...
func main() {
	fDir := flag.String("dir", ".", "source root directory")
	fPattern := flag.String("pkg", "./legacy", "package to fix")
	flag.Parse()
	if err := run(*fDir, *fPattern); err != nil {
...
func run(dir, pattern string) error {
	for i := 0; i < 25; i++ {
		...
		changed, err := runOnce(dir, pattern)
		if err != nil {
			return err
		} else if !changed {
			break
...
func runOnce(dir, pattern string) (bool, error) {
	...
	legacyName := path.Base(pattern)
	var legacy *packages.Package
	for _, pkg := range pkgs {
		if pkg.Name == legacyName {
			legacy = pkg
			continue
		}
