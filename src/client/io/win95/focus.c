...
void __cdecl sub_42EBB0(unsigned int a1, void (*fnc)(int), int field_4, const char* name) {
	...
	if (a1 == 1) {
		...
		if (!arr) {
			v6 = nox_strman_loadString_40F1D0((char*)getMemAt(0x587000, 80600), 0, "C:\\NoxPost\\src\\Client\\Io\\Win95\\focus.c",
									   37);
			sub_4516C0(v6);
			nox_exit(-1);
		}
		...
		if (!arr) {
			v8 = nox_strman_loadString_40F1D0((char*)getMemAt(0x587000, 80656), 0, "C:\\NoxPost\\src\\Client\\Io\\Win95\\focus.c",
									   52);
			sub_4516C0(v8);
			nox_exit(-1);
		}
