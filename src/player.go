...
func (s *Server) newPlayer(ind ntype.PlayerInd, opts *PlayerOpts) int {
	...
	*info = opts.Info
	...
	s.nox_xxx_netSendPacket_4E5030(int(ind)|0x80, v30[:129], 0, 0)
	pl.Field3676 = 2
	...
	if !noxflags.HasGame(noxflags.GameModeCoop) {
		...
		buf[1] = 1
		buf[2] = 1
		s.nox_xxx_netSendPacket_4E5030(int(ind), buf[:], 0, 0)
	}
...
func nox_xxx_playerDisconnFinish_4DE530(pli ntype.PlayerInd, a2 int8) {
	...
	if pl != nil {
		...
		s.nox_xxx_netSendPacket0_4E5420(int(pli)|0x80, buf[:3], 0)
	}
	if int32(a2) == 4 {
		var buf [1]byte
