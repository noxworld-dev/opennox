...
func keyByInd(ind int) []byte {
	out := table[tableElemSize*ind:]
	return out[:tableKeySize:tableKeySize]
}

func u64to32(v uint64) (lo, hi uint32) {
...
func step(v uint32, key []uint32) uint32 {
	v2 := key[(v>>24)&0xff+0x000]
	v2 += key[(v>>16)&0xff+0x100]
	v2 ^= key[(v>>8)&0xff+0x200]
	v2 += key[(v>>0)&0xff+0x300]
	return v2
}

func loop(v uint64, keyB, keyA []uint32) uint64 {
	lo, hi := u64to32(v)
	for i := 0; i < 8; i++ {
		lo ^= keyB[0]
		keyB = keyB[1:]

		hi ^= step(lo, keyA) ^ keyB[0]
		keyB = keyB[1:]
	...
	lo, hi = hi, lo
	return u32to64(lo, hi)
}

func newCoder(key int, reverse bool) (*coder, error) {
	e := &coder{}
	if err := e.initKeyA(key); err != nil {
		return nil, err
	}
	e.initKeyB()
	if reverse {
		for i := 0; i < len(e.keyA)/2; i++ {
			j := len(e.keyA) - 1 - i
			e.keyA[i], e.keyA[j] = e.keyA[j], e.keyA[i]
		}
	}
	return e, nil
}
...
func computeKey(out []uint32, v uint64, keyB, keyA []uint32) {
	for i := 0; i < len(out); i += 2 {
		v = loop(v, keyB, keyA)
		out[i+0], out[i+1] = u64to32(v)
	}
}

func (e *coder) initKeyA(ind int) error {
	if ind < 0 || ind > maxKeyInd {
		return fmt.Errorf("invalid key index: %d", ind)
	}
	key := keyByInd(ind)
	copy(e.keyA[:], staticA[:])
	for i := 0; i < len(e.keyA); i++ {
		ind := i * 4
		v := (uint32(key[(ind+0)%tableKeySize]) << 24) |
			(uint32(key[(ind+1)%tableKeySize]) << 16) |
			(uint32(key[(ind+2)%tableKeySize]) << 8) |
			(uint32(key[(ind+3)%tableKeySize]))
		e.keyA[i] ^= v
	}
	computeKey(e.keyA[:], 0, e.keyA[:], staticB[:])
	return nil
}

func (e *coder) initKeyB() {
	k := e.keyA[:]
	copy(e.keyB[:], staticB[:])
	computeKey(e.keyB[:], u32to64(k[16], k[17]), k[:], e.keyB[:])
}
...
func reverse64(v uint64) uint64 {
	lo, hi := u64to32(v)
	lo, hi = reverse(lo), reverse(hi)
	return u32to64(lo, hi)
}

func (e *coder) EncodeUin64(v uint64) uint64 {
	v = reverse64(v)
	v = loop(v, e.keyA[:], e.keyB[:])
	v = reverse64(v)
	return v
}

func (e *coder) EncodeBlock(p []byte) {
	v := endiness.Uint64(p)
	v = e.EncodeUin64(v)
	endiness.PutUint64(p, v)
}

type keyA [keyASize]uint32
