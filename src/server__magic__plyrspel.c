...
void nox_xxx_playerSpell_4FB2A0_magic_plyrspel(nox_object_t* u) {
	...
	int v3 = ud;
	if (nox_xxx_isArgB8EqSome_424850(ud->spell_phoneme_leaf)) {
		v2 = 0;
	} else {
		uint32_t* v4 = ud->spell_phoneme_leaf;
		if (v4 && *v4) {
			int spellInd = *v4;
					...
					if (v8 < 0) {
						...
						*(float*)&v14[2] = (double)pl->field_2288;
						if (nox_xxx_castSpellByUser_4FDD20(spellInd, u, v14)) {
							nox_xxx_netInformTextMsg_4DA0F0(pl->playerInd, 1, ud->spell_phoneme_leaf);
						} else {
							sub_4FD030(u, v8);
							a1 = 8;
						}
