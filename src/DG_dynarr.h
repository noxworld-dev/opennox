...
// set a single value v at index idx - like "a.p[idx] = v;" but with checks (unless disabled)
#define dg_dynarr_set(a, idx, v) (dg__dynarr_checkidx((a), (idx)), (a).p[dg__dynarr_idx((a).md, (idx))] = (v))
...
#define dg_dynarr_pop(a)                                                                                               \
	(dg__dynarr_check_notempty((a), "Don't pop an empty array!"), (a).p[((a).md.cnt > 0) ? (--(a).md.cnt) : 0])
...
// removes and returns the last element of the array
#define dg_dynarr_pop(a) (dg__dynarr_check_notempty((a), "Don't pop an empty array!"), (a).p[--(a).md.cnt])

// returns the last element of the array
#define dg_dynarr_last(a)                                                                                              \
	(dg__dynarr_check_notempty((a), "Don't call da_last() on an empty array!"), (a).p[(a).md.cnt - 1])
...
#define dg_dynarr_getptr(a, idx)                                                                                       \
	(dg__dynarr_checkidx((a), (idx)), ((size_t)(idx) < (a).md.cnt) ? ((a).p + (size_t)(idx)) : NULL)
...
DG_DYNARR_INLINE int dg__dynarr_maybegrow(void** arr, dg__dynarr_md* md, size_t itemsize, size_t min_needed) {
	if ((md->cap & DG__DYNARR_SIZE_T_ALL_BUT_MSB) >= min_needed)
		...
		return dg__dynarr_grow(arr, md, itemsize, min_needed);
}
...
DG_DYNARR_INLINE int dg__dynarr_insert(void** arr, dg__dynarr_md* md, size_t itemsize, size_t idx, size_t n,
	...
	if (idx <= oldCount && dg__dynarr_maybegrow(arr, md, itemsize, newCount)) {
		...
		if (idx < oldCount)
			memmove(p + (idx + n) * itemsize, p + idx * itemsize, itemsize * (oldCount - idx));

		// if the memory is supposed to be zeroed, do that
		if (init0)
			memset(p + idx * itemsize, 0, n * itemsize);
...
DG_DYNARR_INLINE int dg__dynarr_add(void** arr, dg__dynarr_md* md, size_t itemsize, size_t n, int init0) {
	...
	if (dg__dynarr_maybegrow(arr, md, itemsize, cnt + n)) {
		...
		// if the memory is supposed to be zeroed, do that
		if (init0)
			memset(p + cnt * itemsize, 0, n * itemsize);
...
DG_DYNARR_INLINE void dg__dynarr_delete(void** arr, dg__dynarr_md* md, size_t itemsize, size_t idx, size_t n) {
	...
	if (idx < cnt) {
		if (idx + n >= cnt)
			md->cnt = idx; // removing last element(s) => just reduce count
		else {
...
#define DG_DYNARR_REALLOC(ptr, elemSize, oldNumElems, newCapacity) realloc(ptr, elemSize* newCapacity);
...
DG_DYNARR_DEF int dg__dynarr_grow(void** arr, dg__dynarr_md* md, size_t itemsize, size_t min_needed) {
	...
	if (min_needed < DG__DYNARR_SIZE_T_MSB) {
		...
		if (md->cap & DG__DYNARR_SIZE_T_MSB) {
			...
			if (p != NULL)
				memcpy(p, *arr, itemsize * md->cnt);
			*arr = p;
		} else {
			void* p = DG_DYNARR_REALLOC(*arr, itemsize, md->cnt, newcap);
			if (p == NULL)
				DG_DYNARR_FREE(*arr); // realloc failed, at least don't leak memory
			*arr = p;
		}
