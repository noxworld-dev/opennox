...
func nox_xxx_netDrawRays_49BDD0(a1 *uint8) int32 {
	...
	if *memmap.PtrInt32(0x5D4594, 1304308) < 96 {
		...
		if result != 0 {
			*(*uint8)(unsafe.Add(unsafe.Pointer(uintptr(result)), 432)) = 0
			*(*uint32)(unsafe.Add(unsafe.Pointer(uintptr(result)), 437)) = *(*uint32)(unsafe.Pointer(v3))
			*(*uint32)(unsafe.Add(unsafe.Pointer(uintptr(result)), 441)) = *((*uint32)(unsafe.Add(unsafe.Pointer(v3), 4*1)))
