...
func nox_xxx_updDrawMagic_4CDD80(vp *noxrender.Viewport, dr *client.Drawable) int {
	...
	for {
		...
		v11 = v10
		if v10 != 0 {
			*(*uint32)(unsafe.Add(unsafe.Pointer(uintptr(v10)), 300)) = uint32(uintptr(ccall.FuncAddr(nox_thing_magic_sparkle_draw)))
			nox_xxx_spriteChangeLightColor_484BE0((*uint32)(unsafe.Add(unsafe.Pointer(uintptr(v10)), 136)), 128, 128, math.MaxUint8)
			*(*uint32)(unsafe.Add(unsafe.Pointer(uintptr(v11)), 432)) = *(*uint32)(unsafe.Add(unsafe.Pointer(v2), 4*3)) << 12
			*(*uint32)(unsafe.Add(unsafe.Pointer(uintptr(v11)), 436)) = *(*uint32)(unsafe.Add(unsafe.Pointer(v2), 4*4)) << 12
			*(*uint8)(unsafe.Add(unsafe.Pointer(uintptr(v11)), 299)) = 0
			*(*uint32)(unsafe.Add(unsafe.Pointer(uintptr(v11)), 440)) = 0
			*(*uint32)(unsafe.Add(unsafe.Pointer(uintptr(v11)), 448)) = gameFrame() + uint32(nox_common_randomIntMinMax_415FF0(10, 20, internCStr("C:\\NoxPost\\src\\Client\\Drawable\\Update\\mtailup.c"), 90))
			*(*uint32)(unsafe.Add(unsafe.Pointer(uintptr(v11)), 444)) = gameFrame()
