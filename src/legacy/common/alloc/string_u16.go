...
// GoString16B is similar to GoString16S, but accepts a slice of bytes.
func GoString16B(p []byte) string {
	if len(p) == 0 {
		return ""
	}
	ptr := (*uint16)(unsafe.Pointer(&p[0]))
	return GoString16S(unsafe.Slice(ptr, len(p)/2))
}
...
// StrCopy16B will copy a UTF16 C string from src to dst.
// It will NOT add a zero terminator if len(src) == len(dst).
func StrCopy16B(dst []byte, src string) int {
	wstr := utf16.Encode([]rune(src))
	if len(wstr) == 0 {
		dst[0], dst[1] = 0, 0
		return 0
	}
	cwstr := unsafe.Slice((*byte)(unsafe.Pointer(&wstr[0])), len(wstr)*2)
	n := copy(dst, cwstr)
	if n < len(dst) {
		dst[n+0] = 0
		dst[n+1] = 0
