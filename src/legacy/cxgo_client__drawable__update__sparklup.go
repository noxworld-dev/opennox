...
func nox_xxx_updDrawSparkleTrail_4CDBF0(vp *noxrender.Viewport, dr *client.Drawable) int {
	...
	for {
		...
		if v5 != 0 {
			*(*uint32)(unsafe.Add(unsafe.Pointer(uintptr(v5)), 300)) = uint32(uintptr(ccall.FuncAddr(nox_thing_pixie_dust_draw)))
			nox_xxx_spriteChangeLightColor_484BE0((*uint32)(unsafe.Add(unsafe.Pointer(uintptr(v5)), 136)), math.MaxUint8, 200, 75)
			*(*uint32)(unsafe.Add(unsafe.Pointer(uintptr(v5)), 432)) = uint32(v3 << 12)
			*(*uint32)(unsafe.Add(unsafe.Pointer(uintptr(v5)), 436)) = uint32(v4 << 12)
			*(*uint8)(unsafe.Add(unsafe.Pointer(uintptr(v5)), 299)) = 0
			*(*uint32)(unsafe.Add(unsafe.Pointer(uintptr(v5)), 440)) = 0
			*(*uint32)(unsafe.Add(unsafe.Pointer(uintptr(v5)), 448)) = gameFrame() + uint32(nox_common_randomIntMinMax_415FF0(2, 10, internCStr("C:\\NoxPost\\src\\Client\\Drawable\\Update\\sparklup.c"), 51))
