...
func (s *serverMapSend) CountQueued(pli ntype.PlayerInd) int {
	...
	return s.s.NetStr.ByPlayerInd(nind).CountInQueue(noxnet.MSG_MAP_SEND_START, noxnet.MSG_MAP_SEND_PACKET)
}
...
func (s *serverMapSend) abort(p *playerMapSend, errCode byte) {
	...
	buf[1] = byte(errCode)
	s.s.NetStr.ByPlayerInd(p.PlayerInd).Send(buf[:2], netstr.SendQueue|netstr.SendFlush)
	if s.activeCnt != 0 {
		s.activeCnt--
	}
...
func (s *serverMapSend) SendMore(p *playerMapSend) {
	...
	if p.SentSize == 0 {
		...
		s.s.NetStr.ByPlayerInd(p.PlayerInd).Send(buf[:88], netstr.SendQueue|netstr.SendFlush)
	}
	...
	s.s.NetStr.ByPlayerInd(p.PlayerInd).Send(packet[:6+psz], netstr.SendQueue|netstr.SendFlush)
	p.Sequence++
	p.SentSize += psz
