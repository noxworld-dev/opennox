#include "defs.h"
#include "memfile.h"

// '...' differs in levels of indirection from '... *'
#pragma warning(disable : 4047)

// incompatible types - from '... *' to '... *'
#pragma warning(disable : 4133)

// '...' undefined; assuming extern returning int
#pragma warning(disable : 4013)

// signed/unsigned mismatch
#pragma warning(disable : 4018)

// convertion from '...' to '...', possible loss of data
#pragma warning(disable : 4244)

// truncation from '...' to '...'
#pragma warning(disable : 4305)

// integral size mismatch in argument; conversion supplied
#pragma warning(disable : 4761)

#pragma warning(disable : 4028 4024 4700 4026 4070 4142 4022 4098 4113 4146 4090 4172 4101)

#ifdef __thiscall
#undef __thiscall
#endif
#define __thiscall __cdecl // Test compile in C mode

#include "MixPatch.h"

// angrykirc: variables to be moved
extern _DWORD nox_common_gameFlags;
extern int nox_server_gameSettingsUpdated;

extern char byte_563002[4];
extern char byte_563006[26];

void nox_common_setEngineFlag(const nox_engine_flag flags);
void nox_common_resetEngineFlag(const nox_engine_flag flags);
void nox_common_toggleEngineFlag(const nox_engine_flag flags);
bool nox_common_getEngineFlag(const nox_engine_flag flags);

void init_data();

#ifdef USE_SDL
void change_windowed_fullscreen();
void updateGamma(int value);
#endif

#ifdef __linux__
char* dos_to_unix(const char* path);
#endif

int map_download_loop(int);

int nox_get_audio_enabled();
void sub_401010();
wchar_t* nox_xxx_getNoxVer_401020();
BOOL sub_401040();
int sub_401060();
int __cdecl cmain(int argc, const char* argv[]);
void sub_401B10();
void __cdecl sub_401B20(char*);
// int __stdcall WinMain(HINSTANCE hInstance, int, int, int); // idb
#ifdef USE_SDL
SDL_Window* getWindowHandle_nox_xxx_getHWND_401FD0();
#else
HWND getWindowHandle_nox_xxx_getHWND_401FD0();
#endif // USE_SDL
void __cdecl sub_401FE0(LPCSTR);
// void __cdecl srand(unsigned int);
// int __cdecl rand();
// int swprintf(wchar_t *, const wchar_t *, ...);
// unsigned int __cdecl _control87(unsigned int, unsigned int);
// unsigned int __cdecl _controlfp(unsigned int, unsigned int);
// char *__cdecl _getcwd(char *, int);
// int __cdecl atoi(const char *);
// char *__cdecl strtok(char *, const char *);
// int __cdecl nox_xxx_unknown_libname_1_40256C(int, PVOID TargetFrame, int); idb
// int __cdecl _getptd(_DWORD, _DWORD); weak
// int __cdecl _flsbuf(int, FILE *);
// int _setdefaultprecision(void); weak
// void *__cdecl malloc(size_t);
// _DWORD __cdecl _dosmaperr(_DWORD); weak
// int *__cdecl _errno();
// unsigned int *__cdecl __doserrno();
// int __cdecl _isctype(WORD CharType, int); idb
// void __cdecl __noreturn exit(int);
// void *__cdecl calloc(size_t, size_t);
// int __cdecl free(LPVOID lpMem); idb
// int __cdecl _lseek(int, int, int);
// int __cdecl _lseek_lk(int, LONG lDistanceToMove, DWORD dwMoveMethod); idb
// int __cdecl _write(int, const void *, unsigned int);
// _DWORD __cdecl _lock_file(_DWORD); weak
// _DWORD __cdecl _unlock_file(_DWORD); weak
// _DWORD __cdecl _output(_DWORD, _DWORD, _DWORD); weak
// size_t __cdecl strlen(const char *);
// int __cdecl mbtowc(wchar_t *, const char *, size_t);
// unsigned int __cdecl memcpy(unsigned int a1, _BYTE *a2, unsigned int a3);
// char *__cdecl strncpy(char *, const char *, size_t);
// void *__cdecl memset(void *, int, size_t);
// int __cdecl fflush(FILE *);
// unsigned int __cdecl memmove(unsigned int a1, _BYTE *a2, unsigned int a3);
// int __cdecl fclose(FILE *);
// size_t __cdecl strcspn(const char *, const char *);
// int __cdecl strncmp(const char *, const char *, size_t);
// int __cdecl _close(int);
// char *__cdecl _itoa(int, char *, int);
// int __cdecl _strcmpi(const char *, const char *);
// int __cdecl _strnicmp(const char *, const char *, size_t);
// int __cdecl realloc(LPVOID lpMem, int); idb
// char *__cdecl _strdup(const char *);
// char *__cdecl strchr(const char *, int);
FILE* __cdecl nox_xxx_openFileBin_408CC0(const char* path, int mode);
int __cdecl nox_xxx_cryptOpen_408D40(FILE* a1, int a2);
int __cdecl nox_xxx_fileBinClose_408D90(FILE* a1);
signed int __cdecl nox_xxx_fread_408E40_fread(char* a1, int a2, signed int a3, FILE* a4);
signed int __cdecl nox_xxx_skip2nextboundary_408FE0(char* a1, int a2, int a3, FILE* a4);
int __cdecl nox_xxx_fseek_409050(FILE* a1, int a2, int a3);
int __cdecl sub_409110(FILE* a1);
void nox_xxx_file_409190(FILE* file, int data, int file_offset);
size_t __cdecl sub_409200(char* a1, int a2, int a3, FILE* a4);
int sub_409370();
int nox_xxx_fileBinGetInPtr_409390();
void sub_4093A0();
void sub_4093D0(void); // weak
void __cdecl nox_xxx_parseRead_4093E0(FILE* a1, char* a2, int a3);
int __cdecl nox_xxx_parseString_409470(FILE* a1, _BYTE* a2);
int __cdecl sub_409520(FILE* a1);
void sub_409560();
void __cdecl sub_409580(int* a1, int a2);
int __cdecl nox_xxx_crypt_4096E0(int* a1, int* a2);
void sub_4097C0(uint8_t* src, size_t sz, uint8_t* dest);
_BYTE* __cdecl nox_xxx_parseThingsRule_409860(_BYTE* a1, _BYTE* a2, _DWORD* a3);
int __cdecl sub_4099C0(_BYTE* a1, signed int a2, _BYTE* a3);
int nox_xxx_crypt_409A60();
int __cdecl sub_409A70(__int16 a1);
unsigned int sub_409AA0();
int sub_409AB0();
int sub_409AD0();
int __cdecl sub_409AE0(int a1);
int nox_xxx_mapCrcGetMB_409B00();
int __cdecl nox_xxx_mapSetCrcMB_409B10(int a1);
char* nox_server_currentMapGetFilename_409B30();
char* nox_xxx_mapGetMapName_409B40();
unsigned int __cdecl sub_409B50(const char* a1);
char* sub_409B80();
char* nox_server_get_current_map_path_409B90();
char* sub_409C70();
char* __cdecl nox_xxx_gameSetMapPath_409D70(char* a1);
char* nox_common_get_data_path_409E10();
void nox_common_set_data_path_409E20(char* wd);
int __cdecl sub_409E40(int a1);
int nox_xxx_getServerSubFlags_409E60();
int __cdecl sub_409E70(int a1);
int __cdecl sub_409EC0(int a1);
int __cdecl sub_409EF0(int a1);
BOOL __cdecl sub_409F40(int a1);
int __cdecl nox_xxx_servSetPlrLimit_409F80(int a1);
int nox_xxx_servGetPlrLimit_409FA0();
__int16 __cdecl nox_xxx_servGamedataGet_40A020(__int16 a1);
unsigned __int8 __cdecl sub_40A180(__int16 a1);
BOOL sub_40A1A0();
int __cdecl sub_40A1F0(int a1);
int sub_40A220();
int sub_40A230();
__int64 sub_40A250();
const char* __cdecl nox_xxx_servStartCountdown_40A2A0(int a1, const char* a2);
int sub_40A300();
__int64 __cdecl sub_40A310(int a1);
int __cdecl sub_40A340(int a1);
int sub_40A350();
int __cdecl nox_xxx_mapCrcMb_40A360(int a1);
int nox_xxx_crc_40A370();
void nox_xxx_mapLoad_40A380();
unsigned int __cdecl sub_40A3C0(unsigned int a1);
void __cdecl nox_xxx_setPortParam_40A3E0(int);
int __cdecl nox_xxx_setClientNetPort_40A410(int a1);
int sub_40A420();
int nox_xxx_servGetPort_40A430();
char* __cdecl nox_xxx_gameSetServername_40A440(char* a1);
char* nox_xxx_serverOptionsGetServername_40A4C0();
int __cdecl nox_xxx_setGameFlags_40A4D0(int a1);
void nox_common_gameFlags_unset_40A540(int a1);
int __cdecl sub_40A590(int a1);
int nox_common_gameFlags_getVal_40A5B0();
BOOL __cdecl nox_common_gameFlags_check_40A5C0(int a1);
int __cdecl nox_xxx_ruleSetNoRespawn_40A5E0(int a1);
int nox_server_doPlayersAutoRespawn_40A5F0();
wchar_t* __cdecl nox_xxx_sysopSetPass_40A610(wchar_t* a1);
wchar_t* nox_xxx_sysopGetPass_40A630();
wchar_t* __cdecl sub_40A640(wchar_t* a1);
wchar_t* sub_40A660();
void nox_xxx____setargv_40A670();
int nox_server_gameDoSwitchMap_40A680();
void nox_server_gameUnsetMapLoad_40A690();
int __cdecl sub_40A6A0(int a1);
int sub_40A6B0();
int nox_xxx_rateGet_40A6C0();
int __cdecl nox_xxx_rateUpdate_40A6D0(int a1);
int __cdecl sub_40A710(int a1);
BOOL sub_40A740();
int sub_40A770();
int __cdecl nox_xxx_countNonEliminatedPlayersInTeam_40A830(int a1);
int nox_xxx_gamePlayIsAnyPlayers_40A8A0();
void sub_40A970();
BOOL sub_40AA00();
int sub_40AA20();
int __cdecl sub_40AA30(int a1);
int sub_40AA40();
int sub_40AA50();
int __cdecl sub_40AA60(int a1);
int __cdecl sub_40AA70(int a1);
BOOL nox_xxx_checkConjSoloMap_40ABD0();
signed int __cdecl nox_xxx_fileBinRead_40ADD0_fread(char* a1, size_t a2, size_t a3, FILE* a4);
int __cdecl nox_xxx_PtFuncCompare_40AE90(const void*, const void*); // idb
int __cdecl sub_40AEB0(const void*, const void*); // idb
void sub_40AED0();
void sub_40AF30();
int __cdecl nox_xxx_utilFindSound_40AF50(void* a1);
const char* __cdecl nox_xxx_getSndName_40AF80(int a1);
void __cdecl nox_xxx_soloGameEscMenuCallback_40AF90(int a1, int a2, char a3, int a4, _BYTE* a5, unsigned int a6);
int __cdecl sub_40B170(int a1);
char* __cdecl sub_40B1F0(unsigned __int8 a1);
void __cdecl sub_40B250(int a1, unsigned __int8 a2, unsigned __int16 a3, const void* a4, size_t a5);
int __cdecl nox_xxx_netXfer_40B4B0(unsigned int a1, char a2);
int __cdecl sub_40B4E0(unsigned __int8 a1);
char* __cdecl sub_40B530(unsigned __int8 a1, char a2);
int __cdecl nox_xxx_neXfer_40B590(unsigned int a1, char a2, char a3);
int __cdecl sub_40B5D0(unsigned int a1, char a2, const char* a3, size_t a4, char a5);
int __cdecl sub_40B690(unsigned int a1, char a2, char a3);
char* __cdecl sub_40B6D0(_BYTE* a1);
char* __cdecl sub_40B720(int a1, unsigned __int8 a2);
void sub_40B740();
int sub_40B790();
void __cdecl sub_40B810(int a1, int a2, int a3);
void __cdecl sub_40B850(int a1, char a2);
void* __cdecl sub_40B890(int a1);
int __cdecl sub_40B930(int a1);
void sub_40B970();
int __cdecl sub_40BA90(unsigned int a1, char a2, __int16 a3, __int16 a4, const void* a5);
char* __cdecl sub_40BB20(unsigned int a1, int a2, char a3);
void __cdecl sub_40BBC0(int a1, char a2);
char* __cdecl sub_40BC10(int a1, char a2);
int __cdecl sub_40BC60(int a1, int a2, const char* a3, int a4, int a5, int a6);
int __cdecl nox_xxx_netXferMsg_40BE80(unsigned int a1, char a2, const char* a3, int a4, char a5);
char* __cdecl sub_40BF10(_BYTE* a1);
char* __cdecl sub_40BF60(int a1, char a2, int a3);
char* __cdecl sub_40BFF0(int a1, char a2, char* a3);
char* __cdecl sub_40C030(int a1, char a2);
char* __cdecl sub_40C070(int a1, int a2, char a3);
void sub_40C0D0();
void __cdecl sub_40C0E0(int a1);
int __stdcall sub_40C140(int a1, int a2, _DWORD* a3);
int __stdcall sub_40C1B0(int a1, int a2, int a3);
int __stdcall sub_40C1F0(int a1, int a2, const void* a3);
int __stdcall sub_40C240(int a1, int a2, int a3);
int __stdcall sub_40C2E0(int a1, int a2, int a3, const char* a4);
int __stdcall sub_40C420(int a1, int a2, int a3, int* a4);
int __stdcall sub_40C4E0(int a1, int a2, int a3, int a4);
int __stdcall sub_40C4F0(int a1, int a2, int a3);
int __stdcall sub_40C500(int a1, int a2, int a3);
int __stdcall sub_40C510(int a1, int a2, int a3);
int __stdcall sub_40C520(int a1, int a2, int a3);
int __stdcall sub_40C530(int a1, int a2, int a3, _BYTE* a4);
int __stdcall sub_40C580(int a1, int a2, int a3, int a4, _BYTE* a5);
int __stdcall sub_40C5C0(int a1, int a2, int a3, int a4, _BYTE* a5);
int __stdcall sub_40C600(int a1, int a2, int a3, _BYTE* a4);
int __stdcall sub_40C650(int a1, int a2, int a3, int a4);
int __stdcall sub_40C660(int a1, int a2, int a3, int a4, int a5);
int __stdcall sub_40C670(int a1, int a2, int a3, int a4, int a5);
int __stdcall sub_40C690(int a1, int a2, int a3, int a4, int a5);
int __stdcall sub_40C710(int a1, int a2, int a3);
int __stdcall sub_40C720(int a1, int a2, _DWORD* a3);
int __stdcall sub_40C7A0(int a1, int a2);
int __stdcall sub_40C7D0(int a1, int a2, int a3, _BYTE* a4);
int __stdcall sub_40C830(int a1, int a2, int a3);
int __stdcall sub_40C880(int a1, int a2, const char* a3, int a4, int a5);
int __stdcall sub_40C8C0(int a1, int a2, int a3, int a4);
int __stdcall sub_40C8F0(int a1, int a2, int a3, int a4);
int __stdcall sub_40C960(int a1, int a2, _DWORD* a3);
int __stdcall sub_40C9D0(int a1, int a2, int* a3);
int __stdcall sub_40CA40(int a1, int a2);
int __stdcall sub_40CA50(int a1, int a2);
BOOL __stdcall sub_40CA60(int a1, int a2, const void* a3);
int __stdcall sub_40CB90(int a1, int a2, int a3);
int __stdcall sub_40CBA0(int a1, int a2, int a3);
int __stdcall sub_40CBE0(int a1, int a2);
int sub_40CD00();
int __stdcall sub_40CD50(int a1, int a2, int a3);
int __stdcall nox_xxx_somejoinproc_40CD60(int a1, int a2, int a3);
int __stdcall sub_40CDC0(int a1, int a2, int a3, int a4);
int sub_40CE60();
void sub_40D0F0();
BOOL sub_40D250();
BOOL __cdecl sub_40D280(int a1, int a2, int a3, int a4, int a5);
BOOL __cdecl sub_40D2C0(int a1, int a2, int a3);
BOOL __cdecl sub_40D2F0(int a1, int a2);
BOOL __cdecl sub_40D320(int a1);
BOOL __cdecl sub_40D350(int a1);
BOOL sub_40D380();
BOOL sub_40D3B0();
BOOL __cdecl sub_40D3E0(int a1);
BOOL __cdecl sub_40D410(int a1, int a2);
int sub_40D440();
BOOL __cdecl sub_40D470(int a1, int a2);
BOOL __cdecl sub_40D4A0(int a1);
BOOL __cdecl sub_40D4D0(int a1);
BOOL __cdecl sub_40D500(int a1, int a2);
BOOL __cdecl sub_40D530(int a1);
int __cdecl sub_40D560(int a1);
BOOL __cdecl sub_40D5B0(int a1);
BOOL __cdecl sub_40D5E0(int a1);
BOOL __cdecl sub_40D610(int a1);
BOOL __cdecl sub_40D640(int a1);
BOOL __cdecl sub_40D670(int a1);
BOOL __cdecl sub_40D6A0(int a1);
BOOL __cdecl sub_40D6D0(int a1);
BOOL __cdecl sub_40D700(int a1, int a2, int a3, int a4, int a5, int a6);
BOOL __cdecl sub_40D740(int a1);
BOOL __cdecl sub_40D770(int a1, int a2);
BOOL __cdecl sub_40D7A0(int a1, int a2);
BOOL __cdecl sub_40D7D0(int a1, int a2);
BOOL __cdecl sub_40D800(int a1);
BOOL __cdecl sub_40D830(int a1);
BOOL sub_40D860();
BOOL __cdecl sub_40D890(int a1);
BOOL __cdecl sub_40D8C0(int a1, int a2);
BOOL __cdecl sub_40D8F0(int a1);
BOOL __cdecl sub_40D920(int a1);
BOOL __cdecl sub_40D950(int a1);
BOOL __cdecl sub_40D980(int a1, int a2, int a3);
BOOL __cdecl sub_40D9C0(int a1, int a2, int a3, int a4);
BOOL __cdecl sub_40DA00(int a1);
BOOL __cdecl sub_40DA30(int a1);
BOOL __cdecl sub_40DA60(int a1, int a2);
BOOL __cdecl sub_40DA90(int a1, int a2);
BOOL __cdecl sub_40DAC0(int a1);
BOOL sub_40DAF0();
BOOL __cdecl sub_40DB20(int a1);
BOOL __cdecl sub_40DB50(int a1, int a2);
BOOL __cdecl sub_40DB80(int a1, int a2);
int __stdcall sub_40DBB0(int a1);
int __stdcall sub_40DBE0(int a1, int a2);
int __stdcall sub_40DC10(int a1, int a2, int a3, int a4, int a5);
int __stdcall sub_40DC50(int a1, int a2);
int __stdcall sub_40DC60(int a1);
BOOL sub_40DC70();
BOOL __cdecl sub_40DC90(int a1, int a2, int a3, int a4, int a5, int a6);
BOOL sub_40DCD0();
int __stdcall sub_40DD00(int a1, int a2, int a3, int a4, int a5);
int __stdcall sub_40DD10(int a1, int a2, int a3, int a4, int a5, int a6);
int __stdcall sub_40DD60(int a1, int a2);
int __stdcall sub_40DD70(int a1, int a2, int a3, const char* a4, const char* a5);
int __stdcall sub_40DE10(int a1, int a2, int a3, int a4);
int __stdcall sub_40DE90(int a1, int a2, int a3, int a4);
BOOL __cdecl sub_40DEA0(int a1, int a2, int a3, int a4);
BOOL __cdecl sub_40DED0(int a1, int a2, int a3, int a4, int a5, int a6, int a7, int a8, int a9);
BOOL __cdecl sub_40DF20(int a1, int a2, int a3, int a4, int a5, int a6, int a7);
BOOL __cdecl sub_40DF60(int a1, int a2, int a3);
BOOL sub_40DF90();
BOOL __cdecl sub_40DFC0(int a1, int a2);
BOOL __cdecl sub_40DFE0(int a1, int a2, int a3, int a4, int a5, int a6);
BOOL __cdecl sub_40E020(int a1, int a2, int a3, int a4);
BOOL __cdecl sub_40E050(int a1, int a2);
void sub_40E070();
void sub_40E090();
void nox_xxx____setargv_0_40E0A0();
BOOL sub_40E0B0();
int sub_40E0C0();
int __cdecl sub_40E0D0(int, LPCSTR lpSubKey, int); // idb
int __stdcall sub_40E260(int(__stdcall*** a1)(_DWORD, void*, int*), int a2, int a3, int a4);
int __stdcall sub_40E320(int(__stdcall*** a1)(_DWORD, void*, int*), int a2, int a3);
int __thiscall sub_40E3D0(int* this);
int __stdcall sub_40E3E0(_DWORD* a1);
char* __thiscall sub_40E470(char* this);
LONG __stdcall sub_40E490(int a1);
LONG __stdcall sub_40E4B0(volatile LONG* lpMem);
int __stdcall sub_40E4F0(int a1, _DWORD* a2, int* a3);
void __fastcall sub_40E5F0(_DWORD* a1);
int __stdcall sub_40E630(_DWORD* a1);
LONG __stdcall sub_40E680(volatile LONG* lpMem);
int __stdcall sub_40E6C0(int a1, _DWORD* a2, int* a3);
void __fastcall sub_40E7C0(_DWORD* a1);
int __stdcall sub_40E800(_DWORD* a1);
LONG __stdcall sub_40E850(volatile LONG* lpMem);
int __stdcall sub_40E890(int a1, _DWORD* a2, int* a3);
void __fastcall sub_40E990(_DWORD* a1);
int __cdecl sub_40E9D0(int a1, int a2);
int __cdecl sub_40E9F0(int a1, int a2);
_DWORD* nox_xxx_netInitMsgBufs_40EA10();
void nox_xxx_net_40EA70();
BOOL __cdecl nox_xxx_netSend_40EAC0(int a1, int a2, int a3);
BOOL __cdecl nox_xxx_net_40EB60(int a1, int a2, int a3);
int __cdecl nox_xxx_netAddToMsgListCli_40EBC0(int a1, int a2, const void* a3, int a4);
char* __cdecl nox_xxx_netSend_40EC30(int a1, int a2, const void* a3, signed int a4);
int __cdecl nox_xxx_netClientSend_0_40ECA0(int a1, int a2, const void* a3, int a4);
FILE* __cdecl sub_40ED10(int a1, int a2);
int __cdecl nox_xxx_netBufs_40ED40(int a1, int a2);
unsigned __int8* __cdecl nox_xxx_netCopyCheckPacketList_40ED60(int a1, unsigned int a2, _DWORD* a3);
int __cdecl nox_xxx_netSendBySock_40EE10(unsigned int a1, int a2, unsigned int a3);
int sub_40EE60();
FILE* __cdecl nox_xxx_netBuf_40EE90(int a1);
int** __cdecl nox_xxx_net_40EEB0(int a1, int a2, _DWORD* a3);
BOOL __cdecl sub_40EEF0(int a1, int a2);
int __cdecl nox_xxx_netAddToMsgListServ_40EF40(int a1, const void* a2, int a3);
char* __cdecl nox_xxx_netlistAdd_40EFA0(int a1, const void* a2, signed int a3);
void __cdecl sub_40F000(int a1, int a2);
int __cdecl nox_xxx_servBuildMsgPool4Plr_40F020(int a1);
int __cdecl sub_40F040(int a1);
int sub_40F060();
int** __cdecl nox_xxx_netListGetData_40F080(int a1, _DWORD* a2);
int __cdecl sub_40F0B0(int a1);
int __cdecl sub_40F0D0(int a1);
void __cdecl sub_40F0F0(int a1, int(__cdecl* a2)(_DWORD, int), int a3);
unsigned __int8* __cdecl nox_xxx_netlist_40F120(int a1, _DWORD* a2);
wchar_t* __cdecl nox_xxx_loadString_40F1D0(const char* stringName, char** storeTo, const char* srcLocation, int srcLine);
int __cdecl nox_strman_readfile(char* a1);
int sub_40F4E0();
char __cdecl sub_40F5C0(_BYTE* a1);
char __cdecl sub_40F640(FILE* a1, char* a2, char* a3, _BYTE* a4, int a5);
int __cdecl sub_40F7A0(char* a1);
int __cdecl sub_40F830(const char* a1);
void __cdecl sub_40FB60(wchar_t* a1);
int sub_40FBE0();
_WORD* __cdecl sub_40FE00(_WORD* a1, _BYTE* a2);
void __cdecl sub_40FFE0(char*, char*);
void sub_410020();
int __cdecl nox_xxx_cliCanTalkMB_4100F0(__int16* a1);
BOOL sub_410120();
void nox_xxx____setargv_1_410150();
int nox_xxx_wall_410160();
int nox_xxx_mapAlloc_4101D0();
LPVOID __cdecl nox_xxx_wallCreateAt_410250(int a1, int a2);
_BYTE* __cdecl nox_xxx_doorAttachWall_410360(int a1, int a2, int a3);
_DWORD* __cdecl sub_410390(int a1, int a2, int a3);
_DWORD* __cdecl nox_xxx_mapDelWallAtPt_410430(int a1, int a2);
int* __cdecl nox_xxx_wallDestroyedByWallid_410520(__int16 a1);
int __cdecl sub_410550(__int16 a1);
int __cdecl nox_server_getWallAtPoint_410580(int a1, int a2);
int __cdecl nox_xxx_wall_4105E0(int a1, int a2);
int __cdecl nox_xxx_wallForeachFn_410640(void(__cdecl* a1)(int, int), int a2);
int __cdecl sub_410670(void(__cdecl* a1)(int, int), int a2);
int __cdecl sub_4106A0(int a1);
void sub_4106C0();
_DWORD* sub_410730();
_DWORD* __cdecl nox_xxx_wallSecretBlock_410760(_DWORD* a1);
LPVOID nox_xxx_wallSecretGetFirstWall_410780();
int __cdecl nox_xxx_wallSecretNext_410790(int* a1);
int* __cdecl sub_4107A0(LPVOID lpMem);
_DWORD* nox_xxx_wallBreackableListClear_410810();
_DWORD* __cdecl nox_xxx_wallBreackableListAdd_410840(int a1);
LPVOID nox_xxx_wallGetFirstBreakableCli_410870();
int __cdecl nox_xxx_wallGetNextBreakableCli_410880(int* a1);
int* __cdecl nox_xxx_wallBreakableRemove_410890(int* lpMem);
int __cdecl nox_thing_read_WALL_410900(_DWORD* a1, char* a2);
char* __cdecl sub_410D40(int a1);
int __cdecl nox_xxx_wallTileByName_410D60(const char* a1);
unsigned __int8 __cdecl nox_xxx_mapWallMaxVariation_410DD0(int a1, int a2, int a3);
unsigned __int8 __cdecl nox_xxx_map_410E00(int a1);
unsigned __int8 __cdecl nox_xxx_mapWallGetHpByTile_410E20(int a1);
unsigned __int8 __cdecl nox_xxx_wallGetBrickTypeMB_410E40(int a1);
char* __cdecl nox_xxx_wallGetBrickObj_410E60(int a1, int a2);
char* __cdecl nox_xxx_wallSoundByTile_410EA0(int a1);
char* __cdecl nox_xxx_wallFindOpenSound_410EE0(int a1);
char* __cdecl nox_xxx_wallFindCloseSound_410F20(int a1);
int nox_xxx_tileAlloc_410F60_init();
void nox_xxx_tileFree_410FC0_free();
int nox_xxx_tileUnusedMB_410FF0();
int __cdecl nox_xxx_tileNFromPoint_411160(float2*);
int __cdecl sub_411350(int* a1, int* a2, int a3);
int __cdecl sub_4113A0(int* a1, int a2);
int __cdecl sub_411490(int a1, int a2);
int __cdecl nox_thing_read_FLOR_411540(int a1, _BYTE* a2);
int __cdecl nox_xxx_checkFacade_4117E0(const char* a1);
int __cdecl nox_thing_read_EDGE_411850(int a1, _BYTE* a2);
BOOL __cdecl nox_xxx_mapTileAllowTeleport_411A90(float2* a1); // idb
void sub_411B80();
BOOL __cdecl nox_xxx_parseWeapColor_411C40(const char* a1, char* a2, int a3);
int __cdecl sub_411C80(const char* a1);
char* __cdecl nox_xxx_parseBinColor_411CF0(char* a1, _BYTE* a2);
BOOL __cdecl sub_411D90(const char* a1, char* a2, int a3);
int __cdecl sub_411E60(int a1, char* a2, int a3);
char* __cdecl sub_411ED0(int a1, char* a2, int a3);
char* __cdecl sub_411F20(int a1, char* a2, int a3);
char* __cdecl sub_411F70(int a1, char* a2, int a3);
char* __cdecl sub_411FC0(int a1, char* a2, int a3);
const char* __cdecl nox_xxx_parseWeapDamageType_412010(int a1, char* a2, int a3);
char* __cdecl sub_412060(int a1, char* a2, int a3);
char* __cdecl sub_4120B0(int a1, char* a2, int a3);
int __cdecl nox_xxx_parseEnchWorth_412310_parse_worth(const char* a1, char* a2, obj_412ae0_t* a3);
int __cdecl nox_xxx_parseEnchColor_412360_parse_color(const char* a1, char* a2, obj_412ae0_t* a3);
int __cdecl nox_xxx_parseEnchEffect_412380_parse_attack_effect(const char* a1, char* a2, obj_412ae0_t* a3);
int __cdecl nox_xxx_parseEnchDefend_412490_parse_defend_effect(const char* a1, char* a2, obj_412ae0_t* a3);
int __cdecl nox_xxx_parseEnchEngageOrDisEng_412580_parse_engage_effect(const char* a1, char* a2, obj_412ae0_t* a3);
int __cdecl nox_xxx_parseEnchUpdate_412670_parse_update_effect(const char* a1, char* a2, obj_412ae0_t* a3);
int __cdecl nox_xxx_parseEnchAllowedWeapon_412740_parse_allowed_weapons(const char* a1, char* a2, obj_412ae0_t* a3);
int __cdecl nox_xxx_parseAllowedWeaponArmor_412760(char* a1, int* a2);
int __cdecl nox_xxx_parseEnchAllowedArmor_4128A0_parse_allowed_armor(const char* a1, char* a2, obj_412ae0_t* a3);
int __cdecl nox_xxx_parseAllowedPosition_4128C0_parse_allowed_pos(const char* a1, char* a2, obj_412ae0_t* a3);
int __cdecl nox_xxx_parseModifierBin_412930(char* a1, char* a2);
int __cdecl nox_xxx_parseModifDesc_412AE0(const char* a1, FILE* a2, char* a3);
int __cdecl nox_xxx_parseModifAddByType_412C60(const char* a1, obj_412ae0_t* a2);
int __cdecl nox_xxx_parseWeaponDef_412D40(int a1, FILE* a2, char* a3);
int __cdecl sub_412ED0(int a1, FILE* a2, char* a3);
void nox_xxx_freeWeaponArmorDefAndModifs_413060();
void __cdecl sub_4130C0(LPVOID lpMem); // idb
void __cdecl sub_413100(LPVOID lpMem); // idb
void __cdecl nox_xxx_modifFreeOne_413140(LPVOID lpMem); // idb
_DWORD* __cdecl nox_xxx_getProjectileClassById_413250(int a1);
_DWORD* __cdecl nox_xxx_equipClothFindDefByTT_413270(int a1);
int __cdecl nox_xxx_modifGetIdByName_413290(const char* a1);
int __cdecl sub_413300(int a1);
obj_412ae0_t* __cdecl nox_xxx_modifGetDescById_413330(int a1);
LPVOID sub_413370();
int __cdecl sub_413380(int a1);
LPVOID sub_413390();
int __cdecl sub_4133A0(int a1);
obj_412ae0_t* __cdecl nox_xxx_modifGetModifListByType_4133B0(int a1);
obj_412ae0_t* __cdecl nox_xxx_modifNext_4133C0(obj_412ae0_t* a1);
BOOL __cdecl sub_4133D0(int a1);
int __cdecl sub_413420(char a1);
void sub_4134C0();
int sub_4134D0();
void sub_4134F0();
int sub_413760();
int sub_413780();
BOOL sub_4137A0();
int sub_4137C0();
int sub_4137E0();
int sub_413800();
BOOL nox_xxx_testCD_413830();
BOOL nox_xxx_testCDAndSolo_413840();
int sub_413850();
BOOL __cdecl sub_413870(int a1);
char* sub_413890();
void sub_4138E0(int a1);
void sub_413900(int a1);
int sub_413920();
void sub_413960();
BOOL sub_4139B0();
void sub_413A00(int a1);
BOOL nox_xxx_checkGFlagNoParticles_413A50();
void sub_413A60();
int __cdecl sub_413A80(char* a1);
void __cdecl sub_413AD0(FILE* a1);
char* __cdecl sub_413B00();
int __cdecl sub_413B20(FILE** a1, char* a2, char* a3);
void sub_413B70(char* a1, ...);
void __cdecl sub_413BD0(FILE* a1, int a2);
void sub_413C00();
void sub_413C30();
void sub_413C60();
void sub_413C80(char* a1, ...);
FILE* sub_413CC0();
void sub_413D00();
int nox_xxx_networkLog_413D30(char* a1, ...);
int sub_413E10();
void sub_413E30();
int __cdecl sub_413F60(const void*, const void*); // idb
void nullsub_2(void);                             // weak
char* __cdecl nox_clone_str(const char* a1);
nox_alloc_class* __cdecl nox_new_alloc_class(const char* name, int size, int cnt);
nox_alloc_class* __cdecl nox_new_alloc_class_f30(const char* name, int size, int cnt);
void __cdecl nox_free_alloc_class(nox_alloc_class* p); // idb
void __cdecl nox_free_alloc_class_f30(nox_alloc_class* p);
int __cdecl nox_alloc_class_new_obj(_DWORD* a1);
void* __cdecl nox_alloc_class_new_obj_zero(nox_alloc_class* a1);
void __cdecl nox_xxx_memDeleteAdv_414330(unsigned int* a1, nox_drawable* dr);
int __cdecl sub_4143D0(int a1, nox_drawable* dr);
void __cdecl sub_414400(unsigned int* a1, _QWORD* a2);
int __cdecl sub_4144B0(int a1);
void __cdecl nox_xxx_class_4144D0(_DWORD* a1);
int sub_414580();
int sub_414590();
int sub_4145A0();
int sub_4145B0();
void* __cdecl sub_4145C0(size_t a1, size_t a2);
void __cdecl sub_4145E0(LPVOID lpMem);
int __cdecl sub_4145F0(_DWORD* a1);
char __cdecl sub_414690(unsigned int* a1, void(__stdcall* a2)(char*));
BOOL __cdecl sub_4147E0(HWND hWnd);
int sub_414800();
unsigned int __cdecl sub_4148D0(LPCSTR lpFileName);
int __cdecl sub_4149A0(LPCSTR lpFileName, LPVOID lpBuffer, LPVOID, LPVOID); // idb
int __cdecl sub_414A40(LPCSTR lpSubKey, LPCSTR lpValueName);                // idb
int __cdecl sub_414A90(LPCSTR lpSubKey, LPCSTR lpValueName, BYTE* lpData);  // idb
char* __cdecl sub_414B00(LPCWSTR lpWideCharStr, LPSTR lpMultiByteStr, int cbMultiByte);
void nox_common_readHiddenExeMsg_414B30(uint8_t* src_8byte, uint8_t* dst_12byte);
int __cdecl sub_414BA0(int a1);
int __cdecl sub_414BD0(signed int a1);
int __cdecl sub_414C50(signed int a1);
char nox_xxx_initSinCosTables_414C90();
void sub_414D30();
int __cdecl nox_thing_read_audio_414D40(int a1);
BOOL __cdecl nox_thing_read_FLOR_414DB0(nox_memfile* f);
BOOL __cdecl nox_thing_read_EDGE_414E70(int a1, void* a2);
BOOL __cdecl nox_thing_read_WALL_414F60(_DWORD* a1, void* a2);
int __cdecl nox_thing_read_spells_415100(int a1);
int __cdecl nox_thing_read_image_415240(int a1);
int __cdecl nox_thing_read_ability_415320(int a1);
bool __cdecl nox_xxx_loadAllBinFiles_415470(void);
int __cdecl nox_thing_read_audio_415660(int a1, char* a2);
int __cdecl nox_thing_read_SPEL_4156B0(nox_memfile* f, void* a2);
int __cdecl nox_thing_read_IMAG_415700(int a1, void* a2);
int __cdecl nox_thing_read_ABIL_415750(int a1, void* a2);
void sub_4157A0();
void sub_4157B0();
int __cdecl nox_xxx_unitWeaponInventoryEquipFlags_415820(int a1);
int __cdecl sub_415840(char* a1);
int __cdecl nox_xxx_ammoCheck_415880(char* a1);
int __cdecl sub_415910(char* a1);
int __cdecl sub_415960(wchar_t* a1);
char* __cdecl sub_4159B0(char* a1);
int __cdecl sub_4159F0(int a1);
int __cdecl sub_415A30(char* a1);
void sub_415A90();
void sub_415AA0();
int __cdecl sub_415B20(char* a1);
double __cdecl sub_415BD0(int a1);
double __cdecl nox_xxx_itemApplyDefendEffect_415C00(int a1);
int __cdecl nox_xxx_unitArmorInventoryEquipFlags_415C70(int a1);
int __cdecl sub_415C90(int a1);
int __cdecl sub_415CD0(char* a1);
int __cdecl sub_415D10(char* a1);
int __cdecl sub_415DA0(wchar_t* a1);
int __cdecl sub_415DF0(char* a1);
char* __cdecl sub_415E40(char* a1);
int __cdecl sub_415E80(int a1);
int __cdecl sub_415EC0(char* a1);
void sub_415F20();
int __cdecl nox_xxx_replayWriteRndCounter_415F30(int a1);
int __cdecl nox_xxx_replayReadeRndCounter_415F50(int a1);
int nox_xxx_initRnd_415F70();
int __cdecl nox_common_randomInt_415FA0(int a1, int a2);
int __cdecl nox_xxx_randGetMinMax_415FF0(int, int, const char*, int);
double __cdecl nox_xxx_randFloat_416030(float a1, float a2);
double __cdecl sub_416090(float a1, float a2);
int __cdecl nox_xxx_setKeybTimeout_4160D0(int a1);
BOOL __cdecl nox_xxx_checkKeybTimeout_4160F0(unsigned __int8 a1, unsigned int a2);
BOOL __cdecl sub_416120(unsigned __int8 a1);
int __cdecl sub_416150(int a1, int a2);
int __cdecl sub_416170(int a1);
int nox_xxx_clear18hDD_416190();
BOOL __cdecl sub_4161B0(unsigned __int8 a1);
int sub_4161E0();
void sub_4164F0();
int nox_xxx_mapSetDataDefault_416500();
int sub_416580();
char* __cdecl nox_xxx_cliGamedataGet_416590(int a1);
char* sub_4165B0();
char* __cdecl sub_4165D0(int a1);
int __cdecl sub_4165F0(int a1, int a2);
char* sub_416630();
char* sub_416640();
int sub_416650();
int __cdecl sub_416670(int a1);
void sub_416690();
int sub_416720();
int* __cdecl sub_416770(int a1, wchar_t* a2, const char* a3);
int* __cdecl sub_416820(int a1);
int* __cdecl sub_416860(int a1);
int* __cdecl sub_4168A0(wchar_t* a1);
int* sub_4168E0();
int* __cdecl sub_4168F0(int* a1);
int* sub_416900();
int* __cdecl sub_416910(int* a1);
int sub_416920();
int* sub_416950();
int sub_4169C0();
int __cdecl nox_xxx_cliSetSettingsAcquired_4169D0(int a1);
char* sub_4169E0();
char* sub_4169F0();
unsigned int sub_416A00();
int nox_xxx_createMutexCheck_416A10();
HANDLE sub_416B00();
void nox_init_ticks_func();
__int64 nox_call_get_ticks();
__int64 nox_get_ticks_freq();
__int64 nox_get_ticks_fallback();
void nox_framerate_limit_416C70(int fps);
BOOL nox_ticks_should_update_416CD0();
__int64 nox_ticks_until_next_416D00();
void nox_ticks_xxx_416D40();
BOOL __cdecl sub_416D70();
void nox_ticks_maybe_sleep_416DD0();
void sub_416E20();
void nox_xxx_cliResetAllPlayers_416E30();
char* __cdecl nox_xxx_playerForceSendLessons_416E50(int a1);
nox_playerInfo* nox_common_playerInfoGetFirst_416EA0();
nox_playerInfo* nox_common_playerInfoGetNext_416EE0(nox_playerInfo* p);
int nox_common_playerInfoCount_416F40();
nox_playerInfo* nox_common_playerInfoNew_416F60(int a1);
void nox_common_playerInfoReset_416FD0(nox_playerInfo* a1);
nox_playerInfo* nox_common_playerInfoResetInd_417000(int i);
nox_playerInfo* nox_common_playerInfoGetByID_417040(int a1);
nox_playerInfo* nox_common_playerInfoFromNum_417090(int i);
char* __cdecl nox_xxx_playerByName_4170D0(wchar_t* a1);
wchar_t* __cdecl sub_417120(int a1);
int __cdecl nox_xxx_netMarkMinimapObject_417190(int a1, int a2, int a3);
int __cdecl sub_417270(int a1);
int __cdecl sub_4172C0(int a1);
int __cdecl nox_xxx_netUnmarkMinimapObj_417300(int a1, int a2, int a3);
int __cdecl nox_xxx_playerMapTracksObj_4173D0(int a1, int a2);
char* __cdecl nox_xxx_netMinimapUnmark4All_417430(int a1);
char* __cdecl nox_xxx_netUnmarkMinimapSpec_417470(int a1, int a2);
char* __cdecl nox_xxx_netMarkMinimapForAll_4174B0(int a1, int a2);
int __cdecl nox_xxx_netNeedTimestampStatus_4174F0(int a1, int a2);
char __cdecl nox_xxx_playerUnsetStatus_417530(int a1, int a2);
char* __cdecl nox_xxx_sendAllClientStatus_4175C0(int a1);
int __cdecl nox_xxx_netReportPlayerStatus_417630(int a1);
void __cdecl nox_xxx_cliPlayerRespawn_417680(int a1, char a2);
char* __cdecl nox_xxx_clientEquipWeaponArmor_417AA0(char a1, int a2, int a3, int a4);
char* __cdecl sub_417B80(char a1, int a2, int a3);
char* __cdecl sub_417C00(char* a1, int a2);
void sub_417C50();
int sub_417CF0();
int sub_417D00();
int __cdecl nox_xxx_SetGameplayFlag_417D50(int a1);
int __cdecl nox_xxx_UnsetGameplayFlags_417D70(int a1);
int nox_xxx_GetGameplayFlags_417D90();
BOOL __cdecl nox_xxx_CheckGameplayFlags_417DA0(int a1);
int sub_417DC0();
unsigned __int8 nox_xxx_getTeamCounter_417DD0();
char sub_417DE0();
int nox_xxx_mapInfoSetCapflag_417EA0();
bool sub_417EC0();
char nox_xxx_mapInfoSetFlagball_417F30();
int __cdecl sub_417F50(int a1);
int nox_xxx_mapInfoSetKotr_4180D0();
void sub_4181F0(int a1);
int sub_418390();
int sub_4183C0();
void __cdecl sub_4184D0(wchar_t* a1);
int nox_xxx_wndGuiTeamCreate_4185B0();
int nox_xxx_teamAssignFlags_418640();
char* __cdecl nox_xxx_toggleAllTeamFlags_418690(int a1);
char sub_4187A0();
int sub_4187E0();
void __cdecl sub_418800(wchar_t* a1, wchar_t* a2, int a3);
int __cdecl sub_418830(int a1, int a2);
int nox_xxx_unused_418840();
char* sub_4189D0();
char* sub_418A10();
char* __cdecl sub_418A40(wchar_t* a1);
char* __cdecl sub_418A80(int a1);
char* __cdecl nox_xxx_clientGetTeamColor_418AB0(int a1);
char* __cdecl sub_418AE0(int a1);
char* nox_xxx_TeamGet_418B10();
char* __cdecl nox_xxx_TeamNext_418B60(int a1);
int __cdecl sub_418BC0(int a1);
int __cdecl nox_xxx_teamCheckSmth_418C60(int a1);
_DWORD* __cdecl sub_418C70(_DWORD* a1);
_DWORD* __cdecl nox_xxx_objGetTeamByNetCode_418C80(int a1);
void __cdecl nox_xxx_teamRenameMB_418CD0(wchar_t* a1, wchar_t* a2);
void* __cdecl nox_xxx_materialGetTeamColor_418D50(int a1);
void __cdecl sub_418D80(int a1);
_DWORD* __cdecl sub_418E40(int a1, int a2);
void __cdecl sub_418F20(wchar_t* a1, int a2);
int __cdecl sub_419030(int a1);
void __cdecl nox_xxx_netChangeTeamID_419090(int a1, int a2);
int __cdecl sub_4190F0(wchar_t* a1);
BOOL __cdecl nox_xxx_servObjectHasTeam_419130(int a1);
BOOL __cdecl nox_xxx_servCompareTeams_419150(int a1, int a2);
int __cdecl nox_xxx_teamCompare2_419180(int a1, unsigned __int8 a2);
void __cdecl nox_xxx_netChangeTeamMb_419570(int a1, int a2);
int __cdecl sub_4196D0(int a1, int a2, int a3, int a4);
void __cdecl sub_4197C0(wchar_t* a1, int a2);
void __cdecl sub_4198A0(int a1, int a2, int a3);
char __cdecl sub_419900(int a1, int a2, __int16 a3);
char __cdecl sub_419960(int a1, int a2, __int16 a3);
int __cdecl sub_4199C0(int a1);
double __cdecl sub_419A10(float a1); // idb
unsigned int __cdecl sub_419A30(float a1);
int __cdecl nox_float2int(float); // idb
__int16 __cdecl nox_float2int16(float a1);
__int16 __cdecl nox_float2int16_abs(float a1);
char __cdecl nox_float2int8(float a1);
float __cdecl nox_double2float(double a1);
int __cdecl nox_double2int(double); // idb
int nox_xxx_parseGamedataBin_419B30();
double __cdecl nox_xxx_gamedataGetFloat_419D40(void* a1);
double __cdecl nox_xxx_loadFloatVar_419D70(void* a1, int a2);
void sub_419DB0();
void __cdecl sub_419DE0(int a1, LPVOID* lpMem);
void sub_419E00();
int __cdecl sub_419E10(int a1, int a2);
BOOL __cdecl sub_419E60(int a1);
BOOL sub_419EA0();
int __cdecl sub_419EB0(char a1, int a2);
BOOL __cdecl sub_419EE0(char a1);
BOOL sub_419F00();
int __cdecl sub_419F10(const char* a1, const char* a2);
int __cdecl sub_41A000(char* a1, nox_savegame_xxx* sv);
int __cdecl nox_xxx_playerSaveToFile_41A140(char* a1, int a2);
int __cdecl nox_xxx_mapSavePlayerDataMB_41A230(char* a1);
char* __cdecl nox_xxx_cliPlrInfoLoadFromFile_41A2E0(char* a1, int a2);
int __cdecl nox_xxx_plrLoad_41A480(char* a1);
int __cdecl sub_41A590(int a1, int a2);
char* __cdecl sub_41AA30(int a1);
BOOL __cdecl sub_41AC30(_DWORD* a1);
int sub_41B3B0();
BOOL __cdecl sub_41B3E0(int a1);
BOOL __cdecl nox_xxx_guiFieldbook_41B420(int a1);
int __cdecl nox_xxx_guiSpellbook_41B660(int a1);
BOOL __cdecl nox_xxx_guiEnchantment_41B9C0(_DWORD* a1);
BOOL __cdecl sub_41BEC0(int a1);
int __cdecl sub_41C080(int a1);
int sub_41C200();
int __cdecl sub_41C280(void* a1);
int nox_xxx_parseFileInfoData_41C3B0(int a1);
int sub_41C780(int a1);
FILE* __cdecl sub_41CAC0(char* a1, char** a2);
int __cdecl sub_41CC00(char* a1);
int __cdecl nox_xxx_computeServerPlayerDataBufferSize_41CC50(char* a1);
int __cdecl nox_xxx_SavePlayerDataFromClient_41CD70(char* a1, _BYTE* a2, int a3);
int nox_xxx_netSavePlayer_41CE00();
int __cdecl nox_xxx_unused_41CE30(char* a1, wchar_t* a2);
int __cdecl sub_41CEE0(int a1, int a2);
int __cdecl sub_41CFA0(char* a1, int a2);
int __cdecl sub_41D090(char* a1);
int __cdecl sub_41D110(_DWORD* a1);
int sub_41D170();
int __cdecl sub_41D1A0(int a1);
int sub_41D1B0();
void sub_41D1C0();
int sub_41D440();
int sub_41D4C0();
int nox_xxx_unused_41D4E0();
int sub_41D530();
int sub_41D5D0();
unsigned __int8* sub_41D5E0();
int sub_41D650();
BOOL __cdecl sub_41D670(int a1);
int sub_41D6C0();
void* sub_41D9F0();
int __cdecl sub_41DA10(int a1);
LPVOID sub_41DA40();
int __cdecl sub_41DA70(int a1, __int16 a2);
int sub_41DB90();
int __cdecl sub_41DCC0(int a1);
int sub_41DD10();
int sub_41DD40();
int sub_41DFC0();
int sub_41E080();
int sub_41E0D0();
int sub_41E170();
int sub_41E1B0();
int sub_41E210();
int sub_41E2F0();
int __cdecl sub_41E300(int a1);
int sub_41E370();
int nox_xxx_reconAttempt_41E390();
void nox_xxx_reconStart_41E400();
int sub_41E470();
int __cdecl sub_41E4B0(int a1);
int sub_41E4C0();
void __cdecl sub_41E4D0(_DWORD* a1);
int sub_41E520();
BOOL sub_41E540();
int __cdecl sub_41E560(int a1, int a2, int a3, int a4);
_DWORD* __cdecl sub_41E590(_DWORD* a1, _DWORD* a2, _DWORD* a3, _DWORD* a4);
int __cdecl sub_41E5C0(int a1);
_DWORD* __cdecl sub_41E6C0(char* a1);
void __cdecl sub_41E710(wchar_t* a1, int a2);
int __cdecl sub_41E750(wchar_t* a1);
_DWORD* __cdecl sub_41E7A0(const char* a1);
_DWORD* sub_41E810();
_DWORD* __cdecl sub_41E870(char* a1, _DWORD* a2);
int __cdecl sub_41E8D0(int a1, int a2);
char* __cdecl sub_41E910(char* a1);
_DWORD* __cdecl sub_41E940(int a1);
_DWORD* __cdecl sub_41E970(int a1);
LPVOID __cdecl sub_41E990(char* a1);
int* __cdecl sub_41E9D0(int a1, const char* a2, const char* a3, int a4, int a5);
int* sub_41EAC0();
int sub_41EB40();
void* __cdecl sub_41EBB0(int a1);
int sub_41EC00();
int sub_41EC10();
int sub_41EC30();
void __cdecl sub_41ECB0(int a1);
void __cdecl sub_41EEA0(int a1);
wchar_t* __cdecl sub_41EFB0(int a1);
wchar_t* sub_41F070();
wchar_t* sub_41F0A0();
int sub_41F0C0();
int sub_41F0E0();
int __cdecl sub_41F100(char* a1);
wchar_t* __cdecl sub_41F140(wchar_t* a1);
BOOL __cdecl nox_xxx_unused_41F160(const char* a1);
int __cdecl sub_41F1C0(int a1);
int __cdecl sub_41F1E0(int a1);
int sub_41F210();
int sub_41F220();
void __cdecl sub_41F230(int a1, int a2);
wchar_t* __cdecl sub_41F330(int a1, int a2);
int sub_41F360();
_DWORD* __cdecl sub_41F370(int a1);
void __cdecl sub_41F440(int a1, char a2, int a3);
int __cdecl sub_41F460(int a1, int a2);
int sub_41F4B0();
void __cdecl sub_41F520(const char* a1);
void __cdecl sub_41F6F0(int a1);
char* sub_41F710();
char* __cdecl sub_41F740(int a1, char a2);
char* sub_41F780();
_DWORD* __cdecl sub_41F790(const char* a1);
int __cdecl sub_41F800(const char* a1);
void __cdecl sub_41F820(const char* a1);
_DWORD* __cdecl sub_41F840(const char* a1, int a2);
BOOL __cdecl sub_41F860(const char* a1, wchar_t* a2);
BOOL __cdecl sub_41F8F0(const char* a1, wchar_t* a2);
_DWORD* __cdecl sub_41F980(const char* a1, int a2);
int sub_41F9A0();
BOOL __cdecl sub_41F9E0(int a1);
char* sub_41FA40();
void __cdecl sub_41FA50(const char* a1);
int __cdecl sub_41FA80(const char* a1);
int __cdecl sub_41FAE0(const char* a1);
int sub_41FB60();
int __cdecl sub_41FB70(int a1);
int __cdecl sub_41FB90(int a1, _DWORD* a2, _DWORD* a3);
int __cdecl sub_41FBE0(_DWORD* a1, _DWORD* a2);
int __cdecl sub_41FC20(int a1);
int sub_41FC40();
int sub_41FC50();
int __cdecl sub_41FCA0(const char* a1, const char* a2);
int sub_41FCF0();
int nox_xxx_officialStringCmp_41FDE0();
BOOL sub_41FEE0();
int sub_41FF10();
void __cdecl sub_41FF30(const void* a1);
int sub_41FF60();
BOOL __cdecl sub_41FF70(int a1);
int sub_41FF90();
BOOL __cdecl sub_41FFA0(int a1);
int sub_41FFC0();
BOOL __cdecl sub_41FFD0(int a1);
BOOL sub_41FFF0();
int sub_420020();
LSTATUS sub_420030();
int sub_4200E0();
int sub_4200F0();
int sub_420100();
char* sub_420110();
int __cdecl nox_xxx_regGetSerial_420120(LPBYTE lpData); // idb
void __cdecl sub_4201B0(_DWORD* a1);
LPVOID sub_420200();
int __cdecl sub_420230(char* a1, _WORD* a2);
int __cdecl sub_420360(char* a1, _WORD* a2);
_DWORD* sub_420580();
const char* __cdecl sub_4205B0(int a1);
int __cdecl sub_4206B0(int a1);
int __cdecl sub_4207A0(int a1);
int __cdecl sub_4207D0(int a1);
int sub_4207E0();
void __cdecl sub_4207F0(int a1);
void __cdecl nox_xxx_net_420830(_DWORD* a1);
_DWORD* __cdecl nox_xxx_net_CreateMsgList_420890(int a1, int a2);
void __cdecl sub_4208F0(LPVOID lpMem); // idb
int __cdecl nox_xxx_net_420940(int a1, int a2, int a3, int a4);
void __cdecl nox_xxx_net_4209C0(int a1, _QWORD* a2);
void __cdecl sub_4209E0(int* a1, int a2);
_DWORD* __cdecl sub_420A60(_DWORD* a1, int(__cdecl* a2)(_DWORD, int), int a3);
_DWORD* __cdecl nox_xxx_netlistGetList_420A90(int a1, int ListSizePtr);
int******* __cdecl nox_xxx_unused_420B00(_DWORD* a1, _DWORD* a2);
int __cdecl sub_420B70(_DWORD* a1, _DWORD* a2, int(__cdecl* a3)(_DWORD, int), int a4);
int __cdecl nox_xxx_netSend_420BC0(int a1);
int __cdecl nox_xxx_net_420BD0(int a1);
int __cdecl sub_420BE0(int a1, _DWORD* a2);
int __cdecl sub_420C10(int a1, _DWORD* a2);
_DWORD* __cdecl sub_420C40(int a1, int a2);
_DWORD* sub_420C70();
char* nox_xxx_polygon_420CA0();
char* __cdecl nox_xxx_polygon_420CD0(_DWORD* a1);
int sub_420D10();
unsigned int* __cdecl nox_xxx_polygonSetAngl_420D40(int a1, int a2, unsigned int a3, int a4);
unsigned int* __cdecl sub_420DA0(float a1, float a2);
int __cdecl sub_420E80(float, float, float); // idb
int __cdecl sub_420EE0(int a1);
int __cdecl sub_420EF0(_DWORD* a1);
char* sub_421010();
char* __cdecl nox_xxx_polygonGetUgol_421030(int a1);
void __cdecl sub_421040(int a1);
char* nox_xxx_polygonGetNext_4210A0();
char* __cdecl sub_4210E0(int a1);
int sub_421130();
int __cdecl sub_421160(int a1);
int __cdecl sub_4211D0(int a1);
unsigned __int8* sub_421230();
int __cdecl sub_4212C0(int a1);
LPVOID sub_421430();
char* __cdecl nox_xxx_polygonGetByIdx_4214A0(int a1);
void sub_4214D0();
int __cdecl nox_xxx_polygon_421660(int* a1, int a2);
int __cdecl nox_xxx_polygonGetIdxA_421790(int2* a1, int a2); // idb
struc_19* __cdecl nox_xxx_polygonIsPlayerInPolygon_4217B0(int2*, int);
int __cdecl sub_421880(int, int, float); // idb
int __cdecl sub_421960(int a1, float a2, int a3);
int* __cdecl sub_421990(int2*, float, int);
__int16 sub_421A30();
_DWORD* sub_421B10();
int __cdecl sub_421B40(_DWORD* a1);
void nox_xxx_polygonDrawColor_421B80();
void __cdecl nox_xxx_questCheckSecretArea_421C70(int a1);
unsigned __int8* __cdecl sub_421F10(int* a1, int a2);
void __cdecl nox_xxx_monsterPolygonEnter_421FF0(int a1);
int __cdecl sub_422140(int a1);
int* __cdecl nox_xxx_tileListAddNewSubtile_422160(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_tileFreeTileOne_4221E0(int a1);
int __cdecl nox_xxx_tileFreeTile_422200(int a1);
int __cdecl nox_server_mapRWFloorMap_422230(int a1);
unsigned __int8 __cdecl nox_xxx_tileReadOne_422A40(int a1, _BYTE* a2);
int __cdecl nox_xxx_tile_422C10(int a1, int a2);
int __cdecl nox_xxx_effectFloatValueLoad_4235C0(const char* a1, char* a2, obj_412ae0_t* a3);
int __cdecl nox_xxx_effectLoadFloatParam_4235F0(const char* a1, float a2, obj_412ae0_t* a3); // idb
double nox_xxx_effectLoadFloat_423730_parse_float();
int __cdecl nox_xxx_effectDwordValueLoad_423780(const char* a1, char* a2, obj_412ae0_t* a3);
int __cdecl nox_xxx_effectLoadDwordParam_4237B0(const char* a1, int a2, obj_412ae0_t* a3);
int nox_xxx_effectLoadDword_4238F0_parse_int();
void __cdecl set_bitmask_flags_from_plus_separated_names_423930(const char* input, _DWORD* bitmask,
																const char** allowed_names);
int __cdecl set_one_bitmask_flag_by_name_4239C0(char* name, _DWORD* bitmask, const char** allowed_names);
void __cdecl set_bitmask_flags_from_plus_separated_names_multiple_423A10(const char* input, _DWORD* bitmask);
int __cdecl nox_parse_shape(nox_shape*, char*);
int sub_423BB0();
BOOL sub_423C60();
BOOL __cdecl sub_423CC0(int a1);
int nox_xxx_noxCDinDriver_423CF0();
unsigned __int8 sub_423EC0();
BOOL __cdecl sub_423ED0(LPCSTR lpRootPathName, DWORD SectorsPerCluster);
HANDLE __cdecl sub_423F80(LPCSTR lpFileName, int a2, int a3, int a4);
int __cdecl sub_4240F0(int a1, const char* a2, int a3);
int __cdecl nox_xxx_parseSoundSetBin_424170(char* a1);
void sub_4242C0();
int __cdecl nox_xxx_monsterGetSoundSet_424300(int a1);
int __cdecl nox_xxx_setNPCVoiceSet_424320(int a1, int a2);
const char** __cdecl nox_xxx_getDefaultSoundSet_424350(const char* a1);
LPVOID sub_4243C0();
int __cdecl sub_4243D0(int a1);
void sub_4243E0();
int __cdecl nox_xxx_spellNameToN_4243F0(const char* a1);
int __cdecl nox_xxx_spellGetAud44_424800(int a1, int a2);
void* nox_xxx_spellGetDefArrayPtr_424820();
int __cdecl nox_xxx_updateSpellRelated_424830(int a1, int a2);
char* __cdecl nox_xxx_spellNameByN_424870(int a1);
int __cdecl nox_xxx_enchantByName_424880(const char* a1);
char* __cdecl sub_4248F0(int a1);
int __cdecl sub_424900(int a1);
int __cdecl nox_xxx_getEnchantSpell_424920(int a1);
int __cdecl nox_xxx_spellLoadName_424930(int a1);
int __cdecl sub_424960(wchar_t* a1);
int __cdecl nox_xxx_getManaCost_4249A0(int a1, int a2);
char* __cdecl nox_xxx_spellGetPhonemeMB_424A20(int a1);
int __cdecl nox_xxx_spellGet_424A30(int a1);
BOOL __cdecl nox_xxx_spellDefHasFlags_424A50(int a1, int a2);
int __cdecl nox_xxx_spellGetFlags_424A70(int a1);
int __cdecl nox_xxx_spellGetIcon_424A90(int a1);
int __cdecl nox_xxx_spellGetIconHighlight_424AB0(int a1);
int nox_xxx_bookFirstKnownSpell_424AD0();
int __cdecl nox_xxx_bookNextEnabledSpell_424AF0(int a1);
int __cdecl sub_424B20(int a1);
int __cdecl nox_xxx_spellGetValidMB_424B50(int a1);
int __cdecl nox_xxx_spellIsEnabled_424B70(int a1);
int __cdecl sub_424B90(int a1);
int __cdecl nox_xxx_spellDisable_424BB0(int a1);
int sub_424BD0();
unsigned int __cdecl nox_xxx_spellMayTrap_424BF0(int a1);
__int16 __cdecl nox_xxx_spellGetWorth_424C40(float a1);
char __cdecl sub_424CB0(int a1);
int sub_424D00();
int __cdecl sub_424D20(int a1);
void sub_424D60();
void sub_424D70();
int __cdecl nox_xxx_abilityNameToN_424D80(const char* a1);
int nox_xxx_loadWariorParams_424DF0();
int __cdecl sub_425230(int a1, int a2);
char* __cdecl nox_xxx_abilityGetName_425250(int a1);
int __cdecl nox_xxx_abilityGetName_0_425260(int a1);
int __cdecl nox_xxx_abil_425290(wchar_t* a1);
int __cdecl nox_xxx_abilityCooldown_4252D0(int a1);
int __cdecl sub_4252F0(int a1);
int __cdecl nox_xxx_spellGetAbilityIcon_425310(int a1, int a2);
int nox_xxx_bookFirstKnownAbil_425330();
int __cdecl nox_xxx_bookNextKnownAbil_425350(int a1);
int __cdecl sub_425380(int a1);
int __cdecl sub_4253B0(int a1);
int __cdecl sub_4253D0(int a1);
int __cdecl sub_4253F0(int a1);
int __cdecl sub_425410(int a1);
BOOL __cdecl sub_425430(int a1, int a2);
int __cdecl sub_425450(int a1);
int __cdecl sub_425470(int a1);
void sub_425490();
int __cdecl sub_4254A0(int a1, _BYTE* a2);
bool __cdecl sub_4254C0(unsigned __int8** a1);
_BYTE* __cdecl sub_425500(int a1, _BYTE* a2, char a3);
char __cdecl sub_425520(int a1, char a2);
int __cdecl sub_425550(_BYTE* a1, _BYTE* a2, int a3);
unsigned int __cdecl sub_4255F0(_BYTE* a1, _BYTE* a2, int a3);
int __cdecl sub_425680(_BYTE* a1, _BYTE* a2);
int __cdecl sub_425710(int a1, int a2);
_DWORD* __cdecl sub_425760(_DWORD* a1);
_DWORD* __cdecl sub_425770(_DWORD* a1);
int __cdecl sub_425790(int* a1, _DWORD* a2);
_DWORD* __cdecl sub_4257F0(int* a1, _DWORD* a2);
_DWORD* __cdecl sub_425840(_DWORD* a1, int a2);
int __cdecl sub_425870(_DWORD** a1);
int* __cdecl nox_xxx_gameMapsValidateListMB_425890(int* a1);
int* __cdecl sub_4258A0(int* a1);
_DWORD* __cdecl sub_4258C0(_DWORD** a1, int a2);
_DWORD* __cdecl sub_4258E0(int a1, _DWORD* a2);
_DWORD* __cdecl sub_425900(_DWORD* a1, _DWORD* a2);
_DWORD** __cdecl sub_425920(_DWORD** a1);
int __cdecl sub_425940(int* a1);
int __cdecl sub_425960(int a1);
_DWORD* __cdecl sub_425980(_DWORD* a1);
int __cdecl sub_4259A0(int a1);
_DWORD* sub_4259C0();
int* sub_4259F0();
int* sub_425A50();
int* __cdecl sub_425A60(int* a1);
int* __cdecl sub_425A70(int a1);
int __cdecl sub_425AA0(int a1);
wchar_t* __cdecl sub_425AD0(int a1, wchar_t* a2);
_DWORD* __cdecl sub_425B30(int a1, int a2);
char* __cdecl sub_425B60(LPVOID lpMem, int a2);
int* __cdecl sub_425BC0(int a1);
int* __cdecl sub_425BE0(int* a1);
int nox_xxx_countObserverPlayers_425BF0();
char* nox_xxx_firstReplaceablePlayer_425C40();
char* __cdecl nox_xxx_nextReplaceablePlayer_425C70(int a1);
char* __cdecl sub_425CA0(int a1, int a2);
int __cdecl sub_425E90(int a1, char a2);
int __cdecl sub_425ED0(int a1, char a2);
int __cdecl sub_425F10(int a1);
char* sub_426150();
char* nox_xxx_net_4263C0();
BOOL sub_4264D0();
void* sub_426590();
void __cdecl sub_4265A0(LPVOID lpMem); // idb
int __cdecl sub_426600(int a1, void(__cdecl* a2)(_DWORD, _DWORD));
int __cdecl sub_426650(int a1, char* a2, int a3);
CHAR* __cdecl sub_426680(int a1, char* a2);
CHAR* __cdecl sub_426740(int a1, CHAR* a2);
int __cdecl sub_426780(int a1, char* a2, int a3);
int __cdecl sub_4267B0(int a1, char* a2, int a3);
int __cdecl sub_4267E0(int a1, void* a2);
void* __cdecl sub_426800(int a1, void* a2);
int __cdecl sub_426840(const void*, const void*); // idb
double __cdecl sub_426860(int a1, void* a2);
int __cdecl sub_426890(int a1, void* a2);
int __cdecl sub_4268B0(int a1);
void sub_4268D0();
int nox_xxx_file_4268E0();
int __cdecl nox_xxx_cryptOpen_426910(char* a1, int a2, int a3);
FILE* nox_xxx_cryptClose_4269F0();
int __cdecl sub_426A20(int a1);
int nox_xxx_wallGet_426A30();
int nox_xxx_cryptGet_426A40();
int __cdecl nox_xxx_cryptSetTypeMB_426A50(int a1);
FILE* nox_xxx_mapgenGetSomeFile_426A60();
char* nox_xxx_mapGetWallSize_426A70();
int* __cdecl nox_xxx_mapWall_426A80(int* a1);
int __cdecl sub_426AA0(int a1);
size_t __cdecl nox_xxx_fileReadWrite_426AC0_file3_fread(_BYTE* a1, size_t a2);
int __cdecl sub_426BD0(unsigned __int8* a1, int a2);
void __cdecl nox_xxx_fileCryptReadCrcMB_426C20(_BYTE* a1, size_t a2);
void __cdecl nox_xxx_crypt_426C90();
void __cdecl nox_xxx_crypt_426D40();
int __cdecl nox_xxx_mapWriteSectionsMB_426E20(int a1);
BOOL __cdecl nox_xxx_mapReadSection_426EA0(int a1, const char* a2, _DWORD* a3);
BOOL __cdecl sub_426F40(int a1, const char* a2, _DWORD* a3, int(__cdecl* a4)(int));
void sub_426FF0();
void sub_427000();
int __cdecl nox_xxx_guide_427010(const char* a1);
char* __cdecl nox_xxx_guideNameByN_427230(int a1);
int __cdecl nox_xxx_guiCreatureGetName_427240(int a1);
int __cdecl nox_xxx_guidesTTByName_427270(wchar_t* a1);
int __cdecl nox_xxx_creatureIsCharmableByTT_4272B0(int a1);
int __cdecl nox_xxx_guideGetDescById_4272E0(int a1);
int nox_xxx_bookGetFirstCreMB_427300();
int __cdecl nox_xxx_bookGetNextCre_427320(int a1);
int __cdecl nox_xxx_unused_427350(int a1);
BOOL __cdecl nox_xxx_unused_427380(int a1);
int __cdecl sub_4273A0(int a1);
int __cdecl sub_4273C0(int a1);
int __cdecl sub_4273E0(int a1);
int __cdecl nox_xxx_bookGetCreatureImg_427400(int a1);
int __cdecl sub_427430(int a1);
unsigned __int8 __cdecl nox_xxx_guideGetUnitSize_427460(int a1);
void sub_427480();
_BYTE* __cdecl nox_xxx_journalEntryAdd_427490(int a1, char* a2, __int16 a3);
void __cdecl nox_xxx_comJournalEntryAdd_427500(int a1, char* a2, __int16 a3);
int __cdecl nox_xxx_comAddEntryAll_427550(char* a1, __int16 a2);
int __cdecl nox_xxx_journalEntryRemove_427590(int a1, const char* a2);
void __cdecl nox_xxx_comJournalEntryRemove_427630(int a1, const char* a2);
int __cdecl nox_xxx_comRemoveEntryAll_427680(const char* a1);
int __cdecl nox_xxx_journalUpdateEntry_4276B0(int a1, const char* a2, __int16 a3);
int __cdecl nox_xxx_comJournalEntryUpdate_427720(int a1, const char* a2, __int16 a3);
int __cdecl nox_xxx_comUpdateEntryAll_427770(const char* a1, __int16 a2);
int __cdecl sub_4277B0(int a1, unsigned __int16 a2);
_DWORD* __cdecl sub_427820(int a1);
_DWORD* __cdecl sub_427850(int a1);
BOOL __cdecl sub_427880(const char* a1);
int __cdecl sub_4278B0(float* a1, float* a2, float* a3);
int __cdecl sub_427980(float4* a1, float4* a2);    // idb
int __cdecl sub_427C80(int4* a1, int4* a2);        // idb
int __cdecl sub_427DF0(int a1, int* a2, float a3); // idb
int __cdecl nox_xxx_wallMath_427F30(int2*, int*);
int __cdecl sub_4280E0(int2* a1, _DWORD* a2); // idb
int __cdecl sub_428170(_DWORD*, int4*);
int __cdecl sub_4281F0(int2*, int4*);
BOOL __cdecl sub_428220(float2* a1, float4* a2); // idb
void __cdecl nox_shape_box_calc(nox_shape*);
char* __cdecl sub_4282D0(char* a1, int a2);
void* __cdecl sub_4282F0(int a1, int a2, size_t a3);
unsigned int __cdecl sub_428540(int a1, char* a2, int a3);
char* __cdecl sub_4285C0(__int16* a1);
BOOL __cdecl sub_428810(int a1, int a2);
BOOL __cdecl sub_428890(__int16* a1);
LPVOID __cdecl sub_428910(LPVOID* a1);
LPVOID __cdecl sub_4289D0(LPVOID* a1);
int nox_server_mapRWObjectTOC_428B30();
int __cdecl nox_server_mapRWPolygons_428CD0(int a1);
int nox_server_mapRWAmbientData_429200();
int __cdecl nox_server_mapRWWindowWalls_4292C0(_DWORD* a1);
void __cdecl sub_429450(_BYTE* a1, _DWORD* a2);
void __cdecl sub_4294B0(_BYTE* a1, _DWORD* a2);
void nox_xxx_wallBreakableCounterClear_429520();
int __cdecl nox_server_mapRWDestructableWalls_429530(_DWORD* a1);
void __cdecl nox_xxx_wall_4296E0(_BYTE* a1, _DWORD* a2);
void __cdecl sub_429740(_BYTE* a1, _DWORD* a2);
void nox_xxx_wallSecretCounterClear_4297B0();
int __cdecl nox_server_mapRWSecretWalls_4297C0(_DWORD* a1);
void __cdecl sub_429A00(_BYTE* a1, _DWORD* a2);
void __cdecl sub_429A60(int a1, _DWORD* a2);
int __cdecl nox_server_mapRWWallMap_429B20(_DWORD* a1);
int __cdecl sub_42A0F0(int a1);
int __cdecl sub_42A150(__int16 a1, _DWORD* a2);
int __cdecl sub_42A650(unsigned __int8* a1);
unsigned __int8 __cdecl nox_xxx_wall_42A6C0(unsigned __int8 a1, unsigned __int8 a2);
int nox_server_mapRWMapInfo_42A6E0();
uint16_t* __cdecl sub_42A8B0(_BYTE* a1, int* a2);
int __cdecl sub_42A970(_BYTE* a1, _BYTE* a2, int* a3);
double __cdecl sub_42AAA0(int* a1);
void __cdecl sub_42ABF0(int a1, int a2, int a3);
_BYTE* __cdecl sub_42AC50(_BYTE* a1, size_t* a2);
uint16_t* __cdecl sub_42ADA0(int a1, int a2, __int16 a3, unsigned int* a4);
uint16_t* __cdecl sub_42B810(__int16* a1, unsigned int* a2);
int __thiscall sub_42BCE0(_DWORD* this, char* a2, char a3);
int __thiscall sub_42BD50(_DWORD* this, char* a2, char a3);
int __thiscall sub_42BDC0(_DWORD* this, char* a2, char a3);
int __thiscall sub_42BE30(_DWORD* this, char* a2, const char* a3);
int __thiscall sub_42BEA0(_DWORD* this, char* a2, const void* a3, unsigned __int16 a4);
int sub_42BFB0();
int sub_42BFE0();
int __cdecl sub_42C210(int a1, _WORD* a2, _WORD* a3);
__int16 __cdecl nox_xxx_objectTOCgetTT_42C2B0(__int16 a1);
__int16 __cdecl sub_42C2E0(int a1);
__int16 sub_42C300();
LPVOID __cdecl sub_42C310(unsigned __int16 a1, __int16 a2);
void __thiscall sub_42C330(_DWORD* this);
int __thiscall sub_42C360(_DWORD* this, int a2);
uint16_t* __thiscall sub_42C370(uint16_t* this, uint16_t* a2);
uint16_t* __thiscall sub_42C480(_DWORD* this, unsigned int* a2);
int __thiscall sub_42C580(_DWORD* this, char* a2);
bool __thiscall sub_42C5C0(_DWORD* this, char* a2, _BYTE* a3);
bool __thiscall sub_42C5E0(_DWORD* this, char* a2, _BYTE* a3);
bool __thiscall sub_42C600(_DWORD* this, char* a2, _WORD* a3);
bool __thiscall sub_42C630(_DWORD* this, char* a2, _WORD* a3);
bool __thiscall sub_42C660(_DWORD* this, char* a2, _DWORD* a3);
bool __thiscall sub_42C680(_DWORD* this, char* a2, _DWORD* a3);
bool __thiscall sub_42C6A0(_DWORD* this, char* a2, char* a3);
bool __thiscall sub_42C6E0(_DWORD* this, char* a2, _DWORD* a3);
bool __thiscall sub_42C700(_DWORD* this, char* a2, _DWORD* a3);
bool __thiscall sub_42C720(_DWORD* this, char* a2, void* a3, unsigned int* a4);
int __thiscall sub_42C770(LPVOID* this);
int __thiscall sub_42C7C0(int this, char* a2, char a3);
int __thiscall sub_42C7F0(int this, char* a2, char a3);
int __thiscall sub_42C820(int this, char* a2, char a3);
int __thiscall sub_42C850(int this, char* a2, char a3);
int __thiscall sub_42C880(int this, char* a2, char a3);
int __thiscall sub_42C8B0(int this, char* a2, char a3);
int __thiscall sub_42C8E0(int this, char* a2, const char* a3);
int __thiscall sub_42C910(int this, char* a2, const void* a3, unsigned __int16 a4);
char* __thiscall sub_42C940(LPVOID* this, char* a2, char a3);
char* __thiscall sub_42C9A0(LPVOID* this, char* a2, char a3);
char* __thiscall sub_42CA00(LPVOID* this, char* a2, char a3);
char* __thiscall sub_42CA60(LPVOID* this, char* a2, char a3);
char* __thiscall sub_42CAC0(LPVOID* this, char* a2, char a3);
char* __thiscall sub_42CB20(LPVOID* this, char* a2, char a3);
char* __thiscall sub_42CB80(LPVOID* this, char* a2, const char* a3);
void* __thiscall sub_42CBF0(LPVOID* this, char* a2, const void* a3, unsigned __int16 a4);
int __thiscall sub_42CC50(LPVOID* this); // weak
int __thiscall sub_42CC60(unsigned __int16* this);
uint16_t __thiscall sub_42CC70(int this);
__int16 __thiscall sub_42CCE0(uint16_t* this);
char* __cdecl nox_xxx_netGetBuffSize_42CD50(_BYTE* a1);
LPVOID nox_xxx_getBindKeysBuf_42CD70();
void* __cdecl sub_42CD80(void* a1);
void sub_42CD90();
_DWORD* __cdecl sub_42CDF0(FILE* a1);
int __cdecl sub_42CF50(const char* a1);
unsigned int sub_42D220();
int __cdecl sub_42D440(int a1);
unsigned __int8 __cdecl sub_42D450(int a1);
int __cdecl nox_xxx_keyCheckWarrorKeys_42D460(int a1);
int __cdecl nox_xxx_keyCanPauseMode_42D4B0(int a1);
unsigned int nox_xxx_netBuf_42D510();
char* sub_42E600();
void sub_42E620();
int sub_42E630();
void nox_xxx_keys_42E670(int a1, int a2);
void nox_xxx_keys_42E780(int a1, int a2);
void __cdecl nox_xxx_clientTalk_42E7B0(int a1);
void __cdecl nox_xxx_clientCollideOrUse_42E810(int a1);
void __cdecl nox_xxx_clientTrade_42E850(int a1);
void __cdecl nullsub_33(_DWORD, _DWORD, _DWORD); // weak
void __cdecl sub_42E8C0(int a1);
char* __cdecl sub_42E8E0(int a1, int a2);
char* __cdecl sub_42E960(wchar_t* a1);
char* __cdecl sub_42E9B0(char* a1);
char* __cdecl sub_42EA00(char* a1);
int __cdecl sub_42EA40(wchar_t* a1);
char* __cdecl sub_42EA90(char* a1);
int __cdecl sub_42EB90(int a1);
int sub_42EBA0();
void sub_42EBB0(unsigned int a1, void (*fnc)(int), int field_4, const char* name);
void sub_42ED20();
void sub_42ED70();
void sub_42EDC0();
int sub_42EE00();
int __cdecl nox_video_read_videobag(int a1);
int sub_42EFF0();
char* sub_42F030();
FILE* __cdecl nox_xxx_fopenVideoBag_42F0B0(int a1);
int __cdecl sub_42F1B0(int* a1, int* a2);
int __cdecl sub_42F200(unsigned __int8* a1, int a2);
int __cdecl nox_video_parse_videobag(int a1);
BOOL __cdecl nox_video_reopen_videobag(char* a1);
void sub_42F4D0();
int __cdecl nox_xxx_imgLoadAlloc_42F610(int a1);
int __cdecl nox_xxx_imgLoad_42F660(int a1, void* a2);
char* __cdecl nox_xxx_gLoadImg_42F970(const char* a1);
char* __cdecl nox_xxx_gLoadAnim_42FA20(const char* a1);
char* __cdecl nox_xxx_readImgMB_42FAA0(int known_idx, const char* a2, char* a3);
int sub_42FAD0();
_DWORD* __cdecl sub_42FAE0(int a1);
int __cdecl nox_xxx_tileEdgeCrashHere_42FB30(int a1);
int __cdecl sub_42FE30(int a1);
int __cdecl sub_42FF90(_DWORD* a1);
BOOL __cdecl sub_42FFF0(FILE* a1);
BOOL __cdecl sub_4300D0(int a1);
char* __cdecl nox_video_getbagfilename(int a1);
char* __cdecl sub_430120(int a1);
void sub_430140(int a1);
int __cdecl nox_xxx_initInput_430190();
void sub_430210();
void sub_430230();
void sub_4302A0_process_events(int a1, int a2);
void __cdecl nox_client_readMouseBuffer_4306A0(int a1);
int nox_xxx_getKeyFromKeyboard_430710();
int sub_4307D0();
int __cdecl sub_430880(int a1);
int nox_xxx_get_430890();
int __cdecl nox_client_processMouseInput_4308A0(int a1);
char* nox_xxx_wndKeyGet_430940();
unsigned __int8 __cdecl sub_430950(unsigned __int8 a1);
unsigned __int8 __cdecl sub_430970(unsigned __int8 a1);
int __cdecl sub_430990(unsigned __int8 a1);
int __cdecl sub_4309B0(unsigned __int8 a1, char a2);
int __cdecl sub_4309D0(unsigned __int8 a1, char a2);
int2* __cdecl nox_client_getMousePos_4309F0();
void __cdecl sub_430A00_change_mouse_pos(int x, int y, int abs);
int __cdecl sub_430A70_set_mouse_bounds(int x_min, int x_max, int y_min, int y_max);
int __cdecl sub_430AA0(int a1);
int nox_xxx_cursor_430AF0();
int nox_xxx_cursor_430B00();
void __cdecl sub_430B10_set_mouse_pos(int x, int y);
int sub_430B30();
int nox_xxx_bookGet_430B40_get_mouse_prev_seq();
int __cdecl sub_430B50(int a1, int a2, int a3, int a4);
char* __cdecl sub_430B80(_DWORD* a1);
void __cdecl sub_430BB0(_DWORD* a1, _DWORD* a2, _DWORD* a3);
BOOL __cdecl nox_video_resizewnd(int a1, int a2, int a3);
void __cdecl sub_430C30_set_video_max(int w, int h);
void __cdecl nox_xxx_screenGetSize_430C50_get_video_max(int* w, int* h);
BOOL nox_xxx_testMMXSupport_430D40();
int __cdecl nox_xxx_tileInitBuf_430DB0(int a1, int a2);
int sub_430E70();
int nox_video_freeFloorBuffer_430EC0();
void sub_430EF0();
int __cdecl nox_audio_initall(int a3);
void sub_4311B0();
BOOL sub_4311F0();
void sub_431270();
int* sub_431290();
void sub_4312B0();
int sub_4312C0();
BOOL sub_431330();
int sub_431340();
BOOL sub_431370();
void sub_431380();
int nox_client_initScreenParticles_431390();
int nox_xxx_particlesLoadColor_4313E0();
void sub_4314D0();
int sub_431510();
nox_screenParticle* nox_client_newScreenParticle_431540(int a1, int a2, int a3, int a4, int a5, int a6, char a7, char a8, char a9, char a10);
void nox_client_addScreenParticle_431680(nox_screenParticle* p);
void sub_4316C0(nox_screenParticle* p);
void __cdecl sub_431700(_QWORD* a1);
void __cdecl nox_client_screenParticlesDraw_431720(int* a1);
char* nox_xxx_getHostInfoPtr_431770();
char* nox_xxx_unused_431780();
char* __cdecl nox_xxx_copyServerIPAndPort_431790(char* a1);
int nox_common_readcfgfile(const char* path, int a2);
int nox_common_parsecfg_all(FILE* a1);
int sub_431FC0();
int sub_432010();
int sub_4320B0();
int sub_432100();
int sub_432150();
int sub_4321A0();
int sub_4321F0();
int sub_432240();
int sub_432290();
int sub_4322E0();
int sub_432340();
int sub_432390();
int sub_4323E0();
int sub_432430();
int sub_432480();
int sub_4324D0();
int sub_432520();
int sub_432590();
int sub_4325D0();
int sub_432620();
int sub_432660();
int sub_4326B0();
int sub_432700();
int sub_432740();
int sub_432780();
int sub_4327C0();
int sub_432810();
int sub_432860();
int sub_4328C0();
int sub_432920();
int sub_432970();
int sub_4329D0();
int sub_432A30();
int sub_432A90();
int __cdecl sub_432AD0(int* a1);
int nox_common_parsecfg_videomode();
int nox_common_parsecfg_videosize();
int sub_432C00();
int sub_432C30();
int __cdecl sub_432C70(int a1);
int __cdecl sub_432CB0(int a1);
int sub_432D10();
int sub_432D80();
int sub_432DF0();
char* __cdecl sub_432E50(int a1);
int sub_433050();
int sub_4330C0();
int sub_433130();
int sub_433190();
int __cdecl sub_4331E0(FILE* a1, int a2);
void nox_common_writecfgfile(char* a1);
int __cdecl sub_4332E0(FILE* a1);
int __cdecl sub_4337B0(FILE* a1);
int __cdecl sub_433820(FILE* a1);
char* sub_433890();
int sub_4338D0();
void sub_433C20();
int __cdecl sub_433CD0(BYTE a1, BYTE a2, BYTE a3);
int __cdecl sub_433E40(int a1);
void __cdecl sub_433E80(unsigned __int8 a1, unsigned __int8 a2, unsigned __int8 a3);
void __cdecl sub_433ED0(int a1);
int __cdecl nox_xxx_drawMakeRGB_433F10(unsigned __int8 a1, unsigned __int8 a2, unsigned __int8 a3);
int __cdecl sub_434040(int a1);
int __cdecl sub_434080(int a1);
void __cdecl sub_4340A0(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_drawPlayer_4341D0(int a1, int a2);
void nox_set_color_rgb_434320(int a1, int a2, int a3);
int __cdecl nox_xxx_drawSelectColor_434350(int a1);
int sub_434370();
int __cdecl nox_xxx_drawSetTextColor_434390(int a1);
void nox_set_color_rgb_4343B0(int a1, int a2, int a3);
int __cdecl nox_xxx_drawSetColor_4343E0(int a1);
void nox_set_color_rgb_434400(int a1, int a2, int a3);
void nox_set_color_rgb_434430(int a1, int a2, int a3);
int __cdecl nox_client_drawSetColor_434460(int a1);
void __cdecl sub_434480(int a1, int a2, int a3, int a4);
uint32_t nox_color_rgb_4344A0(int r, int g, int b);
int __cdecl nox_xxx_unused_4344D0(_BYTE* a1);
char* __cdecl sub_434510(char* a1, unsigned __int8 a2, unsigned __int8 a3, unsigned __int8 a4);
int __cdecl nox_client_drawEnableAlpha_434560(int a1);
int __cdecl nox_client_drawSetAlpha_434580(unsigned __int8 a1);
int __cdecl sub_4345F0(int a1);
int __cdecl nox_xxx_draw_434600(int a1);
int __cdecl sub_434610(int a1);
int __cdecl sub_434620(int a1);
unsigned __int8 __cdecl sub_434630(unsigned __int8 a1, unsigned __int8 a2, unsigned __int8 a3);
char __cdecl sub_4347C0(char a1);
BOOL __cdecl sub_4347F0(char* a1, int a2);
int __cdecl sub_434820(int a1, char a2, char a3, char a4);
int __cdecl sub_434870(int a1, _BYTE* a2, _BYTE* a3, _BYTE* a4);
int sub_4348C0();
int sub_434920();
int sub_434950();
int __cdecl sub_434990(int a1, int a2, int a3);
int __cdecl sub_4349C0(_DWORD* a1);
int sub_434A00();
_DWORD* __cdecl sub_434A10(_DWORD* a1, _DWORD* a2, _DWORD* a3);
char __cdecl sub_434A60(int a1);
int __cdecl sub_434AA0(int a1, int a2, int a3);
int __cdecl sub_434AC0(int a1);
int nox_video_getGammaSetting_434B00();
int __cdecl nox_video_setGammaSetting_434B30(int a1);
BOOL sub_434B60();
int sub_434CC0();
int sub_434DA0();
char __cdecl sub_434E80(char a1, char a2, char a3);
char __cdecl sub_434EC0(char a1, char a2, char a3);
int sub_434F00();
int sub_434FB0();
void sub_435040();
_BYTE* __cdecl sub_4350E0(_BYTE* a1, _BYTE* a2);
void sub_435120(_BYTE* a1, char* a2);
void sub_435150(_BYTE* a1, char* a2);
void nox_color_func_435180(uint8_t r, uint8_t g, uint8_t b, uint32_t* color);
void nox_color_func_4351C0(uint8_t r, uint8_t g, uint8_t b, uint32_t* color);
void nox_color_func_435200(uint8_t r, uint8_t g, uint8_t b, uint32_t* color);
void __cdecl sub_435240(unsigned __int8 a1, _BYTE* a2, _BYTE* a3, char* a4);
void __cdecl sub_435280(__int16 a1, _BYTE* a2, _BYTE* a3, _BYTE* a4);
void __cdecl sub_4352E0();
void sub_4353C0();
void sub_4353F0();
void sub_4354F0();
void sub_435550();
__int64 nox_xxx_initTime_435570();
int sub_435590();
void sub_4355A0();
int __cdecl sub_4355B0(int a1);
BOOL sub_4355C0();
int __cdecl nox_xxx_cliUpdateCameraPos_435600(int a1, int a2);
void __cdecl nox_xxx_getSomeCoods_435670(int2*);
_DWORD* __cdecl sub_435690(_DWORD* a1);
int nox_xxx_gameGetPlayState_4356B0();
BOOL __cdecl nox_xxx_spriteTestBuf_4356C0(nox_drawable* dr, char a2);
void nox_xxx_spriteLoadError_4356E0();
char* nox_xxx_unused_4356F0();
wchar_t* __cdecl sub_435700(wchar_t* a1, int a2);
char* __cdecl sub_435720(char* a1);
DWORD sub_435740();
DWORD sub_435750();
DWORD nox_xxx_time_startProfile_435770();
DWORD nox_xxx_time_endProfile_435780();
int nox_xxx_cliToggleObsWindow_4357A0();
BOOL __cdecl nox_xxx_clientResetSpriteAndGui_4357D0(int argc, const char** argv);
int __cdecl nox_xxx_whenServerHostServer_435A10(signed int* a1);
int nox_xxx_initGameSession_435CC0();
int sub_435EB0();
int sub_435F40();
int sub_435F60();
int nox_xxx_client_435F80_draw();
BOOL sub_436550();
int sub_437060();
int sub_437100();
int4* sub_437180();
int nox_xxx_cliSetupSession_437190();
char* sub_437250();
int4* sub_437260();
int4* sub_437290();
BOOL nox_xxx_playerAnimCheck_4372B0();
BOOL nox_xxx_clientIsObserver_4372E0();
void sub_437310();
int __cdecl sub_437320(int a1);
void __cdecl sub_4375C0(int a1);
int __cdecl sub_4375F0(const char* a1, __int16 a2, char* a3, int a4);
int __cdecl sub_437860(int a1, int a2);
void sub_4379C0();
int sub_438330();
int sub_438370();
int sub_438480();
int nox_xxx_initSomethingNetGame_438A90();
int sub_438C80(int a1, int a2);
int __cdecl sub_438DD0(unsigned int a1, unsigned int a2);
int __cdecl sub_438E30(_DWORD* a1, int a2);
int __cdecl sub_438EF0(_DWORD* a1, int a2, unsigned int a3, int a4);
int __cdecl sub_439050(int a1, unsigned int a2, int* a3, unsigned int a4);
_DWORD* __cdecl sub_439450(int a1, int a2, _DWORD* a3);
char* __cdecl sub_439CC0(int a1, char* a2);
int __cdecl sub_439D00(int* a1, int a2, unsigned int a3, int a4);
int __cdecl sub_439D90(unsigned int a1, unsigned int a2);
int sub_43A920();
int sub_43A9D0();
char* sub_43AA70();
void sub_43ACC0();
int sub_43AF30();
int sub_43AF40();
int __cdecl sub_43AF50(int a1);
int sub_43AF70();
int sub_43AF80();
int __cdecl sub_43AF90(int a1);
int __cdecl sub_43AFA0(int a1);
void __cdecl sub_43AFC0(int a1);
void __cdecl nox_xxx_failconn_43B0E0(int a1);
int __cdecl sub_43B2A0(int a1);
unsigned int nox_xxx_inServerGetAddr_43B300();
int sub_43B320();
int sub_43B340();
int nox_xxx_cliDrawConnectedLoop_43B360();
int sub_43B440();
int sub_43B460();
int sub_43B490();
_DWORD* nox_xxx_serverHost_43B4D0();
char* sub_43B510();
_DWORD* sub_43B670();
int sub_43B6D0();
unsigned __int16* __cdecl sub_43BC10(wchar_t* a1, unsigned __int8 a2);
int __cdecl sub_43BC80(int a1, unsigned __int16 a2, char* a3);
int __cdecl sub_43BD90(int a1);
int sub_43BDB0();
void sub_43BDC0();
int __cdecl sub_43BDD0(int a1);
int sub_43BE10();
int sub_43BE30();
int __cdecl sub_43BE40(int a1);
int sub_43BE50_get_video_mode_id();
nox_video_mode* __cdecl sub_43BE80_video_mode_by_id(int a1);
void __cdecl nox_xxx_gameGetScreenBoundaries_43BEB0_get_video_mode(int* w, int* h, int* d);
void __cdecl nox_xxx_gameResizeScreen_43BEF0_set_video_mode(int w, int h, int d);
int __cdecl nox_xxx_video_43BF10_upd_video_mode(int defaults);
int nox_xxx_cliWaitForJoinData_43BFE0();
int nox_xxx_gameStateWait_43C020();
BOOL sub_43C060();
int sub_43C0A0();
void sub_43C1E0();
_DWORD* sub_43C220();
int sub_43C260();
int sub_43C2F0();
void sub_43C380();
nox_wnd_xxx* sub_43C500();
int __cdecl sub_43C510(int a1);
nox_wnd_xxx* sub_43C520(int a1);
nox_wnd_xxx* nox_wnd_sub_43C540();
void sub_43C570(nox_wnd_xxx* p); // idb
nox_wnd_xxx* nox_wnd_sub_43C5B0(_DWORD* a1, int a2, int a3, int a4, int a5, int a6, int a7, int a8, int a9);
void sub_43C640();
int sub_43C650();
BOOL sub_43C6E0();
int nox_client_isConnected_43C700();
BOOL sub_43C710();
int __cdecl nox_xxx_gameSetCliConnected_43C720(int a1);
int nox_xxx_netGet_43C750();
int sub_43C760();
int sub_43C790();
int __cdecl sub_43C7A0(int a1);
int __cdecl nox_xxx_netAddNetStruct4Host_43C7B0(char* cp, int hostshort, int a3, signed int* a4);
int __cdecl nox_xxx_netHandleCliPacket_43C860(int a1, unsigned __int8* a2, int a3);
int __cdecl nox_xxx_clientSendInput_43C8F0(int a1, int a2, int a3);
int nox_xxx_netSendClientReady_43C9F0();
int nox_xxx_netKeepAliveSocket_43CA20();
int nox_xxx_netRequestMap_43CA50();
int nox_xxx_netMapReceived_43CA80();
int nox_xxx_cliSendCancelMap_43CAB0();
int nox_xxx_netSendIncomingClient_43CB00();
int nox_xxx_cliSendOutgoingClient_43CB50();
int sub_43CBB0();
BOOL __cdecl sub_43CC60(int a1);
int sub_43CC80();
int sub_43CCA0();
int sub_43CEB0();
int sub_43CF40();
int sub_43CF70();
BOOL sub_43CFD0();
int sub_43CFF0();
void sub_43D0A0(int a1);
void sub_43D2C0();
void sub_43D2D0();
void __cdecl sub_43D3C0(int a1, int a2);
int sub_43D440();
int sub_43D650();
int sub_43D680();
int sub_43D6A0();
int __cdecl nox_xxx_musicStartPlay_43D6C0(int* a1);
int sub_43D8E0();
int sub_43D970();
void sub_43D990();
void sub_43D9B0(int a1, int a2);
void sub_43D9E0(int4*);
int sub_43DA80();
void sub_43DAD0();
void sub_43DB10();
int sub_43DB20();
int __cdecl sub_43DB30(int a1);
char* __cdecl sub_43DB40(int a1);
int sub_43DB60();
void sub_43DBA0();
int sub_43DBD0();
int sub_43DBE0();
void sub_43DC00();
int sub_43DC10();
int sub_43DC30();
int sub_43DC40();
int sub_43DC80();
int sub_43DCC0();
int __cdecl sub_43DD10(int* a1);
void sub_43DD70(int a1, int a2);
void sub_43DDA0();
void sub_43DDC0();
int __cdecl nox_xxx_setExitMenuOrHost_43DDD0(int a1);
int __cdecl sub_43DDE0(int a1);
int __cdecl nox_xxx_setGameState_43DDF0(int (*a1)(void));
int nox_return_1_43DE10();
int __cdecl nox_game_SetCliDrawFunc(int (*a1)(void));
int __cdecl sub_43DE40(int (*a1)(void));
int sub_43DE60();
_DWORD* __cdecl nox_xxx_gui_43E1A0(int a1);
int sub_43E200();
int sub_43E230();
void mainloop_43E290();
int nox_xxx_debugSub_43E8B0();
int __cdecl sub_43E8C0(int a1);
void sub_43E8D0();
void sub_43E8E0(int a1);
void sub_43E910(int a1);
int __cdecl sub_43E940(int a1);
int __cdecl sub_43E9C0(); // idb
_DWORD* __stdcall sub_43E9D0(int a1);
void sub_43E9F0();
int __cdecl sub_43EA20(int a1);
int __cdecl sub_43EA90(int a1, int a2);
int __cdecl sub_43EAD0(int a1);
int sub_43EC10();
int __cdecl sub_43EC30(int a1);
int __cdecl sub_43ECB0(int a1);
int __cdecl sub_43ED00(_DWORD* a1);
int __stdcall sub_43EDB0(int a1);
int __stdcall sub_43EDE0(int a1);
int __cdecl sub_43EE00(int a1);
int __cdecl sub_43EFD0(int a1);
int __cdecl sub_43F010(int a1);
int __cdecl sub_43F030(int a1);
int sub_43F050();
int __cdecl sub_43F060(_DWORD* a1);
int sub_43F0D0();
int __cdecl sub_43F0E0(_DWORD* a1);
int sub_43F130();
void sub_43F140(int a1);
int sub_43F1A0();
int nox_xxx_fontLoadMB_43F1C0();
void nox_xxx_FontDestroy_43F2E0();
int __cdecl nox_xxx_guiFontHeightMB_43F320(int a1);
int __cdecl nox_xxx_Font_43F340(int* a1);
int __cdecl nox_xxx_guiFontPtrByName_43F360(char* a1);
_DWORD* __cdecl nox_xxx_FontLoadFile_43F3B0(char* a1);
int __cdecl sub_43F620(LPVOID lpMem);
int __cdecl sub_43F670(int a1);
int __cdecl sub_43F680(int a1);
int __cdecl sub_43F690(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_drawString_43F6E0(int a1, __int16* a2, int a3, int a4);
int __cdecl sub_43F730(int a1, __int16* a2, int a3, int a4);
int __cdecl sub_43F7B0(int a1, __int16* a2, int a3, int a4);
int __cdecl nox_xxx_drawGetStringSize_43F840(int a1, unsigned __int16* a2, int* a3, _DWORD* a4, int a5);
int __cdecl sub_43F9E0(int a1, unsigned __int16* a2, int a3);
int __cdecl nox_xxx_bookGetStringSize_43FA80(int a1, unsigned __int16* a2, int* a3, _DWORD* a4, int a5);
int __cdecl nox_xxx_drawString_43FAF0(int a1, _WORD* a2, int a3, int a4, int a5, int a6);
int __cdecl sub_43FD00(int a1, _WORD* a2, int a3, int a4, int a5, int a6);
int __cdecl nox_xxx_bookDrawString_43FA80_43FD80(int a1, _WORD* a2, int a3, int a4, int a5, int a6);
int __cdecl sub_43FE20(int a1);
int __cdecl nox_xxx_FontGetChar_43FE30(int a1, unsigned __int16 a2);
int __cdecl nox_xxx_StringDraw_43FE90(int a1, __int16 a2, int xLeft, int yTop);
int __cdecl sub_440360(int, int, int xLeft, int yTop); // idb
int __cdecl nox_xxx_guiDrawString_4407F0(int a1, __int16* a2, int a3, int a4);
void __cdecl nox_xxx_Font_440840(LPVOID lpMem); // idb
int __cdecl nox_xxx_fontLoad_440870(int a1);
__int16 __cdecl sub_4408A0(_WORD* a1);
__int16 __cdecl sub_4408C0(_BYTE* a1);
int sub_4408E0();
void nullsub_3(void); // weak
void sub_440900();
int __cdecl nox_xxx_netServerCmd_440950(char a1, wchar_t* a2);
int __cdecl sub_440AC0(wchar_t* a1);
char* __cdecl sub_440BC0(int a1, unsigned __int8 a2, int a3);
void __cdecl sub_4417E0(wchar_t* a1, const char* a2);
int sub_4420F0();
int nox_xxx_conStartSoloQuest_4423A0();
int __cdecl nox_xxx_doExecrul_4438A0(int a1);
void __cdecl sub_4443B0(unsigned __int8 a1);
char* __cdecl sub_444410(int a1);
void sub_4445C0();
BOOL nox_xxx____crtGetStringTypeA_0_444830();
int __cdecl nox_xxx_GfxInit_444930(HWND wnd, int w, int h, int depth, int flags);
int __cdecl sub_4449D0(HWND wnd, int w, int h, int depth, int flags);
int __cdecl sub_444AC0(HWND wnd, int w, int h, int depth, int flags);
void sub_444C50();
void sub_444D00();
void sub_444D30();
void __cdecl sub_444D50(obj_5D4594_3799572_t* a1);
void __cdecl sub_444D70(obj_5D4594_3799572_t* a1);
BOOL nox_xxx_testCPUID2_444D90();
signed int sub_444DD0();
HDC sub_444F90();
void __cdecl sub_444FC0(HDC a1);
int __stdcall nox_xxx_windowProc_444FF0(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam); // idb
int nox_xxx_processWinMessages_4453A0_poll_events();
int sub_445440();
wchar_t* sub_445450();
int nox_xxx_drawMessageLines_445530();
int __cdecl nox_xxx_guiChatMode_4456E0(int* a1);
int __cdecl nox_xxx_guiChatShowHide_445730(int a1);
BOOL sub_445750();
int sub_445770();
void nox_xxx_quitDialogYes_445B20(); // weak
int nox_xxx_quitDialogNo_445B30();
void sub_445BA0();
int sub_445BB0();
int __cdecl nox_xxx_wndDrawQuiteMenu_445BC0(_DWORD* a1);
void sub_445C00();
void sub_445C20();
_DWORD* sub_445FF0();
BOOL sub_446030();
int sub_446050();
void sub_446060();
void sub_446070();
BOOL sub_446090();
int __cdecl sub_4460A0(int a1);
int sub_4460B0();
int sub_4460C0();
int sub_446140();
int nox_xxx_serverIsClosing_446180();
unsigned int sub_446360();
void sub_446380();
int __cdecl nox_motd_4463E0(int a1);
LPVOID __cdecl sub_446490(void* a1);
LPVOID __cdecl sub_4464D0(int a1, _DWORD* a2);
int __cdecl sub_4464F0(int a1, _DWORD* a2);
void __cdecl sub_446520(int a1, const void* a2, int a3);
int __cdecl sub_446580(int a1);
int nox_xxx_guiMotdLoad_4465C0();
int __cdecl sub_4466C0(int a1, int a2, int a3, int a4);
char* __cdecl sub_4466F0(char* a1, _BYTE* a2);
_BYTE* __cdecl nox_xxx_motdAddSomeTextMB_446730(_BYTE* a1);
int sub_446780();
void nox_xxx_motd_4467F0();
int __cdecl sub_446940(int a1);
BOOL sub_446950();
wchar_t* __cdecl sub_4469D0(int a1);
int sub_446A90();
void sub_446AD0(int (*fnc)(void));
char* __cdecl sub_446BC0(int a1);
int __cdecl sub_446C10(int a1, int a2);
int sub_446C70();
BOOL sub_446C90();
wchar_t* __cdecl sub_446CC0(int a1);
void sub_446D50();
void __cdecl sub_446D80(char* a1);
char* __cdecl sub_446E20(char* a1, _BYTE* a2);
wchar_t* sub_446E60();
char* sub_446EB0();
char* __cdecl sub_446F40(int a1);
const wchar_t* __cdecl sub_4471A0(const char* a1, int a2, int a3, int a4);
const wchar_t* __cdecl sub_447250(const char* a1, int a2, int a3, int a4);
int __cdecl sub_447310(int a1, int a2);
wchar_t* __cdecl sub_447380(const char* a1, char* a2);
unsigned int __cdecl sub_447410(char* a1);
int sub_4475E0();
void sub_447600();
int sub_447BD0();
int __cdecl sub_447BF0(_DWORD* a1, int a2, unsigned int a3, int a4);
int __cdecl sub_447C70(int a1, int a2, int a3, int a4);
int __cdecl sub_447CC0(int a1, unsigned int a2, int* a3, int a4);
int __cdecl sub_448140(int yTop); // idb
int __cdecl sub_448240(_DWORD* a1, _BYTE* a2);
int __cdecl sub_4482D0(int a1, int a2, int a3, int a4, _WORD* a5);
int __cdecl sub_448340(int a1, unsigned int a2);
void sub_448380();
int __cdecl sub_4483A0(int a1, int a2);
int sub_448450();
int sub_448490();
void __cdecl sub_4484D0(wchar_t* a1, int a2, int a3);
int __cdecl sub_448550(int a1);
void __cdecl sub_448570(const char* a1, char a2, int a3);
int __cdecl sub_448620(int a1);
int sub_448640(void); // weak
int __cdecl sub_448650(_BYTE* a1, wchar_t* a2);
int __cdecl sub_448680(wchar_t* a1, char* a2);
int __cdecl sub_448F00(_DWORD* a1, int a2, int a3, int a4);
int __cdecl sub_448F60(_DWORD* a1, int a2, int a3, int a4);
int __cdecl sub_448FC0(int a1, int a2);
int sub_449280();
int __cdecl sub_4493D0(int a1, int a2, int* a3, int a4);
void sub_4494A0();
int nox_xxx_guiServerListLoad_449530();
_DWORD* __cdecl nox_xxx_dialogMsgBoxCreate_449A10(int a1, int a2, int a3, int a4, void (*a5)(void), void (*a6)(void));
int __cdecl sub_449BE0(int a1, int a2, int a3, int a4);
int __cdecl sub_449C30(_DWORD* a1, int a2, int a3, int a4);
int __cdecl nox_xxx_windowDlgProc_449CA0(int a1, int a2, int* a3, int a4);
int __cdecl sub_449E00(int a1);
int __cdecl sub_449E30(int a1);
int __cdecl sub_449E60(char a1);
void __cdecl sub_449EA0(int a1);
int __cdecl sub_44A0B0(int a1);
int __cdecl sub_44A200(int a1);
int __cdecl sub_44A340(int a1, int a2);
void sub_44A360(int a1);
int sub_44A400();
BOOL sub_44A4A0();
int sub_44A4B0();
int __cdecl sub_44A4D0(int a1);
int sub_44A4E0();
char* __cdecl sub_44A520(int a1);
int sub_44AA40();
int sub_44AA70();
int sub_44AAC0();
int __cdecl sub_44AAD0(_DWORD* a1, int a2, int a3, int a4);
int __cdecl sub_44AB30(int a1, unsigned int a2, int* a3, int a4);
int sub_44AF70();
_DWORD* __cdecl sub_44AFB0(int a1, int a2, int a3);
void nox_xxx____setargv_4_44B000();
int __cdecl sub_44B0F0(int a1, int a2, int* a3, int a4);
int __cdecl sub_44B940(_DWORD* a1, int a2, int a3);
int __cdecl nox_xxx_parse_Armor_44BA60(const char* a1);
int __cdecl sub_44BAC0(const char* a1);
int __cdecl sub_44BB20(const char* a1);
CHAR* __cdecl nox_xxx_spriteDefByAlphabetAdd_44CD10(CHAR* a1);
int __cdecl nox_xxx_keyFirstLetterNumberCli_44CD30(CHAR* a1);
char* __cdecl nox_get_thing_name(int a1);
nox_thing* __cdecl nox_get_thing(int i);
wchar_t* __cdecl nox_get_thing_pretty_name(int a1);
wchar_t* __cdecl nox_get_thing_desc(int a1);
int __cdecl nox_get_thing_pretty_image(int a1);
int __cdecl nox_xxx_getTTByNameSpriteMB_44CFC0(CHAR* a1);
int __cdecl nox_xxx_getvalByName2Imp_44CFD0(CHAR* a1);
int __cdecl sub_44D020(const void*, const void*); // idb
int __cdecl sub_44D040(int a1);
BOOL __cdecl sub_44D060(int a1);
int __cdecl sub_44D090(int a1);
int __cdecl nox_drawable_link_thing(nox_drawable* d, int i); // idb
void sub_44D2C0();
nox_thing* nox_get_things_head();
int __cdecl sub_44D320(int a1);
int __cdecl sub_44D330(CHAR* a1);
int __cdecl sub_44D340(CHAR* a1);
void sub_44D3A0();
void __cdecl sub_44D5C0(int a1, int a2);
int sub_44D640();
BOOL __cdecl sub_44D660(const char* a1);
int __cdecl sub_44D7E0(int a1);
void sub_44D8C0();
int sub_44D8F0();
int __cdecl nox_xxx_playDialogFile_44D900(int a1, int a2);
int sub_44D930();
void sub_44D960();
int sub_44D970();
int sub_44D990();
int sub_44D9A0();
void sub_44D9D0();
int __cdecl sub_44D9F0(int a1);
int __cdecl sub_44DA60(int a1);
int __cdecl nox_client_screenFadeTimeout_44DAB0(int a1, int a2, void (*a3)(void));
int __cdecl sub_44DB30(int a1, int a2, void (*a3)(void));
int __cdecl sub_44DBA0(int a1, int a2, void (*a3)(void));
int __cdecl sub_44DBF0(int a1, int a2, void (*a3)(void));
int __cdecl sub_44DC40(int a1, int a2);
int __cdecl sub_44DCA0(int a1, int a2);
BOOL sub_44DD00();
int __cdecl nox_xxx_screenFadeEffect_44DD20(int a1);
int __cdecl sub_44DD70(int a1);
void nox_xxx_cliClearScreen_44DDC0();
int4* __cdecl sub_44DDF0(int a1);
int4* __cdecl sub_44DE30(int a1);
void __cdecl sub_44DE80(_DWORD* a1);
void __cdecl sub_44DEE0(_DWORD* a1);
struc_36* sub_44DF50();
char* sub_44DF70();
int __cdecl sub_44DF90(int a1);
int __cdecl sub_44DFD0(int a1);
void sub_44E000();
void sub_44E020();
void sub_44E040();
int sub_44E070();
int sub_44E090();
int __cdecl nox_xxx_cliPlayMapIntro_44E0B0(int a1);
BOOL sub_44E0D0();
_DWORD* sub_44E110();
void sub_44E320();
int nox_client_getIntroScreenDuration_44E3B0();
void sub_44E3C0();
int nox_xxx_playGMCAPsmth_44E3E0();
_DWORD* sub_44E560();
int __cdecl nox_client_wndQuestBriefProc_44E630(int a1, int a2, int a3, int a4);
BOOL __cdecl nox_xxx_wndProc_44E6E0(int a1, int a2, int a3, int a4);
int __cdecl sub_44E6F0(_DWORD* a1, int xLeft);
double sub_44E8B0();
int sub_44E8D0();
int __cdecl nox_client_lockScreenBriefing_450160(int a1, int a2, char a3);
int sub_450560();
int sub_450570();
int sub_450580();
int sub_4505B0();
int sub_4505E0();
unsigned __int8 sub_450750();
char __cdecl sub_450760(char a1);
int __cdecl sub_450960(const void*, const void*); // idb
char* __cdecl sub_450AD0(char* a1);
int __cdecl sub_450AF0(int a1);
int __cdecl sub_450B00(int a1);
int sub_450B10();
int nox_xxx_conClear_450B70();
int __cdecl nox_xxx_consolePrint_450B90(unsigned __int8 a1, int a2);
void nox_xxx____setargv_7_450BE0();
void sub_450BF0();
int nox_xxx_consoleVPrint_450C00(unsigned __int8 a1, wchar_t* a2, ...);
int nox_xxx_consoleCreate_450C70();
int nox_xxx_consoleWndFn_450E80();
int __cdecl nox_xxx_consoleWndFn_450E90(int a1, int a2);
int __cdecl nox_xxx_consoleEditProc_450F40(_DWORD* a1, int a2, int a3, int a4);
int sub_451100();
int sub_4512B0();
int sub_451410();
void nox_xxx_unused_451430();
void __cdecl nullsub_4(_DWORD, _DWORD, _DWORD, _DWORD); // weak
int nox_xxx_unused_4514F0();
int __cdecl sub_451850(int a2, int a3);
int __cdecl sub_451920(_DWORD* a2);
void sub_451970();
int sub_4519C0();
int __cdecl sub_451BE0(int a1);
int __cdecl sub_451CA0(_DWORD* a1);
int __cdecl sub_451F30(int a1, int a2);
int __cdecl sub_451F90(int a1);
int __cdecl sub_451FE0(int a1);
int sub_452010();
_DWORD* __cdecl sub_452050(_DWORD* a1);
int* __cdecl sub_452120(int a1);
_DWORD** __cdecl sub_452190(int a1);
int* __cdecl sub_4521A0(int a1);
int sub_4521F0();
int***** sub_452230();
char* __cdecl nox_xxx_draw_452270(int a1);
int __cdecl sub_4522A0(int a1);
int __cdecl sub_4522C0(int a1);
char* __cdecl sub_4522E0(int a1);
_DWORD* __cdecl nox_xxx_draw_452300(_DWORD* a1);
int __cdecl sub_4523D0(_DWORD* a1);
int __cdecl sub_452410(int a1);
int __cdecl sub_452490(_DWORD* a1);
int __cdecl sub_452510(int a3);
__int64 __cdecl sub_452690(int a3, __int64 a4, int a5);
int __cdecl sub_4526D0(int a1);
int __cdecl sub_4526F0(int a1);
int* __cdecl sub_452810(int a1, char a2);
BOOL __cdecl nox_thing_read_AVNT_452890(int a1, void* a2);
BOOL __cdecl nox_thing_read_AVNT_452B00(int a1);
BOOL __cdecl sub_452B30(int a1);
int __cdecl sub_452BD0(int a1, char* a2);
_DWORD* __cdecl nox_xxx_clientPlaySoundSpecial_452D80(int a1, int a2);
_DWORD* __cdecl sub_452DC0(int a1, int a2, int a3);
_DWORD* __cdecl sub_452E10(int a1, int a2, int a3);
_DWORD* __cdecl sub_452E60(_DWORD* a1);
_DWORD* __cdecl sub_452E80(_DWORD* a1);
int __cdecl sub_452E90(_DWORD* a1, int a2);
int __cdecl sub_452EB0(int* a1);
int __cdecl sub_452EE0(int a1, int a2);
unsigned int __cdecl sub_452F10(int a1, int a2);
int __cdecl sub_452F50(int a1, int a2);
_DWORD* __cdecl sub_452F80(int a1, int a2);
int __cdecl sub_452FA0(int a1);
int __cdecl sub_452FE0(int a1, int a2);
int __cdecl sub_453000(_DWORD* a1, unsigned int a2, int a3);
void sub_453050();
void nox_xxx____setargv_9_453060();
int sub_453070();
BOOL __cdecl sub_453080(char a1);
_DWORD* sub_4532E0();
int __cdecl sub_453350(int a1, int a2);
int __cdecl sub_4533D0(int a1, int a2, int a3, int a4);
int* __cdecl sub_4535E0(int* a1);
int __cdecl sub_4535F0(int a1);
char* sub_453600();
int sub_453610();
_BYTE* __cdecl sub_453620(_BYTE* a1, char a2, int a3);
_DWORD* __cdecl sub_453640(_DWORD* a1, int a2, int a3);
BOOL __cdecl sub_453660(int a1);
BOOL __cdecl sub_453690(int a1);
int __cdecl sub_4536B0(_DWORD* a1);
int sub_453710();
char sub_453750();
int sub_4537F0();
_DWORD* sub_453B00();
int __cdecl sub_453B80(int a1, int a2);
void __cdecl sub_453F70(const void* a1);
char* sub_453F90();
int __cdecl sub_453FA0(int a1, int a2, int a3);
BOOL __cdecl sub_454000(int a1, int a2);
int __cdecl sub_454040(_DWORD* a1);
int __cdecl sub_4540E0(int a1);
char sub_454120();
int __cdecl nox_xxx_guiServerAccessLoad_4541D0(int a1);
int __cdecl sub_454A90(int a1, int a2);
int __cdecl nox_xxx_windowAccessProc_454BA0(int a1, int a2, int* a3, int a4);
int sub_455770();
int __cdecl sub_4557D0(int a1);
int* sub_455800();
int __cdecl sub_455920(int a1);
void __cdecl sub_455950(wchar_t* a1);
int __cdecl sub_4559B0(wchar_t* a1);
int __cdecl sub_455A00(int a1);
char __cdecl sub_455A50(char a1);
int sub_455C10();
int __cdecl sub_455CD0(_BYTE* a1, _DWORD* a2);
int __cdecl sub_455E70(unsigned __int8 a1);
int sub_455EE0();
int __cdecl sub_455F10(int a1);
int sub_455F60();
int sub_456050();
int sub_456070();
int __cdecl sub_4560D0(int a1, int a2);
int sub_456240();
int sub_456500();
int __cdecl sub_456640(int a1, int a2);
char __cdecl sub_456BB0(int a1);
wchar_t* __cdecl sub_456D00(int a1, wchar_t* a2);
int* __cdecl sub_456D60(int a1);
int __cdecl sub_456DF0(int a1);
int __cdecl sub_456E40(int a1, int a2);
int __cdecl sub_456EA0(wchar_t* a1);
int __cdecl sub_456F10(wchar_t* a1, int a2);
int sub_456FA0();
unsigned __int8 __cdecl sub_457120(int a1);
int __cdecl sub_457140(int a1, wchar_t* a2);
int __cdecl sub_4571A0(int a1, int a2);
int* __cdecl sub_457350(unsigned __int8 a1, unsigned __int8 a2);
int sub_4573A0();
void sub_4573B0();
int __cdecl nox_xxx_guiServerOptionsGetGametypeName_4573C0(__int16 a1);
int __cdecl sub_457460(int a1);
int nox_xxx_guiServerOptionsTryHide_4574D0();
char* sub_457A10();
int __cdecl sub_457B60(int a1);
int __cdecl sub_457F30(int a1);
int __cdecl nox_xxx_windowServerOptionsDrawProc_458500(_DWORD* a1, int a2);
int __cdecl nox_xxx_guiServerOptionsProc_458590(int a1, int a2, int a3, int a4);
char sub_459150();
int __cdecl sub_4593B0(int a1);
int __cdecl sub_459560(int a1);
int __cdecl sub_459650(wchar_t* a1);
int nox_xxx_windowServerOptionsFillGametypeList_4596A0();
int sub_459700();
int* __cdecl nox_xxx_guiServerOptionsHide_4597E0(int a1);
char* sub_459870();
int __cdecl sub_459A40(char* a1);
char* __cdecl sub_459AA0(int a1);
int sub_459C10();
int sub_459C30();
int __cdecl sub_459D50(int a1);
int sub_459D60();
int sub_459D70();
int __cdecl sub_459D80(int a1);
BOOL sub_459DA0();
BOOL __cdecl sub_459DB0(int a1);
int __cdecl sub_459DD0(int a1, char a2);
int __cdecl nox_xxx_cliRemoveHealthbar_459E30(nox_drawable* dr, char a2);
int nox_xxx_cliFirstMinimapObj_459EB0();
int __cdecl nox_xxx_cliNextMinimapObj_459EC0(int a1);
int __cdecl sub_459ED0_drawable(int a1);
int __cdecl sub_459F00(nox_drawable* dr);
int __cdecl sub_459F40_drawable(int a1);
_DWORD* __cdecl sub_459F70(nox_drawable* dr);
int nox_xxx_cliGetSpritePlayer_45A000();
int __cdecl sub_45A010(int a1);
int nox_get_drawable_count();
int nox_xxx_sprite_45A030();
int __cdecl sub_45A040(int a1);
int sub_45A060();
int __cdecl sub_45A070(int a1);
int sub_45A090();
int __cdecl sub_45A0A0(int a1);
int sub_45A0C0();
int __cdecl sub_45A0E0(int a1);
_DWORD* __cdecl nox_xxx_sprite_45A110_drawable(nox_drawable* a1);
void __cdecl sub_45A160_drawable(nox_drawable* a1);
BOOL __cdecl nox_alloc_drawable_init(int cnt);
void nox_drawable_free();

nox_drawable* __cdecl nox_xxx_spriteLoadAdd_45A360_drawable(int thingInd, int a2, int a3);
void __cdecl nox_xxx_sprite_45A480_drawable(int a1);
int __cdecl nox_xxx_spriteDelete_45A4B0(nox_drawable* dr);
void __cdecl nox_xxx_spriteDeleteStatic_45A4E0_drawable(nox_drawable* dr);
void __cdecl nox_xxx_spriteDeleteAll_45A5E0(int a1);
int __cdecl nox_xxx_spriteIsPlayerSprite_45A630(int a1);
void __cdecl sub_45A670(unsigned int a1);
_DWORD* __cdecl nox_xxx_netSpriteByCodeDynamic_45A6F0(int a1);
_DWORD* __cdecl nox_xxx_netSpriteByCodeStatic_45A720(int a1);
int nox_xxx_unused_45A750();
int __cdecl sub_45A840(_DWORD* a1);
int __cdecl nox_xxx_spriteSetActiveMB_45A990_drawable(int a1);
int __cdecl nox_xxx_cliDestroyObj_45A9A0(int a1);
int* __cdecl sub_45A9B0(int a1, int a2);
int sub_45AB40();
int __cdecl nox_xxx_spriteSetFrameMB_45AB80(int a1, int a2);
int __cdecl nox_xxx_guiSpellSortFn_45ABC0(const void*, const void*); // idb
int nox_xxx_bookSetColor_45AC40();
unsigned int sub_45AC70();
int __cdecl nox_xxx_bookHideMB_45ACA0(int a1);
int __cdecl nox_xxx_guiSpellSortList_45ADF0(int a1);
unsigned int __cdecl nox_xxx_book_45B010(int a1);
int __cdecl nox_xxx_bookWndProc_45B070(int a1, int a2);
int nox_xxx_bookClickSpell_45B1F0();
int nox_xxx_bookClickCreature_45B200();
int __cdecl nox_xxx_book_45B210(int a1, int a2);
int __cdecl nox_xxx_bookChildWndProcMB_45B360(_DWORD* a1, unsigned int a2);
int __cdecl nox_xxx_bookListWndProc_45B5F0(int a1, unsigned int a2, unsigned int a3);
int __cdecl nox_xxx_bookMoveToPage_45B930(int a1);
int nox_xxx_bookInit_45B9D0();
int __cdecl nox_xxx_bookDrawIconFn_45CB30(_DWORD* a1);
int __cdecl nox_xxx_bookWndFn_45CC10(_DWORD* a1, int a2, unsigned int a3);
int sub_45CFA0();
int sub_45CFC0();
void __cdecl nox_xxx_netSpellRewardCli_45CFE0(int a1, int a2, int a3, int a4);
void __cdecl nox_xxx_netGuideRewardCli_45D140(int a1, int a2);
int* __cdecl nox_xxx_bookSetForward_45D200(int* a1, int a2, int2* a3); // idb
void nox_xxx_abilityReward_45D290(int a1, char* a2, int a3);
int __cdecl sub_45D320(int a1);
int __cdecl sub_45D400(int a1);
char* __cdecl nox_xxx_clientQuestDisableAbility_45D4A0(int a1);
int __cdecl sub_45D500(int a1);
_DWORD* __cdecl sub_45D550(_DWORD* a1);
void nox_xxx_bookFillAll_45D570(int a1, int a2);
int __cdecl sub_45D7D0(int* a1, int* a2);
void sub_45D810();
int sub_45D9B0();
BOOL nox_xxx_guiSpellTest_45D9C0();
int __cdecl nox_xxx_guiSpellTargetClickSet_45D9D0(int a1);
int __cdecl nox_xxx_guiSpell_45DA10(int a1);
void nox_xxx_clientUseQWEButton_45DA50(int a1);
void nox_xxx_clientStoreLastButton_45DAD0(int a1);
int __cdecl nox_xxx_clientSendAbil_45DAF0(int a1);
int __cdecl nox_xxx_clientSendSpell_45DB20(char* a1, int a2, char a3);
int sub_45DB90();
void nox_xxx_guiSpellTargetClickCheckSend_45DBB0();
void* __cdecl nox_xxx_book_45DBE0(void* a1, int a2, int a3);
_DWORD* __cdecl nox_xxx_spellKeyPackSetSpell_45DC40(int a1, int a2, int a3);
int __cdecl nox_xxx_bookSpellDrop_45DCA0(int a1, char a2, int a3, int a4, int* a5);
int __cdecl nox_xxx_updateSpellIcons_45DDF0(int a1);
int __cdecl nox_xxx_updateSpellIconDir_45DE10(int a1, int a2);
int __cdecl nox_xxx_spellBoxPointToWnd_45DE60(int a1, int a2, int a3);
int __cdecl nox_xxx_guiSpellSetCursor_45DF60(int a1, char a2);
int __cdecl sub_45DFC0(int a1);
int __cdecl nox_xxx_clientUpdateButtonRow_45E110(int a1);
int nox_xxx_buttonsGetSelectedRow_45E180();
int nox_xxx_quickbarButtonBookDraw_45EF30();
int sub_45EF40();
int __cdecl nox_xxx_quickBarWnd_45EF50(int a1, int a2, unsigned int a3);
wchar_t* __cdecl nox_xxx_clientSwapQuickbarKeys_45F300(int a1, int a2, int a3);
wchar_t* __cdecl sub_45F390(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_quickbarButtonBookWnd_45F450(int a1, unsigned int a2);
int __cdecl sub_45F500(int a1, int a2);
int __cdecl sub_45F520(int a1, unsigned int a2);
int __cdecl sub_45F5D0(_DWORD* a1);
int __cdecl nox_xxx_quickbarTrapUpDownProc_45F630(int a1, unsigned int a2);
int __cdecl nox_xxx_quickbarTrapUpDownDraw_45F6F0(_DWORD* a1);
int __cdecl nox_xxx_quickbarTrapButtonProc_45F7A0(_DWORD* a1, unsigned int a2, unsigned int a3);
int sub_45F890();
BOOL __cdecl nox_xxx_quickbar_45F8D0(int a1, int a2, int a3, int a4);
int sub_45F8F0();
_DWORD* sub_45F900();
int __cdecl nox_xxx_quickbarTrapProc_45FB90(int a1, unsigned int a2, int a3, int a4);
int __cdecl nox_xxx_quickBarDrawFn_45FBD0(int yTop); // idb
int __cdecl nox_xxx_quickBarWarriorDraw_45FDE0(int yTop); // idb
int nox_xxx_quickbarDrawFn_460000();
int __cdecl nox_xxx_quickBarInitWindow_4601F0(int a1, int a2, int a3, int a4, int a5, int a6, int a7);
void* sub_4602F0();
void* sub_460380();
_DWORD* sub_4603A0();
_DWORD* sub_4603F0();
_DWORD* sub_460440();
int __cdecl sub_4604B0(int a1);
int sub_4604E0();
int nox_xxx_spellNext_4604F0();
int nox_xxx_spellPrev_460540();
void nox_xxx_spellReset_460590();
void sub_460630();
int nox_xxx_guiSpell_460650();
int sub_460660();
int nox_xxx_quickBarClose_4606B0();
_DWORD* sub_460920();
int __thiscall sub_460940(void* this);
int __cdecl sub_460A10(int a1, int a2, int a3, char a4);
int __cdecl sub_460B90(int a1);
BOOL sub_460D40();
int sub_460D50();
int nox_xxx_cliPrepareGameplay1_460E60();
int sub_460E90();
int __cdecl sub_460EA0(int a1);
char __cdecl sub_460EB0(int a1, char a2);
_DWORD* sub_461010();
_DWORD* sub_461060();
int sub_461080();
char* __cdecl sub_461090(int a1, int a2);
char* __cdecl sub_4610D0(unsigned __int8 a1);
char* __cdecl sub_461120(int a1, int a2);
BOOL __cdecl sub_461160(int a1);
int sub_4611A0();
int sub_4611B0();
void __cdecl nox_xxx_netAbilityRewardCli_4611E0(int a1, int a2, char* a3);
int nox_xxx_buttonFindFirstEmptySlot_461250();
int sub_4612A0();
int __cdecl nox_xxx_buttonHaveSpellInBarMB_4612D0(int a1);
void __cdecl nox_xxx_buttonSetImgMB_461320(int a1, _DWORD* a2);
int __cdecl sub_461360(int a1);
int sub_4613B0();
int sub_461400();
int __cdecl sub_461440(int a1);
int sub_461450();
int __cdecl nox_xxx_playerInitColors_461460(int a1);
char* sub_461520();
int __cdecl sub_461550(int a1);
int sub_4615C0();
int __cdecl sub_461600(int a1);
int __cdecl nox_xxx_send2ServInvenFail_461630(__int16 a1);
int __cdecl sub_4617C0(int a1, int a2, const void* a3, int2* a4); // idb
int sub_461930();
int* __cdecl sub_461970(int a1, int a2);
char* sub_4619F0();
unsigned __int8* sub_461B50();
_QWORD** __cdecl sub_461E60(_QWORD*** a1);
char* __cdecl sub_461EF0(int a1);
int __cdecl sub_461F90(int a1);
int __cdecl sub_4622E0(int a1);
int __cdecl nox_xxx_clientEquip_4623B0(int a1);
_DWORD* __cdecl sub_4623E0(_DWORD* a1, int a2);
int __cdecl sub_4624D0(int a1);
int __cdecl sub_4625D0(_DWORD* a1);
double __cdecl sub_4626C0(int a1);
double __cdecl sub_462700(int a1);
int __cdecl sub_463370(_DWORD* a1, _DWORD* a2, _DWORD* a3);
int __cdecl sub_4633B0(int a1, float* a2, float* a3);
int __cdecl sub_463420(int a1);
int __cdecl nox_xxx_inventoryDrawAllMB_463430(int a1);
int __cdecl nox_xxx_guiDrawInventoryTray_4643B0(int a1, int a2);
int __cdecl sub_464770(int a1, int a2, unsigned int a3);
BOOL __cdecl sub_464B40(int a1, int a2);
int __cdecl nox_xxx_clientDequip_464B70(int a1);
int nox_xxx_XorEaxEaxSub_464BA0();
BOOL __cdecl nox_xxx_inventoryWndProc_464BB0(int a1, int a2);
int __cdecl nox_xxx_trade_4657B0(__int16 a1);
char __cdecl nox_xxx_clientTradeMB_4657E0(_DWORD* a1);
int __cdecl nox_xxx_clientTrade_465870(__int16 a1);
void __cdecl sub_4658A0(int a1, int2* a2);
int __cdecl sub_465990(_DWORD* a1);
int __cdecl nox_xxx_clientDrop_465BE0(int2* a1); // idb
BOOL __cdecl nox_xxx_clientKeyEquip_465C30(int a1, int a2);
void nox_xxx_clientUse_465C70(int a1);
int sub_465CA0();
void __cdecl sub_465CD0(_DWORD* a1, int a2, int a3, int a4);
int __cdecl sub_465D50_draw(int a1);
int __cdecl sub_465DE0(int a1);
int sub_465DF0();
int nox_xxx_wndCreateInventoryMB_465E00();
int nox_xxx_movEax1Sub_4661C0();
int __cdecl sub_466220(int a1, int a2, int* a3, int a4);
int __cdecl sub_466550(int a1, unsigned int a2);
int __cdecl nox_xxx_inventoryDrawProc_466580(_DWORD* a1);
int __cdecl sub_466620(int a1, int a2, unsigned int a3);
wchar_t* __cdecl sub_466660(int a1, int2* a2); // idb
int __cdecl sub_466950(int a1);
int __cdecl sub_466BA0(_DWORD* a1, int a2, unsigned int a3, int a4);
int __cdecl sub_466BF0(int a1, int a2, unsigned int a3, int a4);
int __cdecl sub_466C40(int a1);
int __cdecl sub_466ED0(int a1);
int __cdecl sub_466F50(_DWORD* a1, int* a2);
char* nox_xxx_inventoryLoadImages_467050();
int nox_client_invAlterWeapon_4672C0();
int __cdecl sub_4673F0(int a1, int a2);
int __cdecl sub_467410(int a1);
char __cdecl sub_467420(char a1);
unsigned __int8 sub_467430();
int __cdecl sub_467440(int a1);
int __cdecl sub_467450(int a1);
int nox_xxx_j_inventoryNameSignInit_467460(void);               // weak
int __cdecl sub_467470(int, float); // idb
int __cdecl sub_467490(int a1);
int sub_4674A0();
void __cdecl nox_window_set_visible_unk5(int visible);
void nox_xxx_cliUseCurePoison_4674E0(int a1);
char* __cdecl nox_xxx_cliInventoryFirstItemByTT_467520(int a1);
int sub_467590();
int sub_4675B0();
int __cdecl sub_4675C0(int a1);
__int16 __cdecl sub_4675E0(int a1, __int16 a2, __int16 a3);
int sub_467650();
void sub_467680();
int nox_xxx_wndGetHandle_4676A0();
char* sub_4676B0();
char* sub_4676C0();
int __cdecl sub_4676D0(int a1);
int __cdecl sub_467700(int a1);
int __cdecl sub_467740(int a1);
int __cdecl sub_467810(int a1, int a2);
int __cdecl sub_467850(int a1);
char* __cdecl sub_467870(int a1, int a2);
int sub_4678B0();
int sub_4678C0();
int sub_4678D0();
char* __cdecl sub_467930(int a1, int a2, int a3);
int sub_467980();
int __cdecl sub_467B00(int a1, int a2);
int sub_467BB0();
int sub_467C10();
int sub_467C60();
BOOL sub_467C80();
int sub_467CA0();
int sub_467CD0();
int __cdecl nox_xxx_gameClearAll_467DF0(int a1);
void nullsub_5(void); // weak
int sub_467F30();
int __cdecl sub_467F70(int a1, int a2, int a3, int a4);
int __cdecl sub_467FC0(int a1, int a2, int* a3, int a4);
void sub_468020();
int sub_468060();
int sub_468080();
int sub_4680B0();
int sub_4680E0();
int sub_468340();
int sub_4683B0();
int __cdecl sub_468480(int a1, int a2, int a3, int a4);
int sub_4684C0();
int __cdecl sub_4685D0(int a1, int a2, int* a3, int a4);
int __cdecl sub_468840(wchar_t* a1);
int sub_468DC0();
int __cdecl sub_468DF0(wchar_t* a1);
int __cdecl sub_468E60(int a1);
char* __cdecl sub_469920(_DWORD* a1);
int __cdecl sub_469B90(int* a1);
char* nox_xxx_getAmbientColor_469BB0();
int sub_469FA0();
int __cdecl sub_469FB0(const char* a1);
BOOL __cdecl nox_thing_read_wall_46A010(_DWORD* a1, char* a2);
int __cdecl nox_xxx_getWallSprite_46A3B0(int a1, int a2, int a3, int a4);
char* __cdecl nox_xxx_getWallDrawOffset_46A3F0(int a1, int a2, int a3, int a4);
void sub_46A430(int a1);
int sub_46A4A0();
size_t __cdecl nox_xxx_cmdSayDo_46A4B0(wchar_t* a1, int a2);
int __cdecl sub_46A5D0(_DWORD* a1, int a2);
int sub_46A6A0();
_DWORD* sub_46A730();
int __cdecl sub_46A7E0(_DWORD* a1, int a2, int a3, int a4);
int __cdecl sub_46A820(int a1, int a2, int a3, int a4);
int sub_46A860();
int nox_xxx_wndRetNULL_46A8A0();
int nox_xxx_wndRetNULL_0_46A8B0();
int __cdecl nox_xxx_wndShowModalMB_46A8C0(int a1);
int __cdecl nox_xxx_wndToTopList_46A920(int a1);
void __cdecl nox_xxx_wndCursorListAdd_46A960(nox_window* win);
int __cdecl nox_wnd_nox_xxx_wndDraw_46A9B0(_DWORD* a1, int a2, int a3);
_DWORD* __cdecl nox_xxx_wndFixCoords_46A9F0(_DWORD* a1);
int __cdecl nox_xxx_wnd_46AA20(int a1, _DWORD* a2, _DWORD* a3);
int __cdecl nox_client_wndGetPosition_46AA60(nox_window* a1, _DWORD* a2, _DWORD* a3);
BOOL __cdecl nox_xxx_wndPointInWnd_46AAB0(_DWORD* a1, int a2, int a3);
int __cdecl sub_46AB20(_DWORD* a1, int a2, int a3);
int __cdecl nox_window_get_size(nox_window* win, int* outW, int* outH);
int __cdecl nox_xxx_wnd_46ABB0(int a1, int a2);
int __cdecl nox_window_set_hidden(nox_window* win, int visible);
void __cdecl sub_46AC60(int a1);
int __cdecl wndIsShown_nox_xxx_wndIsShown_46ACC0(int a1);
void __cdecl sub_46ACE0(_DWORD* a1, int a2, int a3, int a4);
void __cdecl sub_46AD20(_DWORD* a1, int a2, int a3, int a4);
int __cdecl nox_xxx_wnd_46AD60(int a1, int a2);
int __cdecl nox_xxx_wndClearFlag_46AD80(int a1, int a2);
int __cdecl nox_xxx_wndGetFlags_46ADA0(int a1);
int __cdecl nox_xxx_wndSetCaptureMain_46ADC0(int a1);
int __cdecl nox_xxx_wndClearCaptureMain_46ADE0(int a1);
int nox_xxx_wndGetCaptureMain_46AE00();
int __cdecl sub_46AE10(int a1, int a2);
int __cdecl nox_xxx_wndSetOffsetMB_46AE40(int a1, int a2, int a3);
int __cdecl nox_xxx_wndSetIcon_46AE60(int a1, int a2);
int __cdecl sub_46AE80(int a1, int a2);
int __cdecl nox_xxx_wndSetIconLit_46AEA0(int a1, int a2);
int __cdecl sub_46AEC0(int a1, int a2);
int __cdecl sub_46AEE0(int a1, int a2);
int __cdecl sub_46AF00(int a1);
int __cdecl sub_46AF40(int a1);
int __cdecl sub_46AF50(int a1, int a2);
int __cdecl nox_xxx_wndSetDrawData_46AF80_copy_window_part(nox_window* win, const void* p);
int __cdecl sub_46AFB0(int a1, void* a2);
int __cdecl nox_xxx_wndSetRectColor2MB_46AFE0(int a1, int a2);
int __cdecl sub_46B070(int a1, int a2);
int __cdecl nox_xxx_wndSetID_46B080(_DWORD* a1, int a2);
int __cdecl nox_xxx_wndGetID_46B0A0(int* a1);
_DWORD* __cdecl nox_xxx_wndGetChildByID_46B0C0(_DWORD* a1, int a2);
int sub_46B110();
int __cdecl sub_46B120(nox_window* win, int a2);
void __cdecl sub_46B180(nox_window* win);
void __cdecl nox_window_set_ptrs_97(nox_window* win, nox_window* a2);
int __cdecl sub_46B240(int a1);
int __cdecl nox_window_is_child(nox_window* a1, nox_window* a2);
int __cdecl nox_xxx_wnd_46B280(int a1, int a2);
int __cdecl sub_46B2B0(int a1);
int __cdecl nox_xxx_wndSetProc_46B2C0(int a1, int (*a2)(int, int, int, int));
int nox_xxx_wndDefaultProc_46B2F0(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_wndSetWindowProc_46B300(int a1, int (*a2)(int, int, int, int));
int nox_xxx_wndDefaultProc_0_46B330(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_wndSetDrawFn_46B340(int a1, int (*a2)(int, int));
int __cdecl nox_xxx_wndDrawFnDefault_46B370(int a1, int* a2);
int __cdecl nox_window_set_all_funcs(nox_window* win, int (*a2)(int, int, int, int), int (*draw)(nox_window*, void*),
									 void* a4);
int __cdecl nox_window_call_field_94(nox_window* win, int a2, int a3, int a4);
int __cdecl nox_window_call_field_93(nox_window* win, int a2, int a3, int a4);
int nox_xxx_wndGetFocus_46B4F0();
int __cdecl nox_xxx_windowDestroyChildsMB_46B500(nox_window* win);
int __cdecl sub_46B580(int a1);
_DWORD* __cdecl nox_xxx_wnd_46B5B0(_DWORD* a1, int a2, int a3);
int __cdecl sub_46B630(int a1, int a2, int a3);
void __cdecl nox_xxx_windowUpdateKeysMB_46B6B0(unsigned __int8* a1);
void nox_xxx_cursorUpdate_46B740();
void sub_46C200();
int __cdecl nox_xxx_wnd_46C2A0(int a1);
void mainloop_draw();
int __cdecl nox_window_draw_recursive(nox_window* a1);
nox_window* __cdecl nox_window_new(nox_window* a1, int flags, int a3, int a4, int w, int h,
								   int (*fnc)(int, int, int, int));
int __cdecl nox_xxx_windowDestroyMB_46C4E0(_DWORD* a1);
int sub_46C5D0();
int sub_46C610();
int __cdecl sub_46C690(int a1);
int __cdecl nox_xxx_wnd_46C6E0(int a1);
int sub_46C720();
int sub_46CC70();
int sub_46CC90();
int sub_46CCA0(int a1, int a2, int a3, int a4);
int sub_46CCB0();
int __cdecl sub_46CCD0(int a1, int a2);
int sub_46CD70(nox_savegame_xxx* sv);
int nox_savegame_findLatestSave_46CDC0(nox_savegame_xxx* sarr);
int sub_46D6F0();
int __cdecl sub_46D750(int a1, int a2, int a3, int a4, int a5, int a6, int a7, int a8, int a9, int a10);
int sub_46D830();
void __cdecl sub_46D950(_DWORD* a1);
FILE* __cdecl sub_46D9D0(int a1, int a2, LPVOID lpMem, int a4);
int sub_46DB00();
int sub_46DB80();
int __cdecl sub_46DC00(int a1, unsigned __int8 a2, int a3);
int sub_46DC30(int a1, unsigned __int8 a2, wchar_t* a3, ...);
char* sub_46DCC0();
int __cdecl sub_46E080(int a1);
int __cdecl sub_46E130(int a1);
unsigned __int16* __cdecl sub_46E170(wchar_t* a1);
int __cdecl sub_46E1E0(int a1);
char* sub_46E4E0();
int sub_46F060();
int nox_xxx_Proc_46F070();
void sub_46FAE0();
unsigned __int8 __cdecl sub_46FE60(int a1);
unsigned __int8 __cdecl sub_46FEB0(unsigned __int8 a1);
char sub_46FEE0();
char __cdecl sub_46FF70(int a1);
unsigned __int8 sub_46FFD0();
int sub_4703F0();
int sub_470510();
int sub_470550();
BOOL sub_470580();
int sub_4705B0();
char __cdecl sub_4705F0(char a1, char a2, __int16 a3);
char __cdecl sub_470650(char a1, __int16 a2);
int sub_470680();
BOOL sub_4706A0();
int __cdecl sub_4706C0(int a1);
int sub_470710();
int __cdecl nox_xxx_cliDrawLagometer_470770(int a1, int a2);
int __cdecl sub_470A50(int a1);
int sub_470A60();
void sub_470A80();
int nox_xxx_playerGet_470A90();
void nox_xxx_cliShowHideTubes_470AA0(int a1);
unsigned __int8* nox_xxx_guiHealthManaColorInit_470B00();
int __cdecl sub_470C20(_DWORD* a1, _DWORD* a2);
int __cdecl sub_470C40(int a1);
int __cdecl nox_xxx_cliSetTotalHealth_470C80(int a1, int a2);
int __cdecl sub_470CB0(int a1);
int sub_470CC0();
int sub_470CD0();
int __cdecl nox_xxx_cliSetManaAndMax_470CE0(int a1, int a2);
int __cdecl nox_xxx_cliSetMana_470D10(int a1);
int __cdecl sub_470D20(int a1, int a2);
BOOL sub_470D40();
void sub_470D70();
int __cdecl sub_470D90(int a1, int a2);
int nox_xxx_cliGetMana_470DD0();
int sub_470DE0();
int __cdecl sub_470E90(int a1, int a2);
void __cdecl nox_win_init_cur_weapon(nox_window* a1, int a2, int a3, int w, int h);
int __cdecl sub_470F40_draw(nox_window* win);
int __cdecl sub_471250(_DWORD* a1);
int __cdecl sub_471450(_DWORD* a1);
int __cdecl nox_xxx_guiBottleSlotDrawFn_471A80(_DWORD* a1);
int __cdecl nox_xxx_guiBottleSlotProc_471B90(int a1, int a2);
int __cdecl nox_xxx_drawHealthManaBar_471C00(int a1);
int __cdecl nox_xxx_guiHealthManaTubeDraw_471D10(int xLeft); // idb
int sub_472080();
int __cdecl sub_4720C0(int xLeft, int); // idb
int __cdecl nox_xxx_guiHealthManaTubeProc_472100(int a1, int a2);
int __cdecl sub_4721A0(int a1);
int nox_xxx_cliPrepareGameplay2_4721D0();
void sub_472220();
void sub_472240();
void sub_472260();
wchar_t* sub_472280();
unsigned __int8* sub_472310();
void sub_4724E0();
void sub_472500();
int __cdecl nox_xxx_cliSetMinimapZoom_472520(int a1);
int sub_472530();
int __cdecl sub_472540(int a1);
int4* __cdecl nox_xxx_drawMinimap4Sprite_4725C0(int a1);
int __cdecl nox_xxx_cliDrawMinimap_472600(int a1, int a2);
int __cdecl sub_4730D0(int2* a1, unsigned __int8 a2, int a3); // idb
int __cdecl sub_473380(int a1, int a2, int a3, int a4, int a5);
int __cdecl sub_4733B0(_DWORD* a1);
int __cdecl sub_473420(_DWORD* a1);
int __cdecl nox_video_drawCircleRad3_4734F0(int* a1);
int __cdecl nox_client_drawRectLines_473510(int a1, int a2, int a3, int a4);
void __cdecl nox_xxx_minimapDrawPoint_473570(int xLeft, int yTop);
void __cdecl sub_4735C0(int xLeft, int yTop);
char sub_473610();
int sub_473670();
int nox_xxx_wnd_473680();
int sub_473840();
int nox_xxx_drawMinimapAndLines_4738E0();
void nox_xxx____setargv_11_473920();
char* sub_473930();
int sub_473960();
void __cdecl sub_473970(int2*, int2*);
int __cdecl sub_4739A0(int2* a1, int2* a2); // idb
int __cdecl sub_4739D0(int a1);
int __cdecl sub_4739E0(_DWORD* a1, int2* a2, int2* a3); // idb
int __cdecl sub_473A10(_DWORD*, int2*, _DWORD*);
int sub_473A40();
int sub_473B30_free();
char __cdecl nox_xxx_drawWalls_473C10(_DWORD* a1, unsigned __int8* a2);
int __cdecl sub_4745F0(_DWORD* a1);
int __cdecl sub_474B40(int a1);
int __cdecl sub_474BE0(_DWORD* a1, int* a2, int a3, int a4, int a5, int a6, int a7, int a8);
int __cdecl sub_474DD0(_DWORD* a1, int2* a2, int2* a3, int a4, int a5); // idb
size_t __cdecl sub_4754C0(int a1);
void __cdecl sub_4754F0(_DWORD* a1);
void __cdecl nox_xxx_spriteAddQueue_475560_draw(nox_drawable* dr, int a2);
int __cdecl nox_xxx_sprite_4756E0_drawable(_DWORD* a1);
int __cdecl nox_xxx_sprite_475740_drawable(_DWORD* a1);
int __cdecl nox_xxx_sprite_4757A0_drawable(int a1);
int __cdecl sub_4757D0_drawable(_DWORD* a1);
int4* __cdecl nox_xxx_drawAllMB_475810_draw(int* a1);
int __cdecl sub_475F10(_DWORD* a1);
int __cdecl sub_475FE0(_DWORD* a1);
int __cdecl sub_476080(unsigned __int8* a1);
int __cdecl sub_476160(const void*, const void*); // idb
int __cdecl sub_4761B0(int a1);
int __cdecl sub_476240(const void*, const void*); // idb
int __cdecl sub_476270(_DWORD* a1);
int __cdecl sub_4765F0(int a1);
int sub_476680();
int __cdecl sub_4766A0(int a1);
int sub_4766D0();
void sub_4766E0();
void sub_4766F0();
void sub_476700(int a1, int a2);
void __cdecl sub_476850(int a1, unsigned __int8* a2);
int(__cdecl* __cdecl sub_476AE0(int a1, unsigned __int8* a2))(int* a1, int a2);
__int16 __cdecl sub_476D70(_DWORD* a1, int* a2, unsigned int a3);
void __cdecl sub_476DA0(unsigned int* a1, _DWORD* a2, unsigned int a3);
int __cdecl sub_476E00(int a1);
_DWORD* sub_476E20();
int sub_476E90();
int __cdecl nox_xxx_clientDeleteSprite_476F10_drawable(nox_drawable* dr);
unsigned int nox_xxx_packetGetMarshall_476F40();
int sub_476F60();
int nox_xxx_spriteGetMB_476F80();
int nox_xxx_clientGetSpriteAtCursor_476F90();
void nox_xxx_clientEnumHover_476FA0();
void __cdecl nox_xxx_clientOnCursorHover_477050(int, int); // idb
void __cdecl nox_video_drawCursorSelectCircle2_477470(_DWORD* a1, int a2, int a3);
unsigned __int8 __cdecl sub_477530(int a1);
int nox_xxx_guiCursor_477600();
int __cdecl nox_client_setCursorType_477610(int a1);
int nox_client_getCursorType_477620();
int nox_xxx_cursorGetTypePrev_477630();
int __cdecl nox_xxx_bookSaveSpellForDragDrop_477640(int a1, int a2);
int nox_xxx_bookSpellDnDclear_477660();
int nox_xxx_bookGetSpellDnDType_477670();
int nox_xxx_cursorGetDraggedItem_477680();
int __cdecl nox_xxx_cursorSetDraggedItem_477690(int a1);
void nox_xxx_cursorResetDraggedItem_4776A0();
void __cdecl nox_xxx_cursorSetTooltip_4776B0(wchar_t* a1);
char* nox_xxx_cursorLoadAll_477710();
char* nox_client_drawCursorAndTooltips_477830();
int sub_477EA0();
int __cdecl sub_477EF0(int a1, int a2, int a3);
void sub_477F80();
int sub_477FF0();
int sub_478000();
int sub_478030();
int sub_478040();
int __cdecl sub_478080(int a1);
char* __cdecl sub_4780A0(int a1);
int sub_478110();
int __cdecl sub_478480(int a1, int a2, int* a3, int a4);
int __cdecl sub_478650(int a1, int a2, unsigned int a3);
void __cdecl sub_478850(int a1, __int16 a2, int a3, int a4);
int sub_478970();
int __cdecl sub_478A70(int2* a1);      // idb
wchar_t* __cdecl sub_478B10(int2* a1); // idb
int sub_478C80();
int __cdecl sub_478E50(int a1, int a2, unsigned int a3);
_DWORD* sub_478F10();
int sub_478F80();
char* __cdecl nox_xxx_getShopPic_4790F0(int a1);
int sub_479280();
_DWORD* __cdecl sub_479300(int a1, int a2, int a3, __int16 a4, int a5);
char* __cdecl sub_4793C0(int a1);
char* sub_479430();
char* __cdecl sub_479480(int a1);
int __cdecl sub_4794D0(int a1, int a2);
int sub_479590();
void __cdecl sub_4795A0(int a1);
int __cdecl sub_479690(int a1, __int16 a2, __int16 a3, int a4);
int __cdecl sub_4796D0(__int16 a1);
int __cdecl sub_479700(__int16 a1, char a2);
void sub_479810();
int __cdecl sub_479820(int a1, __int16 a2);
int __cdecl sub_479840(__int16 a1);
BOOL sub_479870();
bool __cdecl sub_479880(_DWORD* a1);
int __cdecl sub_4798A0(_DWORD* a1);
int sub_479950();
int sub_4799A0();
int __cdecl nox_xxx_guiDialog_479B00(int a1, int a2, int* a3, int a4);
int __cdecl sub_479BE0(_DWORD* a1, int a2, unsigned int a3, int a4);
int __cdecl sub_479C40(_DWORD* a1, int a2);
int __cdecl sub_479CB0(int a1, int a2);
int sub_479D00();
int sub_479D10();
int sub_47A1F0();
int sub_47A260();
int sub_47A270();
DWORD __cdecl sub_47A310(LPCSTR lpPathName);
int sub_47A650();
char* __cdecl nox_xxx_loadImage_47A8C0(const char* a1, char* a2);
int __cdecl nox_xxx_videoBagSmth_47A960(const char* a1, int a2);
int __cdecl nox_xxx_videoBag_LoadTile_47AD60(int a1, int a2, _WORD* a3);
int __cdecl nox_xxx_videoBag_LoadEdge_47AF30(int, int, unsigned __int8*);
int __cdecl nox_xxx_videoBag_LoadPXImg_47B7F0(int a1, int a2, int a3, unsigned __int8 a4);
int __cdecl sub_47D090(int a1, int a2);
LPVOID sub_47D150();
int sub_47D200();
void nullsub_6(void); // weak
__int16 __cdecl nox_client_drawImageAt_47D2C0(int a1, int a2, int a3);
int __cdecl sub_47D370(int a1);
int __cdecl sub_47D380(int a1, int a2);
int __cdecl sub_47D3F0(int a1);
int __cdecl sub_47D400(int a1, char a2);
int __cdecl nox_xxx_unused_47D420(int a1, int a2, int a3, int a4, int a5, int a6);
int __cdecl nox_xxx_someVideoSwitch_47D480(int* a1, int a2);
__int16 __cdecl nox_xxx_video_ReadTile_Real_47D4E0(_DWORD* a1);
char __cdecl nox_xxx_video_ReadSprite_Real_47D530(int* a1);
int(__cdecl* __cdecl sub_47D5B0(int a1))(_DWORD);
int __cdecl sub_47D5C0(int a1, _DWORD* a2, _DWORD* a3, _DWORD* a4, _DWORD* a5);
void nullsub_7(); // weak
UINT __cdecl nox_xxx_initJoystick_47D660(UINT uJoyID, int a2);
DWORD* __cdecl sub_47D7A0(DWORD* a1, UINT uJoyID);
int sub_47D890(); // weak
void nox_xxx_showWindow_47D8A0();
int unacquireMouse_sub_47D8B0();
int acquireMouse_sub_47D8C0();
int initMouse_sub_47D8D0();
char __cdecl sub_47DB20_get_mouse_data(nox_mouse_state_t* a1);
unsigned __int8 sub_47DBC0();
void sub_47DBD0();
unsigned __int16 __cdecl nox_xxx_conScanCode2Alpha_47F950(unsigned __int16 a1);
void __cdecl nox_xxx_getKeyFromKeyboardImpl_47FA80(int a1);
void sub_47FB10();
void nox_xxx_clientControl_42D6B0_em_not_mouse_down();
int sub_47FCC0();
int __stdcall sub_47FCE0(_DWORD* a1, int a2);
int nox_xxx_setupSomeVideo_47FEF0();
int sub_4800F0();
char __cdecl sub_4801B0(_BYTE* a2, _BYTE* a3);
_WORD* __cdecl sub_4801E0(_BYTE* a1, _WORD* a2);
_BYTE* __cdecl sub_480220(_BYTE*, _BYTE*);
_WORD* __cdecl sub_480250(_BYTE* a1, _WORD* a2);
unsigned int __cdecl sub_480290(__m64* a1, int a2, int a3, unsigned int* a4, unsigned int* a5);
__int16 __cdecl sub_4805A0(int a1, __m64* a2, int a3, unsigned int* a4, unsigned int* a5);
__int16 __cdecl sub_480700(int a1, __m64* a2, int a3, unsigned int* a4, unsigned int* a5);
void __cdecl sub_480860(WORD* a1, WORD* a2, int a3, int* a4, int* a5);
int __cdecl sub_480950(int* a1, int* a2, int a3, int a4, int a5);
void __cdecl sub_480BE0(_DWORD* a1, int* a2, int a3, int a4, int a5);
_WORD* __cdecl sub_480EB0(_WORD* a1, __int16* a2, int a3);
int* __cdecl nox_xxx_someEdgeProcessing_480EF0(int a1, int a2, int a3, int* a4, int* a5, int a6, int a7, int a8, int a9, int a10);
void sub_481410();
int nox_xxx_tileSetDrawFn_481420();
int __cdecl sub_4814F0(int2* a1);
char __cdecl nox_xxx_tileDraw_4815E0(_DWORD* a1, int a2);
char* __cdecl sub_481770(_DWORD* a1, int a2, unsigned __int16 a3);
char __cdecl nox_xxx_drawTexEdgesProbably_481900(_DWORD* a1, _DWORD* a2);
int nullsub_8(int, int); // weak
void __cdecl nox_xxx_tileCallDrawEdges_481BF0(int a1, int a2);
int __cdecl nox_xxx_tileDrawMB_481C20(int* a1);
int __cdecl nox_xxx_tileCheckRedrawMB_482570(_DWORD* a1);
int __cdecl nox_xxx_tileDrawImpl_4826A0(_DWORD* a1);
char __cdecl sub_482910(int a1, int a2);
__int16 __cdecl sub_4831C0(int a1, int a2);
int __cdecl sub_483FE0(int a1, int a2);
int __cdecl sub_484450(int a1, int a2);
void __cdecl sub_484BA0(float a1);
_DWORD* __cdecl nox_xxx_spriteChangeLightColor_484BE0(_DWORD* a1, int a2, int a3, int a4);
__int64 __cdecl sub_484C00(int a1, int a2);
__int64 __cdecl nox_xxx_spriteChangeLightSize_484C30(int a1, int a2);
int __cdecl sub_484C60(float);      // idb
int __cdecl sub_484CE0(int, float); // idb
int __cdecl sub_484D20(int a1, float a2);
int __cdecl nox_xxx_spriteChangeIntensity_484D70_light_intensity(int, float);                     // idb
int __cdecl sub_484DC0(int2* a1, int2* a2, int* a3);                    // idb
int __cdecl sub_4854D0(int2* a1, int2* a2, int2* a3, int a4, float a5); // idb
int __cdecl sub_4855D0(int, int, int, float);                           // idb
unsigned int __cdecl sub_485AE0(int a1, int a2);
int __cdecl nox_thing_read_floor_485B30(int a1, char* a2);
int __cdecl nox_thing_read_edge_485D40(int a1, char* a2);
unsigned int sub_485F80();
int nox_xxx_tile_486060();
int sub_486090();
void nox_free_pixbuffers_486110();
int __cdecl sub_486180(void* a1);
void nox_pixbuffer_swap_4861A0();
int sub_4861D0();
int sub_486230();
int __cdecl sub_4862E0(int a3, int a4);
_DWORD* __cdecl sub_486320(_DWORD* a1, int a2);
_DWORD* __cdecl sub_486340(_DWORD* a1);
int __cdecl sub_486350(int a1, int a2);
int __cdecl sub_486380(_DWORD* a1, unsigned int a2, int a3, int a4);
int __cdecl sub_4863B0(unsigned int* a2);
_DWORD* __cdecl sub_4864A0(_DWORD* a3);
int __cdecl sub_486520(unsigned int* a2);
BOOL __cdecl sub_486550(_BYTE* a1);
int __cdecl sub_486570(unsigned int* a1, _DWORD* a2);
_DWORD* __cdecl sub_486620(_DWORD* a1);
int __cdecl sub_486640(int a1, int a2);
int __cdecl sub_486670(int a1, int a2);
int __cdecl sub_4866A0(int a1);
int __cdecl sub_4866D0(_DWORD* a1, int a2);
char* __cdecl sub_4866F0(const char* a1, const char* a2);
void __cdecl sub_4869C0(LPVOID lpMem); // idb
unsigned int __cdecl sub_486A10(int a1, void* a2);
char* __cdecl sub_486A50(_DWORD* a1, int a2);
int __cdecl sub_486A80(_DWORD* a1, int a2);
int __cdecl sub_486AA0(_DWORD* a1, int a2, _DWORD* a3);
int __cdecl sub_486B10(_DWORD* a1, int a2, _DWORD* a3);
int __cdecl sub_486B60(int a1, int a2);
signed int __cdecl sub_486DB0(int a1, char* a2, signed int a3);
FILE* __cdecl sub_486E00(int a1);
int __cdecl sub_486E30(int a1, _DWORD* a2);
int __cdecl sub_486E90(int a1);
void* sub_486EF0();
int sub_486F30();
_DWORD* __cdecl sub_486FA0(int a1);
_DWORD* __cdecl sub_486FE0(int a1);
void __cdecl sub_487030(LPVOID lpMem); // idb
_DWORD* __cdecl sub_487050(_DWORD* a1);
void __cdecl sub_487070(LPVOID lpMem); // idb
_DWORD** __cdecl sub_487090(_DWORD** a1);
void sub_4870A0();
int* __cdecl sub_4870E0(int* a1);
int* __cdecl sub_487100(int** a1);
int sub_487120();
_DWORD* __cdecl sub_487150(int a1, const void* a2);
_DWORD* __cdecl sub_4871C0(int a1, int a2, const void* a3);
void __cdecl sub_4872C0(LPVOID lpMem); // idb
int __cdecl sub_487310(_DWORD* a1);
int* __cdecl sub_487360(int a1, int** a2, int* a3);
int __cdecl sub_4873C0(int a3);
int __cdecl sub_487590(int a1, const void* a2);
int* __cdecl sub_4875B0(int* a1);
int* __cdecl sub_4875D0(int** a1);
int sub_4875F0();
void __cdecl sub_487650(_DWORD* lpMem);
void __cdecl sub_487680(LPVOID lpMem); // idb
void* __cdecl sub_4876A0(_DWORD** a1);
int sub_4876E0();
_DWORD* __cdecl sub_4876F0(int a1, int a2);
int __cdecl sub_487710(int a1, int a2);
int __cdecl sub_487730(int a1);
int __cdecl sub_487740(int a1);
_DWORD* __cdecl sub_487750(int a1);
int __cdecl sub_487790(int a1, int a2);
int* __cdecl sub_4877D0(int a1, int* a2);
int* __cdecl sub_4877F0(int** a1);
int* __cdecl sub_487810(int a1, int a2);
int* __cdecl sub_4878E0(int a1, int a2);
int __cdecl sub_487910(int a1, int a2);
int* __cdecl sub_487970(int a1, int a2);
int* __cdecl sub_4879C0(int a1, int a2);
int* __cdecl sub_487A10(int a1, int a2);
int __cdecl sub_487A60(int a1);
int __cdecl nox_xxx_unused_487A70(_DWORD* a1, int(__cdecl* a2)(char*));
_DWORD* __cdecl sub_487BC0(size_t a1);
void __cdecl sub_487C10(LPVOID lpMem); // idb
_DWORD* __cdecl sub_487C30(_DWORD* a1);
int __cdecl sub_487C50(int a1, _DWORD* a2);
int __cdecl sub_487C80(int a1);
void __cdecl nullsub_9(_DWORD); // weak
int __cdecl sub_487CB0(int a1, int a2);
__int64 __cdecl sub_487CD0(int a1, int a2);
void __cdecl nullsub_10(_DWORD); // weak
int __cdecl sub_487D00(_DWORD* a1);
_DWORD* __cdecl sub_487D30(_DWORD* a1, int a2, int a3);
int __cdecl sub_487D60(int a1);
int __cdecl nox_xxx_wndEditProc_487D70(_DWORD* a1, int a2, int a3, int a4);
int __cdecl nox_xxx_wndEditDrawNoImage_488160(int a1, int a2);
_DWORD* __cdecl nox_xxx_wndEditCreate_488500(int a1, int a2, int a3, int a4, int a5, int a6, int a7, wchar_t* a8);
int __cdecl nox_xxx_wndEditProcPre_488710(int a1, unsigned int a2, wchar_t* a3, int a4);
int __cdecl nox_xxx_wndEdit_488830(int a1);
int __cdecl nox_xxx_wndEditDrawWithImage_488870(int a1, int a2);
int sub_488B60();
int sub_488BA0();
void __cdecl nox_xxx_onChar_488BD0(unsigned __int16 a1);
int __cdecl nox_xxx_wndStaticDrawNoImage_488D00(int, int xLeft); // idb
wchar_t* __cdecl sub_4892D0(int a1);
_DWORD* __cdecl nox_xxx_wndStaticCreate_489300(int a1, int a2, int a3, int a4, int a5, int a6, _DWORD* a7, _DWORD* a8);
int __cdecl nox_xxx_wndStaticProcPre_489390(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_wndStatic_4893E0(int a1);
int __cdecl nox_xxx_wndStaticProc_489420(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_wndStaticDrawWithImage_489550(_DWORD* a1, _DWORD* a2);
int sub_4896C0();
int __cdecl sub_4896D0(int a1);
int sub_4896E0();
int sub_489870();
BOOL __cdecl nox_xxx_checkSomeFlagsOnJoin_4899C0(int a1);
_DWORD* __cdecl sub_489B80(int a1);
void sub_489DC0();
int __cdecl nox_xxx_windowMplayFilterProc_489E70(int a1, int a2, int* a3, int a4);
int sub_489FB0();
int __cdecl sub_489FF0(int a1, int a2, const void* a3);
int __cdecl sub_48A020(int a1, _DWORD* a2);
int __cdecl sub_48A040(HWND a1, int a2, int a3, int a4);
void sub_48A120();
char sub_48A190();
int __cdecl nox_video_checkIsWinNT_48A1D0(int a1);
int __cdecl sub_48A210(int a1);
void __cdecl nox_xxx_directDrawBlitMB_48A220();
void sub_48A290_call_present();
int nox_xxx_directDrawBlitImpl_48A2A0();
int sub_48A3D0();
int sub_48A5D0();
int sub_48A5E0();
void sub_48A7F0();
void __cdecl sub_48A820(UINT uFlags);
void __cdecl sub_48A9C0(int a1);
void sub_48AA40();
void __cdecl sub_48AAF0();
void __cdecl sub_48AB50();
void __cdecl sub_48ABC0();
int sub_48ACC0();
int nox_xxx_initDDraw_48B000();
int nox_video_initCursorDrawVars_48B1F0();
void nox_video_stopCursorDrawThread_48B350();
int __cdecl sub_48B3E0(int a1);
int __cdecl sub_48B3F0(int a1, int a2, int a3);
void sub_48B590(_DWORD* a1, _DWORD* a2, _DWORD* a3);
int __cdecl nox_video_waitVBlankAndDrawCursorFromThread_48B5D0(int a1, int a2);
void __cdecl sub_48B680(int a1);
int __cdecl sub_48B6B0(int a1, int a2, int a3);
int __cdecl sub_48B800(int a1);
int __cdecl sub_48B840(int a1);
int __cdecl sub_48BA50(int a1);
void __cdecl sub_48BD90(int a1);
BOOL nox_xxx_makeFillerColor_48BDE0();
int __cdecl sub_48BE50(int a1);
int nox_video_createCursorDrawThread_48BE70();
int nox_video_cursorThreadWorker_48BEB0();
int nox_video_createCursorSurface_48BF70();
int sub_48C060();
int __cdecl sub_48C0C0(int a1, _DWORD* a2, _DWORD* a3);
void sub_48C110();
void sub_48C170(int4* a1, int4* a2); // idb
unsigned __int8* __cdecl sub_48C200(int a1, int a2, int a3);
unsigned __int8* __cdecl sub_48C320(int a1, int a2, int a3);
char sub_48C420();
__int16 sub_48C480();
int sub_48C4D0();
int sub_48C560();
void nullsub_11(void); // weak
void __cdecl sub_48C580(pixel8888* a1, int num);
__int16 __cdecl sub_48C5B0(__int16* a1, int a2);
int __cdecl sub_48C5E0(int a1, int a2);
__int16 sub_48C610();
_DWORD* __cdecl sub_48C650(int a1, int a2, int a3, _DWORD* a4, _DWORD* a5);
unsigned int __cdecl sub_48C690(int a1, int a2, int a3, int a4);
unsigned int __cdecl sub_48C6B0(int a1, int a2);
double __cdecl sub_48C6D0(float a1, float a2, float a3, float a4);
double __cdecl sub_48C700(float a1, float a2);
unsigned int __fastcall sub_48C730(unsigned int a1);
int __cdecl nox_client_winVerGetMajor_48C870(LPSTR); // idb
int __cdecl nox_xxx_msgError_48C940(LPCSTR lpText);               // idb
int __cdecl nox_xxx_showObserverWindow_48CA70(int a1);
BOOL sub_48CA90();
int sub_48CAB0();
int sub_48CAD0();
int sub_48D000();
int __cdecl nox_xxx_guiKick_48D0A0(int a1, int a2, int* a3, int a4);
int sub_48D120();
char* __cdecl nox_xxx_voteSend_48D260(wchar_t* a1);
char* __cdecl nox_xxx_netSendRenameMb_48D2D0(wchar_t* a1);
int sub_48D340();
int sub_48D3B0();
int nox_xxx_clientVote_48D3E0();
_DWORD* sub_48D410();
int sub_48D450();
int sub_48D4A0();
int __cdecl sub_48D4B0(int a1);
int sub_48D4E0();
BOOL __cdecl sub_48D4F0(unsigned __int16 a1, unsigned __int16 a2);
int __cdecl sub_48D560(unsigned __int16 a1);
_DWORD* __cdecl sub_48D5A0(int a1);
int sub_48D660();
int sub_48D740();
_DWORD* sub_48D760();
int* sub_48D7B0();
BOOL nox_xxx_chatInit_48D7D0();
int sub_48D800();
BOOL __cdecl sub_48D830(nox_drawable* dr);
int __cdecl nox_xxx_netCode2ChatBubble_48D850(int a1);
wchar_t* __cdecl nox_xxx_createTextBubble_48D880(int a1, wchar_t* a2);
int4* __cdecl sub_48D990(_DWORD* a1);
void __cdecl sub_48DCF0(_DWORD* a1);
bool __cdecl sub_48E000(int4* a1, _DWORD* a2); // idb
char __cdecl sub_48E240(int a1, _DWORD* a2);
BOOL __cdecl sub_48E480(_DWORD* a1, _DWORD* a2);
int __cdecl sub_48E530(int a1, int a2);
int __cdecl sub_48E5C0(_DWORD* a1, int a2, int a3);
int* __cdecl sub_48E6A0(char a1, _DWORD* a2, _DWORD* a3, int* a4, int* a5);
void __cdecl sub_48E8E0(int a1);
void sub_48E940();
void sub_48E960();
_DWORD* __cdecl nox_xxx_spriteCreate_48E970(int a1, unsigned int a2, int a3, int a4);
char* __cdecl sub_4947E0(int a1);
int __cdecl sub_4948B0(int a1);
int __cdecl nox_xxx_netCliProcUpdateStream_494A60(unsigned __int8* a1, int a2, _DWORD* a3);
unsigned __int8* __cdecl nox_xxx_netCliUpdateStream2_494C30(unsigned __int8* a1, int a2, int* a3);
int __cdecl nox_xxx_netlist_494E90(int a1);
int sub_494F00();
void sub_494FE0();
char* sub_494FF0();
char* __cdecl sub_495020(int a1);
int __cdecl sub_495060(int a1, __int16 a2, __int16 a3);
int __cdecl sub_4950C0(int a1);
int __cdecl sub_4950F0(int a1, char a2);
int __cdecl sub_495120(int a1, __int16 a2, __int16 a3);
int __cdecl sub_495150(int a1, __int16 a2);
int __cdecl sub_495180(int a1, _WORD* a2, _WORD* a3, _BYTE* a4);
char* sub_4951C0();
BOOL __cdecl nox_xxx_unitSpriteCheckAlly_4951F0(int a1);
int __cdecl sub_495210(int a1);
int sub_495430();
int* __cdecl sub_495500(int* a1);
int sub_4958F0();
BOOL nox_xxx_allocClassListFriends_495980();
void sub_4959B0();
int sub_4959D0();
_DWORD* __cdecl nox_xxx_cliAddObjFriend_4959F0(int a1);
void __cdecl sub_495A20(int a1);
int __cdecl sub_495A80(int a1);
int nox_xxx_allocArrayDrawableFX_495AB0();
int sub_495AE0();
void __cdecl sub_495B00(nox_drawable* dr);
_DWORD* __cdecl sub_495B50(_DWORD* a1);
_DWORD* __cdecl sub_495BB0(_DWORD* a1, _DWORD* a2);
int __cdecl sub_495BF0(int a1, int a2, int a3);
int __cdecl sub_495D00(_DWORD* a1, int a2, _DWORD* a3);
void __cdecl sub_495F30(int a1, int a2);
void __cdecl sub_495F70(int a1);
_DWORD* __cdecl sub_495FC0(_DWORD* a1, int a2);
int __cdecl sub_496020(int a1, int a2);
void __cdecl sub_496050(int a1);
int sub_4960B0();
int sub_496120();
int __cdecl nox_xxx_drawBlack_496150(int* a1);
int __cdecl sub_497180(float* a1, float* a2, float* a3);
int __cdecl sub_497260(int* a1);
void __cdecl sub_4974B0(int a1);
int __cdecl sub_497650(float); // idb
int __cdecl sub_4977C0(int a1);
char __cdecl sub_497B80(float* a1, int* a2);
int __cdecl nox_xxx_drawBlackofWall_497C40(int a1, int a2, char a3);
int __cdecl sub_497F60(int a1, int a2, char a3, int a4, int a5);
int __cdecl sub_498030(_DWORD* a1);
int sub_498110();
int __cdecl sub_498130(int a1);
int __cdecl sub_4981D0(int a1);
int __cdecl sub_498220(int a1, int a2);
int __cdecl sub_498290(int a1);
int __cdecl sub_4982E0(int a1);
int __cdecl sub_498330(int a1);
void __cdecl sub_498380(int a1, int a2);
int __cdecl nox_xxx_client_4984B0_drawable(int a1);
void __cdecl sub_4989A0();
void sub_498AE0();
int __cdecl sub_498B50(int a1, int a2, int a3, int a4);
int __cdecl sub_498C20(int2* a1, int2* a2, int a3); // idb
char __cdecl sub_4990D0(_DWORD* a1, _DWORD* a2);
int __cdecl sub_499130(int* a1);
BOOL __cdecl sub_499160(int2* a1, int2* a2, int2* a3); // idb
void __cdecl sub_4991E0(_DWORD* a1);
_QWORD __cdecl sub_499290(int a1);
BOOL __cdecl sub_4992B0(int a1, int a2);
int nox_xxx_loadReflSheild_499360();
int sub_499450();
int __cdecl nox_xxx_drawShield_499810(int a1, int a2);
_DWORD* __cdecl nox_xxx_fxDrawTurnUndead_499880(__int16* a1);
int __cdecl nox_xxx_makeLightningParticles_4999D0(int a1, int2* a2, int2* a3); // idb
void __cdecl nox_xxx_drawPointMB_499B70(int xLeft, int yTop, int a3);
void nox_xxx_bookRewardCli_499CF0(int* a1, int a2, int a3);
_DWORD* __cdecl sub_499F60(int a1, int a2, int a3, __int16 a4, char a5, char a6, char a7, char a8, char a9, int a10);
int __cdecl sub_49A150(int2* a1, int a2, unsigned __int8 a3); // idb
void nox_alloc_npcs();
void nox_xxx_j_resetNPCRenderData_49A2E0(void);           // weak
void __cdecl nox_alloc_npcs_2(); // weak
nox_npc* __cdecl nox_new_npc(int id);
nox_npc* __cdecl nox_npc_by_id(int id);
int __cdecl nox_init_npc(nox_npc* ptr, int id);
nox_npc* __cdecl nox_npc_set_328(int id, int a2);
char* __cdecl nox_xxx_clientEquip_49A3D0(char a1, int a2, int a3, int a4);
int nox_xxx_allocArrayHealthChanges_49A5F0();
void sub_49A630();
_WORD* __cdecl nox_xxx_cliAddHealthChange_49A650(int a1, __int16 a2);
void __cdecl sub_49A6A0(_DWORD* a1, int a2);
void __cdecl sub_49A880(int a1);
int sub_49A8C0();
int sub_49A8E0_init();
int sub_49A950_free();
int __cdecl nox_xxx_sprite_49A9B0_drawable(nox_drawable* dr);
void __cdecl nox_xxx_sprite_49AA00_drawable(nox_drawable* a1);
void __cdecl nox_xxx_updateSpritePosition_49AA90(nox_drawable* dr, int a2, int a3);
void __cdecl nox_xxx_spriteEnum_49AB00(int4*, void(__cdecl*)(nox_drawable*, int), int);
_DWORD* __cdecl sub_49ABF0(_DWORD* a1, int a2);
int __cdecl sub_49AD20(_DWORD* a1, int a2);
int __cdecl nox_xxx_unused_49ADD0(int a1);
int sub_49AEA0();
void sub_49AEE0();
int* __cdecl nox_xxx_unused_49AF80(int a1, int a2, int a3);
int __cdecl nox_xxx_unused_49B0A0(int a1, int a2, int a3);
int __cdecl sub_49B1A0(int a1);
int __cdecl sub_49B1D0(int a1, int a2);
int sub_49B260();
int sub_49B370();
int sub_49B380();
int sub_49B3A0();
int sub_49B3B0();
int sub_49B3C0();
int sub_49B3E0();
int __cdecl sub_49B420(int a1, int a2, int* a3, int a4);
int sub_49B490();
int sub_49B6B0();
void nox_xxx_consoleEsc_49B7A0();
_DWORD* __cdecl nox_xxx_spriteTransparentDecay_49B950(_DWORD* a1, int a2);
_DWORD* __cdecl nox_xxx_sprite_49BA10(nox_drawable* dr);
int sub_49BA70();
_DWORD* __cdecl nox_xxx_spriteToSightDestroyList_49BAB0_drawable(_DWORD* a1);
_DWORD* __cdecl sub_49BAF0(nox_drawable* dr);
void sub_49BB40();
void* __cdecl sub_49BB80(char a1);
void sub_49BBB0();
void sub_49BBC0();
_DWORD* __cdecl nox_xxx_spriteToList_49BC80_drawable(_DWORD* a1);
_DWORD* __cdecl sub_49BCD0(nox_drawable* dr);
int nox_xxx_getSomeSprite_49BD40();
int __cdecl nox_xxx_getSprite178_49BD50(int a1);
int(__cdecl* __cdecl sub_49BD70(int a1))(int, int);
_DWORD* __cdecl nox_xxx_clientAddRayEffect_49C160(int a1);
void __cdecl nox_xxx_clientRemoveRayEffect_49C450(int a1);
void nox_xxx_spriteDeleteSomeList_49C4B0();
void nox_xxx_sprite_49C4F0();
int __cdecl sub_49C520(int a1);
int __cdecl nox_xxx_wnd_49C760(int a1, int a2, int* a3, int a4);
int sub_49C7A0();
BOOL sub_49C810();
int __cdecl sub_49CA60(int a1, int a2, int* a3, int a4);
BOOL sub_49CB40();
int nox_video_initRectDrawingFuncs_49CB50();
void nullsub_12(void); // weak
void __cdecl nox_client_drawBorderLines_49CC70(int xLeft, int yTop, int a3, int a4);
void __cdecl sub_49CD30(int xLeft, int yTop, int a3, int a4, int a5, int a6);
void __cdecl nox_client_drawRectFilledOpaque_49CE30(int xLeft, int yTop, int a3, int a4);
int4* __cdecl nox_client_drawRectFilledAlpha_49CF10(int xLeft, int yTop, int a3, int a4);
int4* __cdecl sub_49CFB0(int xLeft, int yTop, int a3, int a4);
int4* __cdecl sub_49D050(int xLeft, int yTop, int a3, int a4);
int4* __cdecl nox_client_drawRectFadingScreen_49D0F0(int xLeft, int yTop, int a3, int a4);
int __cdecl nox_client_drawRectStringSize_49D190(int a1, int a2, int a3, int a4, int a5);
int __cdecl sub_49D1C0(int a1, int a2, int a3);
void __cdecl sub_49D1E0(int a1, int a2, unsigned int a3, int a4);
void __cdecl nox_xxx_draw_49D270_MBRect_49D270(int a1, int a2, unsigned int a3, int a4);
int __cdecl sub_49D2F0(int a1, int a2, int a3, int a4);
void __cdecl sub_49D370(int a1, int a2, int a3, int a4);
int __cdecl sub_49D540(int a1, int a2, int a3, int a4);
int __cdecl sub_49D680(int a1, int a2, int a3, int a4);
__int16 __cdecl sub_49D6F0(int a1, int a2, unsigned int a3, int a4);
void __cdecl sub_49D770(int a1, int a2, unsigned int a3, int a4);
void __cdecl sub_49D880(int a1, int a2, int a3, int a4);
int __cdecl sub_49D8E0(int a1, int a2, int a3, int a4);
int __cdecl sub_49D9A0(int a1, int a2, int a3, int a4);
unsigned __int8 __cdecl sub_49DA90(int a1, int a2, int a3, int a4);
unsigned __int8 __cdecl sub_49DB20(int a1, int a2, int a3, int a4);
int __cdecl sub_49DBB0(int a1, int a2, int a3, int a4);
int __cdecl sub_49DC70(int a1, int a2, int a3, int a4);
int4* __cdecl sub_49DD60(int a1, int a2, int a3, int a4, int a5);
int4* __cdecl sub_49E060(int a1, int a2, int a3, int a4, int a5);
void __cdecl sub_49E380(_DWORD* a1, int a2, unsigned int a3);
void __cdecl sub_49E3C0(_DWORD* a1, int a2, unsigned int a3);
int nox_video_initLineDrawingFuncs_49E3F0();
void nullsub_13(void); // weak
int nox_client_drawLineFromPoints_49E4B0();
int __cdecl sub_49E4C0(int a1, int a2, int a3, int a4);
int __cdecl sub_49E4F0(int a1);
int __cdecl sub_49E510(int a1);
int __cdecl sub_49E540(int a1);
int __cdecl sub_49E6C0(int a1);
int __cdecl sub_49E930(int a1);
int __cdecl sub_49EAB0(int a1);
int __cdecl sub_49ED80(unsigned __int8 a1, int a2);
int __cdecl sub_49EFA0(int a1, int a2);
int __cdecl sub_49EFC0(int a1, int a2);
int __cdecl sub_49F010(int a1, int a2);
void __cdecl sub_49F060(int a1, int a2, int a3);
unsigned __int8* __cdecl sub_49F0F0(int a1, int a2, int a3);
void __cdecl sub_49F180(int a1, int a2, int a3);
__int16 __cdecl sub_49F210(int a1, int a2, int a3);
void __cdecl sub_49F3A0(int a1, int a2, int a3);
void __cdecl sub_49F420(int a1, int a2, int a3);
BOOL sub_49F4A0();
void sub_49F4D0();
int __cdecl nox_client_drawAddPoint_49F500(int a1, int a2);
int __cdecl nox_xxx_rasterPointRel_49F570(int a1, int a2);
int __cdecl sub_49F5B0(_DWORD* a1, _DWORD* a2, int a3);
int sub_49F610();
void nullsub_14(void); // weak
int __cdecl sub_49F6D0(int a1);
int sub_49F6E0();
int4* __cdecl nox_client_copyRect_49F6F0(int xLeft, int yTop, int a3, int a4);
int4* __cdecl sub_49F780(int xLeft, int a2);
int4* __cdecl sub_49F7C0(int a1);
int nox_xxx_wndDraw_49F7F0();
int sub_49F860();
BOOL __cdecl sub_49F8E0(int a1, int a2, int a3);
int4* __cdecl nox_xxx_utilRect_49F930(int4* a1, int4* a2, int4* a3); // idb
int __cdecl sub_49F990(int* a1, int* a2, int* a3, int* a4);
int __cdecl sub_49FC20(int* a1, int* a2, int* a3, int* a4);
void __cdecl sub_49FDB0(int a1);
_DWORD* sub_49FF20();
char* __cdecl sub_49FF40(__int16* a1);
int* __cdecl sub_49FFA0(int a1);
char* sub_4A0020();
int __cdecl sub_4A0030(const void* a1);
int __cdecl sub_4A0290(int a1);
int __cdecl sub_4A0330(int* a1);
int* sub_4A0360();
int* sub_4A0390();
int __cdecl sub_4A0410(const char* a1, __int16 a2);
int* __cdecl sub_4A0490(int a1);
int* __cdecl sub_4A04C0(int a1);
int* __cdecl sub_4A04F0(char* a1);
void __cdecl sub_4A0540(LPVOID lpMem); // idb
int nox_xxx_getConnResult_4A0560();
int __cdecl nox_xxx_parseColor_4A0570(int* a1, char* a2);
int __cdecl nox_xxx_guiParseColor_4A05E0(int* a1, FILE* a2, char* a3);
int __cdecl sub_4A0650(int a1, char* a2);
int __cdecl sub_4A0690(int a1, char* a2);
int __cdecl sub_4A06D0(int a1, char* a2);
int __cdecl sub_4A0710(int a1, char* a2);
int __cdecl sub_4A0750(int a1, char* a2);
int __cdecl sub_4A0790(int a1, char* a2);
int __cdecl sub_4A07D0(int a1, char* a2);
int __cdecl sub_4A0830(int a1, char* a2);
int __cdecl sub_4A0870(int a1, char* a2);
int __cdecl sub_4A08C0(int a1, char* a2);
int __cdecl sub_4A0910(int a1, char* a2);
int __cdecl sub_4A0960(int a1, char* a2);
int __cdecl sub_4A09B0(int a1, char* a2);
int __cdecl nox_xxx_parseWindowStatus_4A0A00(int a1, const char* a2);
int __cdecl nox_xxx_parseWindowStyle_4A0A30(int a1, const char* a2);
int __cdecl sub_4A0A60(int a1, char* a2);
nox_window* __cdecl nox_new_window_from_file(const char* a1, int (*a2)(int, int, int, int));
int nox_xxx_wndResetParentStack_4A0CF0();
int sub_4A0D10();
BOOL __cdecl sub_4A0D40(int* a1, FILE* a2, char* a3);
_DWORD* __cdecl nox_xxx_guiWndLoad2_4A0D80_parse_window(FILE* a1, char* a2, int (*a3)(int, int, int, int));
nox_window* __cdecl nox_xxx_wndLoadControlOrUser_4A1440_parse_window(const char* a1, int a2, int a3, wchar_t* a4, int a5, int a6, int a7,
											_DWORD* a8, float* a9, int (*a10)(int, int, int, int));
int nox_xxx_wndLoadStackPop_4A14F0();
nox_window* __cdecl nox_xxx_wndLoadControl_4A1510_parse_window(const char* a1, int a2, int a3, wchar_t* a4, int a5, int a6, int a7,
											_DWORD* a8, float* a9);
BOOL __cdecl nox_xxx_guiParse_4A1780(int a1, FILE* a2, char* a3);
int sub_4A18A0();
int __cdecl sub_4A18C0(int a1);
int __cdecl sub_4A18E0(int, int, int, int); // weak
int sub_4A19D0();
int __cdecl sub_4A1A40(int a1);
int sub_4A1A60();
int sub_4A1BD0();
int __cdecl sub_4A1BE0(int a1);
int sub_4A1C00();
int sub_4A1D40();
int sub_4A1D80();
int sub_4A2200();
int nox_xxx_wndLoadMainBG_4A2210();
BOOL __cdecl sub_4A2490(int a1, int a2, int a3, int a4);
int sub_4A24A0();
int __cdecl sub_4A24C0(int a1);
int sub_4A24F0();
int sub_4A2500();
int sub_4A2530();
BOOL __cdecl sub_4A2560(_DWORD* a1, int a2);
int __cdecl sub_4A25C0(_DWORD* a1, int* a2);
int __cdecl sub_4A2610(int a1, _DWORD* a2, int* a3);
_DWORD* __cdecl sub_4A2830(int a1, int a2, _DWORD* a3);
int sub_4A2890();
BOOL sub_4A28B0();
int __cdecl sub_4A28C0(int a1);
int __cdecl nox_xxx_wndListboxProcWithoutData10_4A28E0(_DWORD* a1, int a2, unsigned int a3, int a4);
int __cdecl nox_xxx_wndListBox_4A2D10(int a1, int a2, int a3);
int __cdecl nox_xxx_wndListboxProcWithData10_4A2DE0(int a1, int a2, unsigned int a3, int a4);
__int16* __cdecl sub_4A3090(__int16* a1, int a2);
int __cdecl nox_xxx_wndListboxProcPre_4A30D0(int a1, unsigned int a2, wchar_t* a3, int a4);
int __cdecl nox_xxx_wndListBox_4A3A70(int a1);
int __cdecl nox_xxx_wndListBoxAddLine_4A3AC0(wchar_t* a1, int a2, _DWORD* a3);
void __cdecl nox_xxx_wndListboxInit_4A3C00(int a1, int a2);
int __cdecl nox_xxx_wndListboxDrawNoImage_4A3C50(_DWORD* a1, int a2);
int __cdecl nox_xxx_wndListboxDrawWithImage_4A3FC0(_DWORD* a1, int a2);
int __cdecl sub_4A4800(int a1);
int sub_4A4840();
int sub_4A4970();
int sub_4A49A0();
int __cdecl sub_4A49D0(int yTop, int a2);         // idb
int __cdecl sub_4A4CB0(const void*, const void*); // idb
int sub_4A50A0();
int sub_4A50D0();
int __cdecl sub_4A5690(_BYTE* a1);
void* sub_4A56E0();
void sub_4A5700();
int sub_4A5C70();
int sub_4A5E90();
unsigned __int8* __cdecl sub_4A61E0(_DWORD* a1, int a2, _BYTE* a3);
void sub_4A62B0();
int sub_4A6890();
int __cdecl sub_4A6B50(wchar_t* a1);
int sub_4A6C90();
int __cdecl sub_4A6D20(int a1, int a2);
int __cdecl sub_4A6DC0(_DWORD* a1, int a2);
int __cdecl sub_4A7270(int a1, int a2, unsigned int a3, int a4);
_DWORD* __cdecl sub_4A72D0(unsigned __int16 a1);
int __cdecl sub_4A7330(int a1, int a2, int* a3, unsigned int a4);
_DWORD* __cdecl sub_4A7530(unsigned __int16 a1);
int __cdecl sub_4A7580(int a1, int a2);
int __cdecl sub_4A7A60(int a1);
int __cdecl sub_4A7A70(int a1);
int __cdecl sub_4A7A80(const char* a1);
int __cdecl sub_4A7AC0(const char* a1);
int __cdecl sub_4A7B00(const char* a1);
int __cdecl sub_4A7B40(char* a1);
int __cdecl sub_4A7BA0(char* a1);
int __cdecl sub_4A7BC0(const char* a1);
int __cdecl sub_4A7C00(const char* a1);
int __cdecl sub_4A7C40(char* a1);
int __cdecl sub_4A7C60(char* a1);
int __cdecl sub_4A7CE0(char* a1);
int __cdecl sub_4A7D00(const char* a1);
int __cdecl sub_4A7D50(char* a1);
int __cdecl sub_4A7D70(char* a1);
int __cdecl sub_4A7DF0(char* a1);
char* sub_4A7E70();
char* sub_4A7E80();
char* sub_4A7E90();
int sub_4A7EA0();
int sub_4A7EB0();
char* sub_4A7EC0();
char* sub_4A7ED0();
int sub_4A7EE0();
char* sub_4A7EF0();
int sub_4A7F00();
char* sub_4A7F10();
int sub_4A7F20();
int sub_4A7F30();
int __cdecl sub_4A7F40(int a1);
int __cdecl nox_xxx_wndButtonProc_4A7F50(_DWORD* a1, int a2, int a3, int a4);
int __cdecl nox_xxx_wndButtonDrawNoImg_4A81D0(int a1, int a2);
int __cdecl nox_xxx_wndButtonInit_4A8340(int a1);
int __cdecl nox_xxx_wndButtonDraw_4A8380(_DWORD* a1, int a2);
int __cdecl nox_xxx_wndRadioButtonProc_4A84E0(_DWORD* a1, int a2, int a3, int a4);
int __cdecl nox_xxx_wndRadioButtonSetAllFn_4A87E0(int a1);
int __cdecl nox_xxx_wndRadioButtonDrawNoImg_4A8820(int a1, int a2);
int __cdecl nox_xxx_wndRadioButtonDraw_4A8A20(int a1, int a2);
int __cdecl nox_xxx_wndCheckBoxProc_4A8C00(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_wndCheckBoxInit_4A8E60(int a1);
int __cdecl nox_xxx_wndDrawCheckBoxNoImg_4A8EA0(int a1, int a2);
int __cdecl nox_xxx_wndDrawCheckBox_4A9050(_DWORD* a1, int a2);
int __cdecl nox_xxx_wndCreateButtonOrChekBox_4A91A0(int a1, int a2, int a3, int a4, int a5, int a6, _DWORD* a7);
int __cdecl nox_xxx_wndButtonProcPre_4A9250(int a1, int a2, wchar_t* a3, int a4);
int __cdecl nox_xxx_wndCheckboxProcMB_4A92C0(int a1, int a2, wchar_t* a3, int a4);
int __cdecl nox_xxx_wndCreateRadioButton_4A9330(int a1, int a2, int a3, int a4, int a5, int a6, int a7, _DWORD* a8);
int __cdecl nox_xxx_wndRadioButtonProcPre_4A93C0(int a1, int a2, wchar_t* a3, int a4);
int nox_xxx_loadDefColor_4A94A0();
FILE* __cdecl nox_xxx_loadPal_4A96C0_video_read_palette(char* a1);
int sub_4A9A10();
int __cdecl sub_4A9A30(unsigned __int8 a1, unsigned __int8 a2, unsigned __int8 a3);
__int16 __cdecl sub_4A9B20(int a1);
__int16 __cdecl sub_4A9B70(__int16* a1);
int __cdecl nox_xxx_unused_4A9BC0(unsigned __int8 a1, unsigned __int8 a2, unsigned __int8 a3);
int __cdecl sub_4A9C50(int a1);
int nox_xxx_compassGenStrings_4A9C80();
__int16 __cdecl sub_4A9E90(int a1, int a2, int a3, int a4);
__int16 __cdecl sub_4AA030(_DWORD* a1, int a2);
char* nox_xxx_wndLoadBorder_4AA1F0();
int sub_4AA270();
int sub_4AA450();
int sub_4AA490();
int sub_4AA6B0();
int sub_4AA9C0();
int sub_4AAA10();
_DWORD* sub_4AAA70();
int __cdecl sub_4AABE0(int a1, int a2, int* a3, int a4);
int sub_4AB0C0();
int nox_xxx_gameStartup_4AB0F0();
int sub_4AB170();
int sub_4AB1F0();
int sub_4AB260();
int __cdecl sub_4AB340(int a1, int a2, int a3, int a4);
int __cdecl sub_4AB390(int a1, int a2, int* a3, int a4);
int __cdecl sub_4AB420(int* a1);
int sub_4AB470();
int __cdecl sub_4AB4A0(int a1);
int __cdecl sub_4AB4D0(int a1);
int __cdecl nox_xxx_gameStopDownload_4AB560(int a1);
int __cdecl nox_xxx_mapSetDownloadOK_4AB570(int a1);
LPCSTR sub_4AB580();
int nox_xxx_gameDownloadMap_4AB5E0();
int nox_xxx_mapDeleteFile_4AB720();
void __cdecl nox_xxx_netMapDownloadPart_4AB7C0(unsigned __int16 a1, void* a2, size_t a3);
int __cdecl sub_4AB9B0(char* a1);
LPCSTR nox_xxx_cliCancelMapDownload_4ABA90();
int __cdecl nox_xxx_mapDownloadStart_4ABAD0(char* a1, unsigned int a2);
int __cdecl sub_4ABDA0(int a1, __int16 a2, __int16 a3, _DWORD* a4);
int __cdecl nox_xxx_spriteLoadFromMap_4AC020(int thingInd, __int16 a2, _DWORD* a3);
char* __cdecl nox_xxx_mapCliReadAll_4AC2B0(char* a1);
int nox_client_mapSpecialRWObjectData_4AC610();
int __cdecl nox_xxx_clientLoadSomeObject_4AC6E0(unsigned __int16 a1);
int __cdecl sub_4AC7B0(int a1);
int __cdecl nox_xxx_colorLightClientLoad_4AC980(int a1);
int __cdecl nox_xxx_cliLoadTeamBase_4ACE00(int a1);
int __cdecl sub_4ACEF0(int a1);
int __cdecl sub_4AD040(int a1);
int sub_4AD100();
void nullsub_15(void); // weak
void sub_4AD150();
void sub_4AD170_call_copy_backbuffer(void); // weak
void sub_4AD180();
void sub_4AD1E0();
void sub_4AD2A0();
int sub_4AD570();
int __cdecl nox_xxx_windowServerOptionsGeneralProc_4AD5D0(int a1, int a2, int* a3, int a4);
int sub_4AD820();
int __cdecl sub_4AD9B0(int a1);
int sub_4ADA40();
int sub_4ADAD0();
int __cdecl sub_4ADEF0(_DWORD* a1, int a2);
int __cdecl nox_xxx_windowOptionsProc_4ADF30(int a1, int a2, int* a3, int a4);
int sub_4AE3B0();
BOOL sub_4AE3D0();
int sub_4AE400();
void nullsub_16(void); // weak
int sub_4AE420();
int sub_4AE470();
int sub_4AE520();
void sub_4AE540();
int __cdecl sub_4AE580(int a1);
int __cdecl sub_4AE5C0(int a1);
BOOL __cdecl sub_4AE5E0(int a1, int a2);
int __cdecl sub_4AE6F0(int a1, int a2, int a3, int a4, int a5);
int sub_4AEBD0();
int __cdecl sub_4AEC20(int a1, int a2);
int __cdecl sub_4AEDA0(int* a1, int* a2, int a3, int a4);
void* sub_4AEDF0();
__int64 sub_4AEE30();
int sub_4AEE80();
void nullsub_17(void); // weak
unsigned __int8* __cdecl nox_xxx_ParticleFxT0_4AEEA0(unsigned __int8** a1);
char* __cdecl sub_4AEED0(int* a1, int a2, int a3);
int __cdecl nox_xxx_ParticleFxT1_4AEF80(int* a1);
int __cdecl sub_4AF060(int a1);
unsigned __int8* __cdecl nox_xxx_ParticleFxT2_4AF0F0(unsigned __int8** a1);
int __cdecl sub_4AF200(int a1);
unsigned __int8* __cdecl nox_xxx_ParticleFxT3_4AF2A0(unsigned __int8** a1);
void __cdecl sub_4AF2D0(int* a1, int a2, int a3);
_DWORD* __cdecl nox_xxx_ParticleFxT4_4AF3D0(_DWORD* a1);
int __cdecl sub_4AF400(int a1, int a2, int a3);
char* __cdecl nox_xxx_ParticleFxT5_4AF450(int* a1);
int __cdecl sub_4AF4C0(int a1);
char* __cdecl nox_xxx_ParticleFxT6_4AF5A0(int* a1);
int __cdecl nox_xxx_partFx_4AF600(int a1);
_DWORD* __cdecl nox_xxx_partfxSwitch_4AF690(_DWORD* a1, void(__cdecl* a2)(_DWORD*, _DWORD*, int));
unsigned __int8* __cdecl sub_4AF890(unsigned __int8** a1);
int __cdecl sub_4AF8C0(_DWORD* a1);
int sub_4AF8D0();
void sub_4AF950();
char* __cdecl nox_xxx_drawPartFx2_4AF990(int a1, int a2, int a3, int a4);
void __cdecl sub_4AFA40(int a1);
char* __cdecl sub_4AFA70(int a1);
int __cdecl sub_4AFAF0(_DWORD* a1, int a2);
int __cdecl sub_4AFB00(int a1, int a2);
int __cdecl sub_4AFB10(int a1, int a2);
int __cdecl sub_4AFB40(int a1, int a2);
int __cdecl sub_4AFB50(int a1, int a2);
int __cdecl sub_4AFB60(int a1, int a2);
int __cdecl sub_4AFB70(int a1, int a2);
int __cdecl sub_4AFB80(int a1, int a2);
int __cdecl sub_4AFB90(int a1, int a2);
int __cdecl sub_4AFBA0(int a1, int a2);
_DWORD* __cdecl sub_4AFBB0(_DWORD* a1, int a2, int a3, int a4);
int __cdecl sub_4AFBD0(int a1, int a2);
int __cdecl sub_4AFBE0(int a1, int a2);
int __cdecl sub_4AFBF0(int a1, int a2);
_DWORD* __cdecl sub_4AFC00(_DWORD* a1, int a2, int a3, int a4);
int __cdecl sub_4AFC20(int a1, int a2);
int __cdecl sub_4AFC30(int a1, int a2);
int __cdecl sub_4AFC40(int a1, int a2);
int __cdecl sub_4AFC50(int a1, int a2);
int __cdecl sub_4AFC60(int a1, int a2, int a3);
void __cdecl sub_4AFC90(_DWORD* a1, int a2, int a3, int a4);
_DWORD* __cdecl sub_4AFCD0(_DWORD* a1);
int __cdecl nox_xxx_registerParticleFx_4AFCF0(int a1, int a2, int a3, int a4);
void sub_4AFD40();
int __cdecl nox_xxx_partfxLoadParticle_4AFE20(_DWORD* a1, CHAR* a2);
int __cdecl nox_xxx_drawParticlefx_4AFEB0(int* a1);
int __cdecl sub_4B0020(_DWORD* a1);
void __cdecl sub_4B01A0(int a1);
char* nox_xxx_partfxAllocSmth_4B01B0();
int __cdecl sub_4B0220(size_t a1);
int sub_4B02D0();
void nullsub_18(void); // weak
int __cdecl sub_4B0300(char* a1);
int __cdecl sub_4B0340(int a1);
void sub_4B05D0();
void __cdecl sub_4B0610(int a1);
int __cdecl sub_4B0640(int (*a1)(void));
int sub_4B0650();
void sub_4B0660();
int* __cdecl sub_4B0680(unsigned __int8 a1, unsigned __int8 a2);
int __cdecl sub_4B07D0(LPVOID lpMem); // idb
int(__cdecl* __cdecl sub_4B0820(int a1, int a2, int a3))(_DWORD);
int __cdecl sub_4B0870(int* a1);
int __cdecl sub_4B0B20(int a1);
int nox_video_assignCircleDrawFuncs_4B0B30();
void nullsub_19(void); // weak
int __cdecl nox_video_drawCircle_4B0B90(int a1, int a2, int a3);
int __cdecl sub_4B0BC0(int a1, int a2, int a3);
char __cdecl nox_video_drawCircle8Opaque_4B0D30(int a1, int a2, int a3);
int __cdecl sub_4B0F50(int a1, int a2, int a3);
__int16 __cdecl nox_video_drawCircle16Opaque_4B1380(int a1, int a2, int a3);
int __cdecl sub_4B15E0(int a1, int a2, int a3);
int __cdecl nox_video_drawCircle8Alpha_4B1A60(int a1, int a2, int a3);
int __cdecl sub_4B1E30(int a1, int a2, int a3);
_WORD* __cdecl nox_video_drawCircle16Alpha_4B2480(int a1, int a2, int a3);
int __cdecl sub_4B3450(int a1, int a2, int a3);
int __cdecl sub_4B4860(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_wndScrollBoxDraw_4B4BA0(int a1, int a2, unsigned int a3, int a4);
_DWORD* __cdecl nox_xxx_wndScrollBoxCreate_4B4EE0(int a1, int a2, int a3, int a4, int a5, int a6, _DWORD* a7, float* a8);
int __cdecl sub_4B5010(int a1, unsigned int a2, int a3, int a4);
int __cdecl sub_4B51A0(int a1);
int __cdecl sub_4B51E0(int a1, int a2);
int __cdecl sub_4B52C0(int a1, int a2);
int __cdecl nox_xxx_wndScrollBoxProc_4B5320(int a1, unsigned int a2, int a3, unsigned int a4);
int __cdecl nox_xxx_wndScrollBoxSetAllFn_4B5500(int a1);
int __cdecl nox_xxx_wndScrollBoxDraw_4B5540(int a1, int a2);
int __cdecl nox_xxx_wndScrollBoxDraw_4B5620(_DWORD* a1, int a2);
int __cdecl nox_xxx_wndScrollBoxButtonCreate_4B5640(int a1, int a2, int a3);
void __cdecl sub_4B5700(int a1, int a2, int a3, int a4, int a5, int a6);
void* sub_4B5990();
int __cdecl sub_4B5AB0(int a1, int a2, int* a3, int a4);
void sub_4B5BF0();
int sub_4B5CD0();
int __cdecl sub_4B63B0(int2* a1, int2* a2); // idb
int sub_4B64C0();
void __cdecl sub_4B6720(int2*, int, int, char);
int __cdecl sub_4B6880(_DWORD* a1, int a2, int a3, int a4);
int __cdecl sub_4B6970(_DWORD* a1, nox_drawable* dr, int a3, int a4);
__int16 __cdecl sub_4B69F0(int a1);
int __cdecl sub_4B6B80(int* a1, nox_drawable* dr, int a3);
int __cdecl sub_4B71A0(_DWORD* a1, int a2);
_DWORD* __cdecl nox_xxx_netHandleSummonPacket_4B7C40(__int16 a1, unsigned __int16* a2, unsigned __int16 a3, unsigned __int8 a4, __int16 a5);
void __cdecl sub_4B7EE0(__int16 a1);
int nox_xxx_spriteShieldLoad_4B7F90();
_DWORD* __cdecl nox_xxx_fxShield_4B8090(unsigned int a1, int a2);
void __cdecl nox_xxx_spriteScanForShield_4B81E0(int, int); // idb
int nox_xxx_gameDeleteSpiningCrownSkull_4B8220();
__int16 __cdecl sub_4B8960(int* a1, nox_drawable* dr, int a3, _DWORD* a4, int a5, int a6);
_DWORD* __cdecl sub_4B8CA0(_DWORD* a1, char* a2);
__int16 __cdecl sub_4B8D40(int* a1, nox_drawable* dr, int a3, _DWORD* a4, int a5, int a6);
_DWORD* __cdecl sub_4B8E10(_DWORD* a1, char* a2);
void __cdecl nox_xxx_drawOtherPlayerHP_4B8EB0(_DWORD* a1, nox_drawable* dr, unsigned __int16 a3, char a4);
int __cdecl sub_4B8FA0(nox_drawable* dr, int* a2, int* a3);
int __cdecl sub_4B9470(const char** a1);
int __cdecl sub_4B94E0(nox_drawable* dr);
_DWORD* __cdecl sub_4B95D0(nox_drawable* dr);
_DWORD* __cdecl sub_4B9650(int a1);
_DWORD* __cdecl sub_4B96F0(nox_drawable* dr);
void __cdecl sub_4BA670(int a1, int a2, int a3, int a4, int a5);
int nox_xxx_prepareLightningEffects_4BAB30();
void nullsub_20(void); // weak
void __cdecl nox_xxx_spriteDrawMonsterHP_4BC080(_DWORD* a1, int a2, unsigned __int16 a3, unsigned __int16 a4, char a5);
int __cdecl sub_4BC6B0(int* a1, nox_drawable* dr, int a3);
int __cdecl sub_4BC720(int a1);
int nox_xxx_updDrawMonsterGen_4BC920();
int __cdecl sub_4BD010(nox_drawable* dr, int2* a2, int a3); // idb
_DWORD* __cdecl sub_4BD280(int a1, int a2);
void __cdecl sub_4BD2D0(LPVOID lpMem); // idb
_DWORD* __cdecl sub_4BD2E0(_DWORD** a1);
int __cdecl sub_4BD300(_DWORD* a1, int a2);
int __cdecl sub_4BD320(_DWORD** a1);
_DWORD* __cdecl sub_4BD340(int a1, int a2, int a3, int a4);
void __cdecl sub_4BD3C0(LPVOID lpMem); // idb
_DWORD* __cdecl sub_4BD420(int a1, int a2);
_DWORD* __cdecl sub_4BD450(int a1);
_DWORD* __cdecl sub_4BD470(_DWORD** a1, int a2);
int __cdecl sub_4BD600(int a1);
int __cdecl sub_4BD650(int a1);
int __cdecl sub_4BD660(int a1);
int __cdecl sub_4BD680(int a1);
int __cdecl sub_4BD690(int a1);
int __cdecl nox_xxx___fileno_0_4BD700(int a1);
int __cdecl sub_4BD710(int a1);
_DWORD* __cdecl sub_4BD720(int a1);
void __cdecl sub_4BD7A0(LPVOID lpMem); // idb
_DWORD* __cdecl sub_4BD7C0(_DWORD* a1);
int __cdecl sub_4BD840(int a3);
int __cdecl sub_4BD8C0(int a1);
int __cdecl sub_4BD940(int a1);
int __cdecl sub_4BD9B0(_DWORD* a2);
int __cdecl sub_4BDA00(int a1);
int __cdecl sub_4BDA10(int a1, int a2);
_DWORD* __cdecl sub_4BDA30(_DWORD* a1);
void __cdecl sub_4BDA60(LPVOID lpMem); // idb
int __cdecl sub_4BDA80(int a1);
void __cdecl sub_4BDAC0(int a1);
void __cdecl sub_4BDAF0(int a1);
int __cdecl sub_4BDB20(int a1);
int __cdecl sub_4BDB30(int a1);
int __cdecl sub_4BDB40(int a2);
void __cdecl sub_4BDB90(_DWORD* a1, _DWORD* a2);
int __cdecl sub_4BDC00(int a1);
int __cdecl nox_xxx_loadAdvancedWnd_4BDC10(int* a1);
int __cdecl sub_4BDC70(int* a1);
int sub_4BDD10();
int sub_4BDDA0();
int __cdecl nox_xxx_windowAdvancedServProc_4BDDB0(int a1, int a2, int* a3, int a4);
int sub_4BDF30();
int __cdecl sub_4BDF70(int* a1);
int __cdecl sub_4BDF90(int* a1);
int sub_4BDFD0();
int __cdecl sub_4BE120(int a1);
int sub_4BE320();
int __cdecl sub_4BE330(int a1, unsigned int a2, int* a3, int a4);
int sub_4BE610();
int __cdecl sub_4BE640(int a1, int a2);
__int16 __cdecl nox_video_drawAnimatedImageOrCursorAt_4BE6D0(int a1, int a2, int a3);
__int16 __cdecl sub_4BE710(int a1, int a2, int a3, int a4);
__int16 __cdecl sub_4BE750(int a1, int a2, int a3);
int __cdecl sub_4BE7B0(int a1);
int __cdecl sub_4BE800(int a1);
char __cdecl sub_4BE810(int a1, int a2, int a3, char a4);
int __cdecl sub_4BE840(int* a1, int* a2, int* a3, int* a4, int a5);
void __cdecl sub_4BEAD0(int2*, int2*, int2*, int2*, int, int);
void __cdecl sub_4BEDE0(int2*, int2*, int2*, int2*, int, float, int, int);
void sub_4BF000();
int __cdecl nox_xxx_clientReportSecondaryWeapon_4BF010(int a1);
void sub_4BF040();
__int16 __cdecl sub_4BF7E0(_DWORD* a1);
__int16 __cdecl sub_4BF9F0(int a1, int a2, int a3, int a4, int a5, int a6, int a7);
int sub_4BFAD0();
_DWORD* __cdecl sub_4BFB70(int a1);
_DWORD* __cdecl sub_4BFBB0(_DWORD* a1);
int sub_4BFBF0();
int sub_4BFC70();
int sub_4BFC90();
int __cdecl sub_4BFCD0(int a1, int a2, int* a3, int a4);
_DWORD* sub_4BFD10();
int sub_4BFD30();
int sub_4BFD40();
int __cdecl sub_4BFDD0(_DWORD* a1, int a2, unsigned int a3);
int sub_4BFE40();
int sub_4BFEF0();
int __cdecl sub_4C0030(int a1);
int __cdecl sub_4C01C0(int a1, int a2, int* a3, int a4);
int sub_4C03E0();
int __cdecl sub_4C0430(int a1, int a2, int a3, int a4, int a5, const void* a6, int a7, int a8, int a9, int a10);
int __cdecl sub_4C0560(int a1, int a2);
int __cdecl sub_4C05F0(int a1, int a2);
int nox_xxx_func_4C0610();
int __cdecl sub_4C0630(int a1, unsigned int a2, unsigned int a3);
int __cdecl nox_xxx_clientTrade_0_4C08E0(int a1);
char __cdecl sub_4C0910(int2* a1); // idb
int __cdecl sub_4C0C90(int a1, int a2, int* a3, int a4);
int nox_xxx_clientTrade_4C0CE0();
int sub_4C0D00();
int __cdecl sub_4C1120(int a1, int a2, wchar_t** a3);
char __cdecl sub_4C11E0(_DWORD* a1);
int nox_xxx_closeP2PTradeWnd_4C12A0();
int sub_4C12C0();
int nox_xxx_showP2PTradeWnd_4C12D0();
BOOL __cdecl sub_4C12F0(int* a1);
int __cdecl nox_xxx_netP2PStartTrade_4C1320(int a1);
int sub_4C1410();
int sub_4C1590();
int __cdecl sub_4C1710(int a1, int a2);
int __cdecl sub_4C1760(int a1, int a2);
char* __cdecl nox_xxx_tradeClientAddItem_4C1790(int a1);
int __cdecl sub_4C18E0(int a1, _DWORD* a2);
char* __cdecl sub_4C1910(int a1);
char* __cdecl sub_4C19C0(int a1);
char* __cdecl sub_4C1A70(int a1, int* a2);
int __cdecl sub_4C1B50(int a1);
int __cdecl sub_4C1BC0(int);
int nox_xxx_prepareP2PTrade_4C1BF0();
int __fastcall sub_4C1C60(int a1, int a2);
int __cdecl sub_4C1C80(int a1);
int sub_4C1C90();
int __cdecl sub_4C1CA0(int a1);
int sub_4C1D70();
int __cdecl nox_xxx_guiDrawSummonBox_4C1FE0(_DWORD* a1);
int __cdecl nox_xxx_wndSummonGet_4C2410(int2* a1); // idb
int __cdecl nox_xxx_guiDrawSummon_4C2440(int a1);
int sub_4C2470();
int sub_4C24A0();
int __cdecl nox_xxx_wndSummonBigButtonProc_4C24B0(int a1, int a2, unsigned int a3);
_DWORD* __cdecl nox_xxx_wndSummonCreateList_4C2560(int2* a1); // idb
int __cdecl sub_4C26F0(int yTop);     // idb
int __cdecl sub_4C2A00(int a1, int a2, int a3, int a4, __int16* a5);
int __cdecl nox_xxx_clientOrderCreature_4C2A60(int a1, unsigned int a2);
int __cdecl nox_xxx_wndSummonProc_4C2B10(_DWORD* a1, unsigned int a2, unsigned int a3);
int sub_4C2BD0();
int sub_4C2BE0();
int* sub_4C2BF0();
int __cdecl sub_4C2C20(_DWORD* a1, int a2, unsigned int a3);
int __cdecl sub_4C2C60(_DWORD* a1, int2* a2); // idb
char* sub_4C2D60();
char* __cdecl sub_4C2D90(int a1);
BOOL __cdecl sub_4C2DD0(int a1);
int sub_4C2E00();
char __cdecl nox_xxx_cliSummonCreat_4C2E50(int a1, int a2, int a3);
int __cdecl sub_4C2EF0(int a1);
char* sub_4C2F20();
char* sub_4C2F70();
int __cdecl sub_4C2FD0(int a1);
int __cdecl sub_4C3030(int* a1, int a2, int a3);
int __cdecl sub_4C30C0(int* a1, int a2);
int* __cdecl nox_xxx_cliSummonOnDieOrBanish_4C3140(int a1, void* a2);
char* __cdecl sub_4C31D0(int a1);
int __cdecl sub_4C3210(int a1);
BOOL __cdecl nox_xxx_sprite_4C3220(int a1);
BOOL __cdecl sub_4C3240(int a1);
BOOL sub_4C3260();
int __cdecl sub_4C3270(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_spriteDrawCircleMB_4C32A0(int a1, int a2, int a3, int a4);
int sub_4C3390();
int __cdecl sub_4C3410(int* a1);
int __cdecl sub_4C3460(int a1);
BOOL sub_4C3480();
int sub_4C34A0();
void __cdecl sub_4C34C0(int a1);
int sub_4C3500();
int sub_4C3560();
int sub_4C3580();
int __cdecl sub_4C35B0(int a1);
int __cdecl sub_4C3A60(_DWORD* a1, unsigned int a2, unsigned int a3, int a4);
int __cdecl sub_4C3A90(int a1, int a2, int* a3, int a4);
char* sub_4C3B70();
char* sub_4C3CB0();
int __cdecl sub_4C3EB0(int a1, int a2, char* a3, int a4);
int __cdecl sub_4C3FC0(char* a1);
int __cdecl sub_4C4100(char* a1);
int sub_4C4220();
void sub_4C4260();
BOOL sub_4C4280();
int __cdecl sub_4C42A0(int2* a1, int2* a2, int* a3, int* a4); // idb
__int16 __cdecl nox_xxx_drawObject_4C4770_draw(int* a1, nox_drawable* dr, int a3);
char __cdecl sub_4C4EC0(_DWORD* a1, int a2);
__int16 __cdecl nox_xxx_drawShinySpot_4C4F40(_DWORD* a1, int a2);
int nox_xxx_colorInit_4C4FD0();
int __cdecl sub_4C5020(int a1);
void sub_4C5050();
int __cdecl sub_4C5060(_DWORD* a1);
int __cdecl sub_4C51D0(int2* a1, int2* a2); // idb
int __cdecl sub_4C52E0(int* a1, int a2);
void __cdecl sub_4C5430(int a1, int a2);
void nox_xxx____inc_tmpoff_4C54D0();
int sub_4C54E0();
void sub_4C5500(int* a1);
int __cdecl sub_4C5630(int a1, int a2, int a3);
int __cdecl sub_4C5680(int a1, int a2, int a3);
int __cdecl sub_4C56D0(_DWORD* a1);
int __cdecl sub_4C57C0(FILE* a1, int a2, _DWORD* a3, _DWORD* a4);
BOOL __cdecl sub_4C5850(FILE* a1);
int __cdecl sub_4C5A60(FILE* a1, int a2);
int __cdecl sub_4C5B80(FILE* a1, int a2, _BYTE* a3, _BYTE* a4);
BOOL __cdecl sub_4C5CB0(FILE* a1);
int __cdecl sub_4C5D20(FILE* a1, int a2);
void __cdecl sub_4C5EB0(int a1, int a2, int a3);
void __cdecl sub_4C60D0(int a1, int a2, int a3);
int __cdecl sub_4C6260(int a1, int a2, int a3);
unsigned __int8* __cdecl sub_4C64E0(int a1, int a2, int a3, int* a4);
char* __cdecl sub_4C6900(int a1, int a2);
void sub_4C69A0();
void sub_4C6A00();
void sub_4C6B50();
void sub_4C6BA0();
void sub_4C6BF0();
void sub_4C6C60();
void sub_4C6DE0();
void sub_4C6E40();
void sub_4C6FD0();
void sub_4C7060();
unsigned int sub_4C7110();
unsigned int sub_4C7240();
unsigned __int8 sub_4C73A0();
void __cdecl sub_4C7440(int a1, int a2, int a3);
void __cdecl nox_xxx_smthPlayerAnim_4C7670(int a1, int a2, int a3);
void __cdecl sub_4C7860(int a1, int a2, int a3);
int __cdecl sub_4C79F0(int a1, int a2, int a3);
unsigned __int8* __cdecl sub_4C7C80(int a1, int a2, int a3, int* a4);
char* __cdecl sub_4C8040(int a1, int a2);
void sub_4C80E0();
void sub_4C8130();
__int16 sub_4C82C0();
void sub_4C8410();
__int16 sub_4C8570();
void sub_4C86B0();
__int32 sub_4C8760();
void sub_4C8850();
void sub_4C8A30();
__int16 sub_4C8C00();
void sub_4C8D60();
void sub_4C8DF0();
void sub_4C8EC0();
void sub_4C9050();
void sub_4C91C0();
void sub_4C92F0();
void sub_4C94D0();
void sub_4C96A0();
void sub_4C97F0();
void sub_4C9970();
void sub_4C9B20();
int __cdecl nox_xxx_sprite_4CA540(_DWORD* a1, int a2);
int __cdecl sub_4CA650(int a1, int a2);
int __cdecl sub_4CA720(int a1, int a2);
int sub_4CA860();
int __cdecl sub_4CA8B0(int a1, int a2);
void __cdecl sub_4CA960(_DWORD* a1, char a2, float4* a3, float2* a4);      // idb
void __cdecl sub_4CAA90(int2* a1, float4* a2, float2* a3, int a4, int a5); // idb
void __cdecl sub_4CAC30(int2* a1, float4* a2, float2* a3, int a4, int a5); // idb
int sub_4CADD0();
int __cdecl sub_4CAE40(int a1);
int sub_4CAE60();
int __cdecl sub_4CAE90(int a1);
int sub_4CAEB0();
int __cdecl sub_4CAED0(int a1);
_DWORD* __cdecl sub_4CAF10(int a1, int a2, int a3, int a4, int a5, int a6, _DWORD* a7);
int __cdecl sub_4CAF80(int a1, int a2, int a3, int a4);
int __cdecl sub_4CAFB0(int a1);
int __cdecl sub_4CAFF0(_DWORD* a1, _DWORD* a2);
int __cdecl sub_4CB1A0(_DWORD* a1, int a2);
BOOL __cdecl sub_4CB230(const char* a1, char* lpFileName);
_DWORD* sub_4CB330();
int __cdecl sub_4CB590(int a1);
int __cdecl sub_4CB5D0(int a1, int a2, int* a3, int a4);
int sub_4CB880();
int sub_4CBB70();
int sub_4CBBB0();
char* sub_4CBBF0();
int __cdecl sub_4CBE70(int a1, int a2, int* a3, int a4);
char* sub_4CBF40();
int __cdecl sub_4CC140(_DWORD* a1, unsigned int a2, unsigned int a3, int a4);
int __cdecl sub_4CC170(int a1, int a2, char* a3, int a4);
int __cdecl sub_4CC280(char* a1);
int __cdecl sub_4CC3C0(char* a1);
int sub_4CC4E0();
int __cdecl sub_4CC660(_DWORD* a1, int a2, int a3, int a4);
int __cdecl nox_xxx_windowMainBGProc_4CC6A0(_DWORD* a1, int a2, int* a3, int a4);
int sub_4CC6F0(int a1, int a2);
int nox_xxx_gameDownloadShowDialog_4CC770();
int __cdecl nox_xxx_guiDownloadAbort_4CC830(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_guiDownloadProc_4CC890(int a1, int a2, int* a3, int a4);
int __cdecl nox_xxx_guiDownloadSetPercent_4CC900(unsigned __int8 a1);
int nox_xxx_guiDownloadClose_4CC930();
int nox_xxx_updDrawUndeadKiller_4CCCF0();
int __cdecl sub_4CCD00(int a1, int a2);
int __cdecl nox_xxx_updDrawFist_4CCDB0(int a1, int a2);
int __cdecl sub_4CCE70(int a1, _DWORD* a2);
int __cdecl sub_4CD090(int a1, _DWORD* a2);
int __cdecl sub_4CD0C0(int a1, _DWORD* a2);
int __cdecl sub_4CD0F0(int a1, _DWORD* a2);
int __cdecl sub_4CD120(int a1, _DWORD* a2);
int __cdecl sub_4CD400(_DWORD* a1, int a2);
int __cdecl nox_xxx_updDrawDBall_4CDF80(int a1, int a2);
int __cdecl sub_4CE0A0(int a1, int a2);
int __cdecl nox_xxx_updDrawCloud_4CE1D0(int a1, int a2);
int __cdecl sub_4CE340(int a1, int a2);
int __cdecl sub_4CE360(int a1, int a2);
int __cdecl nox_xxx_updDrawColorlight_4CE390(_DWORD* a1, int a2);
void __cdecl nox_xxx_colorLightAlterB0ArrayColor_4CE440(int a1);
void __cdecl nox_xxx_colorLightAlterIntensity_4CE610(int a1);
int __cdecl nox_xxx_colorLightAlterRadius_4CE760(int a1);
void __cdecl sub_4CE8C0(int a1);
__int16 __cdecl sub_4CE960(int a1);
int __cdecl sub_4CEA90(float* a1, int2* a2, int a3); // idb
int __cdecl sub_4CEBA0(int a1, char* a2);
HANDLE __cdecl sub_4CED40(char* a1);
int __cdecl sub_4CEED0(int a1, int a2);
int __cdecl sub_4CF060(int a1, unsigned int a2, int* a3, int a4);
void sub_4CF460();
int __cdecl nox_xxx_mapValidateMB_4CF470(char* a1, int a2);
char nox_xxx_mapReadSetFlags_4CF990();
void nox_xxx_mapFindCrown_4CFC30();
int __cdecl sub_4CFC90(int a1);
int nox_xxx_serverParseEntireMap_4CFCE0();
int __cdecl sub_4CFDF0(int a1);
int sub_4CFE00();
int __cdecl sub_4CFE10(const char* a1);
int __cdecl sub_4CFF50(int a1);
int __cdecl nox_xxx_mapGetTypeMB_4CFFA0(int a1);
int __cdecl sub_4CFFC0(int a1);
int __cdecl sub_4CFFE0(int a1);
int __cdecl nox_xxx_interesting_xfer_4D0010(_DWORD* a1, int a2);
int __cdecl sub_4D0250(char* a1, char* a2, int a3, int a4);
int __cdecl sub_4D0550(char* a1);
int __cdecl sub_4D0670(char* a1);
_DWORD* __cdecl sub_4D0760(int a1);
HANDLE sub_4D07F0();
void sub_4D0970();
int* nox_xxx_validateMapList_4D09B0();
int* __cdecl sub_4D09C0(int* a1);
int* __cdecl sub_4D09D0(char* a1);
void sub_4D0A20();
FILE* nox_xxx_loadMapCycle_4D0A30();
int __cdecl sub_4D0C70(int a1);
int __cdecl sub_4D0C80(char* a1);
void __cdecl sub_4D0CC0(char* a1);
int __cdecl sub_4D0D50(int a1);
BOOL sub_4D0D70();
int __cdecl sub_4D0D90(int a1);
void sub_4D0DA0();
int __cdecl sub_4D0DC0(int a1, int a2);
int __cdecl sub_4D0DE0(int a1);
int nox_xxx_mapSelectFirst_4D0E00();
void sub_4D0F30();
char* sub_4D0F60();
_DWORD* sub_4D11A0();
void sub_4D11D0();
void* __cdecl sub_4D1210(int a1);
int* __cdecl sub_4D1250(int a1);
int __cdecl sub_4D12A0(int a1);
void nox_xxx_mapSwitchLevel_4D12E0(int a1);
void nox_xxx____setargv_13_4D15B0();
void sub_4D15C0();
int sub_4D15D0();
void sub_4D15E0();
int nox_xxx_scavengerTreasureMax_4D1600();
void sub_4D1610();
int sub_4D1620();
int nox_xxx_parseGamedataBinPre_4D1630();
_DWORD* nox_xxx_unknown_libname_11_4D1650();
int nox_xxx_servNewSession_4D1660();
int nox_xxx_servInitialMapLoad_4D17F0();
int nox_xxx_servResetPlayers_4D23C0();
char* __cdecl nox_xxx_mapLoad_4D2450(char* a1);
char* sub_4D2FF0();
char* nox_xxx_netReportAllLatency_4D3050();
BOOL nox_xxx_servEndSession_4D3200();
int __cdecl sub_4D3310(int a1);
int sub_4D3320();
int nox_xxx_unused_4D3330();
void sub_4D3360();
int __cdecl nox_xxx_replayStartSave_4D3370(char* a1);
void nox_xxx_replayStopSave_4D33B0();
int __cdecl nox_xxx_replaySaveConsole_4D33E0(void* a1, int a2);
int __cdecl nox_xxx_replayWriteMSgMB_4D3450(int a1, void* a2, unsigned int a3);
int __cdecl nox_xxx_replayFileOpen_4D34C0(char* a1);
void nox_xxx_replayStopReadMB_4D3530();
void __cdecl nox_xxx_replay_4D3860(void* a1);
int __cdecl nox_xxx_replayStartReadingOrSaving_4D38D0();
void nox_xxx_replayWriteSomeInt_4D39B0();
int __cdecl sub_4D39F0(const char* a3);
__int64 sub_4D3C30();
void __cdecl nox_xxx_tileInitdataClear_4D3C50(const void* a1);
unsigned __int8* sub_4D3C70();
_DWORD* __cdecl sub_4D3C80(_DWORD* a1);
int __cdecl nox_xxx_mapGenFixCoords_4D3D90(float2* a1, float2* a2); // idb
int __cdecl sub_4D3E30(float2* a1, float2* a2); // idb
BOOL __cdecl sub_4D3ED0(float2* a1);            // idb
int __cdecl sub_4D3F60(int a1);
int __cdecl sub_4D3F80(int a1);
int __cdecl sub_4D3FF0(int a1);
float2* __cdecl sub_4D4060(float2* a1, float2* a2, int a3);
void sub_4D4160();
int __cdecl sub_4D42D0(int a1);
unsigned int __cdecl sub_4D42E0(const char* a1);
char* nox_xxx_getRandMapName_4D4310();
BOOL nox_xxx_mapGenStart_4D4320();
int nox_xxx_mapGenStep_4D44E0();
float* nox_xxx_mapgen_Doors_4D4790();
float* __cdecl nox_xxx_mapGenMkSmallRoom_4D4F40(_DWORD* a1);
void sub_4D52F0();
int __cdecl sub_4D5350(_DWORD* a1, int a2, int a3, int a4, int a5);
int __cdecl nox_xxx_mapGenFillRoom_4D53B0(int a1, int a2, int a3, int a4, int a5);
int __cdecl sub_4D5630(int a1, int a2, int a3, int a4, int a5);
int __cdecl sub_4D5D20(_DWORD* a1);
unsigned int __cdecl nox_xxx_mapGenMakeInfo_4D5DB0(int a1);
BOOL nox_xxx_mapGenStartAlt_4D5F30();
int __cdecl sub_4D6000(int a1);
int sub_4D60B0();
_DWORD* __cdecl sub_4D60E0(int a1);
int __cdecl sub_4D6130(int a1);
int __cdecl sub_4D6170(int a1);
void sub_4D61B0(int a1);
int __cdecl sub_4D61F0(int a1);
int __cdecl nox_xxx_unused_4D6240(int a1, int a2);
int __cdecl nox_xxx_unused_4D6290(int a1);
int __cdecl sub_4D6360(int a1);
int __cdecl sub_4D63B0(int a1);
int __cdecl nox_xxx_unused_4D6400(int a1);
int __cdecl nox_xxx_unused_4D6450(int a1);
int __cdecl nox_xxx_unused_4D64A0(int a1);
int __cdecl nox_xxx_unused_4D64F0(int a1);
unsigned int __cdecl sub_4D6540(int a1);
int __cdecl sub_4D66E0(unsigned int a1, unsigned int a2, unsigned int a3, unsigned int a4);
int __cdecl sub_4D6770(int a1);
int __cdecl sub_4D6880(int a1, int a2);
char* sub_4D6940();
char* sub_4D6950();
int __cdecl sub_4D6960(int a1);
int __cdecl sub_4D6A20(int a1, int a2);
int sub_4D6A60();
int __cdecl sub_4D6B10(int a1);
int sub_4D6BE0();
int nox_server_setupQuestGame_4D6C70();
int sub_4D6F30();
int __cdecl sub_4D6F40(int a1);
int sub_4D6F50();
int __cdecl sub_4D6F60(int a1);
int sub_4D6F70();
int __cdecl sub_4D6F80(int a1);
int __cdecl sub_4D6F90(int a1);
int sub_4D6FA0();
int __cdecl sub_4D6FB0(int a1);
int sub_4D6FC0();
int nox_server_conCmdHandler_startSoloQuest_4D7080();
char* sub_4D70B0();
BOOL __cdecl nox_xxx_bookCreatureTest_4D70C0(int a1);
BOOL __cdecl sub_4D7100(int a1);
int sub_4D7150();
int __cdecl sub_4D71E0(int a1);
unsigned int sub_4D71F0();
int __cdecl sub_4D7280(int a1, char a2);
int __cdecl sub_4D72B0(int a1);
int sub_4D72C0();
int __cdecl sub_4D72D0(int a1);
int sub_4D72F0();
int sub_4D7300();
int sub_4D7310();
void __cdecl sub_4D7390(int a1);
int sub_4D7430();
int __cdecl sub_4D7440(int a1);
int __cdecl sub_4D7450(int a1, __int16 a2);
void __cdecl sub_4D7480(int a1);
int __cdecl sub_4D74F0(int a1);
char __cdecl sub_4D7520(int a1);
int sub_4D75E0();
int __cdecl sub_4D75F0(int a1);
int sub_4D7600();
int __cdecl sub_4D76E0(int a1);
int sub_4D76F0();
bool nox_client_checkQuestExp_SKU2_4D7700();
BOOL nox_xxx_checkAccessToGLava_4D7790();
LSTATUS __cdecl sub_4D77D0(int a1);
LSTATUS nox_common_readSKU_fromRegistry_4D78C0();
int __cdecl nox_xxx_player_4D7960(char a1);
BOOL __cdecl nox_xxx_player_4D7980(char a1);
int __cdecl sub_4D79A0(char a1);
int __cdecl sub_4D79C0(int a1);
int __cdecl sub_4D7A60(int a1);
int sub_4D7A80();
int sub_4D7B40();
int __cdecl sub_4D7B60(int a1);
int __cdecl nox_xxx_netSendInterestingId_4D7BE0(int a1);
int sub_4D7C60();
int sub_4D7CC0();
int __cdecl sub_4D7E50(int a1);
int sub_4D7EA0();
void sub_4D7ED0();
int __cdecl nox_xxx_netCreatureCmd_4D7EE0(int a1, char a2);
int __cdecl nox_xxx_netNotifyRate_4D7F10(int a1);
int __cdecl nox_xxx_netReportObjectPoison_4D7F40(int a1, _DWORD* a2, char a3);
int __cdecl sub_4D80C0(int a1, char a2);
int __cdecl nox_xxx_netAbilRepotState_4D8100(int a1, char a2, char a3);
int __cdecl nox_xxx_netReportActiveAbils_4D8150(int a1, char a2, char a3);
void __cdecl sub_4D81A0(int a1);
int __cdecl nox_xxx_netReportAnimFrame_4D81F0(int a1, _DWORD* a2);
int __cdecl nox_xxx_netReportXStatus_4D8230(int a1, _DWORD* a2);
int __cdecl nox_xxx_netReportPlrStatus_4D8270(int a1);
int __cdecl nox_xxx_netReportCharges_4D82B0(int a1, _DWORD* a2, char a3, char a4);
_DWORD* __cdecl sub_4D82F0(int a1, _DWORD* a2);
int __cdecl nox_xxx_netReportDequip_4D84C0(int a1, int a2);
_DWORD* __cdecl nox_xxx_netReportEquip_4D8540(int a1, _DWORD* a2, int a3);
int __cdecl nox_xxx_netReportDequip_4D8590(int a1, _DWORD* a2);
int __cdecl nox_xxx_netReportTotalHealth_4D85C0(int a1, _DWORD* a2);
int __cdecl nox_xxx_netReportUnitCurrentHP_4D8620(int a1, _DWORD* a2);
int __cdecl nox_xxx_netSendTeam_4D8670(int a1, _DWORD* a2);
char* __cdecl nox_xxx_netSendPlrHealthToTeam_4D86E0(int a1);
__int16 __cdecl nox_xxx_netReportHealthDelta_4D8760(int a1, __int16 a2, __int16 a3);
int __cdecl nox_xxx_itemReportHealth_4D87A0(int a1, _DWORD* a2);
int __cdecl nox_xxx_netReportStamina_4D8800(int a1, int a2);
int __cdecl sub_4D8840(int a1, int a2);
int __cdecl sub_4D8870(int a1, int a2);
int __cdecl nox_xxx_netReportTotalMana_4D88C0(int a1, int a2);
int __cdecl nox_xxx_netReportMana_4D8930(int a1, int a2);
char __cdecl nox_xxx_servSendStats_4D8990(int a1, int a2, char a3);
int __cdecl nox_xxx_netReportArmorVal_4D8A30(int a1, int a2);
int __cdecl nox_xxx_netReportPickup_4D8A60(int a1, int a2);
int __cdecl nox_xxx_netReportModifiablePickup_4D8AD0(int a1, int a2);
int __cdecl nox_xxx_netReportDrop_4D8B50(int a1, int a2);
int __cdecl nox_xxx_netSendDMWinner_4D8B90(int a1, char a2);
int __cdecl nox_xxx_netSendDMTeamWinner_4D8BF0(int a1, char a2);
int __cdecl nox_xxx_netFlagballWinner_4D8C40(int a1);
int __cdecl nox_xxx_netFlagWinner_4D8C40_4D8C80(int a1, char a2);
int __cdecl nox_xxx_scavengerHuntReport_4D8CD0(int a1);
char* __cdecl nox_xxx_playerIncrementElimDeath_4D8D40(int a1);
int __cdecl nox_xxx_changeScore_4D8E90(int a1, int a2);
int __cdecl nox_xxx_playerSubLessons_4D8EC0(int a1, int a2);
int __cdecl nox_xxx_netReportLesson_4D8EF0(int a1);
int __cdecl nox_xxx_netTimerStatus_4D8F50(int a1, int a2);
int __cdecl nox_xxx_netReportEnchant_4D8F90(int a1, _DWORD* a2);
void __cdecl nox_xxx_netReportObjHidden_4D8FD0(int a1, _DWORD* a2);
int __cdecl nox_xxx_netReportUnitHeight_4D9020(int a1, int a2);
int __cdecl sub_4D90E0(int a1, char a2);
int __cdecl nox_xxx_earthquakeSend_4D9110(float* a1, int a2);
int __cdecl nox_xxx_netReportAcquireCreature_4D91A0(int a1, int a2);
int __cdecl nox_xxx_netFxShield_0_4D9200(int a1, int a2);
int __cdecl nox_xxx_netMonitorCreature_4D9250(int a1, int a2);
int __cdecl nox_xxx_netSendUnMonitorCrea_4D92A0(int a1, _DWORD* a2);
int __cdecl nox_xxx_netReportTeamBase_4D92D0(int a1, int a2);
int __cdecl nox_xxx_netReportStatsSpeed_4D9360(int a1, _DWORD* a2, char a3, int a4);
_DWORD* __cdecl nox_xxx_netSendReportNPC_4D93A0(int a1, int a2);
int __cdecl nox_xxx_netSendJournalAdd_4D9440(int a1, int a2);
int __cdecl nox_xxx_netSendJournalRemove_4D94A0(int a1, const char* a2);
int __cdecl nox_xxx_netSendJournalUpdate_4D9500(int a1, int a2);
int __cdecl nox_xxx_netSendChapterEnd_4D9560(int a1, char a2, int a3);
int __cdecl nox_xxx_netSendFlagStatus_4D95A0(int a1, char a2, char a3, char a4, __int16 a5);
int __cdecl nox_xxx_netSendBallStatus_4D95F0(int a1, char a2, __int16 a3);
int __cdecl nox_xxx_netReportSpellStat_4D9630(int a1, int a2, char a3);
int __cdecl nox_xxx_netSendSecondaryWeapon_4D9670(int a1, _DWORD* a2, char a3);
int __cdecl nox_xxx_netMsgLastQuiver_4D96B0(int a1, _DWORD* a2);
int __cdecl nox_xxx_netMsgInventoryLoaded_4D96E0(int a1);
int __cdecl sub_4D9700(int a1);
int __cdecl nox_xxx_netScriptMessageKill_4D9760(int a1);
int __cdecl nox_xxx_netFriendAddRemove_4D97A0(int a1, _DWORD* a2, int a3);
int __cdecl sub_4D97E0(int a1);
int __cdecl nox_xxx_netMsgFadeBegin_4D9800(int a1, int a2);
void nox_xxx_netHarpoonAttach_4D9840(_DWORD* a1, _DWORD* a2);
int __cdecl nox_xxx_netHarpoonBreak_4D98A0(_DWORD* a1, _DWORD* a2);
void __cdecl nox_xxx_playerReportAnything_4D9900(int a1);
int __cdecl nox_xxx_netStatsMultiplier_4D9C20(int a1);
int __cdecl sub_4D9CF0(int a1);
int __cdecl sub_4D9D20(int a1, int a2);
int __cdecl sub_4D9D60(int a1, int a2);
int __cdecl nox_xxx_unused_4D9DB0(int a1, int a2, int a3);
int __cdecl sub_4D9DF0(int a1, int a2, char a3);
int __cdecl sub_4D9E30(int a1, int a2, char a3);
int __cdecl nox_xxx_netGauntlet_4D9E70(int a1);
void sub_4D9EA0();
int nox_xxx_netSendLineMessage_4D9EB0(int a1, wchar_t* a2, ...);
int nox_xxx_printToAll_4D9FD0(char a1, wchar_t* a2, ...);
int __cdecl nox_xxx_netInformTextMsg_4DA0F0(int a1, int a2, int* a3);
int __cdecl nox_xxx_netInformTextMsg2_4DA180(int a1, _BYTE* a2);
void __cdecl nox_xxx_netPriMsgToPlayer_4DA2C0(int a1, const char* a2, char a3);
int __cdecl nox_xxx_netPrintLineToAll_4DA390(const char* a1);
_DWORD* nox_get_and_zero_server_objects_4DA3C0(void);
void nox_set_server_objects_4DA3E0(void* p);
BOOL __cdecl nox_server_strcmpWithoutMapname_4DA3F0(const char* a1, const char* a2);
int __cdecl nox_xxx_getObjectByScrName_4DA4F0(char* a1);
int __cdecl sub_4DA5C0(int a1, const char* a2);
int __cdecl sub_4DA660(int a1, const char* a2);
int nox_server_getFirstObject_4DA790();
int __cdecl nox_server_getNextObject_4DA7A0(int a1);
int nox_xxx_getFirstPlayerUnit_4DA7C0();
int __cdecl nox_xxx_getNextPlayerUnit_4DA7F0(int a1);
int nox_xxx_getFirstUpdatable2Object_4DA840();
int __cdecl nox_xxx_getNextUpdatable2Object_4DA850(int a1);
int nox_server_getFirstObjectUninited_4DA870();
int __cdecl nox_server_getNextObjectUninited_4DA880(int a1);
int nox_xxx_getFirstUpdatableObject_4DA8A0();
int __cdecl nox_xxx_getNextUpdatableObject_4DA8B0(int a1);
int __cdecl nox_xxx_unitAddToUpdatable_4DA8D0(int a1);
_DWORD* __cdecl nox_xxx_unitRemoveFromUpdatable_4DA920(_DWORD* a1);
_DWORD* __cdecl nox_xxx_unitNewAddShadow_4DA9A0(_DWORD* a1);
_DWORD* __cdecl nox_xxx_action_4DA9F0(_DWORD* a1);
void nox_xxx_createAt_4DAA50(int a1, int a2, float a3, float a4); // idb
void nox_xxx_unitsNewAddToList_4DAC00();
char __cdecl nox_xxx_servFinalizeDelObject_4DADE0(int a1);
int __cdecl sub_4DAE50(int a1);
void sub_4DAF10();
void sub_4DAFD0(_DWORD* a1);
_DWORD* nox_xxx_unitClearPendingMB_4DB030();
int sub_4DB090();
int sub_4DB0A0();
int sub_4DB100();
unsigned int __cdecl sub_4DB130(const char* a1);
int __cdecl sub_4DB170(int a1, int a2, int a3);
int nox_xxx_gameGet_4DB1B0();
int sub_4DB1C0();
int __cdecl nox_xxx_gameSetSwitchSolo_4DB220(int a1);
int __cdecl nox_xxx_gameSetNoMPFlag_4DB230(int a1);
int nox_xxx_gameIsSwitchToSolo_4DB240();
int nox_xxx_gameIsNotMultiplayer_4DB250();
char* nox_xxx_mapFilenameGetSolo_4DB260();
unsigned int __cdecl nox_xxx_gameSetSoloSavePath_4DB270(const char* a1);
unsigned int __cdecl nox_xxx_cliCheckMapExists_4DB2A0(const char* a1);
BOOL __cdecl nox_xxx_saveMakeFolder_4DB540(char* saveName);
char* __cdecl sub_4DB790(const char* a1);
void sub_4DB9C0();
void nox_savegame_rm_4DBE10(char* saveName, int rmDir);
int __cdecl sub_4DC100(int a1, char* saveName);
int sub_4DC550();
int sub_4DC630();
int sub_4DC7D0();
void nox_savegame_nameFromPath_4DC970(char* src, char* dst);
char* __cdecl sub_4DC9B0(char* a1);
char* nox_xxx_unused_4DCA00();
int __cdecl sub_4DCBD0(int a1);
int sub_4DCBE0();
int __cdecl sub_4DCBF0(int a1);
int __cdecl sub_4DCC10(int a1);
int __cdecl nox_xxx_mapLoadOrSaveMB_4DCC70(int a1);
BOOL sub_4DCC90();
BOOL nox_xxx_game_4DCCB0();
int sub_4DCD40();
int sub_4DCE00();
int sub_4DCE30();
int __cdecl sub_4DCE60(int a1);
int sub_4DCE70();
int __cdecl sub_4DCE80(const char* a1);
unsigned __int8* sub_4DCED0();
void __cdecl sub_4DCEE0(const char* a1);
int __cdecl nox_xxx_sendGauntlet_4DCF80(int a1, char a2);
char __cdecl sub_4DCFB0(int a1);
char* __cdecl sub_4DD0B0(int a1);
void sub_4DD130();
LPVOID __cdecl nox_xxx_playerSendMOTD_4DD140(int a1);
char* __cdecl nox_xxx_gameServerReadyMB_4DD180(int a1);
void nox_xxx____setargv_15_4DD310();
_DWORD* __cdecl nox_xxx_playerNew_4DD320(int a1, int a2);
int __cdecl nox_xxx_netGuiGameSettings_4DD9B0(char a1, const void* a2, int a3);
char* __cdecl nox_xxx_playerCheckName_4DDA00(int a1);
unsigned int __cdecl nox_xxx_netNewPlayerMakePacket_4DDA90(int a1, int a2);
_DWORD* __cdecl nox_xxx_servSendSettings_4DDB40(_DWORD* a1);
FILE* __cdecl nox_xxx_netSendBySock_4DDDC0(int a1);
_DWORD* __cdecl sub_4DDE10(int a1, int a2);
int __cdecl nox_xxx_playerObserveMonster_4DDE80(int a1, int a2);
int __cdecl nox_xxx_playerObserveClear_4DDEF0(int a1);
int __cdecl nox_xxx_playerGetPossess_4DDF30(int a1);
int __cdecl nox_xxx_netPlayerIncomingServ_4DDF60(int a1);
int __cdecl nox_xxx_sendAllPlayerIDs_4DE270(int a1);
char* __cdecl nox_xxx_newPlayerSendAllPlayers_4DE300(int a1);
int __cdecl nox_xxx_servMinimapRevealFlag_4DE380(int a1);
_DWORD* __cdecl sub_4DE410(int a1);
int __cdecl sub_4DE4D0(char a1);
char __cdecl nox_xxx_playerDisconnFinish_4DE530(int a1, char a2);
void __cdecl sub_4DE790(int a1);
char* __cdecl nox_xxx_playerForceDisconnect_4DE7C0(int a1);
char* __cdecl nox_xxx_playerCallDisconnect_4DEAB0(int a1, char a2);
char* __cdecl nox_xxx_playerDisconnByPlrID_4DEB00(int a1);
int __cdecl nox_xxx_netAddPlayerHandler_4DEBC0(int a1);
int __cdecl nox_xxx_netlist_ServRecv_4DEC30(int a1, unsigned __int8* a2, signed int a3);
int __cdecl sub_4DEC50(int a1);
int __cdecl nox_xxx_netCloseHandler_4DEC60(unsigned int a1);
int nox_xxx_netGameSettings_4DEF00();
char* sub_4DF020();
int __cdecl nox_xxx_wallSendDestroyed_4DF0A0(int a1, int a2);
int __cdecl sub_4DF120(int a1);
int __cdecl sub_4DF180(int a1);
int __cdecl nox_xxx_wall_4DF1E0(int a1);
int* __cdecl sub_4DF2A0(char a1);
void __cdecl sub_4DF2E0(int a1);
int __cdecl nox_xxx_netSendSimpleObject2_4DF360(int a1, int a2);
void sub_4DF3C0(int a1);
int sub_4DF550();
void sub_4DF560();
void sub_4DF570();
int __cdecl sub_4DF580(int a1, _DWORD* a2);
int __cdecl sub_4DF5E0(int a1, int a2);
int __cdecl nox_xxx_netFn_UpdateStream_4DF630(int a1, char* a2, unsigned int a3);
BOOL __cdecl nox_xxx_chkIsMsgTimestamp_4DF7F0(_BYTE* a1);
int __cdecl sub_4DF810(char* a1, int a2, int a3);
int __cdecl sub_4DF8C0(_BYTE* a1, int a2);
_BYTE* __cdecl sub_4DF8F0(int a1, _BYTE* a2, int a3);
int __cdecl sub_4DF9B0(int a1, int a2, int a3, int a4);
signed int __cdecl sub_4DFAF0(void* a1, int a2, const void* a3, signed int a4);
int sub_4DFB20();
void sub_4DFB30();
void sub_4DFB40();
_DWORD* __cdecl sub_4DFB50(int a1, int a2);
_DWORD* __cdecl sub_4DFB80(int a1, int a2);
int __cdecl nox_xxx_enchantItemTestInventory_4DFBB0(int a1, char a2);
void __cdecl nox_xxx_effectSpeedEngage_4DFC30(int a1, int a2);
void __cdecl nox_xxx_effectSpeedDisengage_4DFCA0(int a1, int a2);
_DWORD* __cdecl sub_4DFD10(int a1, int a2);
void __cdecl nox_xxx_modifFireProtection_4DFD40(int a1, int a2, int a3);
_DWORD* __cdecl nox_xxx_buff_4DFD80(int a1, int a2);
_DWORD* __cdecl sub_4DFDB0(int a1, int a2);
_DWORD* __cdecl nox_xxx_checkPoisonProtectEnch_4DFDE0(int a1, int a2);
_DWORD* __cdecl sub_4DFE10(int a1, int a2);
double __cdecl nox_xxx_checkFireProtect_4DFE40(_DWORD* a1);
double __cdecl nox_xxx_checkElectrProtect_4DFF40(_DWORD* a1);
double __cdecl nox_xxx_getPoisonDmg_4E0040(_DWORD* a1);
_DWORD* __cdecl sub_4E0140(int a1, int a2);
void __cdecl sub_4E0170(int a1, int a2);
void sub_4E01B0();
void sub_4E01C0();
void __cdecl nox_xxx_effectRegeneration_4E01D0(int a1, int a2);
void __cdecl nox_xxx_attribContinualReplen_4E02C0(int a1, _DWORD* a2);
void sub_4E0350();
void sub_4E0360();
float* __cdecl sub_4E0370(int a1, int a2, int a3, int a4, int a5, float* a6);
float* __cdecl sub_4E0380(int a1, int a2, int a3, int a4, int a5, float* a6);
BOOL __cdecl nox_xxx_inversionEffect_4E03D0(int a1, int a2, int a3, int a4, int a5, BOOL* a6);
int __cdecl nox_xxx_unusedCheckGripEffect_4E03F0(int a1, int a2, int a3, int a4);
BOOL __cdecl nox_xxx_gripEffect_4E0480(int a1, int a2, int a3, int a4, int a5, BOOL* a6);
void sub_4E04A0();
void sub_4E04B0();
float* __cdecl nox_xxx_effectDamageMultiplier_4E04C0(int a1, int a2, int a3, int a4, float* a5);
void __cdecl nox_xxx_stunEffect_4E04D0(int a1, int a2, int a3, int a4);
_DWORD* __cdecl nox_xxx_fireEffect_4E0550(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_fireRingEffect_4E05B0(int a1, int a2, int a3);
int __cdecl nox_xxx_blueFREffect_4E05F0(int a1, int a2, int a3);
void __cdecl nox_xxx_recoilEffect_4E0640(int a1, int a2, int a3, int a4);
void __cdecl nox_xxx_confuseEffect_4E0670(int a1, int a2, int a3, int a4);
void __cdecl nox_xxx_lightngEffect_4E06F0(int a1, int a2, int a3, int a4);
void __cdecl nox_xxx_drainMEffect_4E0740(int a1, int a2, int a3, int a4, int* a5);
void __cdecl nox_xxx_vampirismEffect_4E07C0(int a1, int a2, int a3, int a4, int* a5);
void __cdecl nox_xxx_poisonEffect_4E0850(int a1, int a2, int a3, int a4);
void __cdecl nox_xxx_sympathyEffect_4E08E0(int a1, int a2, int a3, int a4, int* a5);
void nullsub_22(); // weak
int __cdecl nox_xxx_itemCheckReadinessEffect_4E0960(int a1);
int __cdecl nox_xxx_effectProjectileSpeed_4E09B0(int a1, int a2, int a3, int a4, int a5);
void nullsub_36(); // weak
void sub_4E09E0();
void sub_4E09F0();
int __cdecl nox_xxx_parseDamageTypeByName_4E0A00(const char* a1);
int __cdecl nox_xxx_projectileReflect_4E0A70(int a1, int a2);
int __cdecl nox_xxx_damageDefaultProc_4E0B30(int a1, int a2, int a3, int a4, int a5);
void __cdecl nox_xxx_gameballOnPlayerDamage_4E1230(int a1, int a2, int a3);
int __cdecl nox_xxx_itemApplyDefendEffect2_4E1320(int a1, int a2, int a3, int* a4, int a5);
int __cdecl nox_xxx_itemApplyPreDamageEffect_4E13B0(int a1, int a2, int a3, int a4);
int __cdecl sub_4E1400(int a1, _DWORD* a2);
BOOL __cdecl sub_4E1470(int a1);
int sub_4E14A0();
int __cdecl sub_4E14B0(int a1, int a2, int a3, int a4, int a5);
int __cdecl nox_xxx_damageArmor_4E1500(int a1, int a2, int a3, int a4, int a5);
void __cdecl nox_xxx_playerDamageWeapon_4E1560(int a1, int a2, int a3, int a4, float a5, int a6);
int __cdecl nox_xxx_itemDestroyed_4E1650(int a1, _DWORD* a2, unsigned __int16 a3, unsigned __int16 a4);
void __cdecl nox_xxx_equipDamage_4E16D0(int a1, int a2, int a3, int a4, float a5, int a6);
int __cdecl nox_server_handler_PlayerDamage_4E17B0(int a1, int a2, int a3, int a4, int a5);
void __cdecl nox_xxx_playerDecrementHPMana_4E20F0(int a1, int a2, float a3);
void __cdecl nox_xxx_playerDamageItems_4E2180(int a1, int a2, int a3, int a4, float a5);
double __cdecl sub_4E2220(int a1);
int __cdecl sub_4E22A0(int, int, int, int, float, int); // idb
int __cdecl sub_4E2330(int, int, int, int, float, int); // idb
int __cdecl sub_4E23C0(int a1, int a2, int a3, int a4, int a5);
int __cdecl sub_4E24B0(int a1, int a2, int a3, int a4, int a5);
int __cdecl sub_4E24E0(int a1, int a2, int a3, int a4, int a5);
int __cdecl nox_xxx_damageFlammable_4E2520(int a1, int a2, int a3, int a4, int a5);
int __cdecl nox_xxx_damageBlackPowder_4E2560(int a1, int a2, int a3, int a4, int a5);
int __cdecl nox_xxx_gameSetWallsDamage_4E25A0(int a1);
bool __cdecl nox_xxx_mapDamageUnitsAround_4E25B0(int a1, float a2, float a3, int a4, int a5, int a6, int a7);
void __cdecl nox_xxx_damageFn_4E2700(float*, int); // idb
int __cdecl nox_xxx_damageMonsterGen_4E27D0(int a1, int a2, int a3, int a4, int a5);
void sub_4E29B0();
void sub_4E29C0();
void sub_4E29D0();
int __cdecl nox_read_things_alternative_4E2B60(void);
int __cdecl nox_xxx_keyFirstLetterNumber_4E30A0(CHAR* a1);
int __cdecl nox_xxx_unitDefProtectMB_4E31A0(int a1);
int __cdecl sub_4E31E0(_DWORD* a1);
int __cdecl nox_thing_read_xxx_4E3220(int a1, char* a2, int a3);
void nox_xxx_unitDefFindMaxDataSize_4E3320();
int __cdecl nox_xxx_allocClassArrayObjects_4E3360(unsigned int a1);
int sub_4E3420();
void nullsub_23(void); // weak
_DWORD* __cdecl nox_new_objMem_2_4E3450(int a1);
_DWORD* __cdecl nox_new_objMem_4E3470(int a1);
_DWORD* __cdecl nox_xxx_objectCreateByName_4E3810(CHAR* a1);
int __cdecl nox_xxx_unitDefByName_4E3830(CHAR* a1);
int __cdecl nox_xxx_objBsearchFn_4E3880(const void*, const void*); // idb
int __cdecl nox_xxx_objectFreeMem_4E38A0(int a1);
int __cdecl nox_xxx_getUnitName_4E39D0(int a1);
int __cdecl nox_xxx_getUnitNameByThingType_4E3A80(int a1);
int __cdecl nox_xxx_getNameId_4E3AA0(CHAR* a1);
int nox_xxx_unitDefGetCount_4E3AC0();
BOOL __cdecl sub_4E3AD0(int a1);
LPVOID sub_4E3B30();
int __cdecl sub_4E3B40(int a1);
int __cdecl nox_xxx_utilThingDefByName_4E3B60(CHAR* a1);
int __cdecl nox_xxx_unitDefGet_4E3B70(int a1);
BOOL __cdecl sub_4E3B80(int a1);
int __cdecl nox_xxx_getUnitDefDd10_4E3BA0(int a1);
int __cdecl sub_4E3BF0(int a1);
int nox_xxx_protectUnitDefUpdateMB_4E3C20();
int __cdecl nox_xxx_cliSetPlrGuid_4E3C60(int a1);
int nox_xxx_servResetObjectGIDs_4E3C70();
void sub_4E3C90();
double sub_4E3CA0();
int __cdecl sub_4E3CB0(float a1); // idb
int nox_xxx_GetQuestStage_4E3CC0();
int __cdecl sub_4E3CD0(int a1);
int nox_xxx_player_4E3CE0();
int sub_4E3D50();
__int16 sub_4E3DD0();
void __cdecl sub_4E4080(float a1);
double sub_4E40B0();
void __cdecl sub_4E40C0(float a1);
double sub_4E40F0();
int sub_4E4100();
int __cdecl sub_4E41B0(char* a1);
int __cdecl sub_4E42C0(FILE* a1);
int __cdecl sub_4E4390(FILE* a1);
FILE* __cdecl sub_4E43F0(char* a1);
void sub_4E44D0();
int __cdecl nox_xxx_objectMakeUnseenByNoone_4E44E0(int a1);
int __cdecl nox_xxx_unitNeedSync_4E44F0(int a1);
int* __cdecl sub_4E4500(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_unitSetHP_4E4560(int a1, unsigned __int16 a2);
int* __cdecl nox_xxx_unitSetOnOff_4E4670(int a1, int a2);
void __cdecl nox_xxx_unitRaise_4E46F0(int a1, float a2);
void __cdecl nullsub_35(_DWORD, _DWORD); // weak
int* __cdecl nox_xxx_unitUnsetXStatus_4E4780(int a1, int a2);
int* __cdecl nox_xxx_unitSetXStatus_4E4800(int a1, int* a2);
int* __cdecl nox_xxx_servMarkObjAnimFrame_4E4880(int a1, int a2);
int* __cdecl nox_xxx_setUnitBuffFlags_4E48F0(int a1, int a2);
int* __cdecl nox_xxx_modifSetItemAttrs_4E4990(int a1, int* a2);
double __cdecl nox_xxx_objectGetMass_4E4A70(int a1);
int __cdecl sub_4E4A80(int a1);
int* __cdecl nox_xxx_setNPCColor_4E4A90(int a1, unsigned __int8 a2, int a3);
int* __cdecl nox_xxx_npcSetItemEquipFlags_4E4B20(int a1, int a2, int a3);
int __cdecl sub_4E4C00(int a1);
int __cdecl sub_4E4C10(int a1);
int __cdecl sub_4E4C30(int a1);
int __cdecl sub_4E4C50(int a1);
int __cdecl sub_4E4C80(int* a1);
BOOL __cdecl sub_4E4C90(int a1, unsigned int a2);
void sub_4E4DC0();
void sub_4E4DD0();
int sub_4E4DE0();
int __cdecl sub_4E4E50(int a1);
int sub_4E4ED0();
int sub_4E4EF0();
int __cdecl sub_4E4F30(int a1);
int __cdecl nox_xxx_playerResetImportantCtr_4E4F40(int a1);
int sub_4E4F80();
void __cdecl sub_4E4FC0(int a1);
int __cdecl nox_xxx_netSendPacket_4E5030(int a1, const void* a2, signed int a3, int a4, int a5, char a6);
int nox_xxx_importantCheckRate_4E52B0();
char* __cdecl nox_xxx_playerKickDueToRate_4E5360(int a1);
int __cdecl nox_xxx_netSendPacket1_4E5390(int a1, int a2, int a3, int a4, int a5);
int __cdecl nox_xxx_netClientSend2_4E53C0(int a1, const void* a2, int a3, int a4, int a5);
int __cdecl nox_xxx_netSendPacket0_4E5420(int a1, const void* a2, signed int a3, int a4, int a5);
int __cdecl sub_4E5450(int a1, char* a2, signed int a3, int a4, int a5);
void __cdecl sub_4E54D0(int a1, int a2, int a3);
int __cdecl sub_4E55A0(unsigned __int8 a1, int a2);
int __cdecl sub_4E55F0(unsigned __int8 a1);
int __cdecl sub_4E5630(unsigned __int8 a1, _DWORD* a2, _DWORD* a3, _DWORD* a4);
unsigned int __cdecl nox_xxx_importantCheckRate2_4E5670(unsigned __int8 a1);
void __cdecl nox_xxx_netImportant_4E5770(unsigned __int8 a1, int a2);
int nox_xxx_unused_4E5A90();
void __cdecl nullsub_24(_DWORD); // weak
void sub_4E5AC0();
void __cdecl nox_xxx_playerRemoveSpawnedStuff_4E5AD0(int a1);
BOOL __cdecl nox_xxx_isUnit_4E5B50(int a1);
BOOL __cdecl sub_4E5B80(int a1);
void __cdecl sub_4E5BF0(int a1);
void __cdecl nox_xxx_delayedDeleteObject_4E5CC0(int a1);
int __cdecl nox_xxx_unitDeleteFinish_4E5E80(_DWORD* a1);
_DWORD* nox_xxx_finalizeDeletingUnits_4E5EC0();
void __cdecl sub_4E5F00(int(__cdecl* a1)(int, int), int a2);
int __cdecl sub_4E5F40(int a1);
void __cdecl sub_4E5FC0(int a1);
int __cdecl nox_xxx_playerCameraUnlock_4E6040(int a1);
int __cdecl nox_xxx_playerCameraFollow_4E6060(int a1, int a2);
char* __cdecl nox_xxx_playerLeaveObsByObserved_4E60A0(int a1);
int __cdecl nox_xxx_playerLeaveMonsterObserver_4E60E0(int a1);
int __cdecl sub_4E6150(int a1);
int sub_4E6230();
int __cdecl nox_xxx_playerObserverFindGoodSlave0_4E6280(int a1);
unsigned __int8* __cdecl nox_xxx_updatePlayerObserver_4E62F0(int a1);
void __cdecl sub_4E6800(float*, int); // idb
int __cdecl nox_xxx_playerGoObserver_4E6860(int a1, int a2, int a3);
void __cdecl nox_xxx_playerLeaveObserver_0_4E6AA0(int a1);
BOOL __cdecl sub_4E6BD0(int a1);
long double __cdecl nox_xxx_calcDistance_4E6C00(int a1, int a2);
int __cdecl sub_4E6CE0(float2* a1, float2* a2);                                          // idb
int __cdecl nox_server_testTwoPointsAndDirection_4E6E50(float2* a1, int a2, float2* a3); // idb
int __cdecl sub_4E6EA0(int, float, int);                                                 // idb
void __cdecl sub_4E6EF0(int a1, _DWORD* a2);
void __cdecl nox_xxx_unitMove_4E7010(int, float2*);
void __cdecl nox_xxx_teleportToMB_4E7190(_BYTE* a1, float* a2);
void __cdecl sub_4E71F0(int a1);
int __cdecl nox_xxx_objectUnkUpdateCoords_4E7290(int a1);
int __cdecl sub_4E7350(int a1);
BOOL __cdecl nox_xxx_unused_4E7410(int a1);
void nox_xxx_spawnSomeBarrel_4E7470(int a1, int a2);
void __cdecl sub_4E7540(int a1, int a2);
char __cdecl nox_xxx_objectSetOn_4E75B0(int a1);
int __cdecl nox_xxx_objectSetOff_4E7600(int a1);
char __cdecl nox_xxx_objectToggle_4E7650(int a1);
int __cdecl sub_4E7680(int a1);
int __cdecl sub_4E7700(int a1);
int __cdecl nox_xxx_inventoryGetFirst_4E7980(int a1);
int __cdecl nox_xxx_inventoryGetNext_4E7990(int a1);
int __cdecl sub_4E79B0(int a1);
char __cdecl nox_xxx_unitFreeze_4E79C0(int a1, int a2);
char __cdecl nox_xxx_unitUnFreeze_4E7A60(int a1, int a2);
void __cdecl nox_xxx_unitBecomePet_4E7B00(int a1, int a2);
void __cdecl nox_xxx_monsterRemoveMonitors_4E7B60(int a1, _DWORD* a2);
int __cdecl sub_4E7BC0(int a1);
int __cdecl nox_xxx_unitIsCrown_4E7BE0(int a1);
int __cdecl nox_xxx_unitIsGameball_4E7C30(int a1);
int __cdecl nox_xxx_unitIsUnitTT_4E7C80(int a1, int a2);
int __cdecl sub_4E7CC0(int a1, int a2);
int __cdecl nox_xxx_unitCountSlaves_4E7CF0(int a1, int a2, int a3);
int __cdecl nox_xxx_inventoryCountObjects_4E7D30(int a1, int a2);
int __cdecl sub_4E7D70(int a1, int a2);
int __cdecl sub_4E7DA0(int a1, int a2, int a3);
int __cdecl sub_4E7DE0(int a1, int a2);
int __cdecl sub_4E7EC0(int a1, int a2);
char* __cdecl nox_xxx_unitPostCreateNotify_4E7F10(int a1);
int __cdecl nox_xxx_unitIsHostileMimic_4E7F90(int a1, int a2);
char* __cdecl nox_xxx_monsterMarkUpdate_4E8020(int a1);
int __cdecl sub_4E80C0(char a1);
char* __cdecl sub_4E8110(int a1);
int __cdecl sub_4E81D0(int a1);
int __cdecl sub_4E8210(int a1, int a2);
int __cdecl sub_4E8290(char a1, __int16 a2);
int __cdecl sub_4E82C0(unsigned __int8 a1, char a2, char a3, __int16 a4);
char* sub_4E8310();
unsigned __int8* __cdecl sub_4E8320(unsigned __int8 a1);
void __cdecl nox_xxx_fnFindCloseDoors_4E8340(float*, int); // idb
int __cdecl sub_4E8390(int a1);
unsigned __int8* __cdecl nox_xxx_collideMonsterEventProc_4E83B0(int a1, int a2);
unsigned __int8* __cdecl nox_xxx_collideMimic_4E83D0(int a1, int a2);
void __cdecl nox_xxx_collidePlayer_4E8460(int a1, int a2);
void __cdecl sub_4E86E0(int a1, float* a2);
void __cdecl nullsub_34(_DWORD, _DWORD, _DWORD); // weak
void __cdecl nox_xxx_collideProjectileGeneric_4E87B0(int a1, int a2);
void __cdecl nox_xxx_collideProjectileSpark_4E8880(int a1, int a2);
int __cdecl nox_xxx_doorGetSomeKey_4E8910(int a1, int a2);
int sub_4E8A10();
void __cdecl nox_xxx_collideDoor_4E8AC0(int a2, int a3);
int __cdecl nox_xxx_collidePickup_4E8DF0(int a1, int a2);
unsigned __int8* sub_4E8E50();
int sub_4E8E60();
BOOL sub_4E8F60();
int sub_4E9010();
char* __cdecl nox_xxx_collideExit_4E9090(int a1, int a2, int a3);
void __cdecl nox_xxx_collideDamage_4E9430(int a1, int a2);
_DWORD* __cdecl nox_xxx_collideManadrain_4E9490(int a1, int a2);
void __cdecl nox_xxx_spellFlyCollide_4E9500(int a1, int a2, float* a3);
void __cdecl nox_xxx_collideBomb_4E96F0(int a1, int a2);
void __cdecl nox_xxx_collideBoom_4E9770(int a1, int a2, float* a3);
void __cdecl nox_xxx_collideDie_4E99B0(int a1, int a2);
char __cdecl nox_xxx_collideGlyph_4E9A00(int a1, int a2);
BOOL __cdecl sub_4E9A30(int a1, int a2);
void __cdecl nox_xxx_fireballCollide_4E9AC0(int a1, int a2);
void __cdecl nox_xxx_collideChest_4E9C40(_DWORD* a1, int a2);
void __cdecl nox_xxx_collideSulphurShot2_4E9D80(int a1, int a2, float* a3);
void __cdecl nox_xxx_collideSulphurShot_4E9E50(int a1, int a2, int a3);
int* __cdecl nox_xxx_collideDeathBall_4E9E90(int a1, int a2, float* a3);
void __cdecl nox_xxx_collideDeathBallFragment_4E9FE0(int a1, int a2, float* a3);
void __cdecl nox_xxx_collidePixie_4EA080(int a1, int a2, float* a3);
void __cdecl nox_xxx_collideWallReflectSpark_4EA200(int a1, int a2, float2* a3);
void __cdecl sub_4EA2C0(int a1, int a2);
void __cdecl nox_xxx_collideSpark_4EA300(int a1, int a2, float* a3);
void __cdecl nox_xxx_collideWebbing_4EA380(int a1, int a2);
void __cdecl sub_4EA400(int a1, int a2);
void __cdecl nox_xxx_pickupFlagCtf_4EA490(int a1, int a2);
int __cdecl sub_4EA7A0(int a1);
__int16 __cdecl sub_4EA800(int a1, int a2);
_DWORD* __cdecl sub_4EAAA0(int a1);
_DWORD* __cdecl sub_4EAAD0(int a1, int a2);
int __cdecl nox_xxx_collidePentagram_4EAB20(int a1);
void __cdecl nox_xxx_collideSign_4EAB40(int a1, int a2);
void __cdecl nox_xxx_collideTrapDoor_4EAB60(int a1, int a2);
_DWORD* __cdecl sub_4EACA0(int a1, int a2);
int __cdecl nox_xxx_collideSpellPedestal_4EAD20(int a1, int a2);
void __cdecl nox_xxx_unused_4EAD50(int a1, int a2);
void __cdecl nox_xxx_collideFist_4EADF0(int a1, int a2);
void __cdecl nox_xxx_collideTeleportWake_4EAE30(int a1, int a2);
void __cdecl nox_xxx_collideChakram_4EAF00(int a1, int a2, float* a3);
int __cdecl sub_4EB250(int a1);
void __cdecl sub_4EB340(float*, int); // idb
void __cdecl sub_4EB3E0(int a1);
void __cdecl nox_xxx_collideArrow_4EB490(int a1, int a2);
void __cdecl nox_xxx_collideHarpoon_4EB6A0(int a1, int a2);
void __cdecl nox_xxx_collideMonsterArrow_4EB800(int a1, int a2);
void __cdecl nox_xxx_collideBearTrap_4EB890(int* a1, int a2);
void __cdecl nox_xxx_collidePoisonGasTrap_4EB910(int* a1, int a2);
int __cdecl sub_4EB9B0(int a1, int a2);
_DWORD* __cdecl nox_xxx_collideBall_4EBA00(int a1, int a2);
int __cdecl sub_4EBB50(int a1, int a2);
__int16 __cdecl nox_xxx_collideHomeBase_4EBB80(int a1, int a2);
void __cdecl nox_xxx_collideUndeadKiller_4EBD40(int a1, int a2, int a3);
void __cdecl nox_xxx_collideMonsterGen_4EBE10(int a1, int a2);
void __cdecl sub_4EBE40(int a1, int a2);
void __cdecl nox_xxx_collideAnkhQuest_4EBF40(int, int);
void nox_xxx_unitSetOwner_4EC290(int a1, int a2);
void __cdecl nox_xxx_unitClearOwner_4EC300(int a1);
int __cdecl nox_xxx_playerObserverFindGoodSlave2_4EC3E0(int a1);
int __cdecl nox_xxx_playerObserverFindGoodSlave_4EC420(int a1);
void __cdecl nox_xxx_unitRemoveChild_4EC470(int a1);
void __cdecl nox_xxx_unitTransferSlaves_4EC4B0(int a1);
int __cdecl nox_xxx_unitHasThatParent_4EC4F0(int a1, int a2);
int __cdecl nox_xxx_unitsHaveSameTeam_4EC520(int a1, int a2);
int __cdecl nox_xxx_findParentChainPlayer_4EC580(int a1);
void sub_4EC5B0();
_DWORD* __cdecl nox_xxx_respawnAdd_4EC5E0(int a1);
void __cdecl sub_4EC6A0(int a1);
BOOL nox_xxx_allocItemRespawnArray_4ECA60();
void sub_4ECA90();
char* __cdecl sub_4ECAA0(char a1);
unsigned __int8 __cdecl sub_4ECAC0(const char* a1);
char* __cdecl sub_4ECB20(char* a1);
int __cdecl sub_4ECB60(const char* a1);
int __cdecl sub_4ECBD0(int a1);
int __cdecl sub_4ECC00(const char** a1);
int __cdecl sub_4ECC70(int a1);
int __cdecl nox_server_getObjectFromNetCode_4ECCB0(int a1);
int __cdecl nox_server_netCodeCache_lookupObj_4ECD90(int a1);
int __cdecl sub_4ECDE0(_DWORD* a1, int a2);
int __cdecl sub_4ECE10(_DWORD* a1, int a2);
int nox_server_netCodeCache_initArray_4ECE50();
int __cdecl nox_server_netCodeCache_addObj_4ECEA0(int a1);
int nox_server_netCodeCache_nextUnused_4ECEF0();
int __cdecl sub_4ECF10(int a1);
int __cdecl sub_4ECFA0(int a1);
int sub_4ECFE0();
int __cdecl nox_xxx_netGetUnitByExtent_4ED020(int a1);
void __cdecl sub_4ED050(int a1, int a2);
void __cdecl sub_4ED0C0(int a1, int* a2);
int __cdecl nox_xxx_dropDefault_4ED290(int a1, int a2, float2* a3); // idb
int __cdecl sub_4ED500(int a1, int a2, float2* a3); // idb
int __cdecl nox_xxx_dropTrap_4ED580(int a1, int a2, float2* a3); // idb
int __cdecl nox_xxx_dropCrown_4ED5E0(int a1, int a2, int* a3);
int __cdecl nox_xxx_dropTreasure_4ED710(int a1, int a2, int* a3);
int __cdecl nox_xxx_drop_4ED790(int a1, _DWORD* a2, float2* a3); // idb
int __cdecl nox_xxx_drop_4ED810(int a1, int a2, float* a3);
int __cdecl nox_xxx_invForceDropItem_4ED930(int a1, _DWORD* a2);
float2* __cdecl sub_4ED970(float a1, float2* a2, float2* a3); // idb
_DWORD* __cdecl nox_xxx_dropAllItems_4EDA40(_DWORD* a1);
BOOL __cdecl nox_xxx_unitDropCheckSome_4EDCD0(int a1, int a2);
void sub_4EDD00(int a1, int a2);
int __cdecl sub_4EDDE0(int a1, _DWORD* a2, int* a3);
int __cdecl nox_xxx_dropFood_4EDE50(int a1, int a2, int* a3);
void __cdecl nox_xxx_chest_4EDF00(int a1, int a2);
double __cdecl sub_4EE2A0(int a1);
int __cdecl sub_4EE2F0(int a1, int a2, int* a3);
int __cdecl nox_xxx_dropAnkhTradable_4EE370(int a1, int a2, int* a3);
int __cdecl nox_xxx_unused_4EE390(int a1);
void __cdecl nox_xxx_unused_4EE3E0(int a1);
int sub_4EE430();
int __cdecl sub_4EE440(int a1);
void __cdecl nox_xxx_unitAdjustHP_4EE460(int a1, int a2);
void __cdecl nox_xxx_mobInformOwnerHP_4EE4C0(_DWORD* a1);
void __cdecl nox_xxx_unitDamageClear_4EE5E0(int a1, int a2);
void __cdecl nox_xxx_unitHPsetOnMax_4EE6F0(int a1);
void __cdecl nox_xxx_playerHP_4EE730(int a1);
__int16 __cdecl nox_xxx_unitGetHP_4EE780(int a1);
__int16 __cdecl nox_xxx_unitGetMaxHP_4EE7A0(int a1);
int __cdecl nox_xxx_unitSetMaxHP_4EE7C0(int a1, __int16 a2);
int __cdecl nox_xxx_activatePoison_4EE7E0(int a1, int a2, int a3);
void __cdecl nox_xxx_updatePoison_4EE8F0(int a1, int a2);
void __cdecl nox_xxx_removePoison_4EE9D0(int a1);
void __cdecl nox_xxx_setSomePoisonData_4EEA90(int a1, int a2);
unsigned __int16 __cdecl nox_xxx_playerManaAdd_4EEB80(int a1, __int16 a2);
_DWORD* __cdecl nox_xxx_playerManaSub_4EEBF0(int a1, int a2);
__int16 __cdecl nox_xxx_unitGetOldMana_4EEC80(int a1);
__int16 __cdecl nox_xxx_playerGetMaxMana_4EECB0(int a1);
int __cdecl nox_xxx_playerSetMaxMana_4EECD0(int a1, __int16 a2);
_DWORD* __cdecl nox_xxx_playerManaRefresh_4EECF0(int a1);
void sub_4EED30();
void __cdecl nox_xxx_abilGivePlayerAll_4EED40(int a1, char a2, int a3);
int __cdecl nox_xxx_plrReadVals_4EEDC0(int a1, int a2);
int __cdecl sub_4EF140(int a1);
double __cdecl nox_xxx_calcBoltDamage_4EF1E0(int a1, int a2);
void __cdecl sub_4EF410(int a1, unsigned __int8 a2);
void __cdecl nullsub_26(_DWORD); // weak
char* __cdecl nox_xxx_godSetOrUnSet_4EF500(int a1);
void sub_4EF560();
void sub_4EF570();
char nox_xxx_getRespawnWeaponFlags_4EF580();
int __cdecl sub_4EF6F0(int a1);
_DWORD* __cdecl nox_xxx_playerRespawnItem_4EF750(int a1, CHAR* a2, int* a3, int a4, int a5);
char __cdecl nox_xxx_playerMakeDefItems_4EF7D0(int a1, int a2, int a3);
int __cdecl nox_xxx_netSendPlayerRespawn_4EFC30(int a1, char a2);
int __cdecl nox_xxx_spellAdward_4EFC80(int a1);
int __cdecl nox_xxx_spellAdward_4EFD80(int a1);
char __cdecl nox_xxx_spellAdward_4EFE10(int a1);
char __cdecl nox_xxx_unitInitPlayer_4EFE80(int a1);
int __cdecl sub_4EFF10(int a1);
int __cdecl nox_xxx_unitMonsterInit_4F0040(int a1);
_DWORD* __cdecl nox_xxx_unitSparkInit_4F0390(int a1);
int __cdecl nox_xxx_initFrog_4F03B0(int a1);
int* __cdecl nox_xxx_initChest_4F0400(int a1);
_DWORD* __cdecl nox_xxx_unitBoulderInit_4F0420(_DWORD* a1);
int __cdecl sub_4F0450(int a1);
int __cdecl sub_4F0490(int a1);
int __cdecl nox_xxx_unitInitGold_4F04B0(int a1);
int* __cdecl nox_xxx_breakInit_4F0570(int a1);
int __cdecl nox_xxx_unitInitGenerator_4F0590(int a1);
void sub_4F0630();
_DWORD* __cdecl nox_server_rewardgen_activateMarker_4F0720(int a1, unsigned int a2);
_DWORD* __cdecl nox_xxx_rewardSpellBook_4F09F0(int a1, unsigned int a2);
int __cdecl nox_server_rewardGen_pickRandomSlots_4F0B60(unsigned int a1);
_DWORD* __cdecl nox_xxx_rewardAbilityBook_4F0C70(int a1);
_DWORD* __cdecl nox_xxx_rewardFieldGuide_4F0D20(int a1, unsigned int a2);
_DWORD* __cdecl nox_xxx_rewardMakeArmor_4F0E80(int a1, unsigned int a2);
int __cdecl nox_xxx_rewardMakeWeapon_4F14E0(int a1, unsigned int a2);
_DWORD* __cdecl nox_xxx_rewardMakePotion_4F1C40(int a1, unsigned int a2);
_DWORD* __cdecl nox_xxx_createGem_4F1D30(int a1, unsigned int a2);
_DWORD* __cdecl nox_xxx_createGem2_4F1F00(int a1, unsigned int a2);
void sub_4F2110();
int sub_4F2210();
int __cdecl sub_4F24E0(int a1);
BOOL __cdecl sub_4F2530(int a1);
BOOL __cdecl sub_4F2570(int a1);
int __cdecl sub_4F2590(int a1);
int __cdecl sub_4F2700(int a1);
int __cdecl sub_4F27A0(int a1);
int __cdecl sub_4F27E0(int a1);
int __cdecl sub_4F28C0(int a1);
int __cdecl sub_4F2960(int a1);
int __cdecl sub_4F2B20(int a1);
int __cdecl sub_4F2B60(int a1);
BOOL __cdecl sub_4F2C30(int a1);
BOOL __cdecl nox_xxx_spell_4F2E70(int a1);
BOOL __cdecl sub_4F2EF0(int a1);
int __cdecl nox_xxx_playerTryEquip_4F2F70(_DWORD* a1, int a2);
int __cdecl nox_xxx_playerTryDequip_4F2FB0(_DWORD* a1, int a2);
int __cdecl nox_xxx_itemApplyEngageEffect_4F2FF0(int a1, int a2);
int __cdecl nox_xxx_itemApplyDisengageEffect_4F3030(int a1, int a2);
void __cdecl nox_xxx_inventoryPutImpl_4F3070(int a1, int a2, int a3);
bool __cdecl nox_xxx_playerCheckStrength_4F3180(int a1, int a2);
int __cdecl nox_xxx_pickupDefault_4F31E0(int a1, int a2, int a3);
int __cdecl nox_xxx_pickupFood_4F3350(int a1, int a2, int a3);
int __cdecl sub_4F3400(int a1, int a2, int a3);
int __cdecl nox_xxx_pickupUse_4F34D0(int a1, int a2, int a3);
int __cdecl nox_xxx_pickupTrap_4F3510(int a1, int a2, int a3);
int __cdecl nox_xxx_pickupTreasure_4F3580(int a1, int a2, int a3);
int __cdecl nox_xxx_inventoryServPlace_4F36F0(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_pickupPotion_4F37D0(int a1, int a2, int a3);
int __cdecl nox_xxx_pickupAmmo_4F3B00(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_pickupSpellbook_4F3C60(int a1, int a2, int a3);
int __cdecl nox_xxx_pickupAbilitybook_4F3CE0(int a1, int a2, int a3);
int __cdecl sub_4F3D50(int a1, int a2, int a3);
int __cdecl sub_4F3DD0(int a1, int a2);
void sub_4F3E20();
int __cdecl nox_xxx_xfer_4F3E30(unsigned __int16 a1, int a2, int a3);
int __cdecl nox_xxx_servMapLoadPlaceObj_4F3F50(int a1, int a2, int* a3);
char __cdecl sub_4F40A0(int a1);
int __cdecl nox_xxx_readObjectOldVer_4F4170(int a1, int a2, int a3);
int __cdecl nox_xxx_mapReadWriteObjData_4F4530(int* a1, int a2);
int __cdecl nox_xxx_XFerDefault_4F49A0(int a1);
int __cdecl nox_xxx_XFerSpellPagePedistal_4F4A20(int a1);
int __cdecl nox_xxx_XFerReadable_4F4AB0(int a1);
int __cdecl nox_xxx_XFerExit_4F4B90(int a1);
int __cdecl nox_xxx_XFerDoor_4F4CB0(int a1);
int __cdecl nox_xxx_unitTriggerXfer_4F4E50(float a1);
int __cdecl nox_xxx_XFerHole_4F51D0(int a1);
int __cdecl nox_xxx_XFerTransporter_4F5300(int a1);
int __cdecl nox_xxx_XFerElevator_4F53D0(int a1);
int __cdecl nox_xxx_XFerElevatorShaft_4F54A0(int a1);
int __cdecl sub_4F5540(int a1);
int __cdecl nox_xxx_xferReadScriptHandler_4F5580(int a1, char* a2);
int __cdecl nox_xxx_XFerMover_4F5730(int a1);
int __cdecl nox_xxx_XFerGlyph_4F5890(int a1);
int __cdecl nox_xxx_XFerInvLight_4F5AA0(int* a1);
int __cdecl nox_xxx_XFerSentry_4F5E50(int a1);
int __cdecl nox_xxx_XFerSpellReward_4F5F30(int* a1);
int __cdecl nox_xxx_XFerAbilityReward_4F6240(int* a1);
int __cdecl nox_xxx_XFerFieldGuide_4F6390(int* a1);
int __cdecl nox_xxx_XFerWeapon_4F64A0(int a1);
int __cdecl nox_xxx_XFerArmor_4F6860(int a1);
int __cdecl nox_xxx_XFerAmmo_4F6B20(int* a1);
int __cdecl nox_xxx_XFerTeam_4F6D20(int* a1);
int __cdecl nox_xxx_XFerGold_4F6EC0(int a1);
int __cdecl nox_xxx_XFerObelisk_4F6F60(int* a1);
int __cdecl nox_xxx_XFerToxicCloud_4F70A0(int a1);
int __cdecl nox_xxx_XFerMonsterGen_4F7130(int* a1);
int __cdecl nox_xxx_XFerRewardMarker_4F74D0(int* a1);
void sub_4F78D0();
int __cdecl nox_xxx_unitInventoryContains_4F78E0(int a1, int a2);
int __cdecl nox_xxx_equipedItemByCode_4F7920(int a1, int a2);
void __cdecl sub_4F7950(int a1);
void __cdecl nox_xxx_playerSetCustomWP_4F79A0(int a1, int a2, int a3);
int __cdecl nox_xxx_playerConfusedGetDirection_4F7A40(int a1);
_DWORD* __cdecl nox_xxx_mapFindPlayerStart_4F7AB0(float2* a1, int a2); // idb
BOOL __cdecl sub_4F7CE0(int a1, int a2);
int __cdecl nox_xxx_playerSubStamina_4F7D30(int a1, int a2);
void __cdecl sub_4F7DB0(int a1, char a2);
int __cdecl nox_xxx_checkWinkFlags_4F7DF0(int a1);
int __cdecl nox_xxx_weaponGetStaminaByType_4F7E80(int a1);
__int16 __cdecl nox_xxx_playerRespawn_4F7EF0(int a1);
BOOL __cdecl sub_4F80C0(int a1, float2* a3); // idb
char __cdecl nox_xxx_updatePlayer_4F8100(_DWORD* a1);
void __cdecl nox_xxx_playerInventory_4F8420(int a1);
void __cdecl nox_xxx_unitUpdatePlayerImpl_4F8460(int a1);
BOOL __cdecl sub_4F9A80(int a1);
int __cdecl sub_4F9AB0(int a1);
void* __cdecl nox_xxx_plrSetSpellType_4F9B90(int a1);
int __cdecl nox_xxx_playerCanMove_4F9BC0(int a1);
BOOL __cdecl nox_xxx_playerCanAttack_4F9C40(int a1);
void __cdecl nox_xxx_playerInputAttack_4F9C70(_DWORD* a1);
BOOL __cdecl nox_xxx_playerAimsAtEnemy_4F9DC0(int a1);
int __cdecl sub_4F9E10(int a1);
void __cdecl sub_4F9E70(int a1);
void __cdecl sub_4F9ED0(_DWORD* a1);
int __cdecl nox_xxx_animPlayerGetFrameRange_4F9F90(int a1, _DWORD* a2, int* a3);
int __cdecl nox_xxx_anim_4F9FB0(int a1, int a2, int a3);
int __cdecl nox_xxx_unitGetStrength_4F9FD0(int a1);
int __cdecl nox_xxx_playerSetState_4FA020(_DWORD* a1, int a2);
int __cdecl sub_4FA280(int a1);
int __cdecl nox_common_mapPlrActionToStateId_4FA2B0(int a1);
int __cdecl nox_xxx_itemApplyUpdateEffect_4FA490(int a1);
int __cdecl nox_xxx_checkInversionEffect_4FA4F0(int a1, int a2);
_DWORD* __cdecl nox_xxx_playerAddGold_4FA590(int a1, int a2);
_DWORD* __cdecl nox_xxx_playerSubGold_4FA5D0(int a1, unsigned int a2);
_DWORD* __cdecl sub_4FA620(int a1, int a2);
int __cdecl nox_xxx_playerGetGold_4FA6B0(int a1);
int __cdecl sub_4FA6D0(int a1);
int __cdecl nox_xxx_playerBotCreate_4FA700(int a1);
char __cdecl nox_xxx_mobMorphFromPlayer_4FAAC0(_DWORD* a1);
char __cdecl nox_xxx_mobMorphToPlayer_4FAAF0(_DWORD* a1);
int __cdecl nox_xxx_updatePlayerMonsterBot_4FAB20(_DWORD* a1);
char __cdecl nox_xxx_monsterActionToPlrState_4FABC0(int a1);
int __cdecl nox_xxx_respawnPlayerBot_4FAC70(int a1);
int __cdecl nox_xxx_netSendRewardNotify_4FAD50(int a1, int a2, int a3, char a4);
void __cdecl sub_4FADD0(int a1, const char* a2, char a3);
void sub_4FAE70();
int __cdecl sub_4FB000(int a1, int a2);
int __cdecl sub_4FB050(int a1, int a2, int* a3);
void sub_4FB090();
void sub_4FB0A0();
int __cdecl nox_xxx_playerDoSchedSpell_4FB0E0(int a1, int a2);
int __cdecl nox_xxx_playerDoSchedSpellQueue_4FB1D0(int a1, int a2);
void sub_4FB940();
void sub_4FB950();
char* nox_xxx_allocArrayExecAbilities_4FB990();
void __cdecl nox_xxx_playerInvokeAbility_4FBAF0(_DWORD* a1, int a2);
void __cdecl nox_xxx_playerExecuteAbil_4FBB70(int a1, int a2);
int __cdecl sub_4FBE60(int a1, int a2);
char* __cdecl sub_4FBEA0(int a1, int a2, int a3);
int __cdecl sub_4FC030(int a1, int a2);
_DWORD* __cdecl sub_4FC070(int a1, int a2, int a3);
void __cdecl sub_4FC0B0(int a1, int a2);
void __cdecl nox_xxx_playerCancelAbils_4FC180(int a1);
int __cdecl nox_common_playerIsAbilityActive_4FC250(int a1, int a2);
int __cdecl nox_xxx_playerIsExecutingAbility_4FC2B0(int a1);
void __cdecl sub_4FC300(_DWORD* a1, int a2);
int __cdecl sub_4FC3C0(int a1, char a2, char a3);
int __cdecl nox_xxx_probablyWarcryCheck_4FC3E0(int a1, int a2);
void __cdecl sub_4FC440(int a1, int a2);
int __cdecl nox_xxx_unused_4FC4A0(int a1, int a2, int a3);
int __cdecl nox_xxx_unused_4FC4C0(int a1);
void sub_4FC560();
int __cdecl nox_xxx_resetMapInit_4FC570(int a1);
int __cdecl sub_4FC580(int a1);
int __cdecl sub_4FC670(int a1);
void sub_4FC950();
int __cdecl sub_4FC960(int a1, char a2);
int nox_xxx_allocSpellRelatedArrays_4FC9B0();
int nox_xxx_freeSpellRelated_4FCA80();
int __cdecl nox_xxx_Fn_4FCAC0(int a1, int a2);
void nox_xxx_spellCastByBook_4FCB80();
int __cdecl sub_4FCEB0(int a1);
int __cdecl nox_xxx_spellCheckSmth_4FCEF0(int a1, int* a2, int a3);
int __cdecl sub_4FCF90(int a1, int a2, int a3);
unsigned __int16 __cdecl sub_4FD030(int a1, __int16 a2);
void __cdecl nox_xxx_teleportPixie_4FD050(_DWORD* a1, int a2);
void __cdecl sub_4FD090(int a1);
int __cdecl sub_4FD0E0(int a1, int a2);
int __cdecl nox_xxx_checkPlrCantCastSpell_4FD150(int a1, int a2, int a3);
int __cdecl nox_xxx_spellAccept_4FD400(int a1, int a2, _DWORD* a3, int a4, int* a5, int a6);
int __cdecl nox_xxx_gameCaptureMagic_4FDC10(int a1, int a2);
int __cdecl nox_xxx_castSpellByUser_4FDD20(int a1, _DWORD* a2, int* a3);
_DWORD* __cdecl nox_xxx_createSpellFly_4FDDA0(int a1, int a2, int a3);
void nox_xxx_collide_4FDF90(int a1, int a2);
int __cdecl nox_xxx_unused_4FE060(int a1, int a2);
int __cdecl sub_4FE100(int a1);
int __cdecl nox_xxx_spellGetPhoneme_4FE1C0(int a1, char a2);
int __cdecl nox_xxx_spellByBookInsert_4FE340(int a1, int* a2, int a3, int a4, int a5);
void __cdecl nox_xxx_spell_4FE680(int a1, float a2);
int __cdecl nox_xxx_spellGetPower_4FE7B0(int a1, int a2);
void sub_4FE840();
BOOL nox_xxx_spellCreateDurations_4FE850();
void sub_4FE880();
void __cdecl sub_4FE8A0(int a1);
int __cdecl sub_4FE900(int a1);
int nox_xxx_spellCastedFirst_4FE930();
int __cdecl nox_xxx_spellCastedNext_4FE940(int a1);
_WORD* nox_xxx_spellDurationAdd_4FE950();
void __cdecl sub_4FE980(int a1);
char __cdecl nox_xxx_spellCancelSpellDo_4FE9D0(int a1);
BOOL __cdecl sub_4FEA70(int a1, float2* a2); // idb
int __cdecl nox_xxx_playerCancelSpells_4FEAE0(int a1);
_DWORD* __cdecl nox_xxx_spellCancelDurSpell_4FEB10(int a1, int a2);
unsigned int __cdecl sub_4FEB60(int a1, int a2);
int __cdecl nox_xxx_spellDurationBased_4FEBA0(int a1, int a2, _DWORD* a3, int a4, _DWORD* a5, int a6, int(__cdecl* a7)(_WORD*), int a8, int a9,
					   int a10);
int __cdecl sub_4FED40(int a1);
int sub_4FED70();
void __cdecl nox_xxx_plrCastSmth_4FEDA0(int* a1);
int __cdecl sub_4FEE50(int a1, int a2);
char __cdecl nox_xxx_cancelAllSpells_4FEE90(int a1);
void nox_xxx_clientMB_4FEEF0();
void __cdecl nox_xxx_netStopRaySpell_4FEF90(int a1, _DWORD* a2);
char* __cdecl nox_xxx_netStartDurationRaySpell_4FF130(int a1);
int __cdecl sub_4FF2D0(int a1, int a2);
void __cdecl sub_4FF310(int a1);
BOOL __cdecl nox_xxx_testUnitBuffs_4FF350(int a1, char a2);
__int16 __cdecl nox_xxx_buffApplyTo_4FF380(int a1, int a2, __int16 a3, char a4);
int __cdecl nox_xxx_unitGetBuffTimer_4FF550(int a1, int a2);
char __cdecl nox_xxx_buffGetPower_4FF570(int a1, int a2);
int __cdecl nox_xxx_unitClearBuffs_4FF580(int a1);
int __cdecl nox_xxx_spellBuffOff_4FF5B0(int a1, int a2);
void __cdecl nox_xxx_updateUnitBuffs_4FF620(int a1);
BOOL nox_xxx_allocMagicWallArray_4FF730();
int sub_4FF770();
void nox_xxx_mapWall_4FF790();
void __cdecl sub_4FF7B0(int a1);
int __cdecl nox_xxx_wallDestroyMagicwallSysuse_4FF840(int a1);
void __cdecl nox_xxx_wallDestroy_4FF870(int a1);
int __cdecl sub_4FF900(int a1);
int __cdecl sub_4FF990(int a1);
char __cdecl nox_xxx_spellWallCreateCalcDirMB_4FF9B0(int a1, int a2, int a3, int a4, int a5);
char __cdecl sub_4FFA40(int a1);
int __cdecl nox_xxx_spellWallCreate_4FFA90(int a1);
BOOL __cdecl sub_4FFD00(int a1, int a2, int a3, unsigned __int8 a4);
_DWORD* __cdecl nox_xxx_netWallCreate_4FFE80(int a1, _BYTE* a2, int a3, char a4, char a5, char a6);
char __cdecl nox_xxx_spellWallCreateCalcXMB_4FFEF0(int a1, int a2, int a3, char a4);
char __cdecl nox_xxx_spellWallCreateCalcYMB_4FFFB0(int a1, int a2, int a3, char a4);
int nox_xxx_spellWallUpdate_500070();
int __cdecl nox_xxx_spellWallDestroy_500080(int a1);
int __cdecl sub_5000B0(_DWORD* a1);
int __cdecl sub_5002D0(_DWORD* a1);
void __cdecl sub_500330(char a1, char a2, int a3, char a4, char a5, char a6, char a7, char a8, char a9, char a10);
int nox_xxx_mapSetWallInGlobalDir0pr1_5004D0();
int nox_xxx_map_5004F0();
char* __cdecl nox_xxx_journalQuestSet_500540(char* a1, int a2);
char* __cdecl nox_xxx_scriptGetJournal_5005E0(char* a1);
char* __cdecl nox_xxx_journalQuestSetBool_5006B0(char* a1, int a2);
int __cdecl sub_500750(char* a1);
double __cdecl sub_500770(char* a1);
void __cdecl sub_500790(LPVOID lpMem); // idb
char* __cdecl sub_5007E0(char* a1);
unsigned int __cdecl sub_5009B0(char* a1);
int sub_500A60();
int sub_500B70();
int __cdecl sub_500C70(int a1, int a2);
int __cdecl sub_500CA0(int a1, int a2);
int __cdecl nox_xxx_creatureIsMonitored_500CC0(int a1, int a2);
int __cdecl nox_xxx_countControlledCreatures_500D10(int a1);
int __cdecl sub_500D50(int a1);
bool __cdecl nox_xxx_checkSummonedCreaturesLimit_500D70(int a1, int a2);
int __cdecl nox_xxx_summonStart_500DA0(int a1);
int __cdecl sub_500F40(int a1, float a2);
int __cdecl nox_xxx_summonFinish_5010D0(int a1);
void __cdecl nox_xxx_summonCancel_5011C0(int a1);
int __cdecl nox_xxx_charmCreature1_5011F0(int* a1);
int __cdecl nox_xxx_charmCreatureFinish_5013E0(int* a1);
int __cdecl nox_xxx_charmCreature2_501690(int a1);
_DWORD* __cdecl nox_xxx_unitDoSummonAt_5016C0(int a1, int* a2, int a3, unsigned __int8 a4);
void __cdecl sub_5017F0(int a1);
int nox_xxx_allocAudEventArray_501860();
void sub_5018D0();
int __cdecl sub_501900(int a1);
int __cdecl sub_501920(int a1);
int __cdecl nox_xxx_getSevenDwords3_501940(int a1);
_DWORD* __cdecl nox_xxx_aud_501960(int a1, int a2, int a3, int a4);
void __cdecl nox_xxx_audCreate_501A30(int, float2*, int, int);
int __cdecl nox_xxx_gameSetAudioFadeoutMb_501AC0(int a1);
int sub_501AE0();
int __cdecl sub_501AF0(int a1, float* a2, float* a3);
unsigned int __cdecl nox_xxx_unused_501BB0(int a1, float* a2, float* a3);
char __cdecl sub_501C00(float* a1, int a2);
int __cdecl nox_xxx_netUpdateRemotePlr_501CA0(int a1);
int sub_501E80();
_DWORD* __cdecl sub_501EA0(_DWORD* a1, int a2);
int __cdecl sub_501EF0(unsigned int a1);
unsigned int __cdecl sub_501F10(unsigned int a1);
_DWORD* __cdecl sub_501F30(int a1, _DWORD* a2);
int __cdecl sub_501FD0(int a1, int a2, __int16 a3);
int __cdecl sub_502060(int a1);
BOOL __cdecl nox_thing_read_AVNT_502120(int a1, void* a2);
int __cdecl nox_thing_read_audio_502320(int a1, void* a2);
int __cdecl sub_502370(int a1, void* a2);
unsigned __int8* __cdecl nox_xxx_scriptCallByEventBlock_502490(int* a1, int a2, int a3);
void __cdecl sub_5025A0(int a1, int a2, int a3);
int __cdecl sub_5025E0(int a1, int a2, int a3);
void __cdecl nox_server_scriptExecuteFnForEachGroupObj_502670(unsigned __int8* a1, int a2, void(__cdecl* a3)(int, int),
															  int a4);
int __cdecl nox_xxx_mapgenMakeScript_502790(FILE* a1, char* a2);
char* nox_xxx_mapReset_5028E0();
int __cdecl sub_5029A0(char* a1);
int __cdecl sub_5029F0(int a1);
int sub_502A20();
BOOL __cdecl sub_502A30(char* a1);
int __cdecl sub_502A50(char* a1);
int sub_502A90();
int __cdecl sub_502AB0(char* a1);
int sub_502AF0();
int sub_502B10();
BOOL __cdecl sub_502D70(int a1);
FILE* __cdecl sub_502DA0(char* a1);
FILE* sub_502DF0();
FILE* __cdecl sub_502E10(int a1);
FILE* __cdecl sub_502E50(char* a1);
double __cdecl sub_502E70(int a1);
double __cdecl sub_502EA0(int a1);
FILE* __cdecl sub_502ED0(const char* a1);
BOOL sub_503140();
FILE* __cdecl sub_503230(const char* a1, char* a2);
int __cdecl sub_5034B0(char* a1);
int __cdecl sub_5036D0(char* a1, LPCSTR lpFileName);
int __cdecl nox_xxx_mapgenSaveMap_503830(int a1);
int __cdecl sub_503B30(float2* a1); // idb
int __cdecl sub_503EC0(int a1, float* a2);
char* nox_xxx_free_503F40();
_DWORD* __cdecl nox_xxx_tileAllocTileInCoordList_5040A0(int a1, int a2, float a3);
int __cdecl nox_xxx_tileInit_504150(int a1, int a2);
_DWORD* __cdecl sub_504290(char a1, char a2);
_DWORD* __cdecl nox_xxx_cliWallGet_5042F0(int a1, int a2);
int __cdecl sub_504330(int a1, int a2);
_DWORD* __cdecl sub_5044B0(int a1, float a2, float a3); // idb
int __cdecl sub_504560(int a1, int a2);
int sub_5045B0();
int __cdecl sub_5045F0(int a1);
_DWORD* __cdecl sub_504600(const char* a1, int a2, char a3);
int __cdecl sub_5046A0(_DWORD* a1, int a2);
int __cdecl sub_504720(int a1, int a2);
int __cdecl sub_504760(int a1, int a2);
_DWORD* __cdecl nox_xxx_unitAddToList_5048A0(int a1);
int __cdecl sub_504910(int a1, int a2);
int sub_504980();
int __cdecl sub_5049C0(int a1);
LPVOID sub_5049D0();
int __cdecl sub_5049E0(int a1);
int __cdecl sub_5049F0(FILE* a1, int a2);
int __cdecl sub_504A10(int a1);
int __cdecl sub_504AB0(char* a1);
int __cdecl nox_server_mapRWObjectData_504CF0(_DWORD* a2); // idb
int nox_server_mapRWScriptData_504F90();
LPVOID sub_505050();
LPVOID sub_505060();
int nox_server_mapRWMapIntro_505080();
int nox_xxx_parseNoxCObj_505360();
int nox_xxx_ncobjReadInt_505800();
BOOL __cdecl sub_505830(size_t a1, void* a2);
BOOL __cdecl nox_xxx_ncobjReadAndCheck_505870(const char* a1);
char* nox_xxx_scriptFreeAnything_5058F0();
int nox_server_mapRWScriptObject_505A40();
int nox_server_mapRWGroupData_505C30();
int nox_server_mapRWDebugData_5060D0();
int __cdecl nox_server_mapRWWaypoints_506260(_DWORD* a1);
int nox_xxx_allocVoteArray_5066D0();
void sub_506700();
int sub_506720();
int __cdecl sub_506740(int a1);
void __cdecl sub_5067B0(int a1);
int __cdecl sub_506810(int a1);
int __cdecl nox_xxx_netSendVote_506840(int a1);
char __cdecl sub_506870(int a1, int a2, wchar_t* a3);
char __cdecl sub_5068E0(int a1, int a2, wchar_t* a3);
_DWORD* __cdecl sub_506A20(int a1, int a2);
int __cdecl nox_xxx_voteAddMB_506AD0(int a1);
_DWORD* __cdecl sub_506B00(int a1, int a2);
_DWORD* __cdecl sub_506B80(int a1, int a2, wchar_t* a3);
void __cdecl sub_506C90(int a1, int a2, wchar_t* a3);
void __cdecl sub_506D00(int a1, wchar_t* a2);
void __cdecl sub_506DE0(int a1);
void __cdecl sub_506E50(int a1, wchar_t* a2);
void __cdecl sub_506F80(int a1);
BOOL __cdecl sub_507000(int a1);
void __cdecl sub_507090(int a1);
void __cdecl sub_507100(int a1);
int __cdecl sub_507190(int a1, char a2);
BOOL sub_5071C0();
int __cdecl nox_xxx_scriptByNameMB_5071D0(const char* a1);
int __cdecl nox_xxx_scriptPushValue_507230(int a1);
int script_pop();
int __cdecl nox_xxx_getNextArrayItem_507270(int** a1);
double __cdecl sub_507280(float** a1);
LPVOID nox_xxx_scriptRunFirst_507290();
int __cdecl nox_server_doMapScript_507310(int a1, int a2, int a3);
int __cdecl nox_server_scriptCallBuiltinFn_508B70(int a1, int a2);
int __cdecl sub_508C30(int a1);
int __cdecl sub_508C70(int a1);
int __cdecl sub_508CB0(_DWORD* a1, int a2);
void __cdecl sub_509120(_DWORD* a1, int a2, const char* a3);
int sub_5095E0();
int sub_5096F0();
int sub_5098A0();
int sub_5099B0();
char sub_509A60();
_DWORD* __cdecl sub_509C30(int a1);
int* sub_509CB0();
int __cdecl sub_509CF0(const char* a1, char a2, int a3);
int __cdecl sub_509D80(int a1);
int __cdecl nox_xxx_xferDirectionToAngle_509E00(_DWORD* a1);
int __cdecl nox_xxx_xferIndexedDirection_509E20(int a1, int2* a2); // idb
int __cdecl nox_xxx_mathDirection4ToAngle_509E90(int a1);
int __cdecl nox_xxx_math_509EA0(int a1);
int __cdecl nox_xxx_math_509ED0(float2*);
void __cdecl nox_xxx_utilNormalizeVector_509F20(float2* a1); // idb
void __cdecl nox_xxx_debugPrintStack_509F60(int a1, int a2);
int __cdecl sub_509FF0(int a1);
int __cdecl nox_xxx_monsterActionIsCondition_50A010(int a1);
int __cdecl nox_xxx_mobActionGet_50A020(int a1);
int __cdecl sub_50A040(int a1);
int __cdecl nox_xxx_monsterIsActionScheduled_50A090(int a1, int a2);
int __cdecl nox_xxx_checkMobAction_50A0D0(int a1, int a2);
int __cdecl nox_xxx_monsterActionReset_50A110(int a1);
char __cdecl nox_xxx_monsterPopAction_50A160(int a1);
int* __cdecl nox_xxx_monsterPushAction_50A260(int a1, int a2);
int* __cdecl nox_xxx_monsterAction_50A360(int a1, int a2);
void __cdecl nox_xxx_monsterClearActionStack_50A3A0(int a1);
int __cdecl nox_xxx_monsterCallDieFn_50A3D0(_DWORD* a1);
int __cdecl nox_xxx_unitUpdateMonster_50A5C0(float a1);
char __cdecl nox_xxx_updateNPCAnimData_50A850(int a1);
int __cdecl nox_xxx_mobAction_50A910(int a1);
int __cdecl sub_50AA90(int a1);
int nox_xxx_minimapFirstMonster_50AAE0();
int nox_xxx_minimapNextMonster_50AB10();
__int16 __cdecl sub_50AB50(int a1, int a2);
int nox_xxx_allocVisitNodesArray_50AB90();
void sub_50ABF0();
int __cdecl sub_50AC20(int a3, _WORD* a2); // idb
void __cdecl sub_50AE80(int a1, int a2);
int __cdecl sub_50AEA0(int a1, float2* a2, _DWORD* a3); // idb
int __cdecl nox_xxx_aiTestUnitDangerous_50B2C0(int a1);
void sub_50B500();
int sub_50B510();
int sub_50B520();
int __cdecl nox_xxx_mapTraceObstacles_50B580(int a1, float4* a3);       // idb
void __cdecl sub_50B600(float* arg0, float4* a1); // idb
BOOL __cdecl sub_50B810(int a1, float* a2);
BOOL __cdecl sub_50B870(int a1, int a2, int a3);
BOOL __cdecl nox_xxx_pathfind_preCheckWalls2_50B8A0(int a1, int a2, int a3);
unsigned int __cdecl sub_50B8E0(int a1, int a2, int a3);
int __cdecl sub_50B950(int a1, int a2, int a3);
int __cdecl nox_xxx_genPathToPoint_50B9A0(int a1, int a2, int a3, float* a4);
LPVOID __cdecl nox_xxx_pathFind_50BA00(int a1, int a2, float* a3, float* a4, int(__cdecl* a5)(int, int, int), int a6);
int __cdecl sub_50C320(int a1, int a2, _DWORD* a3);
int __cdecl sub_50C830(int a1, int a2, int a3);
void __cdecl nox_xxx_pathfind_preCheckWalls_50C8D0(int a1, int2* a2); // idb
int __cdecl nox_xxx_appendWorkPath_50C990(int a1, int a2, int a3);
int __cdecl nox_xxx_generateRetreatPath_50CA00(int a1, int a2, int a3, float* a4);
BOOL __cdecl sub_50CA60(int a4, int a2, int a3); // idb
void __cdecl sub_50CAC0(int a1, int a2);
int nox_xxx_pathFindStatus_50CAF0();
int sub_50CB00();
LPVOID sub_50CB10();
int __cdecl sub_50CB20(int a1, float* a2);
unsigned __int8* sub_50CD30();
void nox_xxx_audioAddAIInteresting_50CD40(int a1, int a2, _DWORD* a3);
void sub_50CDD0(int a1);
BOOL __cdecl sub_50CF10(int a1, int a2);
int __cdecl sub_50D000(int a1, int a2);
int __cdecl sub_50D0C0(char a1, int a2);
unsigned __int8* __cdecl sub_50D110(int a1, _DWORD* a2, int a3);
_DWORD* __cdecl sub_50D190(int a1, _DWORD* a2, _DWORD* a3);
void sub_50D1C0();
void sub_50D1E0();
int sub_50D210();
int __cdecl nox_xxx_creatureSetDetailedPath_50D220(int a1, int a2);
int __cdecl sub_50D2A0(int a1, int a2);
BOOL __cdecl sub_50D2E0(int a1);
int __cdecl nox_xxx_creatureActuallyMove_50D3B0(float* a1);
int __cdecl nox_xxx_creatureSetMovePath_50D5A0(int a1);
int nox_xxx_allocMonsterRelatedArrays_50D780();
void sub_50D7E0();
void sub_50D820();
int __cdecl sub_50D860(int a1, int a2);
void sub_50D8D0();
int sub_50D960();
int sub_50DE10();
_DWORD* __cdecl sub_50DE60(int a1);
int __cdecl sub_50DE80(int a1, float* a2);
void __cdecl sub_50DFB0(float*, int); // idb
int __cdecl sub_50E030(int a1, _DWORD* a2);
int __cdecl sub_50E110(int a1);
void __cdecl sub_50E140(int a1);
int __cdecl sub_50E1B0(int a1);
void __cdecl sub_50E1E0(int a1);
void __cdecl sub_50E210(int a1);
int nox_xxx_registerShopClasses_50E2A0();
int nox_xxx_deleteShopInventories_50E300();
int sub_50E360();
int __cdecl nox_xxx_shopGetItemCost_50E3D0(int, int, float); // idb
int __cdecl sub_50E820(int a1, int a2);
_DWORD* nox_xxx_createShopStruct_50E870();
void __cdecl nox_xxx_loadShopItems_50E970(int a1);
float* __cdecl nox_xxx_addItemToShopSession_50EE00(int a1, float a2);
int __cdecl sub_50EEC0(_DWORD* a1);
int __cdecl sub_50F2B0(int a1, _DWORD* a2);
void __cdecl sub_50F3A0(_DWORD* a1);
int __cdecl sub_50F450(int a1);
int __cdecl sub_50F490(_DWORD* a1, int a2);
void __cdecl nox_xxx_shopExit_50F4C0(_DWORD* a1);
int __cdecl nox_xxx_sendEndTradeMsg_50F560(int a1);
void __cdecl nox_xxx_tradeAccept_50F5A0(int a1, int a2);
int __cdecl sub_50F6B0(int a1);
int __cdecl sub_50F6E0(int a1);
int __cdecl sub_50F720(int a1, _DWORD* a2);
void __cdecl sub_50F790(int a1, int a2, int* a3);
_DWORD* __cdecl sub_50F7F0(int a1, int a2);
int __cdecl nox_xxx_tradeP2PUpdStuff_50FA00(int a1, _DWORD* a2);
_DWORD* __cdecl sub_50FB90(_DWORD* a1);
int __cdecl sub_50FD20(_DWORD* a1, int a2);
int __cdecl nox_xxx_tradeP2PAddOfferMB_50FE20(int a1, int a2);
int __cdecl sub_50FF90(int a1, int a2, int a3);
_DWORD* __cdecl sub_50FFE0(_DWORD* a1, int a2);
void __cdecl sub_510000(int a1);
int __cdecl nox_xxx_unused_5100A0(int a1, int a2);
int __cdecl nox_xxx_getSomeShopData_5103A0(int a1, int a2);
int __cdecl sub_5103F0(int a1, int a2);
_DWORD* __cdecl sub_5108D0(int a1, int a2, int a3);
_DWORD* __cdecl sub_510AE0(int* a1, int a2, _DWORD* a3);
unsigned __int16 __cdecl sub_510D10(int* a1, int a2, int a3, unsigned int a4);
void __cdecl nox_xxx_shopCancelSession_510DC0(_DWORD* a1);
int __cdecl sub_510DE0(int a1, int a2);
void __cdecl sub_510E20(int a1);
void sub_510E50();
signed int __cdecl nox_xxx_updateSentryGlobe_510E60(int a1);
_DWORD* __cdecl nox_xxx_sentryUpdateList_510FD0(_DWORD* a1);
void __cdecl nox_xxx_sentry_511020(int a1, int a2);
void __cdecl sub_511100(int a1);
int __cdecl sub_511250(int a1, float* a2);
BOOL nox_xxx_allocSpringsArray_5112C0();
void sub_5112F0();
int sub_511310();
void __cdecl sub_511330(int a1);
void __cdecl sub_511360(int a1);
void nox_xxx_updateDebugObjects_5113A0();
void __cdecl nox_xxx_unused_5114D0(int a1, int a2, int a3, int a4, int a5, int a6);
int __cdecl sub_511560(int a1);
int __cdecl nox_xxx_unitSetDecayTime_511660(_DWORD* a1, int a2);
int __cdecl nox_xxx_decay_5116F0(int a1);
void nox_xxx_decay_511750();
int nox_xxx_decayDestroy_5117B0();
char __cdecl sub_5117F0(int a1);
void __cdecl nox_xxx_unit_511810(int a1);
void nox_xxx_j_allocHitArray_511840(void); // weak
void nox_xxx_collisions_511850();
int __cdecl nox_xxx_updateObjectsVelocity_5118A0(float); // idb
int __cdecl nox_server_scriptValToObjectPtr_511B60(int a1);
int __cdecl nox_xxx_script_511C50(int a1);
int __cdecl sub_511CB0(_DWORD* a1, int a2);
int __cdecl sub_511CE0(_DWORD* a1, int a2);
int sub_511D20();
int __cdecl nox_xxx_scriptPrepareFoundUnit_511D70(int a1);
int sub_511DC0();
int __cdecl sub_511DE0(int a1);
int sub_511E20();
void sub_511E60();
int nox_xxx_scriptGetWall_511EB0();
int nox_xxx_scriptOpenSecretWall_511F50();
void __cdecl sub_511F80(int a1);
int nox_xxx_scriptOpenWallGroup_512010();
int nox_xxx_scriptCloseWall_512040();
void __cdecl sub_512070(int a1);
int nox_xxx_scriptCloseWallGroup_512100();
int nox_xxx_scriptToggleWall_512130();
void __cdecl sub_512160(int a1);
int nox_xxx_scriptToggleWallGroup_512260();
int nox_xxx_scriptWallBreakMB_512290();
int __cdecl nox_xxx_wallPreDestroyByPtr_5122C0(int a1);
int nox_xxx_scriptWallGroupBreak_5122F0();
int nox_xxx_scriptSecondTimer_512320();
int nox_xxx_scriptFrameTimer_512350();
int nox_xxx_scriptMoverOrMonsterGo_512370();
int* __cdecl nox_server_scriptMoveTo_5123C0(int a1, int a2);
int __cdecl nox_xxx_moverGoTo_5124C0(_DWORD* a1, _DWORD* a2);
int nox_server_scriptGroupGoTo_512500();
int nox_xxx_script_LookAtDirection_512560();
float* __cdecl nox_xxx_monsterLookAt_5125A0(int a1, int a2);
int nox_xxx_script_GroupLookAtDirection_512610();
int nox_xxx_script_ObjectOn_512670();
int nox_xxx_scriptObjGroupOn_512690();
char __cdecl sub_5126C0(int a1);
int nox_xxx_scriptWaypointOn_5126D0();
int nox_xxx_scriptWaypointGroupOn_5126F0();
int __cdecl sub_512720(int a1);
int nox_xxx_script_ObjectOff_512730();
int nox_xxx_scriptObjGroupOff_512750();
int __cdecl sub_512780(int a1);
int nox_xxx_scriptWaypointOff_512790();
int nox_xxx_scriptWaypointGroupOff_5127B0();
int __cdecl sub_5127E0(int a1);
int nox_xxx_scriptToggleObject_5127F0();
int nox_xxx_scriptToggleObjectGroup_512810();
char __cdecl sub_512840(int a1);
int nox_xxx_scriptToggleWaypoint_512850();
int nox_xxx_scriptToggleWaypointGroup_512870();
int __cdecl sub_5128A0(int a1);
int nox_xxx_scriptDeleteObject_5128B0();
int nox_xxx_scriptDeleteObjectGroup_5128D0();
void __cdecl sub_512900(int a1);
int nox_xxx_scriptFollowNearestWaypoint_512910();
void __cdecl nox_xxx_scriptMonsterRoam_512930(int a1);
int nox_xxx_scriptGroupRoam_512990();
int nox_xxx_scriptGetObject2_5129C0();
int nox_xxx_scriptGetObject3_5129E0();
int nox_xxx_scriptGotoHome_512A00();
int nox_xxx_scriptAudioEven_512AC0();
int nox_xxx_scriptReturnOne_512C10();
int nox_xxx_scriptUnlockDoor_512C20();
int nox_xxx_scriptLockDoor_512C60();
int nox_xxx_script_IsOn_512CA0();
int nox_xxx_scriptWpIsEnabled_512CE0();
int nox_xxx_scriptDoorIsLocked_512D20();
int nox_xxx_scriptRandomFloat_512D70();
int nox_xxx_scriptRandomInt_512DB0();
int nox_xxx_scriptTimerSecSpecial_512DE0();
int nox_xxx_scriptSpecialTimer_512E10();
int __cdecl nox_xxx_addScriptString_512E40(char* a1);
int nox_xxx_scriptIntToString_512EA0();
int nox_xxx_scriptFloatToString_512ED0();
int nox_xxx_scriptCreate_512F10();
int nox_xxx_scriptDamage_512F80();
void __cdecl sub_512FE0(int a1, int* a2);
int nox_xxx_scriptGroupDamage_513010();
int sub_513070();
_DWORD* __cdecl sub_5130E0(int a1, _DWORD* a2);
int sub_513160();
int nox_xxx_script_AwardSpell_5131C0();
int nox_xxx_script_AwardSpellGroup_513230();
int __cdecl sub_513280(int a1, int* a2);
int nox_xxx_scriptEnchant_5132E0();
__int16 __cdecl nox_xxx_enchantUnit_513390(int a1, int* a2);
int nox_xxx_scriptGroupEnchant_5133B0();
int nox_xxx_scriptGetHost_513460();
int nox_xxx_script_ObjectGet_513490();
int nox_xxx_script_GetObjectX_513530();
int nox_xxx_script_GetWaypointX_513570();
int nox_xxx_script_GetObjectY_5135B0();
int nox_xxx_scriptWaypointY_5135F0();
int nox_xxx_script_UnitHeight_513630();
int nox_xxx_script_GetUnitLook_513670();
int nox_xxx_scriptMoveObject_5136A0();
int nox_xxx_script_MoveWaypoint_513700();
int nox_xxx_script_Raise_513750();
int nox_xxx_scriptFaceAngle_513780();
int nox_xxx_scriptPushObject_5137D0();
int nox_xxx_scriptPushObjectTo_513820();
int nox_xxx_scriptGetFirstInvItem_5138B0();
int nox_xxx_scriptGetNextInvItem_5138E0();
int nox_xxx_scriptHasItem_513910();
int nox_xxx_scriptGetInvHolder_513960();
int nox_xxx_script_Pickup_5139A0();
void __cdecl nox_xxx_playerCanCarryItem_513B00(int a1, int a2);
int nox_xxx_scriptDrop_513C10();
int sub_513C60();
int nox_xxx_script_TestBuffs_513C70();
int nox_xxx_scriptCancelBuff_513D00();
int nox_xxx_scriptGetCurrentHP_513D70();
int nox_xxx_scriptGetMaxHP_513DB0();
int nox_xxx_scriptRestoreHP_513DF0();
int nox_xxx_scriptGetDistance_513E20();
int nox_xxx_scriptCanInteract_513E80();
int nox_xxx_script_Fn58_513F10();
int nox_xxx_script_Fn59_513F20();
int nox_xxx_script_Fn5A_513F30();
int nox_xxx_script_Fn5B_513F40();
int nox_xxx_script_Fn5C_513F50();
int nox_xxx_script_Fn5D_513F60();
int nox_xxx_script_GetHostInfo_513FA0();
int nox_xxx_scriptFaceObject_514050();
int nox_xxx_script_Walk_5140B0();
int* __cdecl nox_xxx_monsterWalkTo_514110(int a1, int a2, int a3);
int nox_xxx_script_GroupWalk_514170();
int nox_xxx_scriptCancelTimer_5141F0();
int nox_xxx_script_Effect_514210();
int nox_xxx_scriptSetOwner_514490();
int nox_xxx_scriptGroupSetOwner_5144C0();
int nox_xxx_scriptSetOwnerGroup_514510();
int sub_514570();
int sub_5145F0();
int sub_514630();
int sub_5146B0();
int sub_514730();
int nox_xxx_script_ClearOwner_5147E0();
int nox_xxx_script_Waypoint_514800();
int nox_xxx_scriptGetWaypointGroup_5148A0();
int nox_xxx_scriptGetObjectGroup_514940();
int nox_xxx_scriptGetWallGroup_5149E0();
int nox_xxx_scriptPop2_74_514BA0();
int nox_xxx_scriptRemoveChat_514BB0();
int nox_xxx_scriptNoChatAll_514BD0();
int nox_xxx_scriptJournalQuest_514BE0();
int nox_xxx_scriptJournalQuestBool_514C10();
int nox_xxx_scriptJournalGetQuest_514C40();
int nox_xxx_scriptJournalGetQuestBool_514C60();
int nox_xxx_scriptRestoreQuestStatus_514C90();
int nox_xxx_scriptObjIsTrigger_514CB0();
int sub_514CF0();
int nox_xxx_scriptGetTrigger_514D30();
int nox_xxx_scriptGetCaller_514D60();
int nox_xxx_scriptSetDialog_514D90();
int nox_xxx_scriptCancelDialog_514DF0();
int nox_xxx_scriptDialogPortrait_514E30();
int nox_xxx_scriptTellStory_514E90();
int nox_xxx_scriptForceDialog_514ED0();
int nox_xxx_script_514F10();
int sub_514FC0();
int nox_xxx_script_515060();
int nox_xxx_cast_515130();
int sub_515200();
int sub_515220();
int nox_xxx_scriptWideScreen_515240();
int nox_xxx_scriptGetElevatorStat_5154A0();
int sub_5154E0();
int nox_xxx_scriptDeleteJournalEntry_515550();
int sub_5155A0();
int nox_xxx_scriptSetGuardPoint_515600();
void __cdecl nox_xxx_monsterGoPatrol_515680(int a1, int a2);
int sub_515700();
int nox_xxx_scriptMonsterHunt_515780();
void __cdecl sub_5157A0(int a1);
int sub_5157D0();
int sub_515800();
void __cdecl sub_515820(int a1);
int sub_515850();
int nox_xxx_script_Follow_515880();
void __cdecl nox_xxx_unitSetFollow_5158C0(int a1, int a2);
int sub_515910();
int nox_xxx_scriptAgresssionLevel_515950();
int __cdecl nox_xxx_monsterSetAgressiveness_515980(int a1, _DWORD* a2);
int sub_5159B0();
int nox_xxx_scriptHitLocation_5159E0();
void __cdecl nox_xxx_monsterActionMelee_515A30(int a1, float2* a2);
int sub_515AE0();
int nox_xxx_scriptShootLocation_515B30();
void __cdecl nox_xxx_monsterMissileAttack_515B80(int a1, _DWORD* a2);
int sub_515BF0();
int __fastcall nox_xxx_scriptMonsterWayFlag_515C40(int a1);
int __cdecl sub_515C80(int a1, _BYTE* a2);
int sub_515CB0();
int nox_xxx_scriptAttack_515CF0();
void __cdecl nox_xxx_mobSetFightTarg_515D30(int a1, int a2);
int sub_515DB0();
int nox_xxx_scriptSetIdleLevel_515DF0();
int __cdecl sub_515E20(int a1, _DWORD* a2);
int nox_xxx_scriptGroupSetIdleHP_515E50();
int nox_xxx_scriptSetResumeLevel_515E80();
int __cdecl sub_515EB0(int a1, _DWORD* a2);
int sub_515EE0();
int nox_xxx_scriptRunAway_515F10();
void __cdecl nox_server_scriptFleeFrom_515F70(int a1, _DWORD* a2);
int nox_server_scriptMonsterGroupFleeFrom_516000();
int nox_xxx_scriptMonsterWait_516060();
void __cdecl sub_516090(int a1, _DWORD* a2);
int sub_5160F0();
int sub_516120();
int sub_516160();
int nox_xxx_scriptGiveExp_516190();
int sub_5161C0();
int nox_xxx_script_GetObjectType_516210();
int sub_5162D0();
int nox_xxx_scriptDialogResult_5163C0();
int nox_xxx_scriptForceAutosave_516400();
int nox_xxx_script_Music_516430();
int __thiscall sub_5164A0(void* this);
int __thiscall sub_5164E0(void* this);
int __thiscall nox_xxx_script_ClearMusic_516520(void* this);
int __thiscall sub_516570(_BYTE* this);
int sub_5165D0();
int sub_516600();
int sub_516680();
int sub_5166A0();
int nox_xxx_scriptPlayerIsTrading_5166E0();
int sub_516720();
int sub_516760();
int __thiscall sub_516790(void* this);
int nox_xxx_scriptBecomePet_5167D0();
int nox_xxx_scriptBecomeEnemy_516810();
int __thiscall sub_516850(void* this);
int nox_xxx_scriptOblivionGive_516890();
int nox_xxx_script_Frozen_516920();
int sub_516960();
int nox_xxx_scriptSetProperty_516970();
int nox_xxx_scriptDecayObject_516A50();
void __cdecl sub_516A80(int a1, int a2, int a3, int a4);
int nox_xxx_scriptTrapSpells_516B40();
int sub_516BC0();
int nox_xxx_scriptSetShopkeeperGreet_516BE0();
int sub_516C10();
int nox_xxx_scriptStartGameMb_516C20();
int nox_xxx_scriptTestMobStatus_516C30();
int sub_516C70();
int __cdecl nox_xxx_zombieSetStayDead_516C90(int a1);
int sub_516CB0();
int sub_516CE0();
unsigned int __cdecl sub_516D00(int a1);
int sub_516D40();
int nox_xxx_scriptObjIsGameball_516D70();
int nox_xxx_scriptObjIsCrown_516DC0();
int nox_xxx_scriptEndGame_516E10();
int nox_xxx_scriptPlayerScore_516E30();
int sub_516EA0();
BOOL nox_xxx_allocPendingOwnsArray_516EE0();
int sub_516F10();
void sub_516F30();
int __cdecl nox_xxx_unused_516F50(int a1, int a2);
_DWORD* __cdecl sub_516F90(int a1, int a2);
void sub_516FC0();
int nox_xxx_loadMonsterBin_517010();
int __cdecl nox_xxx_readStr_517090(FILE* a1, _BYTE* a2);
int __cdecl sub_517140(FILE* a1);
int __cdecl nox_xxx_servParseMonsterDef_517170(FILE* a1, const char* a2);
_DWORD* nox_xxx_monsterListFree_5174F0();
int nox_xxx_monsterList_517520();
_DWORD* __cdecl nox_xxx_monsterDefByTT_517560(int a1);
BOOL __cdecl sub_517590(float a1, float a2);
int __cdecl nox_xxx_roundCoord_5175E0(float, int); // idb
__int16 __cdecl nox_xxx_unitCreateMissileSmth_517640(signed int a1);
int __cdecl nox_xxx_addObjToMapMB_517780(int a1, int a2, int a3, int a4);
__int16 __cdecl sub_517870(int a1);
unsigned __int16* __cdecl sub_5178E0(int a1, unsigned __int16* a2);
void __cdecl nox_xxx_moveUpdateSpecial_517970(int a1);
void __cdecl nox_xxx_waypointMapRegister_5179B0(int a1);
int __cdecl sub_517A70(int a1);
int sub_517AE0();
void sub_517B30();
void __cdecl sub_517B70(float2* a1, void(__cdecl* a2)(_DWORD, int), int a3); // idb
void __cdecl nox_xxx_getUnitsInRect_517C10(float4* a1, void(__cdecl* a2)(float*, int), int a3); // idb
void __cdecl nox_xxx_getUnitsInRectAdvImpl_517DC0(float4* a1, void(__cdecl* a2)(int, int), int a3);    // idb
void __cdecl nox_xxx_getUnitsInRectAdv_517ED0(float4* a1, void(__cdecl* a2)(float*, int), int a3); // idb
int* __cdecl nox_xxx_secretWallCheckUnits_517F00(float* a1, int(__cdecl* a2)(int*, int), int a3);
void __cdecl nox_xxx_unitsGetInCircle_517F90(float2* a1, float a2, int a3, int a4); // idb
void __cdecl nox_xxx_unitsGetInNotFarFn_518000(float*, int);                          // idb
int __cdecl sub_518040(int, float, int, int);                  // idb
void __cdecl sub_5180B0(int a1, int a2);
void __cdecl sub_518170(int a1, float a2, int a3, int a4);
void __cdecl nox_xxx_unused_5182D0(float* a1, float a2, void(__cdecl* a3)(int, int), int a4);
int __cdecl sub_518440(int a1, unsigned __int8 a2);
int __cdecl sub_518460(float2* a1, unsigned __int8 a2, int a3); // idb
int __cdecl sub_518550(int* a1, int* a2, unsigned __int8 a3, int a4);
int __cdecl sub_518740(float2* a1, unsigned __int8 a2); // idb
void sub_518760();
int sub_518770();
int __cdecl nox_xxx_netSendPhantomPlrMb_5187E0(int a1, int a2);
int __cdecl nox_xxx_netSendSimpleObj_5188A0(int a1, int a2);
int __cdecl nox_xxx_netSendComplexObject_518960(int a1, _DWORD* a2, int a3);
int __cdecl nox_xxx_netSpriteUpdate_518AE0(int a1, int a2, _DWORD* a3);
int __cdecl nox_xxx_netPlayerObjSend_518C30(int a1, _DWORD* a2, int a3, signed int a4);
int __cdecl nox_xxx_netUpdate_518EE0(_DWORD* a1);
int __cdecl nox_xxx_netPlayerObjSendCamera_519330(int a1);
char __cdecl nox_xxx_unitAroundPlayerFn_5193B0(_DWORD* a1, int a2);
char __cdecl nox_xxx_netSendObjects2Plr_519410(int a1, int a2);
int __cdecl sub_519660(int a1, int a2);
int __cdecl sub_519710(int a1);
void __cdecl sub_519760(int a1, float* a2);
void sub_519820();
int __cdecl nox_xxx_netMapSendClear_519830(int a1, char a2);
int nox_xxx_netMapSendStop_519870();
void __cdecl nox_xxx_mapSendCancelAll_5198B0(unsigned __int8 a1);
void __cdecl nullsub_27(_DWORD); // weak
int __cdecl sub_519930(int a1);
char* __cdecl nox_xxx_netMapSend_519D20(int a1);
_DWORD* __cdecl sub_519E80(int a1);
LPVOID sub_51A100();
int sub_51A130();
BOOL __cdecl sub_51A140(int a1);
int __cdecl sub_51A170(const char* a1);
char* __cdecl sub_51A1D0(int a1);
int __cdecl sub_51A500(int a1);
CHAR* sub_51A550();
void __cdecl nox_xxx_spawnHecubahQuest_51A5A0(int* a1);
void __cdecl nox_xxx_spawnNecroQuest_51A7A0(int* a1);
int nox_xxx_getQuestStage_51A930();
int __cdecl sub_51A940(int a1);
int sub_51A950();
int __cdecl nox_xxx_playerSaveInput_51A960(int a1, unsigned __int8* a2);
int __cdecl sub_51AA20(int a1);
int __cdecl sub_51AAA0(int a1, int a2, int a3);
unsigned __int8* __cdecl nox_xxx_playerControlBufferFirst_51AB50(int a1);
unsigned __int8* __cdecl nox_xxx_playerGetControlBufferNext_51ABC0(int a1);
int __cdecl nox_xxx_playerCmd_51AC30(int a1);
BOOL __cdecl nox_xxx_playerCmdGet_51AC40(int a1);
void sub_51AC60();
int __cdecl nox_xxx_script_51ACA0(int a1, int a2, int a3);
int nox_xxx_getTimerHandle_51AD20();
void* nox_xxx_scriptActivatorNew_51AD40();
int __cdecl sub_51AD60(int a1);
int __cdecl nox_xxx_scriptAct_51AD90(int a1);
_DWORD* __cdecl sub_51AE60(int a1);
int sub_51AEA0();
int sub_51AF80();
void sub_51B0C0();
int __cdecl sub_51B810(int a1);
char __cdecl sub_51B860(int a1);
void __cdecl nox_xxx_updateFallLogic_51B870(int a1);
void sub_51BAC0();
void sub_51D0D0();
void sub_51D0E0();
int __cdecl sub_51D0F0(char a1);
int __cdecl sub_51D100(int a1);
_DWORD* __cdecl sub_51D120(float* a1);
float* __cdecl sub_51D1A0(float2* a1); // idb
int __cdecl sub_51D1E0(LPVOID lpMem);
float* __cdecl sub_51D270(float* a1);
int __cdecl sub_51D2C0(int a1, int a2);
int __cdecl sub_51D2E0(int a1, int a2);
int __cdecl sub_51D300(int a1, int a2, char a3);
int __cdecl sub_51D370(int a1, int a2, char a3);
float2* __cdecl sub_51D3F0(float2* a1, float2* a2); // idb
float* __cdecl sub_51D440(float* a1, float* a2);
int __cdecl sub_51D490(char* a1);
int __cdecl nox_xxx_tileGetDefByName_51D4D0(char* a1);
int __cdecl nox_xxx_tileCheckImage_51D540(int a1);
int __cdecl nox_xxx_tileCheckImageVari_51D570(int a1);
int __cdecl nox_xxx_tile_51D5C0(int a1);
float* __cdecl sub_51D5E0(float* a1);
float* __cdecl nox_xxx_unused_51D650(float* a1);
int __cdecl nox_xxx_unused_51D6A0(int a1);
int __cdecl nox_xxx_unused_51D740(float* a1, int a2);
int __cdecl sub_51D8B0(int a1);
int __cdecl sub_51D8D0(int a1);
int __cdecl sub_51D8F0(float2* a1); // idb
int __cdecl sub_51D9C0(int a1, int a2, int a3, int a4, int a5);
int __cdecl sub_51DA70(int a1, int a2, int a3, int a4, int a5);
void __cdecl sub_51DD50(int a1, int a2, int a3, int a4);
int __cdecl sub_51DE30(_DWORD* a1, _DWORD* a2, _DWORD* a3);
void sub_51DE90();
int __cdecl nox_xxx_mapCountWallsMB_51DEA0(int a1);
int sub_51DED0();
int __cdecl nox_xxx_xfer_saveObj_51DF90(int a1);
int __cdecl nox_xxx_mapSaveMap_51E010(char* a1, int a2);
int nox_xxx_map_51E140();
void sub_51E1C0();
int __cdecl nox_xxx_mapGenSpellIdByName_51E1D0(const char* a1);
int __cdecl nox_xxx_mapGenReadTheme_51E260(int* a1, int a2);
int __cdecl nox_xxx_mapGenReadLine_51E540(FILE* a1, _BYTE* a2);
int __cdecl sub_51E570(FILE* a1, _BYTE* a2);
int __cdecl sub_51E630(FILE* a1);
int __cdecl sub_51E670(FILE* a1);
int __cdecl sub_51E720(FILE* a1);
int __cdecl sub_51E780(FILE* a1);
int __cdecl sub_51E800(int a1, _DWORD* a2);
int __cdecl sub_51EAF0(int a1, _DWORD* a2);
int __cdecl nox_xxx_genReadAlgData_51EBB0(int a1, FILE* a2);
int __cdecl nox_xxx_genReadSpellSet_51EFB0(int a1, FILE* a2);
int __cdecl nox_xxx_genReadWeaponSet_51F030(int a1, FILE* a2);
void __cdecl nox_xxx_mapGenFreeStr_51F1F0(LPVOID lpMem); // idb
int __cdecl sub_51F230(int a1, FILE* a2);
int __cdecl nox_xxx_genReadArmorSet_51F640(int a1, FILE* a2);
int __cdecl nox_xxx_genReadExit_51F800(int a1, FILE* a2);
int __cdecl nox_xxx_genReadDecor_51F9F0(_DWORD* a1, FILE* a2);
char* __cdecl nox_xxx_genDecorReadWallFloor_51FE00(int a1, FILE* a2);
int __cdecl sub_51FEC0(int a1, int a2, FILE* a3);
int __cdecl nox_xxx_genDecorReadDecorSet_51FFA0(int a1, FILE* a2);
_DWORD* __cdecl nox_xxx_gen_520380(FILE* a1);
_DWORD* __cdecl nox_xxx_gen_5205B0(FILE* a1);
int __cdecl nox_xxx_genDecorReadCopy_520660(_DWORD* a1, const char* a2, FILE* a3);
int __cdecl nox_xxx_genDecorReadOccurConstraint_520810(int a1, FILE* a2);
int __cdecl nox_xxx_genDecorReadOccurLimit_5208D0(int a1, FILE* a2);
int __cdecl nox_xxx_genDecorReadFrequency_520910(int a1, FILE* a2);
int __cdecl nox_xxx_genDecorReadRoomSizeCon_5209F0(int a1, FILE* a2);
BOOL __cdecl nox_xxx_genDecorReadDoor_520A90(int a1, FILE* a2);
BOOL __cdecl nox_xxx_genDecorReadDoubleDoor_520AB0(int a1, FILE* a2);
BOOL __cdecl nox_xxx_mapgenCheckSettings_520AD0(int* a1);
int __cdecl nox_xxx_genReadPrefab_520BF0(int a1, FILE* a2);
char* __cdecl sub_520CE0(int a1, FILE* a2);
_DWORD* __cdecl sub_520D50(_DWORD* a1);
__int64 __cdecl nox_xxx_mapGenRoundFloatToPtr_520DF0(float2* a1, _DWORD* a2); // idb
int __cdecl sub_520E60(int2* a1);                   // idb
int __cdecl sub_520EA0(int a1);
void sub_520F80();
void sub_520FC0();
int __cdecl sub_520FD0(_DWORD* a1);
BOOL __cdecl sub_520FF0(_DWORD* a1);
BOOL __cdecl sub_521020(_DWORD* a1);
int __cdecl sub_521050(_DWORD* a1, __int16 a2);
int __cdecl sub_521080(_DWORD* a1, int a2);
int __cdecl sub_521100(int a1);
int __cdecl sub_521180(int a1);
int __cdecl sub_521200(int a1);
int __cdecl sub_521290(int2* a1); // idb
int __cdecl sub_5212B0(int a1, _DWORD* a2);
BOOL nox_xxx_mapgenAllocBuffer_5213E0();
void nox_xxx_mapgenFreeBuffer_521400();
LPVOID __cdecl sub_521410(int* a1, _DWORD* a2, int a3, int* a4);
int __cdecl sub_521500(_DWORD* a1, _DWORD* a2, int a3);
_DWORD* __cdecl sub_521600(_DWORD* a1, int* a2);
LPVOID nox_xxx_mapGenGetTopRoom_521710();
int __cdecl sub_521720(int a1);
int __cdecl nox_xxx_mapGenAddNewRoom_521730(_DWORD* a1);
int __cdecl sub_521760(int a1);
int __cdecl sub_5217A0(int a1, int a2);
int __cdecl sub_521820(int a1, int a2);
int __cdecl nox_xxx_mapGenUpdateRoomRect_521850(int a1);
int __cdecl nox_xxx_mapGenSetRoomPos_521880(_DWORD* a1, float2* a2); // idb
int __cdecl sub_5218B0(int a1, int a2);
int __cdecl sub_521900(int a1, int a2, int a3);
float* __cdecl nox_xxx_mapGenMakeRoomStruct_521940(int a1, int a2);
float* __cdecl nox_xxx_mapGenPrepareRoom_521990(int a1);
void __cdecl sub_521A10(LPVOID lpMem); // idb
_DWORD* nox_xxx_mapGenFreeTopRoom_521A40();
int __cdecl sub_521A70(int a1, int a2, int a3);
int __cdecl sub_521AA0(_DWORD* a1, int a2);
double __cdecl sub_521B00(int a1, int a2);
double __cdecl sub_521B30(int a1, int a2);
double __cdecl sub_521B60(int a1, int a2);
double __cdecl sub_521B90(int a1, int a2);
float* __cdecl sub_521BC0(int a1, float2* a2, float a3, float a4); // idb
_DWORD* __cdecl sub_521C10(int a1);
int __cdecl nox_xxx_mapgen_521C60(int a1, int a2);
int __cdecl sub_521CB0(int a1, int a2, int a3, int a4);
BOOL __cdecl sub_521EB0(float* a1, float* a2);
int __cdecl sub_521F10(int a1, float* a2);
void __cdecl nox_xxx_mapgen_521FE0(int a1, int a2, _DWORD* a3);
_DWORD* __cdecl nox_xxx_mapGenMakeSpellbook_5220E0(int a1, const char* a2);
_DWORD* __cdecl nox_xxx_mapGenMakeEnchantedItem_5221A0(char* a1, char* a2, int a3);
_DWORD* __cdecl sub_522300(int a1, _DWORD* a2);
void __cdecl nox_xxx_mapgen_522340(int a1, int a2);
void __cdecl nox_xxx_mapgen_522370(int a1, int a2, int* a3);
float* __cdecl nox_xxx_mapgen_5224B0(int a1, int a2, int a3);
int __cdecl sub_5226D0(int, float, int); // idb
int __cdecl sub_5227B0(int a1, float* a2);
int __cdecl nox_xxx_mapGenMakeMonsterInRoom_522810(float* a1, char* a2);
float* __cdecl sub_522C80(float* a1);
char __cdecl sub_522CA0(int a1, float* a2);
float* __cdecl sub_522D30(int a1);
unsigned __int8* __cdecl nox_xxx_mapGenTryNextRoom_522F40(_DWORD* a1);
int __cdecl nox_xxx_netSendPointFx_522FF0(char a1, float2* a2);                // idb
int __cdecl nox_xxx_netSendFxAllCli_523030(float2* a1, const void* a2, int a3); // idb
int __cdecl sub_523150(char a1, char a2, float* a3);
int __cdecl nox_xxx_netSparkExplosionFx_5231B0(float* a1, char a2);
void nox_xxx_sendGeneratorBreakFX_523200(float* a1, char a2);
int __cdecl nox_xxx_sendDeathRayCast_523250(int a1);
int __cdecl nox_xxx_netSendVampFx_523270(char a1, __int16* a2, __int16 a3);
int __cdecl nox_xxx_netSendRayFx_5232F0(char a1, int a2);
int __cdecl nox_xxx_servCode_523340(int* a1, const void* a2, int a3);
int __cdecl nox_xxx_netClientPredictLinear_523530(int a1);
int __cdecl nox_xxx_mapGenSetFlags_5235F0(char a1);
int __cdecl nox_xxx_netSendShieldFx_523670(int a1, float* a2);
int __cdecl nox_xxx_sendSummonStartFX_5236F0(__int16 a1, float* a2, char a3, __int16 a4, __int16 a5);
int __cdecl nox_xxx_sendSummonCancelFX_523760(__int16 a1);
int __cdecl nox_xxx_netSendFxGreenBolt_523790(int4* a1, __int16 a2); // idb
void nox_xxx_sendGeneratorSpawnFX_523830(int4* a1, __int16 a2);        // idb
void nox_xxx_sendArrowTrapFX_5238A0(float* a1, char a2);
BOOL __cdecl nox_xxx_mapGenCheckRoomType_5238F0(int* a1);
int __cdecl sub_523920(int a1);
int __cdecl sub_523960(int a1);
int __cdecl sub_523970(int a1);
int __cdecl sub_5239B0(int a1);
int __cdecl sub_5239D0(int a1);
int nox_xxx___RTC_NumErrors_0_5239EF();
int nox_xxx___RTC_NumErrors_1_5239F5();
BOOL __cdecl sub_523A10(int a1, float* a2);
int __cdecl sub_523C30(int a1, int a2);
int __cdecl sub_523CB0(int a1, int a2);
float* __cdecl sub_523D30(float* a1, float* a2);
float* __cdecl sub_523E30(int a1, int a2, int a3);
float* __cdecl nox_xxx_mapGenMakeHall_523EC0(int a1, int a2, int a3);
int __cdecl sub_523EF0(float* a1, int a2, int* a3, _DWORD* a4, int a5);
float* __cdecl sub_523F70(int a1, int* a2, float* a3, int a4);
int __cdecl sub_524070(int a1, int a2);
int __cdecl sub_524090(int a1, int* a2);
BOOL __cdecl nox_xxx_mapGenDecorChkConstaint_5241C0(int a1, int a2);
BOOL __cdecl nox_xxx_mapGenChkDecorFillsRoom_5241F0(int a1, int a2);
char __cdecl nox_xxx_mapGenDecorChkLimit_524220(int* a1, int a2);
void __cdecl nullsub_28(_DWORD); // weak
int __cdecl nox_xxx_mapGenMakeRooms_524310(int a1);
int __cdecl sub_5244D0(int a1);
int __cdecl sub_524500(float2* a1, int a2);
int __cdecl sub_524550(int* a1, int a2);
float* __cdecl sub_5245A0(int a1, float* a2, int a3, int a4);
float* __cdecl sub_524610(int a1, float* a2, int a3);
BOOL __cdecl sub_524660(float a1, float a2);
int __cdecl nox_xxx_gen_524680(int a1, int a2, int a3);
int __cdecl sub_524950(int a1, int a2, float* a3, int* a4);
int __cdecl sub_5249C0(int a1, int a2, float* a3, int* a4);
void __cdecl sub_524B50(int a1, int a2, float* a3, int* a4);
void __cdecl nox_xxx_gen_524E00(int a1, int a2);
int __cdecl sub_524FB0(int a1, int a2, int a3);
float2* __cdecl sub_525330(float2* a1, int a2);
float2* __cdecl sub_525370(float2* a1, int a2);
int __cdecl sub_5253B0(float* a1);
void __cdecl nox_xxx_mapgen_525510(int a1, int a2);
void __cdecl nox_xxx_mapgen_525570(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_mapgen_525690(int a1, float2* a2, int a3);
int __cdecl nox_xxx_mapgen_525740(int a1, float2* a2, int a3);
int __cdecl nox_xxx_mapgen_525830(int a1, float2* a2, int a3);
int __cdecl nox_xxx_mapgen_5258E0(int a1, float2* a2, int a3);
void __cdecl sub_5259F0(int a1, int a2, float a3);
float* __cdecl sub_525AF0(int a1);
void __cdecl sub_525BF0(int a1);
float* sub_525C90();
int __cdecl nox_xxx_mapGen_InPrefab1_525D20(int a1);
void __cdecl sub_525DF0(float a1);
int __cdecl nox_xxx_mapGenPrefabMkRoom_526100(int a1, int a2);
__int64 __cdecl sub_526260(int a1, float* a2);
int __cdecl sub_5262F0(int a1, int a2);
int __cdecl sub_526550(int a1, int a2);
int __cdecl nox_xxx_mapGen_InPrefab2_5266F0(int a1);
int __cdecl nox_xxx_mapGenPlacePrefabs_526830(int a1);
int __cdecl sub_5268F0(const char* a1);
char* sub_526950();
char* __cdecl sub_526AA0(int a1);
void __cdecl nox_xxx_mapGenSetRngSeed_526AB0(unsigned int a1);
signed int __cdecl nox_xxx_mapGenRandFunc_526AC0(int a1, signed int a2);
int __cdecl nox_xxx_mapGenRandFunc2_526B00(int a1, signed int a2);
int __cdecl sub_526B60(int a1, signed int a2);
double __cdecl sub_526BC0(float a1, float a2);
int __cdecl sub_526BF0(char* a1);
int __cdecl sub_526C40(int a1);
int __cdecl sub_526C60(int a1);
int __cdecl sub_526C80(int a1);
int __cdecl sub_526CA0(char* a1);
int __cdecl sub_526D20(int a1);
int __cdecl sub_526D50(int a1);
int __cdecl sub_526D80(int a1);
int __cdecl sub_526DD0(float2* a1, int* a2);
int __cdecl sub_526E60(float* a1);
int __cdecl sub_527030(float2* a1); // idb
int __cdecl sub_5270E0(int* a1);
int __cdecl sub_527150(int* a1, char a2);
int* __cdecl sub_5271D0(int* a1);
int* __cdecl sub_527210(int* a1);
int __cdecl sub_5272B0(int a1);
int __cdecl sub_5272F0(int a1);
int __cdecl sub_527330(int a1);
int __cdecl sub_527350(float* a1);
int __cdecl sub_527380(float* a1);
int __cdecl sub_527450(_DWORD* a1);
int* __cdecl sub_5277D0(int* a1, int a2);
int* __cdecl sub_527840(int* a1, int a2);
int* __cdecl sub_5278B0(int* a1, int a2);
int __cdecl sub_527920(CHAR* a1);
int __cdecl nox_xxx_mapGenGetObjID_527940(char* a1);
int __cdecl sub_527980(int a1);
float* __cdecl nox_xxx_mapGenPlaceObj_5279B0(float2* a1);
float* __cdecl nox_xxx_mapGenMoveObject_527A10(float* a1, float2* a2);
int __cdecl sub_527B00(float2* a1);
int __cdecl sub_527B70(int a1, int a2, int a3);
int __cdecl sub_527BE0(_DWORD* a1);
int __cdecl sub_527C10(float2* a1);
int __cdecl nox_xxx_mapGenOrientObj_527C60(int a1, int a2);
int __cdecl nox_xxx_mapGenFinishSpellbook_527DB0(int a1, char a2);
int* __cdecl sub_527E00(int a1);
BOOL __cdecl sub_527E20(int a1, unsigned int a2, int a3);
int __cdecl nox_xxx_netUpdateObjectSpecial_527E50(int a1, _DWORD* a2);
__int16 __cdecl sub_528030(int a1);
int __cdecl nox_xxx_checkIsKillable_528190(int a1);
int nox_xxx_frameCounterSetCopyToNextFrame_5281D0();
int nox_xxx_frameCounterSetCopy_5281E0();
void __cdecl nox_xxx_unitUpdateSightMB_5281F0(float a1);
int __cdecl nox_xxx_aiLostSight_528560(int a1, int a2);
void __cdecl sub_528610(int a1);
void __cdecl nox_xxx_monsterUpdateSeenEnemies_5286D0(int a1, int a2);
void __cdecl nox_xxx_monsterVisionSeeEnemy_5287B0(int a1, int a2);
int __cdecl sub_528910(int a1, int a2);
int __cdecl sub_528950(int a1, int a2);
int __cdecl sub_528990(int a1);
char* __cdecl nox_xxx_netReportDestroyObject_5289D0(int a1);
int __cdecl nox_xxx_netObjectOutOfSight_528A60(int a1, _DWORD* a2);
int __cdecl nox_xxx_netObjectInShadows_528A90(int a1, _DWORD* a2);
int __cdecl nox_xxx_monsterCmdSend_528BD0(int a1, int a2, const char* a3, __int16 a4);
int __cdecl nox_xxx_netKillChat_528D00(_DWORD* a1);
int nox_xxx_destroyEveryChatMB_528D60();
int __cdecl nox_xxx_XFerMonster_528DB0(int a1);
void __cdecl nox_xxx_monsterOnSpawnSpellcaster_529BC0(int a1);
int __cdecl nox_xxx_XFer_ActionData_529CE0(int a1);
int __cdecl nox_xxx_AssignIfGreater_52A420(int* a1, int a2);
int __cdecl sub_52A440(int a1, int a2, int a3);
size_t __cdecl sub_52A5F0(int a1);
int __cdecl nox_xxx_XFer_ReadShopItem_52A840(int a1, int a2);
int __cdecl nox_xxx_XFer_ReadMonsterBuffs_52AAB0(_DWORD* a1);
size_t __cdecl nox_xxx_readNPCVoiceSet_52AD10(int a1);
int __cdecl nox_xxx_XFerNPC_52ADE0(int a1);
int __cdecl sub_52BA70(int a1);
int __cdecl sub_52BAF0(int a1);
int sub_52BBA0();
int __cdecl nox_xxx_castCounterSpell_52BBB0(int a1, int a2, int a3, int a4);
void __cdecl nox_xxx_cspellRemoveSpell_52BC90(int a1);
int __cdecl nox_xxx_deathBallCreateFragments_52BD30(int* a1);
int __cdecl sub_52BDB0(int a1, int a2);
void __cdecl nox_xxx_changeOwner_52BE40(int a1, int a2);
int __cdecl sub_52BEB0(int a1, int a2, int a3, int a4);
int sub_52BF00();
int sub_52BF10();
int __cdecl nox_xxx_castSpellWinkORrestoreHealth_52BF20(int a1, int a2, int a3, int a4, int* a5);
int __cdecl sub_52BF50(int a1, int a2, int a3, int a4, int* a5);
int __cdecl nox_xxx_castPull_52BFA0(int a1, int a2, int a3, int a4, int a5, int a6);
int __cdecl nox_xxx_castPush_52C000(int a1, int a2, int a3, int a4, int a5, int a6);
int __cdecl nox_xxx_castFumble_52C060(int a1, int a2, int a3, int a4, int* a5);
int __cdecl nox_xxx_castSlow_52C190(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl nox_xxx_castConfuse_52C1E0(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl sub_52C230(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl sub_52C270(int a1, int a2, int a3, int a4, int* a5, __int16 a6);
int __cdecl nox_xxx_castStun_52C2C0(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl nox_xxx_castFreeze_52C350(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl nox_xxx_castAnchor_52C390(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl nox_xxx_castBurn_52C3E0(int a1, int a2, int a3, int a4, int a5);
int __cdecl nox_xxx_castVamp_52C530(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl nox_xxx_useShock_52C5A0(int a1, int a2, int a3, int a4, int* a5, int a6);
int __cdecl nox_xxx_castHaste_52C640(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl nox_xxx_castRun_52C680(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl nox_xxx_castLight_52C6D0(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl nox_xxx_castPoison_52C720(int a1, int a2, int a3, int a4, int* a5, int a6);
int __cdecl sub_52C750(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl nox_xxx_castFireball_52C790(int a1, int a2, int a3, int a4, int a5, int a6);
int __cdecl sub_52C8D0(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl sub_52C910(int a1);
int __cdecl nox_xxx_castDeath_52C920(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl nox_xxx_castInvis_52C980(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl nox_xxx_castInvul_52C9E0(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl nox_xxx_castInfra_52CA30(int a1, int a2, int a3, int a4, int* a5, char a6);
int sub_52CA70();
int __cdecl sub_52CA80(int a1, int a2, int a3, int a4);
int __cdecl sub_52CBD0(int a1, int a2, int a3, int a4);
int __cdecl sub_52CCD0(int a1, int a2, int a3);
int __cdecl nox_xxx_castCurePoison_52CDB0(int a1, int a2, int a3, int a4, int* a5, int a6);
void __cdecl sub_52CE60(int a1);
int __cdecl nox_xxx_castLock_52CE90(int a1, int a2, int a3, int a4);
void __cdecl sub_52CF90(int a1, int a2);
void __cdecl sub_52D060(int a1, int a2);
int __cdecl nox_xxx_castProtectFire_52D110(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl nox_xxx_castProtectElect_52D150(int a1, int a2, int a3, int a4, int* a5, char a6);
int sub_52D190();
int __cdecl nox_xxx_castProtectPosion_52D1A0(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl nox_xxx_castDeathRay_52D1E0(int a1, int a2, int a3, int a4, int a5);
int __cdecl nox_xxx_castTelekinesis_52D330(int a1, int a2, int a3, int a4, int* a5, char a6);
int __cdecl nox_xxx_castFist_52D3C0(int a1, int a2, int a3, int a4, int a5, int a6);
int __cdecl nox_xxx_spellCastCleansingFlame_52D5C0(int a1, int a2, int a3, int a4, int a5, int a6);
int __cdecl nox_xxx_castMeteorShower_52D8A0(int a1, int a2, int a3, int a4, int a5, int a6);
int __cdecl nox_xxx_castMeteor_52D9D0(int a1, int a2, int a3, int a4, int a5, int a6);
int __cdecl nox_xxx_castToxicCloud_52DB60(int a1, int a2, int a3, int a4, int a5);
int __cdecl nox_xxx_spellArachna_52DC80(int a1, int a2, int a3, int a4, int a5);
int __cdecl sub_52DD50(int, int, int, int, void*); // idb
int __cdecl nox_xxx_castEquake_52DE40(int a1, int a2, int a3, int a4, int a5, int a6);
__int16 __cdecl nox_xxx_equakeDamage_52DEC0(int a1, int a2);
int __cdecl nox_xxx_castFear_52DF40(int a1, int a2, int a3, int a4, int* a5, char a6);
char __cdecl nox_xxx_objectApplyForce_52DF80(int a1, int a2, float a3);
unsigned int __cdecl nox_xxx_isObjectMovable_52E020(int a1);
void __cdecl nox_xxx_mapPushUnitsAround_52E040(int a1, float a2, int a3, float a4, int a5, int a6, int a7);
void __cdecl nox_xxx_unitPushAroundFn_52E0E0(int a1, int** a2);
int __cdecl nox_xxx_spellDrainMana_52E210(float); // idb
int __cdecl sub_52E450(int a1, int a2, int a3);
int __cdecl sub_52E610(int* a1, int a2);
void __cdecl sub_52E660(int a1, int a2);
int __cdecl sub_52E7C0(int a1);
int __cdecl nox_xxx_spellEnergyBoltStop_52E820(int a1);
int __cdecl nox_xxx_spellEnergyBoltTick_52E850(float); // idb
void __cdecl sub_52EC60(int a1, int a2);
void nullsub_29(); // weak
int __cdecl nox_xxx_firewalkTick_52ED40(float* a1);
int __cdecl sub_52EF30(int a1);
int __cdecl sub_52EFD0(int a1);
int __cdecl sub_52F1D0(int a1);
BOOL __cdecl sub_52F220(int* a1);
int __cdecl sub_52F2E0(float a1);
int __cdecl sub_52F460(float); // idb
int __cdecl nox_xxx_castShield1_52F5A0(_DWORD* a1);
BOOL __cdecl sub_52F650(int a1);
int __cdecl sub_52F670(int a1);
char __cdecl nox_xxx_unitShield_52F690(int a1, int a2);
void __cdecl nox_xxx_unitShieldReduceDamage_52F710(int a1, int* a2, int a3, int a4);
int __cdecl nox_xxx_onStartLightning_52F820(int a1);
int __cdecl nox_xxx_onFrameLightning_52F8A0(float); // idb
void __cdecl sub_52FF10(int a1, int a2);
_DWORD* __cdecl nox_xxx_lightningSpellDuration_52FFD0(int a1, int a2, int a3);
void __cdecl sub_530020(int a1, int a2);
char __cdecl sub_530100(_DWORD* a1);
int __cdecl nox_xxx_spellTagCreature_530160(_DWORD* a1);
unsigned int __cdecl sub_530250(int a1);
int __cdecl sub_530270(int a1);
int __cdecl nox_xxx_spellBlink2_530310(_DWORD* a1);
int __cdecl nox_xxx_spellBlink1_530380(int* a1);
_DWORD* __cdecl nox_xxx_spellTeleportCreateWake_530560(int a1, int* a2, _DWORD* a3);
int __cdecl sub_5305D0(_DWORD* a1);
int __cdecl sub_530650(int* a1);
int __cdecl nox_xxx_castTele_530820(int a1);
int __cdecl sub_530880(int* a1);
int __cdecl nox_xxx_castTTT_530B70(int* a1);
int __cdecl sub_530CA0(int a1);
int __cdecl sub_530D30(int* a1);
int __cdecl nox_xxx_manaBomb_530F90(_DWORD* a1);
int __cdecl nox_xxx_manaBombBoom_5310C0(int* a1);
int __cdecl sub_531290(int a1);
int __cdecl nox_xxx_spellTurnUndeadCreate_531310(_DWORD* a1);
int nox_xxx_spellTurnUndeadUpdate_531410();
int __cdecl nox_xxx_spellTurnUndeadDelete_531420(int a1);
int __cdecl sub_531490(_DWORD* a1);
BOOL __cdecl sub_5314F0(int a1);
int __cdecl sub_531560(int a1);
int __cdecl nox_xxx_plasmaSmth_531580(int a1);
int __cdecl nox_xxx_plasmaShot_531600(int a1);
void __cdecl sub_531920(int a1, int a2);
int __cdecl sub_5319E0(int a1);
int __cdecl nox_xxx_spellCreateMoonglow_531A00(_DWORD* a1);
int __cdecl sub_531AF0(int a1);
int* __cdecl nox_xxx_TODOsomeCallingMeleeAttack_531B40(int a1, int a2);
int* __cdecl sub_531C10(int a1, int a2);
int* __cdecl nox_xxx_monsterAction_531C60(int a1, int a2);
int* __cdecl sub_531D50(int a1, int a2);
int __cdecl nox_xxx_mobActionFightStart_531E20(float a1);
int __cdecl sub_531E90(int a1);
char __cdecl nox_xxx_mobActionFight_531EC0(int a1);
int __cdecl sub_532040(int a1, int a2);
char __cdecl nox_xxx_monsterShieldBlockStart_532070(int a1);
char __cdecl nox_xxx_monsterShieldBlockStop_5320E0(int a1);
char __cdecl sub_532100(int a1);
char __cdecl sub_532110(int a1);
int* __cdecl nox_xxx_mobActionMelee1_532130(int a1);
void __cdecl sub_532390(int a1, int a2);
char __cdecl nox_xxx_mobActionMeleeAtt_532440(int a1);
_DWORD* __cdecl sub_532540(int a1);
char __cdecl nox_xxx_mobActionMissileAtt_532610(int a1);
char __cdecl nox_xxx_monsterPlayHurtSound_532800(int a1);
int __cdecl sub_532880(int a1);
int __cdecl nox_xxx_soundPlayerDamageSound_5328B0(int a1, int a2);
int __cdecl sub_532930(int a1, unsigned __int16 a2, unsigned __int16 a3);
int __cdecl nox_xxx_soundDefaultDamageSound_532E20(int a1, int a2);
_DWORD* __cdecl sub_532EC0(int a1, unsigned __int16 a2);
int __cdecl sub_532F70(int a1);
BOOL __cdecl sub_532FB0(__int16 a1);
int __cdecl sub_532FE0(unsigned __int16 a1, int a2);
int __cdecl sub_533010(unsigned __int16 a1, int a2);
unsigned __int8* __cdecl nox_xxx_unused_533030(int a1, int a2);
int __cdecl nox_xxx_projAddVelocitySmth_533080(int, int, float, int); // idb
BOOL __cdecl nox_xxx_unitIsEnemyTo_5330C0(int a1, int a2);
BOOL __cdecl sub_533360(int a1, int a2);
int __cdecl nox_xxx_unused_5333F0(int a1);
void __cdecl nox_xxx_fnPickEnemyAggro_533460(int a1, int a2);
int __cdecl sub_533570(int a1);
int __cdecl sub_5335D0(int, float); // idb
BOOL __cdecl nox_xxx_unused_533610(int a1, int a2);
char* __cdecl sub_533660(int a1);
long double __cdecl sub_5336D0(int a1);
void __cdecl sub_533720(int a1, int a2);
int __cdecl nox_xxx_mobActionToAnimation_533790(int a1);
void __cdecl nox_xxx_orderUnit_533900(int a1, int a2, int a3);
void __cdecl sub_5339A0(int a1, int a2, int a3);
BOOL __cdecl sub_533C80(int a1, int a2);
void __cdecl nox_xxx_mobCalcDir_533CC0(int a1, float* a2);
unsigned __int8* __cdecl nox_xxx_unitNPCActionToAnim_533D00(int a1);
int __cdecl nox_xxx_monsterTestBlockShield_533E70(int a1);
void __cdecl sub_533EB0(int a1, int a2);
int __cdecl sub_534020(int a1);
char __cdecl nox_xxx_monsterMoveAudio_534030(int a1);
BOOL __cdecl sub_534120(int a1, float2* a2); // idb
int __cdecl sub_534160(int a1);
void nox_xxx_debugPrintf_5341A0(char* a1, ...);
int __cdecl sub_5341D0(int a1);
BOOL __cdecl sub_5341F0(int a1);
BOOL __cdecl nox_xxx_monsterCanMelee_534220(int a1);
BOOL __cdecl nox_xxx_monsterCanShoot_534280(int a1);
int __cdecl nox_xxx_monsterHasShield_5342C0(int a1);
int __cdecl nox_xxx_monsterCanCast_534300(int a1);
BOOL __cdecl nox_xxx_monsterIsMoveing_534320(int a1);
BOOL __cdecl sub_534340(int a1);
BOOL __cdecl nox_xxx_monsterCanAttackAtWill_534390(int a1);
BOOL __cdecl sub_5343C0(int a1);
BOOL __cdecl sub_534400(int a1);
BOOL __cdecl sub_534440(int a1);
double __cdecl sub_534470(int a1);
char* __cdecl nox_xxx_unused_534490(float a1);
double __cdecl nox_xxx_unused_5344C0(const char* a1);
char* __cdecl nox_xxx_unused_534530(int a1);
int __cdecl nox_xxx_unused_534550(const char* a1);
char* __cdecl sub_5345B0(int a1);
int __cdecl nox_xxx_actionNByNameMB_5345F0(const char* a1);
char* __cdecl sub_534650(int a1);
int __cdecl nox_xxx_actionByName_534670(const char* a1);
int __cdecl sub_5346D0(int a1);
int __cdecl nox_xxx_monsterResetEnemy_5346F0(int a1);
BOOL __cdecl sub_534710(int a1);
int __cdecl sub_534750(int a1);
int __cdecl sub_534780(int a1);
int __cdecl sub_5347A0(int a1);
BOOL __cdecl sub_5347C0(int a1);
BOOL __cdecl nox_xxx_isNotPoisoned_5347F0(int a1);
BOOL __cdecl nox_xxx_mobGetMoveAttemptTime_534810(int a1);
BOOL __cdecl nox_xxx_unitIsMimic_534840(int a1);
int* __cdecl nox_xxx_unused_534870(int a1);
int* __cdecl nox_xxx_unused_5348A0(int a1);
char __cdecl nox_xxx_mobActionMorphToChest_5348D0(int a1);
char __cdecl nox_xxx_mobActionMorphBackToSelf_534910(int a1);
void __cdecl nox_xxx_monsterMimicCheckMorph_534950(int a1);
BOOL __cdecl nox_xxx_unitIsPlant_534A10(int a1);
BOOL __cdecl nox_xxx_unitIsZombie_534A40(int a1);
char __cdecl nox_xxx_mobActionGetUp_534A90(int a1);
unsigned int __cdecl nox_xxx_mobRaiseZombie_534AB0(int a1);
BOOL __cdecl nox_xxx_unitIsAFish_534B10(int a1);
BOOL __cdecl nox_xxx_unitIsARat_534B60(int a1);
BOOL __cdecl nox_xxx_unitIsAFrog_534B90(int a1);
int __cdecl nox_xxx_damageToMap_534BC0(int a1, int a2, int a3, int a4, int a5);
int __cdecl nox_xxx_wallPreDestroy_534DA0(int* a1);
bool __cdecl nox_xxx_mapDamageToWalls_534FC0(int4* a1, int a2, float a3, int a4, int a5, int a6); // idb
int __cdecl nox_xxx_mapTraceRay_535250(float4* a1, float2* a2, int2* a3, char a4);
void sub_535960();
int __cdecl nox_xxx_parseXP_535970(int a1, int a2, char* a3);
int __cdecl nox_xxx_parseExtent_535990(int a1, int a2, char* a3);
int __cdecl nox_xxx_parseZSize_5359B0(int a1, int a2, char* a3);
int __cdecl nox_xxx_parseWorth_535A00(int a1, int a2, char* a3);
int __cdecl nox_xxx_parseSpeed_535A20(int a1, int a2, char* a3);
int __cdecl nox_xxx_parseHealth_535A60(int a1, int a2, char* a3);
int __cdecl nox_xxx_parseFlags_535AD0(int a1, int a2, const char* a3);
int __cdecl nox_xxx_parseClass_535B00(int a1, int a2, const char* a3);
int __cdecl nox_xxx_parseSubclass_535B30(int a1, int a2, const char* a3);
int __cdecl nox_xxx_parseMass_535B60(int a1, int a2, char* a3);
int __cdecl nox_xxx_parseCapacity_535B80(int a1, int a2, char* a3);
int __cdecl nox_xxx_parseWeight_535BB0(int a1, int a2, char* a3);
int __cdecl nox_xxx_parseMaterial_535BE0(int a1, int a2, const char* a3);
int nox_xxx_parseLightIntensity_535C20(int a1, int a2, char* a3);
int __cdecl nox_xxx_parseMenuIcon_535C30(int a1, int a2, char* a3);
int __cdecl nox_xxx_parsePrettyImage_0_535C80(int a1, int a2);
void sub_535CC0();
int __cdecl nox_xxx_parseDraw_535CD0(int a1, _DWORD* a2, void* a3);
int __cdecl nox_xxx_parseXFer_5360A0(int a1, int a2, char* a3);
char* __cdecl sub_536130(char* a1, int* a2);
char* __cdecl sub_536180(char* a1, int* a2);
char* __cdecl sub_5361B0(char* a1, int a2);
char* __cdecl sub_536260(char* a1, int a2);
char* __cdecl sub_536390(char* a1, int* a2);
char* __cdecl sub_5363C0(char* a1, int* a2);
int __cdecl nox_xxx_parseUseFn_5363F0(_DWORD* a1, int a2, char* a3);
int __cdecl sub_5364E0(char* a1, int a2);
int __cdecl sub_536550(char* a1, _DWORD* a2);
int __cdecl sub_536580(char* a1, int a2);
int __cdecl sub_5365B0(char* a1, int a2);
int __cdecl sub_536600(char* a1, int a2);
int __cdecl nox_xxx_parseUpdate_536620(_DWORD* a1, int a2, char* a3);
int __cdecl nox_xxx_parsePickup_536710(int a1, int a2, char* a3);
char* __cdecl sub_5367B0(__int16* a1);
int __cdecl nox_xxx_parseCreateProc_536830(int a1, int a2, char* a3);
int __cdecl sub_5368C0(char* a1, int a2);
int __cdecl sub_536910(int a1, int a2);
int __cdecl nox_xxx_parseInitProc_536930(_DWORD* a1, int a2, char* a3);
int __cdecl nox_xxx_parseDrop_536A20(int a1, int a2, char* a3);
char* __cdecl sub_536AC0(__int16* a1);
int __cdecl sub_536B40(char* a1, int a2);
int __cdecl nox_xxx_parseDieProc_536B80(int a1, int a2, char* a3);
int __cdecl nox_xxx_parseDamageFn_536C60(int a1, int a2, char* a3);
int __cdecl nox_xxx_parseDamageSound_536CF0(int a1, int a2, char* a3);
int __cdecl sub_536D80(char* a1, int a2);
BOOL __cdecl sub_536DA0(char* a1, int* a2);
int __cdecl sub_536DE0(char* a1, _BYTE* a2);
BOOL __cdecl nox_xxx_collideDamageLoad_536E10(char* a1, int a2);
int __cdecl sub_536E50(char* a1, _BYTE* a2);
int __cdecl sub_536E80(char* a1, int* a2);
int __cdecl nox_xxx_parseCollide_536EC0(_DWORD* a1, int a2, char* a3);
BOOL __cdecl nox_xxx_unitCanSee_536FB0(int a1, int a2, char a3);
int __cdecl nox_xxx_unitCanInteractWith_5370E0(int a1, int a2, char a3);
int __cdecl nox_xxx_mapCheck_537110(int a1, int a2);
void __cdecl nox_xxx_lineCollisionChk_537230(float*, int); // idb
int __cdecl nox_xxx_traceRay_5374B0(float4* a1);   // idb
void __cdecl sub_5374D0(_DWORD* a1);
_DWORD* __cdecl nox_xxx_harpoonBreakForPlr_537520(_DWORD* a1);
void __cdecl sub_537540(int a1);
int __cdecl sub_537580(int a1);
BOOL sub_537590();
void __cdecl sub_5375A0(int a1);
char __cdecl nox_xxx_unitHasCollideOrUpdateFn_537610(int a1);
int sub_537700();
int sub_537740();
int __cdecl sub_537750(int a1);
unsigned int sub_537760();
char __cdecl sub_537770(int a1);
char __cdecl nox_xxx_projectileTraceHit_537850(int a1, int* a2, float2* a3); // idb
_DWORD* __cdecl nox_xxx_sMakeScorch_537AF0(int* a1, int a2);
int nox_xxx_scorchInit_537BD0();
char __cdecl nox_xxx_trapBAH_537C10(int a1, int a2);
void __cdecl sub_537DD0(float*, int); // idb
int __cdecl sub_537E60(int a1, int a2, int a3, int a4);
void __cdecl sub_537F00(float*, int); // idb
int __cdecl nox_xxx_castGlyph_537FA0(int a1, int a2, int a3, int a4, int a5);
int __cdecl sub_538250(int a1);
char __cdecl nox_xxx___mkgmtime_538280(int a1);
int __cdecl nox_xxx_playerPreAttackEffects_538290(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_playerTraceAttack_538330(int a1, int a2);
void __cdecl sub_538510(int a1, int a2);
void __cdecl sub_5386A0(int a3, int a2); // idb
int __cdecl nox_xxx_itemApplyAttackEffect_538840(int a1, int a2, int a3);
int __cdecl nox_xxx_createHarpoonBolt_538890(int a1);
BOOL __cdecl nox_xxx_playerAttack_538960(int a1);
__int16 __cdecl nox_xxx_warcryStunMonsters_539B90(int a1, int a2);
int __cdecl nox_xxx_shootBowCrossbow1_539BD0(int a1, int a2);
_DWORD* __cdecl nox_xxx_shootBowCrossbow2_539D80(int a1, int a2, int a3, char* a4);
int __cdecl nox_xxx_shootApplyEffects_539F40(int a1, int a2, int a3);
int __cdecl sub_539FB0(_DWORD* a1);
int __cdecl nox_xxx_playerTryReloadQuiver_539FF0(_DWORD* a1);
int __cdecl nox_xxx_equipWeaponNPC_53A030(int a1, int a2);
void __cdecl sub_53A0F0(int a1, int a2, int a3);
int __cdecl nox_xxx_playerDequipWeapon_53A140(_DWORD* a1, int a2, int a3, int a4);
int __cdecl nox_xxx_NPCEquipWeapon_53A2C0(int a1, int a2);
void __cdecl sub_53A3D0(_DWORD* a1);
int __cdecl nox_xxx_playerEquipWeapon_53A420(_DWORD* a1, int a2, int a3, int a4);
int __cdecl sub_53A680(int a1);
void __cdecl sub_53A6C0(int a1, int a2);
int __cdecl sub_53A720(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_sendMsgOblivionPickup_53A9C0(int a1, int a2, int a3, int a4);
void sub_53AAB0(int a1);
int __cdecl nox_xxx_dropWeapon_53AB10(int a1, _DWORD* a2, int* a3);
void __cdecl sub_53AB90(int a1, int a2);
void __cdecl nox_xxx_updateProjectile_53AC10(int a1);
char __cdecl nox_xxx_updateDoor_53AC50(int a1);
void __cdecl nox_xxx_updateSpark_53ADC0(int a1);
char __cdecl sub_53AE00(int a1);
float* __cdecl nox_xxx_updateProjTrail_53AEC0(int a1);
void __cdecl nox_xxx_updatePush_53B030(int a1);
char __cdecl nox_xxx_updateToggle_53B060(_DWORD* a1);
char __cdecl nox_xxx_updateTrigger_53B1B0(int a1);
char __cdecl sub_53B300(int a1);
char __cdecl nox_xxx_updateSwitch_53B320(_DWORD* a1);
char __cdecl nox_xxx_updateElevatorShaft_53B380(int a1);
void __cdecl nox_xxx_fnElevatorShaft_53B410(int a1, int a2);
_DWORD* __cdecl nox_xxx_elevatorAud_53B490(int a1, int a2);
void __cdecl nox_xxx_updateElevator_53B5D0(_DWORD* a1);
void __cdecl nox_xxx_elevatorFn_53B750(int a1, int a2);
void __cdecl nox_xxx_updatePhantomPlayer_53B860(int a1);
void __cdecl nox_xxx_updateLifetime_53B8F0(int a1);
void __cdecl nox_xxx_spellFlyUpdate_53B940(int a1);
void __cdecl nox_xxx_updateAntiSpellProj_53BB00(int a1);
void __cdecl sub_53BD10(int a1, int a2);
int __cdecl nox_xxx_updateMagicMissile_53BDA0(int a1);
int __cdecl nox_xxx_updateTeleportPentagram_53BEF0(int a1);
void __cdecl nox_xxx_fnPentagramTeleport_53C060(float*, int); // idb
int __cdecl nox_xxx_updateInvisiblePentagram_53C0C0(int a1);
void __cdecl sub_53C140(float*, int); // idb
void __cdecl nox_xxx_updateBlow_53C160(int a3);      // idb
void __cdecl sub_53C240(float*, int); // idb
int __cdecl nox_xxx_rechargeItem_53C520(int a1, int a2);
signed int __cdecl nox_xxx_updateObelisk_53C580(int a1);
int __cdecl nox_xxx_getRechargeRate_53C940(_DWORD* a1);
void __cdecl nox_xxx_updateBlackPowderBarrel_53C9A0(float* a1);
void __cdecl nox_xxx_updateOneSecondDie_53CB60(int a1);
void __cdecl nox_xxx_updateWaterBarrel_53CB90(int a1);
void __cdecl nox_xxx_waterBarrel_53CC30(float*, int); // idb
void __cdecl nox_xxx_updateSelfDestruct_53CC90(int a1);
void __cdecl nox_xxx_updateBlackPowderBurn_53CCB0(int a1);
void __cdecl nox_xxx_updatePixie_53CD20(int a1);
__int16 __cdecl nox_xxx_pixieIdleAnimate_53CF90(int a1, float2* a2, __int16 a3); // idb
__int16 __cdecl sub_53D010(int a1, int a2);
void __cdecl nox_xxx_updateDeathBall_53D080(int a1);
void __cdecl sub_53D170(int a1, int a2);
void __cdecl nox_xxx_updateDeathBallFragment_53D220(int a1);
void __cdecl nox_xxx_updateMoonglow_53D270(int a1);
void __cdecl nox_xxx_updateTelekinesis_53D330(int a1);
void __cdecl nox_xxx_updateFist_53D400(int a1);
void __cdecl nox_xxx_updateFlameCleanse_53D510(int a1);
void __cdecl nox_xxx_updateMeteorShower_53D5A0(float* a2); // idb
void __cdecl nox_xxx_meteorExplode_53D6E0(int a6);    // idb
void __cdecl nox_xxx_updateToxicCloud_53D850(int a1);
void __cdecl sub_53D8C0(int a1, int a2);
void __cdecl nox_xxx_updateSmallToxicCloud_53D960(int a1);
void __cdecl nox_xxx_toxicCloudPoison_53D9D0(int a1, int a2);
void __cdecl nox_xxx_updateArachnaphobia_53DA60(int* a1);
void __cdecl nox_xxx_updateExpire_53DB00(int a1);
int* __cdecl nox_xxx_updateBreak_53DB30(_DWORD* a1);
int* __cdecl nox_xxx_updateOpen_53DBB0(_DWORD* a1);
void __cdecl nox_xxx_updateBreakAndRemove_53DC30(_DWORD* a1);
void __cdecl nox_xxx_updateChakramInMotion_53DCC0(int a1);
int __cdecl nox_xxx_updateFlag_53DDF0(int a1);
int* __cdecl nox_xxx_updateTrapDoor_53DE80(_DWORD* a1);
void __cdecl nox_xxx_updateGameBall_53DF40(int a3);
void __cdecl nox_xxx_updateUndeadKiller_53E190(int a1);
void __cdecl nox_xxx_updateCrown_53E1D0(int a1);
char __cdecl sub_53E2B0(int a1);
BOOL __cdecl sub_53E2D0(int a1);
int __cdecl nox_xxx_recalculateArmorVal_53E300(_DWORD* a1);
int __cdecl sub_53E3A0(int a1, int a2);
int __cdecl sub_53E430(_DWORD* a1, int a2, int a3, int a4);
int __cdecl nox_xxx_NPCEquipArmor_53E520(int a1, _DWORD* a2);
void __cdecl sub_53E600(_DWORD* a1);
int __cdecl nox_xxx_playerEquipArmor_53E650(_DWORD* a1, int a2, int a3, int a4);
_DWORD* __cdecl nox_xxx_armorHaveSameSubclass_53E7B0(int a1, int a2);
int __cdecl nox_xxx_pickupArmor_53E7F0(int a1, int a2, int a3, int a4);
void sub_53EAE0(int a1);
int __cdecl nox_xxx_dropArmor_53EB70(int a1, _DWORD* a2, int* a3);
int __cdecl nox_xxx_ItemIsDroppable_53EBF0(int a1);
CHAR* sub_53EC40();
BOOL __cdecl sub_53EC80(int a1, int a2);
int __cdecl nox_xxx_useMushroom_53ECE0(int a1, int a2);
int __cdecl nox_xxx_useEnchant_53ED60(int a1, int a2);
int __cdecl nox_xxx_useCast_53ED90(int a1, _DWORD* a2);
int __cdecl nox_xxx_useConsume_53EE10(int a1, int a2);
int __cdecl nox_xxx_useCiderConfuse_53EF00(int a1, int a2);
int __cdecl nox_xxx_usePotion_53EF70(int a1, int a2);
int __cdecl nox_xxx_useLesserFireballStaff_53F290(int a1, _DWORD* a2);
_DWORD* __cdecl nox_xxx_wandShot_53F480(int a1, int a2, int* a3, _DWORD* a4);
int __cdecl nox_xxx_useWandCastSpell_53F4F0(int a1, _DWORD* a2);
int __cdecl nox_xxx_useFireWand_53F670(int a1, int a2);
int __cdecl nox_xxx_useRead_53F7C0(int a1, int a2);
int __cdecl sub_53F830(int a1, int a2);
int __cdecl nox_xxx_useByNetCode_53F8E0(int a1, int a2);
int __cdecl sub_53F930(int a1, int a2);
int __cdecl nox_xxx_useSpellReward_53F9E0(int a1, int a2);
int __cdecl nox_xxx_useAbilityReward_53FAE0(int a1, int a2);
_DWORD* __cdecl nox_xxx_respawnPlayerImpl_53FBC0(float* a1, int a2);
void nox_xxx_createCorpse_53FCA0();
void sub_53FEA0();
void __cdecl nox_xxx_warriorBerserker_53FEB0(int a1);
void __cdecl nox_xxx_warriorWarcry_53FF40(int a1);
int __cdecl sub_53FFB0(_DWORD* a1, _DWORD* a2);
int __cdecl nox_xxx_warriorHarpoon_540070(_DWORD* a1);
void __cdecl nox_xxx_warriorTreadLightly_5400B0(_DWORD* a1, __int16 a2);
void __cdecl nox_xxx_warriorInfravis_540110(int a1, __int16 a2);
void nox_xxx_r3mov3d_Anvil_540150();
int __cdecl nox_xxx_castMissilesOM_540160(int a1, int a2, int a3, int a4, int a5, int a6);
int __cdecl nox_xxx_castPixies_540440(int a1, int a2, int a3, int a4, int a5, int a6);
int __cdecl nox_xxx_spellFlySearchTarget_540610(float2* a1, int a2, int a3, float a4, int a5, int a6); // idb
void __cdecl sub_540780(float*, int);                                         // idb
BOOL __cdecl sub_5408A0(int a1);
int __cdecl nox_xxx_mobCastInversion_5408D0(int a1);
int* __cdecl nox_xxx_monsterCast_540A30(int a1, int a2, int a3);
int __cdecl nox_xxx_unitIsMagicMissile_540B60(int a1, int a2);
int __cdecl nox_xxx_monsterBuffSelf_540B90(int a1);
BOOL __cdecl sub_540CE0(int a1, int a2);
BOOL __cdecl sub_540D20(int a1);
int __cdecl sub_540D40(int a1);
int __cdecl nox_xxx_mobCastRelated2_540D90(int a1, int a2);
int __cdecl nox_xxx_monsterCastOffensive_540F20(int a1, int a2);
int __cdecl nox_xxx_mobCastRelated_541050(int a1);
int __cdecl nox_xxx_mobHealSomeone_5411A0(int a3);       // idb
void __cdecl nox_xxx_mobMayHealThis_5412A0(float*, int); // idb
char __cdecl nox_xxx_mobCast_541300(int a1, _DWORD* a2, int a3);
char __cdecl nox_xxx_mobActionCastOnObj_541360(_DWORD* a1);
char __cdecl nox_xxx_mobActionCast_5413B0(_DWORD* a1, int a2);
void __cdecl nox_xxx_mobCastRandomRecoil_541490(int a1, float* a2, float2* a3); // idb
char __cdecl nox_xxx_mobActionCastOnPoint_541560(_DWORD* a1);
_DWORD* __cdecl nox_xxx_mobActionCastStopMB_541590(int a1);
char __cdecl nox_xxx_mobActionCastFinishMB_5415C0(int a1);
char __cdecl nox_xxx_mobActionCastStart_5415F0(_DWORD* a1);
char __cdecl sub_541630(int a1, int a2);
int __cdecl sub_541670(LPCSTR, LPCSTR lpExistingFileName, LPCSTR lpFileName); // idb
size_t __cdecl sub_5417C0(FILE* a1, FILE* a2, FILE* a3);
int __cdecl sub_5418C0(FILE* a1, FILE* a2, FILE* a3);
int __cdecl sub_541A50(FILE* a1, FILE* a2, FILE* a3);
int __cdecl sub_541D80(FILE* a1, FILE* a2);
int __cdecl sub_541E40(FILE* a1, FILE* a2, FILE* a3);
int __cdecl sub_542380(FILE* a1, FILE* a2, int a3);
int __cdecl sub_542B70(FILE* a1);
double __cdecl sub_542B90(FILE* a1);
size_t __cdecl sub_542BB0(char a1, FILE* a2);
size_t __cdecl sub_542BD0(char a1, FILE* a2);
char* __cdecl sub_542BF0(int a1, int a2, int a3);
int __cdecl sub_543110(LPCSTR lpExistingFileName, int2* a2); // idb
char* __cdecl sub_5435C0(int a1, int a2, int a3, int a4);
char* __cdecl sub_543620(int a1, int a2);
int __cdecl sub_543680(float* a1);
void __cdecl nox_xxx_unused_543720(int a1, int a2, int a3, int a4);
int __cdecl nox_xxx_unused_543770(_DWORD* a1, _DWORD* a2, _DWORD* a3, _DWORD* a4);
BOOL __cdecl sub_5437E0(int* a1, int a2, int a3);
void __cdecl sub_543BC0(int a1, int a2, int a3, int a4, int a5, int a6);
int __cdecl nox_xxx_tile_543C50(_DWORD* a1, int a2, int a3, int a4, int a5, int a6);
int __cdecl sub_543E60(int a1, int a2);
int __cdecl nox_xxx_mapGenEdge_543EB0(int a1, int a2);
int __cdecl sub_543FB0(const char* a1);
int __cdecl sub_544020(char* a1);
int __cdecl nox_xxx_tileCheckByte3_544070(int a1);
int __cdecl nox_xxx_tileCheckByte4_5440A0(int a1);
int __cdecl sub_5440F0(int a1, signed int a2);
int __cdecl sub_544240(int a1);
int __cdecl sub_544270(const char* a1);
int __cdecl sub_5442D0(float* a1);
int __cdecl nox_xxx_tileSubtile_544310(float2* a1); // idb
char __cdecl nox_xxx_mobActionMoveTo_5443F0(int a1);
char __cdecl nox_xxx_mobActionMoveToFar_5445C0(int* a1);
void __cdecl nox_xxx_mobActionDodge_544640(int a1);
int __cdecl sub_544740(int a1);
int __cdecl sub_544750(int a1);
char __cdecl nox_xxx_mobActionFlee_544760(int a1);
int __cdecl nox_xxx_mobActionReturnToHome_544920(int a1);
int __cdecl sub_544930(int a1);
int __cdecl sub_544940(int a1);
char __cdecl sub_544950(int a1);
char __cdecl nox_xxx_mobActionWait_544960(int a1);
char __cdecl nox_xxx_mobActionWaitRelative_544990(int a1);
int* __cdecl nox_xxx_mobActionHunt_5449D0(int a1);
int __cdecl nox_xxx_mobSearchEdible_544A00(int, float); // idb
void __cdecl nox_xxx_mobSearchEdible2_544A40(int a1, int a2);
int __cdecl sub_544AE0(int, float); // idb
void __cdecl sub_544B20(int a1, int a2);
char __cdecl nox_xxx_mobActionPickupObject_544B90(int a1);
int __cdecl sub_544C40(int a1);
void __cdecl sub_544CA0(_DWORD* a1);
void __cdecl nox_xxx_zombieBurnDelete_544CE0(_DWORD* a1);
char __cdecl sub_544D60(int a1);
char __cdecl nox_xxx_mobActionDead1_544D80(_DWORD* a1);
void nox_xxx_createReleasedSoul_544E60(int a1);
void __cdecl nox_xxx_mobActionDead2_544EC0(int a1);
int __cdecl sub_544F70(int a1);
unsigned __int8* __cdecl nox_xxx_mobActionReportComplete_544FF0(int a1);
char __cdecl nox_xxx_mobActionRandomWalk_545020(int a1);
int __cdecl sub_545090(int a1);
char __cdecl nox_xxx_mobActionConfuse_545140(int a1);
char __cdecl sub_545210(int a1);
char __cdecl sub_545240(int a1, float* a2);
char __cdecl sub_545300(int a1);
char __cdecl sub_545340(int a1);
char __cdecl sub_5453E0(int a1);
char __cdecl nox_xxx_mobActionRetreat_545440(int a1);
BOOL __cdecl nox_xxx_monsterCanResumeAttack_545520(int a1);
BOOL __cdecl sub_545580(int a1);
BOOL __cdecl nox_xxx_monsterCanCast2_5455B0(int a1);
int* __cdecl nox_xxx_mobRetreatCheckEdibles_5455E0(int a1);
int __cdecl sub_5456B0(int a1);
int __cdecl sub_5456C0(int a1);
void __cdecl sub_5456D0(int a1);
__int64 __cdecl sub_545790(int a1);
int __cdecl sub_5457C0(int a1);
char __cdecl nox_xxx_mobActionRoam_5457E0(int* a1);
void sub_545B00(int a1, int a2);
int __cdecl sub_545B60(int a1, unsigned __int8 a2);
int __cdecl nox_xxx_monsterRoamDeadEnd_545BB0(int a1, int a2);
int __cdecl sub_545C60(int a1, int a2, unsigned __int8 a3);
int __cdecl sub_545DA0(int a1);
int __cdecl sub_545E60(int* a1);
int* __cdecl nox_xxx_mobActionGuard_546010(int a1);
int __cdecl sub_546410(int a1);
int __cdecl sub_546420(int a1);
char __cdecl nox_xxx_mobActionEscort_546430(int* a1);
int __cdecl nox_xxx_monsterGetObjEscortName_546600(int a1);
int __cdecl nox_xxx_monsterLookAtDamager_5466B0(int a1);
int __cdecl sub_5466F0(_DWORD* a1);
int __cdecl nox_xxx_monsterIdleStarted_546820(int a1);
int* __cdecl nox_xxx_monsterUpdateIdleLogic_546850(int a1);
void __cdecl nox_xxx_mobAction_5469B0(int a1);
char __cdecl nox_xxx_mobActionDependency_546A70(int a1);
__int16 __cdecl nox_xxx_unitIsDangerous_547120(int a1, int a2);
void nox_xxx_monsterPopAttackActions_5471B0(int a1);
char __cdecl nox_xxx_monsterMainAIFn_547210(int a1);
int __cdecl nox_xxx_monsterCheckDodgeables_547C50(int a1);
int __cdecl sub_547DB0(int a1, float2* a2); // idb
BOOL __cdecl sub_547EE0(int a1, unsigned __int8 a2);
int sub_547F10();
int __cdecl sub_547F20(int a1, float* a2);
int __cdecl nox_xxx_BuildWaypointPath_547F70(_DWORD* a1, int a2, _DWORD* a3, int a4);
void __cdecl sub_548100(int2* a1, int a2);
void __cdecl sub_5481C0(int a1);
void __cdecl sub_548220(int* a1, float* a2);
int __cdecl sub_548360(int a1, int a2);
int __cdecl sub_5485B0(int a1, int a2);
void __cdecl nullsub_30(_DWORD); // weak
void __cdecl sub_548600(int a1, float a2, float a3);
void __cdecl nox_xxx_collSysAddCollision_548630(int, unsigned int, float2*);
void nox_xxx_allocHitArray_5486D0();
void nox_xxx_collide_548740();
void sub_548830(int a1);
void sub_548860(int a1, __int16 a2);
void __cdecl sub_5488B0(int* a1, float* a2, int a3);
void sub_548B60();
void __cdecl nox_xxx_scriptSetDialog_548C80(int a1, char a2, int a3, int a4);
void __cdecl nox_xxx_script_forcedialog_548CD0(int a1, int a2);
char __cdecl nox_xxx_scriptDialog_548D30(int a1, char a2);
char __cdecl nox_xxx_startShopDialog_548DE0(int a1, int a2, int a3, const char* a4);
int __cdecl sub_548F40(int a1);
char __cdecl nox_xxx_scriptGetDialogIdx_548F70(const char* a1);
void __cdecl sub_548FE0(int a1, const char* a2);
int __cdecl nox_xxx_monsterLoadStrikeFn_549040(int a1, char* a2);
int __cdecl nox_xxx_monsterLoadDieFn_5490E0(int a1, char* a2);
int __cdecl nox_xxx_monsterLoadDeadFn_549180(int a1, char* a2);
int __cdecl nox_xxx_strikeOgre_549220(float); // idb
void __cdecl sub_549270(int a1, int a2);
int __cdecl nox_xxx_strikeMonsterDefault_549380(float);          // idb
int __cdecl nox_xxx_monsterPickMeleeTarget_549440(int a3, int a2); // idb
void __cdecl sub_5494C0(float*, int);   // idb
int __cdecl nox_xxx_strikeScorpion_5495B0(float);          // idb
BOOL __cdecl sub_549690(int a1, int a2);
int __cdecl nox_xxx_strikeVileZombie_549700(float); // idb
int __cdecl nox_xxx_strikeStoneGolem_5497E0(float a1);
int __cdecl nox_xxx_sendEquakeAfterGolem_549800(float); // idb
void __cdecl nox_xxx_monsterAttackAreaDamage_549860(int a1, float a2);
int __cdecl nox_xxx_strikeMechGolem_549960(float); // idb
int __cdecl nox_xxx_strikeWasp_549980(float); // idb
int __cdecl nox_xxx_strikeGhost_549A60(float); // idb
int nox_xxx_strikeBomber_549BB0();
int __cdecl nox_xxx_strikeSpider_549BC0(float); // idb
int __cdecl nox_xxx_strikeSpittingSpider_549CA0(float); // idb
int __cdecl sub_549D80(int a1);
int __cdecl sub_549E00(int a1);
int __cdecl sub_549E70(int a1);
int __cdecl sub_549E90(int a1);
int __cdecl sub_549FA0(int a1);
int __cdecl sub_54A150(int a1);
int __cdecl sub_54A250(int a1);
int __cdecl nox_xxx_monsterDeadTroll_54A270(int a1);
int __cdecl sub_54A310(int a1);
void sub_54A390(int a1, CHAR* a2, const char* a3, const char* a4, const char* a5, const char* a6, int a7);
void sub_54A4C0(int a1);
int __cdecl sub_54A750(int a1);
int __cdecl sub_54A7D0(int a1);
int __cdecl sub_54A850(int a1);
int __cdecl sub_54A890(int a1);
int __cdecl sub_54A900(int a1);
int __cdecl sub_54A950(int a1);
double __cdecl sub_54A990(float2* a1, float a2, int a3, float2* a4); // idb
void __cdecl sub_54AD50(int a1, int a2, int a3);
int __cdecl nox_xxx_findObjectAtCursor_54AF40(int a1);
void __cdecl nox_xxx_playerCursorScanFn_54AFB0(int a1, float* a2);
int __cdecl sub_54B2D0(int* a1, int a2, _DWORD* a3);
int __cdecl sub_54B810(int a1, int a2, int* a3, int2* a4, int a5); // idb
int __cdecl sub_54BA60(int a1, int a2, int a3, int a4);
int __cdecl sub_54BB20(int a1, int a2, int* a3, _DWORD* a4, int a5);
int __cdecl sub_54BD90(int a1, int a2, int* a3, int* a4, int a5);
int __cdecl sub_54BF20(int a1, int a2, int* a3, int* a4, int a5);
__int16 __cdecl nox_xxx_monsterAutoSpells_54C0C0(int a1);
char __cdecl nox_xxx_monsterCreateFn_54C480(int a1);
int __cdecl nox_xxx_createWeapon_54C710(int a1);
_DWORD* __cdecl sub_54C950(int a1);
int __cdecl nox_xxx_createFnObelisk_54CA10(int a1);
int* __cdecl nox_xxx_createFnAnim_54CA50(int a1);
_BYTE* __cdecl nox_xxx_createTrigger_54CA60(int a1);
_DWORD* __cdecl nox_xxx_createMonsterGen_54CA90(int a1);
_DWORD* __cdecl nox_xxx_createRewardMarker_54CAC0(int a1);
int __cdecl nox_xxx_dieImpEgg_54CAE0(int a1);
void __cdecl nox_xxx_diePolyp_54CB10(int a1);
void __cdecl nox_xxx_diePotion_54CBB0(int a1);
void sub_54CBD0(int a1);
void __cdecl sub_54CC40(int a1);
void __cdecl sub_54CD30(int a1);
void sub_54CE00(int a1);
void sub_54CEE0(int a1);
char __cdecl sub_54CFB0(int a1);
void __cdecl sub_54D080(int a1);
int __cdecl nox_xxx_diePlayer_54D2B0(int a1);
void __cdecl nox_xxx_playerHandleElimDeath_54D7A0(int a1, int a2);
void nox_xxx_playerUpdateScore_54D980(int a1, int a2, int a3, int a4);
void nox_xxx_playerHandleKotrDeath_54DC40(int a1, int a2);
void nox_xxx_netNotifyPlayerDied_54DF00(int a1);
char __cdecl nox_xxx_dieGlyph_54DF30(int a1);
void __cdecl nox_xxx_unused_54DF40(int a1);
void __cdecl nox_xxx_dieBarrel_54DFA0(int a1);
void __cdecl nox_xxx_dieCreateObject_54E010(int a1);
__int16 __cdecl nox_xxx_dieSpawnObject_54E070(int a1);
void __cdecl sub_54E0D0(int a1);
void __cdecl nox_xxx_dieMarker_54E460(int a1);
void __cdecl nox_xxx_dieBoulder_54E4B0(int a1);
int __cdecl nox_xxx_dieGameBall_54E620(int a1);
void __cdecl nox_xxx_dieMonsterGen_54E630(int a1);
int __cdecl sub_54E6F0(int a1, int a2);
int __cdecl sub_54E730(int a1, int a2);
int __cdecl sub_54E810(int a1, float2* a2, int a3); // idb
void __cdecl sub_54E850(int a1, int a2);
char __cdecl nox_xxx_updateMonsterGenerator_54E930(_DWORD* a1);
int __cdecl nox_xxx_mobGeneratorPick_54EBA0(_DWORD* a1, float2* a2, int a4);     // idb
int __cdecl nox_xxx_mgenSetCreaturePos_54ED50(int a1, float2* a2, int a3, int a4); // idb
int __cdecl sub_54EF00(float* a3);                          // idb
void __cdecl sub_54EF60(float*, int);                       // idb
int __cdecl sub_54EF90(float, int, int, int);               // idb
_DWORD* __cdecl nox_xxx_mobGeneratorSpawn_54F070(int a1, int a2, int a3);
void __cdecl nox_xxx_unitCreatureCopyUC_54F2B0(int a1, int a2);
void __cdecl nox_xxx_updateHarpoon_54F380(int a1);
void __cdecl nox_xxx_unitUpdateMover_54F740(int a1);
int __cdecl nox_xxx_updateShootingTrap_54F9A0(int a1);
void __cdecl nox_xxx_createArrowTrapProjectile_54FA80(int a1, int a2);
void sub_54FBB0(int a1);
int __cdecl sub_54FBF0(int a3);    // idb
void __cdecl nox_xxx_unitIsAttackReachable_54FC50(int, int); // idb
void __cdecl nox_xxx_collideTrigger_54FCD0(int a1, int a2);
float* __cdecl nox_xxx_createSpark_54FD80(float a1, float a2, int a3, int a4, float a5, float a6, float a7, int a8); // idb
void __cdecl sub_54FEF0(int a2);                                                                     // idb
int __cdecl sub_54FFC0(int2* a1, int a2);                                                            // idb
int __cdecl sub_550280(int, float, float, int, int, int, int);                                       // idb
int __cdecl sub_5502F0(float2* a1, float a2, float a3, int a4, int a5, float2* a6, float2* a7);      // idb
int __cdecl sub_550380(int a1, int a2, float2* a3);                                                  // idb
BOOL __cdecl sub_550480(int a1);
void __cdecl sub_5504B0(int a2);                                                          // idb
int __cdecl sub_550580(int2* a1, float* a2);                                              // idb
int __cdecl sub_550760(int a1, float2* a2, float2* a3, float4* a4, float2* a5, float a6); // idb
int __cdecl sub_550A10(int a1, float2* a2, float2* a3, float4* a4, float2* a5, float a6); // idb
char __cdecl sub_550CB0(float2* a1, float2* a2);                                          // idb
void __cdecl nox_xxx_collisionCheckCircleCircle_550D00(int a1, int a2);                                                  // idb
void __cdecl sub_550F80(float* a1, int a2);
void __cdecl sub_551250(unsigned int a1, float* a2, int a3);
int __cdecl sub_5516A0(float4* a1, float4* a2, float2* a3, int a4, int a6);           // idb
int __cdecl sub_551780(float4* a1, float a2, float a3, float a4, float2* a5, int a6); // idb
int __cdecl sub_551870(float4* a1, float a2, float a3, float a4, float2* a5, int a6); // idb
int __cdecl sub_551960(float4* a1, float4* a2, float4* a3, float2* a4);               // idb
BOOL __cdecl sub_551A90(float2* a1, float4* a2);                                      // idb
void __cdecl sub_551AE0(int a1, int a2, int a3);
void sub_551BF0();
void __cdecl sub_551C40(int a1, int a2);
void sub_551DF0();
int __cdecl sub_551E00(unsigned __int8 a1, int a2);
LPVOID __cdecl sub_551E60(int a1);
int __cdecl sub_551EB0(int a1, unsigned int a2, unsigned __int8 a3, int a4, int a5);
int __cdecl nox_xxx_sendto_551F90(SOCKET s, char* buf, int len, int flags, struct sockaddr* to, int tolen); // idb
int __cdecl sub_552010(int a1);
int __cdecl nox_xxx_allocNetGQueue_5520B0(int a1, int a2);
int sub_5521A0();
void sub_5522E0(int a1);
int __cdecl nox_xxx_makePacketTime_552340(int a1, int a2);
void sub_552380(int a1);
void sub_5523E0(char a1, int a2);
int sub_552450();
int sub_552460();
int __cdecl sub_552510(unsigned int a1);
int __cdecl sub_552540(unsigned int a1);
int __cdecl sub_552570(unsigned int a1, int a2);
int __cdecl nox_xxx_cliWaitServerResponse_5525B0(unsigned int a1, char a2, int a3, char a4);
int __cdecl nox_xxx_netSendSock_552640(unsigned int a1, const void* a2, signed int a3, char a4);
int __cdecl nox_xxx_netSendReadPacket_5528B0(unsigned int a1, char a2);
int __cdecl nox_xxx_servNetInitialPackets_552A80(unsigned int a1, char a2);
int __cdecl sub_552E70(unsigned int a1);
int __cdecl sub_552F20(unsigned int a1);
u_long __cdecl sub_552FD0(u_long argp);
void* __cdecl nox_xxx_makeNewNetStruct_553000(size_t* a1);
void sub_5531C0(LPVOID lpMem); // idb
int __cdecl nox_xxx_netBigSwitch_553210(unsigned int a1, unsigned __int8* a2, int a3, int a4);
int sub_553D10();
int __cdecl sub_553D30(int a1);
int sub_553D60();
int sub_553D70();
int sub_553D80();
int sub_553D90();
int sub_553DA0();
int sub_553DB0();
__int64 __cdecl sub_553DC0(int a1);
unsigned int __cdecl sub_553E10(int a1);
unsigned int __cdecl sub_553E50(int a1);
unsigned int __cdecl sub_553E90(int a1);
int __cdecl sub_553ED0(int a3);
void sub_553F40(int a1, int a2);
void sub_553FC0(int a1, int a2);
void nox_xxx_netCountData_554030(int a1, int a2);
unsigned int __cdecl sub_554040(int a1, int a2, char* a3);
int* __cdecl nox_xxx_findPlayerID_5541D0(int a1);
int __cdecl sub_554200(unsigned int a1);
char* sub_554230();
int __cdecl sub_554240(int a1);
unsigned int sub_554290();
int sub_554300();
void sub_554370();
int __cdecl nox_xxx_netInit_554380(size_t* a1);
__int16 sub_5545A0();
int __cdecl nox_xxx_netStructReadPackets_5545B0(unsigned int a1);
int __cdecl sub_5546A0(unsigned int a1);
int __cdecl nox_xxx_netPreStructToFull_5546F0(size_t* a1);
int __cdecl sub_554760(int a1, char* cp, int hostshort, int a4, int a5);
int __cdecl sub_5549F0(unsigned int a1);
int __cdecl sub_554A50(unsigned int a1);
void __cdecl nox_xxx_lobbyMakePacket_554AA0(uint16_t hostshort, int, int, int); // idb
void sub_554B30();
int __cdecl nox_xxx_createSocketLocal_554B40(uint16_t hostshort);                 // idb
int __cdecl nox_xxx_sendLobbyPacket_554C80(uint16_t hostshort, char* buf, int); // idb
int sub_554D10();
int sub_554D70(char a1);
int sub_554FF0();
int __cdecl sub_555000(int a1);
int __cdecl nox_xxx_makeTempSocket_555010(int, uint16_t hostshort, char* buf, int); // idb
int __cdecl sub_5550A0(int, uint16_t hostshort, char* buf);      // idb
int __cdecl sub_5550D0(int, uint16_t hostshort, char* buf);      // idb
int sub_555100();
void sub_555110();
void sub_555120();
int __cdecl sub_555130(unsigned int a1, const void* a2, signed int a3);
int sub_5551F0(unsigned int a1, char a2, int a3);
int __cdecl sub_555250(unsigned int a1, _DWORD* a2);
int __cdecl sub_555290(unsigned int a1, _DWORD* a2);
int __cdecl nox_xxx_netSend_5552D0(unsigned int a1, char a2, int a3);
int sub_555360(unsigned int a1, unsigned __int8 a2, int a3);
void sub_555420();
void __cdecl sub_555430(HWND* a1);
char __cdecl sub_555500(char a1);
int sub_555510();
int __cdecl sub_555520(HWND a1);
void sub_5555D0();
int* __cdecl sub_555620(_DWORD* a1);
int sub_555A40();
_DWORD* __cdecl sub_555AE0(int a1, int a2, int a3, _DWORD* a4, _DWORD* a5, _DWORD* a6);
int __cdecl sub_555C10(_DWORD* a1, _DWORD* a2, _DWORD* a3);
int __cdecl nox_xxx_VQA_Test_555C40(_DWORD* a1);
int sub_556010();
int sub_556030();
int sub_556040();
_DWORD* sub_556050();
int __thiscall sub_556240(_DWORD** this);
int __cdecl sub_556280(int a1);
BOOL __cdecl sub_5562A0(int a1);
void* __thiscall sub_5562C0(void* this);
int __thiscall sub_5562D0(_DWORD* this);
void __thiscall sub_5562F0(_DWORD* this, _DWORD* a2, int a3, int a4, int a5, int a6);
int __thiscall sub_5563A0(_DWORD* this, int a2, char a3);
void* __thiscall sub_556470(_DWORD* this, void* a2, int a3, int a4, int a5, char a6, int a7);
void* __thiscall sub_556500(void* this, void* a2, int a3, int a4);
void* __thiscall sub_556570(void* this, void* a2, int a3, char a4);
void* __thiscall sub_5565E0(void* this, void* a2, int a3, char a4, int a5);
int __thiscall sub_556650(_DWORD* this);
int __thiscall sub_556710(_DWORD* this);
int __thiscall sub_556820(_DWORD* this);
int __thiscall sub_556890(_DWORD* this, int a2, int a3, int a4, int a5, int a6, int a7, int a8, int a9);
void __cdecl sub_556920(int*, char);
_DWORD* nox_xxx_j_unknown_libname_13_556990();
_DWORD* nox_xxx_unknown_libname_13_5569A0();
int __cdecl sub_5569B0(HWND a1);
int sub_5569F0();
int nox_xxx_unknown_libname_14_556A10();
char* __cdecl sub_556A20(char* a1);
_DWORD* __thiscall sub_556AC0(_DWORD* this);
int __thiscall sub_556AE0(_DWORD* this, int a2);
int __thiscall sub_556B20(_DWORD* this, int a2);
int __thiscall sub_556B50(_DWORD* this, int a2);
int __thiscall sub_556B80(int* this);
char* __thiscall sub_556C10(_DWORD* this);
int __cdecl sub_556C60(int a1, int a2);
_BYTE* __thiscall sub_557C70(_BYTE* this, const char* a2, int a3, int a4, int a5, int a6, int a7, int a8, int a9);
int __thiscall sub_557F50(char* this);
int __thiscall sub_557F60(int this, size_t a2);
int __thiscall sub_558090(_DWORD* this, int a2, void* a3, unsigned int a4);
_BYTE* __thiscall sub_558180(_BYTE* this, const void* a2);
char* __thiscall sub_5581B0(char* this);
char __thiscall sub_5581C0(_DWORD* this);
bool __thiscall sub_558230(int* this, int a2, int a3);
bool __thiscall sub_558260(int* this, int a2, int a3, int a4);
void __thiscall sub_5582C0(int* this, int a2);
BOOL sub_558300();
BOOL __thiscall sub_558370(int* this, int a2);
char __thiscall sub_5586A0(int this);
char __thiscall sub_5586D0(int this);
_DWORD* __thiscall sub_558710(int* this);
int __thiscall sub_558720(_DWORD* this);
bool __thiscall sub_558730(int* this, int a2, unsigned int a3, unsigned int a4, int a5, int a6);
int __thiscall sub_558790(_DWORD* this, int a2);
unsigned __int16* __thiscall sub_5587A0(_DWORD* this, unsigned __int16* a2, unsigned int a3);
int __thiscall sub_5587E0(_DWORD* this);
int __thiscall sub_5587F0(_DWORD* this);
int __thiscall sub_558800(int (**this)(void)); // weak
int __thiscall sub_558810(int (**this)(void)); // weak
BOOL __thiscall sub_558820(char* this, int a2, CHAR* a3, signed int a4);
BOOL __cdecl sub_558970(int a1, int a2, CHAR* a3, signed int a4);
int __cdecl sub_558990(int a1, int a2, void* a3, unsigned int a4);
_DWORD* __cdecl sub_5589B0(int a1, int a2, _DWORD* lpMem, size_t a4);
int __cdecl sub_558A20(int a1, int a2, void* a3, unsigned int a4);
char __cdecl sub_558AA0(_DWORD* a1, _DWORD* a2, unsigned int a3, unsigned int a4, signed int a5, signed int a6, int a7,
						int a8, char a9);
void __cdecl sub_559030(int a1, int a2, unsigned int a3, unsigned int a4, int a5, int a6, int a7, int a8, int a9,
						int a10, int a11, int a12);
_DWORD* __thiscall sub_559AA0(_DWORD* this);
int __thiscall sub_559AB0(int this);
_BYTE* __thiscall sub_559AD0(_BYTE* this);
int __thiscall sub_559AE0(_BYTE* this); // weak
char __thiscall sub_559AF0(char* this, LPCSTR lpFileName, int a3);
bool __thiscall sub_559B60(int this);
size_t __thiscall sub_559BA0(int this, void* a2, size_t a3);
bool __thiscall sub_559BE0(int this, int a2, int a3);
int __thiscall sub_559C10(_DWORD* this);
int __thiscall sub_559C30(_BYTE* this);
int __cdecl sub_559C40(int a1);
int __cdecl sub_559D00(int a1, unsigned int a2, int a3);
int __cdecl sub_559DB0(DWORD_PTR dwUser, int a2, int a3, int a4);
int __cdecl sub_559E80(DWORD_PTR dwUser, int, int); // idb
int __cdecl sub_55A0C0(int a1);
int __cdecl sub_55A130(int a1);
int __cdecl sub_55A2E0(int a1, int a2, int a3);
int __cdecl sub_55A360(int a1);
int __cdecl sub_55A3B0(int a1);
int __cdecl sub_55A3F0(int a1);
void __stdcall fptc(UINT uTimerID, UINT uMsg, DWORD dwUser, DWORD dw1, DWORD dw2);
int __cdecl sub_55A610(char* a1);
DWORD __cdecl sub_55A740(); // weak
int sub_55A750();
void __cdecl sub_55A760(void* a1);
char* __cdecl sub_55A780(char* a1);
int __cdecl sub_55A790(int a1);
int __cdecl sub_55A7B0(int a1, int a2, _DWORD* a3);
int __cdecl sub_55AE50(int a1);
int __cdecl sub_55AEF0(int a1, int a2, char a3);
int __cdecl sub_55B2D0(int a1, int a2);
int __cdecl sub_55B430(int a1, int a2, int a3);
int __cdecl sub_55B470(int a1, int a2, unsigned int a3, unsigned int a4, unsigned int a5, int a6, int a7);
int __cdecl sub_55B690(int a1, int a2, int a3, int a4);
int __cdecl sub_55B710(int a1, int a2, int a3, int a4, int a5);
_DWORD* __cdecl sub_55B820(int a1);
_DWORD* __cdecl sub_55B8D0(_DWORD* a1);
int __cdecl sub_55B8F0(int a1, int a2);
int __cdecl sub_55B930(unsigned __int8* a1, _DWORD* a2, _DWORD* a3, _DWORD* a4);
int __cdecl sub_55B970(int a1);
int __cdecl sub_55BA50(int a1);
int __cdecl sub_55BA80(int a1, int a2);
int __cdecl sub_55BB30(int a1);
int sub_55BC60();
unsigned __int16* __cdecl sub_55BC80(unsigned __int16* a1, int a2);
char* __cdecl sub_55BCB0(int a1, unsigned __int16* a2, char* a3, int a4, int a5, int a6);
int* __cdecl sub_55C050(int a1, unsigned __int16* a2, int* a3, int a4, int a5, int a6);
int* __cdecl sub_55C3F0(int a1, _WORD* a2, int* a3, unsigned int a4, int a5, int a6);
unsigned int __cdecl sub_55C530(int a1, _WORD* a2, int* a3, unsigned int a4, int a5, int a6);
void* sub_55C8D0();
unsigned __int8* sub_55C8E0();
DWORD __thiscall sub_55C900(DWORD* this);
DWORD nox_xxx_unknown_libname_15_55C920();
DWORD sub_55C930();
__int16 __cdecl sub_55C950(int a1, _BYTE* a2, int* a3, int a4, int a5, int a6);
__int16 __cdecl sub_55CA59(int a1, _BYTE* a2, int* a3, int a4, int a5, int a6);
int __cdecl sub_55CB20(int a1, char a2);
int __cdecl sub_55CDB0(int a1, char a2);
int __cdecl sub_55D6F0(int a1);
unsigned int __cdecl sub_55D740(int a1, int a2);
int __cdecl sub_55D770(int a1, int a2, int a3);
unsigned int __cdecl sub_55D800(int a1, int a2);
int __cdecl sub_55D830(int a1, int a2, int a3);
char __cdecl sub_55D8C0(int a1, int a2);
int __cdecl sub_55D910(int a1, int a2, int a3);
int __cdecl sub_55DA90(int a1, int a2, _DWORD* a3, _DWORD* a4);
int __cdecl sub_55DB20(_DWORD* a1, int a2, int a3, char a4, char a5, char* a6);
int __cdecl sub_55DF50(int a1, int a2, int a3);
int __cdecl sub_55E030(int a1, int a2);
int __cdecl sub_55E100(int a1, int a2, char a3);
int __cdecl sub_55E3D0(int a1, int a2);
int __cdecl sub_55E400(int a1);
int __cdecl sub_55E550(int a1, int a2);
int __cdecl sub_55E580(int a1, int a2);
int __cdecl sub_55E5B0(int a1);
int __cdecl sub_55E820(int a1, int a2);
int __cdecl sub_55E850(int a1, int a2);
int __cdecl sub_55E880(int a1, int a2);
int __cdecl sub_55E8B0(int a1);
int __cdecl sub_55EA60(int a1, int a2);
int __cdecl sub_55EA90(int a1, int a2);
int __cdecl sub_55EAC0(_DWORD* a1, int a2);
unsigned int __cdecl sub_55EBD0(int a1, int a2, int a3, unsigned int a4);
char __cdecl sub_55EC10(int a1, int a2, unsigned int a3);
int __cdecl sub_55ECE0(_DWORD* a1, int a2);
int __cdecl sub_55EDD0(int a1, int a2, _DWORD* a3, _DWORD* a4);
int __cdecl sub_55EE60(int a1);
int __cdecl sub_55EFC0(int a1, int a2);
int __cdecl sub_55EFF0(int a1, int a2);
int __cdecl sub_55F020(int a1);
int __cdecl sub_55F170(int a1, int a2);
int __cdecl sub_55F1A0(int a1, int a2);
int __cdecl sub_55F1D0(int a1, int a2);
int __cdecl sub_55F220(int a1, int a2);
int __cdecl sub_55F280(int a1, int a2);
int __cdecl sub_55F300(int a1, int a2);
int __cdecl sub_55F3F0(int a1, int a2);
int __cdecl sub_55F440(int a1, int a2);
int __cdecl sub_55F570(int a1, unsigned int a2);
int __cdecl sub_55F600(int a1, int a2);
int __cdecl sub_55F6D0(int a1, int a2);
int __cdecl sub_55F710(int a1, int a2);
int __cdecl sub_55F780(int a1, unsigned int a2);
int __cdecl sub_55F860(int a1, int a2);
int __cdecl sub_55FA20(int a1, int a2);
int __cdecl sub_55FBA0(int a1, int a2);
int __cdecl sub_55FC40(int a1);
int __cdecl sub_55FC50(int a1);
int __cdecl sub_55FE20(int a1, int a2);
char __cdecl sub_55FF30(int a1, int* a2);
int __cdecl sub_560030(int a1);
int __cdecl sub_560080(int a1);
signed int __cdecl sub_5603B0(int a1, int a2, int a3, int a4, int a5);
int __cdecl sub_560560(_DWORD* a1);
int __cdecl sub_5605C0(int a1);
int __cdecl sub_5605E0(int a1);
char __cdecl sub_560600(int a1);
int __cdecl sub_5606D0(_DWORD* a1);
int __cdecl sub_560760(int a1);
int __cdecl sub_5607B0(_DWORD* a1);
void __cdecl nullsub_31(_DWORD); // weak
int sub_560850();                // weak
int __cdecl sub_560860(int a1);
void __cdecl sub_560881(unsigned __int16* a1, _WORD* a2, __int16 a3, __int16 a4, unsigned int a5, int a6);
int __cdecl sub_561430(int a1);
int __cdecl sub_5614E0(int a1);
int __cdecl sub_561530(int a1);
char __cdecl sub_561560(int a1);
char __cdecl sub_561590(int a1);
int __cdecl sub_5615C0(int a1);
int __cdecl sub_561760(int a1);
int __cdecl sub_5618B0(_DWORD* a1, int a2);
int __cdecl sub_561980(int a1);
int __cdecl sub_562120(int a1);
int __cdecl sub_562640(int a1);
void* __cdecl sub_562770(int a1, int a2, LPVOID lpMem, int a4);
__int16 __cdecl sub_5627E0(int a1, _DWORD* a2, _DWORD* a3);
int __cdecl sub_562930(int a1, int a2, char* a3, int a4);
int __cdecl sub_562A90(_DWORD* a1, int a2, void* a3, signed int a4);
unsigned int __cdecl sub_562B60(int a1, int a2);
int __cdecl sub_562B90(int a1, int a2);
unsigned int __cdecl sub_562BA0(int a1);
int __cdecl sub_562BD0(_BYTE* a1, char* a2, int a3);
int __cdecl sub_562C9B(unsigned __int8* a1, char* a2, int a3);
int __cdecl sub_562E30(_WORD* a1, char* a2, int a3);
unsigned int __cdecl sub_563020(int a1, unsigned __int16* a2, _BYTE* a3, unsigned int a4, int a5, int a6);
unsigned int __cdecl sub_563470(int a1, unsigned __int16* a2, _BYTE* a3, unsigned int a4, int a5, int a6);
unsigned int __cdecl sub_563880(int a1, unsigned __int16* a2, _BYTE* a3, unsigned int a4, int a5, int a6);
unsigned int __cdecl sub_563C90(int a1, unsigned __int16* a2, _BYTE* a3, unsigned int a4, int a5, int a6);
unsigned int __cdecl sub_5640C0(int a1, unsigned __int16* a2, _BYTE* a3, unsigned int a4, int a5, int a6);
__int16 __cdecl sub_5644B0(int a1, _BYTE* a2, int* a3, int a4, int a5, int a6);
__int16 __cdecl sub_564546(int a1, _BYTE* a2, _DWORD* a3, int a4, int a5, int a6);
__int16 __cdecl sub_564604(int a1, _BYTE* a2, _WORD* a3, int a4, int a5, int a6);
__int16 __cdecl sub_56469E(int a1, _BYTE* a2, _WORD* a3, int a4, int a5, int a6);
__int16 __cdecl sub_564738(int a1, _BYTE* a2, int* a3, int a4, int a5, int a6);
__int16 __cdecl sub_564834(int a1, _BYTE* a2, int* a3, int a4, int a5, int a6);
__int16 __cdecl sub_5648E4(int a1, _BYTE* a2, _DWORD* a3, int a4, int a5, int a6);
__int16 __cdecl sub_5649DA(int a1, _BYTE* a2, _DWORD* a3, int a4, int a5, int a6);
int* __cdecl sub_564AC0(int a1, unsigned __int16* a2, int* a3, int a4, int a5, int a6);
int* __cdecl sub_564E60(int a1, _WORD* a2, int* a3, unsigned int a4, int a5, int a6);
unsigned int __cdecl sub_564FB0(int a1, unsigned __int8* a2, int* a3, unsigned int a4, int a5, int a6);
int* __cdecl sub_565360(int a1, _WORD* a2, int* a3, unsigned int a4, int a5, int a6);
unsigned int __cdecl sub_5654A0(int a1, unsigned __int8* a2, int* a3, unsigned int a4, int a5, int a6);
// int __cdecl operator_delete(LPVOID lpMem); idb
// FILE *__cdecl fopen(const char *, const char *);
// int __cdecl ftell(FILE *);
// int __cdecl _ftell_lk(_DWORD *a1);
// size_t __cdecl _fwrite_lk(void *a1, size_t a2, size_t a3, FILE *a4);
// size_t __cdecl fwrite(const void *, size_t, size_t, FILE *);
// int __cdecl fseek(FILE *, int, int);
// char *__cdecl strrchr(const char *, int);
// int sprintf(char *, const char *, ...);
// wchar_t *__cdecl wcscat(wchar_t *, const wchar_t *);
// wchar_t *__cdecl wcscpy(wchar_t *, const wchar_t *);
// wchar_t *__cdecl wcsncpy(wchar_t *, const wchar_t *, size_t);
// int sub_565D05();
// int _cfltcvt_init_0(void); weak
// _DWORD __cdecl __dtoxmode(_DWORD, unsigned __int8 *); weak
// int __cdecl _stat(LPCSTR lpFileName, int); idb
// _DWORD __cdecl IsRootUNCName(char *); weak
// size_t __cdecl _fread_lk(void *a1, size_t a2, size_t a3, FILE *a4);
// size_t __cdecl fread(void *, size_t, size_t, FILE *);
// void __cdecl qsort(void *, size_t, size_t, int (__cdecl *)(const void *, const void *));
// _DWORD __cdecl shortsort(_DWORD, _DWORD, _DWORD, _DWORD); weak
// _DWORD __cdecl swap(_DWORD, _DWORD, _DWORD); weak
// void *__cdecl bsearch(const void *, const void *, size_t, size_t, int (__cdecl *)(const void *, const void *));
// int __usercall __CxxFrameHandler@<eax>(int a1@<eax>, int a2, int a3, int a4, int a5);
// int __cdecl _CallCatchBlock2(struct EHRegistrationNode *a1, const struct _s_FuncInfo *a2, void *a3, int a4, unsigned
// int a5); int __cdecl _CallSETranslator(struct EHExceptionRecord *a1, struct EHRegistrationNode *a2, int a3, void *a4,
// const struct _s_FuncInfo *a5, int a6, struct EHRegistrationNode *a7); int __cdecl
// TranslatorGuardHandler(PEXCEPTION_RECORD ExceptionRecord, PVOID TargetFrame, int); idb int __cdecl fgetc(FILE *); int
// __cdecl getc(FILE *); void *__cdecl operator_new(unsigned int); weak char *__cdecl fgets(char *, int, FILE *); int
// __cdecl iswctype(WCHAR SrcStr, __int16); idb size_t __cdecl wcslen(const wchar_t *); size_t __cdecl strspn(const char
// *, const char *); double __cdecl atof(const char *); int fprintf(FILE *, const char *, ...); char *__cdecl
// asctime(const struct tm *); struct tm *__cdecl localtime(const time_t *); time_t __cdecl time(time_t *); int __cdecl
// vsprintf(char *, const char *, va_list); double __usercall _CIacos@<st0>(double x@<st0>); idb double __cdecl
// sub_566D1D(_DWORD, _DWORD); weak int __cdecl _read(int, void *, unsigned int); unsigned int __cdecl _read_lk(int a1,
// LPVOID lpBuffer, DWORD nNumberOfBytesToRead); int __cdecl wcscmp(const wchar_t *, const wchar_t *); int sscanf(const
// char *, const char *, ...); int __cdecl toupper(CHAR SrcStr); idb int __cdecl _access(LPCSTR lpFileName, char); idb
// int printf(const char *, ...);
// double __usercall _CIpow@<st0>(double x@<st0>, double y@<st1>); idb
// double __cdecl sub_567362(_DWORD, _DWORD, __int16, _DWORD, __int16); weak
// wchar_t *__cdecl wcstok(wchar_t *, const wchar_t *);
// char *__cdecl strstr(const char *, const char *);
// int __cdecl _itow(int, LPWSTR lpWideCharStr, int); idb
// int __cdecl _chmod(LPCSTR lpFileName, char); idb
// _onexit_t __cdecl _onexit(_onexit_t);
// int __cdecl atexit(void (__cdecl *)());
// int __cdecl vswprintf(wchar_t *, const wchar_t *, va_list);
// int __cdecl wcstol(const wchar_t *, wchar_t **, int);
// char *__cdecl strncat(char *, const char *, size_t);
// wchar_t *__cdecl wcschr(const wchar_t *, wchar_t);
// wchar_t *__cdecl wcsstr(const wchar_t *, const wchar_t *);
// clock_t __cdecl clock();
// size_t __cdecl wcsspn(const wchar_t *, const wchar_t *);
// int __cdecl iswdigit(WCHAR SrcStr); idb
// int __cdecl iswalnum(WCHAR SrcStr); idb
// int __cdecl _beginthread(int, SIZE_T dwStackSize, int); idb
// int fscanf(FILE *, const char *, ...);
// void __cdecl _makepath(char *, const char *, const char *, const char *, const char *);
// void __cdecl _splitpath(const char *, char *, char *, char *, char *);
// void *__cdecl memchr(const void *, int, size_t);
// double __cdecl modf(double, double *);
// int _open(const char *, int, ...);
// int __cdecl rename(LPCSTR lpExistingFileName, LPCSTR lpNewFileName); idb
// int __cdecl remove(LPCSTR lpFileName); idb
// int __cdecl _unlink(LPCSTR lpFileName); idb
// char *__cdecl _strrev(char *);
// int __cdecl _wcsicmp(const wchar_t *, const wchar_t *);
// int __cdecl _mkdir(LPCSTR lpPathName); idb
// int __cdecl _filelength(int);
// int _ms_p5_mp_test_fdiv(void); weak
// _DWORD __cdecl __loctotime_t(_DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD); weak
// int __cdecl _fullpath(LPSTR lpBuffer, LPCSTR lpFileName, DWORD nBufferLength); idb
// int __cdecl _getdrive();
// int __cdecl _mbctolower(CHAR SrcStr); idb
// unsigned __int8 *__cdecl _mbspbrk(const unsigned __int8 *, const unsigned __int8 *);
// void __cdecl terminate(); weak
// void __userpurge sub_569348(EXCEPTION_POINTERS *a1@<ecx>, int a2@<ebx>, int a3@<edi>, int a4@<esi>, int a5);
// _DWORD __cdecl __InternalCxxFrameHandler(_DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD); weak
// _DWORD __stdcall _CallSettingFrame(_DWORD, _DWORD, _DWORD); weak
// _DWORD __cdecl _fltin2(_DWORD, _DWORD); weak
// _DWORD __cdecl _checkTOS_withFB(_DWORD, _DWORD); weak
// int __cdecl _input(FILE *a1, unsigned __int8 *a2, int a3);
// int __cdecl _hextodec(WORD CharType); idb
// int __cdecl fgetc_0(FILE *);
// _DWORD __cdecl _un_inc(_DWORD, FILE *); weak
// _DWORD __cdecl _whiteout(_DWORD, FILE *); weak
// _DWORD __cdecl _ld12cvt(_DWORD, _DWORD, _DWORD); weak
// int __cdecl sub_56C0EA(int a1, int a2);
// int __cdecl sub_56C100(int a1, int a2);
// int __cdecl sub_56C116(int a1, int a2);
// int __cdecl sub_56C143(int a1, int a2);
// LONG __stdcall __CxxUnhandledExceptionFilter(struct _EXCEPTION_POINTERS *ExceptionInfo); idb
// int (__stdcall *sub_56C356())();
// LPTOP_LEVEL_EXCEPTION_FILTER sub_56C367();
// _DWORD __cdecl __strgtold12(_DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD, _DWORD); weak
// int __cdecl isspace(WORD CharType); idb
// int sub_56D9C4();
// HRESULT __stdcall DirectDrawCreate(GUID *lpGUID, LPDIRECTDRAW *lplpDD, IUnknown *pUnkOuter);
// HRESULT __stdcall DirectDrawEnumerateA(LPDDENUMCALLBACKA lpCallback, LPVOID lpContext);
// HRESULT __stdcall DirectSoundCreate(LPGUID, LPDIRECTSOUND *, LPUNKNOWN);
// HRESULT __stdcall DirectInputCreateA(HINSTANCE hinst, DWORD dwVersion, LPDIRECTINPUTA *ppDI, LPUNKNOWN punkOuter);
// u_long __stdcall htonl(u_long hostlong);
// uint16_t __stdcall ntohs(uint16_t netshort);
// uint16_t __stdcall htons(uint16_t hostshort);
// u_long __stdcall ntohl(u_long netlong);
// char *__stdcall inet_ntoa(struct in_addr in);
// unsigned int __stdcall inet_addr(const char *cp);
int sub_56F1C0();
int nox_xxx_protect_56F240();
int sub_56F250();
int __cdecl nox_xxx_protectionCreateStructForInt_56F280(int a1, int a2);
int __cdecl sub_56F2F0(_DWORD* a1);
_DWORD* sub_56F3B0();
int __cdecl nox_xxx_protectionCreateInt_56F400(int a1);
int __cdecl nox_xxx_protectionCreateFloat_56F440(int a1);
int __cdecl nox_xxx_protectionCreateStructForFloat_56F480(int a1, int a2);
int __cdecl sub_56F4F0(int* a1);
int __cdecl sub_56F510(int a1);
_DWORD* __cdecl sub_56F590(int a1);
int nox_xxx_protectData_56F5C0();
_DWORD* __cdecl sub_56F6F0(int a1);
void __cdecl sub_56F720(int* a1, int* a2);
_DWORD* __cdecl sub_56F780(int a1, int a2);
_DWORD* __cdecl nox_xxx_playerUpdateNetBuffs_56F7D0(int a1, int a2);
_DWORD* __cdecl sub_56F820(int a1, unsigned __int8 a2);
_DWORD* __cdecl nox_xxx_protectPlayerHPMana_56F870(int a1, unsigned __int16 a2);
_DWORD* __cdecl sub_56F8C0(int a1, float a2);
_DWORD* __cdecl sub_56F920(int a1, int a2);
_DWORD* __cdecl nox_xxx_protectMana_56F9E0(int a1, __int16 a2);
_DWORD* __cdecl sub_56FA40(int a1, float a2);
int __cdecl nox_xxx_protectionStringCRC_56FAC0(int* a1, unsigned int a2);
int __cdecl nox_xxx_protectionStringCRCLen_56FAE0(int* a1, unsigned int a2);
int __cdecl sub_56FB00(int* a1, unsigned int a2, int a3);
int __cdecl sub_56FB60(int* a1);
int __cdecl nox_xxx_protect_56FBF0(int a1, int* a2);
int __cdecl nox_xxx_protect_56FC50(int a1, int* a2);
int __cdecl sub_56FCB0(int a1, int a2);
int __cdecl nox_xxx_playerAwardSpellProtection_56FCE0(int a1, int a2, int a3);
int __cdecl sub_56FD50(int a1, int a2, int a3);
_BYTE* __cdecl nox_xxx_cryptXor_56FDD0(char a1, _BYTE* a2, int a3);
void __cdecl sub_56FE00(char a1, _BYTE* a2, int a3, _BYTE* a4);
double nox_xxx_unkDoubleSmth_56FE30();
void __cdecl sub_56FF00(int a1);
int __cdecl sub_56FF80(int a1, int a2);
int** __thiscall sub_56FFE0(int** this);
int** __thiscall sub_570070(int*** this);
int __thiscall nox_xxx_changeWinProcToEdit_5700CA(int** this, HWND hWnd);
int* __thiscall nox_xxx_changeWinProcToNormal_5700F6(int** this);
wchar_t* __thiscall nox_xxx_string_57011C(_DWORD** this);
_DWORD* __thiscall sub_570142(_DWORD** this, char a2);
bool __thiscall nox_xxx_string_5702B4(_DWORD** this);
int __thiscall nox_xxx_string_570392(_DWORD** this);
unsigned int __cdecl nox_xxx_netGetUnitCodeServ_578AC0(_DWORD* a1);
unsigned int __cdecl nox_xxx_netGetUnitCodeCli_578B00(int a1);
int __cdecl nox_xxx_netClearHighBit_578B30(__int16 a1);
int __cdecl nox_xxx_packetDynamicUnitCode_578B40(int a1);
unsigned int __cdecl nox_xxx_netTestHighBit_578B70(unsigned int a1);
_DWORD* sub_578B80();
unsigned int __cdecl sub_578BA0(unsigned int a1);
int __cdecl sub_578BB0(void** a1, int a2, unsigned __int8* a3, int a4);
int __cdecl sub_578BD0(LPVOID lpMem); // idb
_DWORD* sub_578BF0();
int __cdecl nox_xxx_nxzDecompress_578C10(_DWORD* a1, _BYTE* a2, _DWORD* a3, unsigned int a4, _DWORD* a5);
int __cdecl sub_578C30(int a1);
int __cdecl sub_578C40(LPVOID lpMem); // idb
int sub_578C60();
int __cdecl sub_578C90(int a1);
int sub_578CD0();
char* nox_xxx_GetEndgameDialog_578D80();
char __cdecl sub_578DE0(char a1);
unsigned __int8 sub_578DF0();
void sub_578E00();
SOCKET sub_578E10();
int __cdecl sub_578F20(int a1);
int __cdecl sub_579350(int a1);
int __cdecl sub_579700(int a1);
BOOL sub_579740();
int __cdecl sub_579750(int a1);
int sub_5797E0();
LPVOID nox_xxx_waypointGetList_579860();
int __cdecl nox_xxx_waypointNext_579870(int a1);
int sub_579890();
int __cdecl sub_5798A0(int a1);
unsigned int nox_xxx_waypoint_5798C0();
_DWORD* __cdecl nox_xxx_waypointNew_5798F0(float a1, float a2);
float* __cdecl nox_xxx_waypointNewNotMap_579970(int a1, float a2, float a3); // idb
char* nox_xxx_waypoint_5799C0();
char* sub_579A30();
float* __cdecl sub_579AD0(float a1, float a2);
void __cdecl sub_579B30(LPVOID lpMem); // idb
_DWORD* sub_579C00();
_DWORD* __cdecl nox_server_getWaypointById_579C40(int a1);
int __cdecl sub_579C60(int a1);
_DWORD* __cdecl sub_579C80(int a1);
int sub_579CA0();
int sub_579D20();
void nox_xxx_waypointDeleteAll_579DD0();
const char* __cdecl nox_xxx_waypointByName_579E30(const char* a1);
_DWORD* sub_579E70();
void __cdecl sub_579E90(int a1);
BOOL __cdecl sub_579EE0(int a1, unsigned __int8 a2);
int __cdecl nox_xxx_waypoint_579F00(_DWORD* a1, int a2);
int __cdecl nox_xxx_playerCanTalkMB_57A160(int a1);
int __cdecl nox_xxx_giant_57A190(int a1);
char* __cdecl sub_57A1B0(__int16 a1);
char __cdecl sub_57A1E0(int* a1, const char* a2, int* a3, char a4, __int16 a5); // idb
int __cdecl sub_57A3F0(char* a1, int a2, int a3, int a4);
const wchar_t* __cdecl sub_57A4D0(wchar_t* a1, int a2, int a3, int a4);
BOOL __cdecl sub_57A620(unsigned __int8 a1, const wchar_t** a2, int a3, int a4);
int __cdecl sub_57A950(char* a1);
int __cdecl sub_57A9F0(const char* a1, const char* a2);
char __cdecl sub_57AAA0(const char* a1, char* a2, int* a3);
int* __cdecl sub_57ADF0(int* a1);
int __cdecl sub_57AE30(const char* a1);
void sub_57AE90();
int __cdecl nox_xxx_playerCheckSpellClass_57AEA0(int a1, int a2);
BOOL __cdecl sub_57AEE0(int a1, int a2);
void sub_57AF10();
int nox_xxx_get_57AF20();
void sub_57B0A0();
__int64 nox_xxx___Getcvt_57B180();
int __cdecl sub_57B190(unsigned __int16 a1, unsigned __int16 a2);
void nox_xxx_loadBaseValues_57B200();
float* sub_57B350();
float* __cdecl nox_xxx_plrGetMaxVarsPtr_57B360(int a1);
char __cdecl sub_57B370(int a1, unsigned __int8 a2, int a3);
BOOL __cdecl nox_xxx_playerClassCanUseItem_57B3D0(int a1, char a2);
int __cdecl nox_xxx_client_57B400(int a1);
BOOL __cdecl sub_57B450(int* a1);
int __cdecl sub_57B4D0(int a1);
char __cdecl sub_57B500(int a1, int a2, char a3);
char __cdecl sub_57B630(int a1, int a2, int a3);
float2* __cdecl sub_57B770(float2* a1, float2* a2); // idb
int __cdecl nox_xxx_collideReflect_57B810(float* a1, int a2);
BOOL __cdecl nox_xxx_map_57B850(float2* a1, float* a2, float2* a3); // idb
int __cdecl sub_57B920(void* a1);
char __cdecl sub_57B930(int a1, int a2, int a3, unsigned int a4);
char __cdecl nox_xxx_cliGenerateAlias_57B9A0(int a1, int a2, int a3, unsigned int a4);
int __cdecl sub_57BA10(int a1, __int16 a2, __int16 a3, int a4);
int __cdecl sub_57BA30(int2* a1, int2* a2, int4* a3); // idb
int __cdecl nox_xxx_getToken_57BBC0(FILE* a1, int a2, int a3);
int __cdecl nox_xxx_mapNxzDecompress_57BC50(char* a1, char* a2);
int __cdecl nox_xxx_mapFile_57BDD0(LPVOID lpMem, int); // idb
BOOL __cdecl sub_57BF20(int a1, int a2);
BOOL __cdecl sub_57BF40(int a1, int a2);
int sub_57BF60();
int sub_57BF70();
unsigned int sub_57BF80();
char* nox_xxx_allocGroupRelatedArrays_57BFB0();
void sub_57C000();
int sub_57C030();
int nox_server_getFirstMapGroup_57C080();
int __cdecl nox_server_getNextMapGroup_57C090(int a1);
int __cdecl nox_server_scriptGetGroup_57C0A0(int a1);
int __cdecl nox_server_mapLoadAddGroup_57C0C0(char* a1, int a2, char a3);
int __cdecl sub_57C130(_DWORD* a1, int a2);
int __cdecl sub_57C1E0(_DWORD* a1, int a2);
int __cdecl nox_server_scriptGetMapGroupByName_57C280(const char* a1, int a2);
void* sub_57C330();
void* sub_57C360();
void __cdecl sub_57C370(_QWORD* a1);
void __cdecl sub_57C390(_QWORD* a1);
int __cdecl nox_server_addNewMapGroup_57C3B0(int a1);
int nox_xxx_getDebugData_57C3E0();
int __cdecl nox_xxx_nextDebugObject_57C3F0(int a1);
BOOL nox_xxx_allocDebugDataArray_57C410();
void sub_57C440();
int sub_57C460();
int __cdecl sub_57C490(const char* a1);
int __cdecl sub_57C500(const char* a1, const char* a2);
void __cdecl sub_57C5A0(const char* a1);
void __cdecl sub_57C650(float* a1, float* a2, float* a3);
void __cdecl sub_57C790(float4* a1, float2* a2, float2* a3, float a4);  // idb
BOOL __cdecl nox_xxx_mathPointOnTheLine_57C8A0(float4* a1, float2* a2, float2* a3);            // idb
char __cdecl nox_xxx_mapTraceRayImpl_57C9A0(int a1, int a2, float* a3, float* a4, char a5); // idb
float2* __cdecl sub_57CD30(float4* a1, float2* a2, float2* a3);         // idb
float2* __cdecl sub_57CD70(float4* a1, float2* a2, float2* a3);         // idb
int __cdecl sub_57CDB0(int2* a1, float* a2, float2* a3);                // idb
void __thiscall sub_57D150(LPVOID* this);
int __thiscall sub_57D1C0(void** this, int a2, unsigned __int8* a3, int a4);
void __thiscall sub_57DD70(LPVOID* this);
int __thiscall nullsub_71(_DWORD); // weak
_DWORD* __thiscall sub_57DD90(_DWORD* this);
void __thiscall sub_57DDC0(LPVOID* this);
int __thiscall sub_57DDD0(void** this);
unsigned int __cdecl sub_57DDE0(int a1, int a2);
int __thiscall sub_57DEA0(_DWORD* this, _WORD* a2);
_DWORD* __thiscall sub_57DF00(_DWORD* this);
void __thiscall sub_57DF70(LPVOID* this);
int __thiscall sub_57DFC0(_DWORD* this, int* a2);
int __thiscall sub_57E2C0(_DWORD** this);
unsigned int __thiscall sub_57E3F0(_DWORD** this, unsigned int a2, int a3);
int __thiscall sub_57E4C0(_DWORD** this, unsigned int a2, int a3, unsigned int a4, unsigned int a5);
int __thiscall sub_57E7D0(_DWORD** this);
_DWORD* __thiscall sub_57E8A0(_DWORD* this);
void __thiscall sub_57E910(LPVOID* this);
int __thiscall sub_57E970(void** this);
_DWORD* __thiscall sub_57E9A0(_DWORD* this);
void __thiscall sub_57EA00(LPVOID* this);
int __thiscall sub_57EA60(int this);
int __thiscall nox_xxx_nxzDecompressImpl_57EA80(_DWORD* this, _BYTE* a2, _DWORD* a3, unsigned int a4, _DWORD* a5);
_DWORD* __thiscall sub_57F160(int this, int a2, int a3);
char __cdecl sub_57F1D0(float2* a1);                // idb
int __cdecl sub_57F2A0(float2* a1, int a2, int a3); // idb
nox_memfile* nox_open_thing_bin(void);
void nox_free_thing_bin(void);
bool nox_ensure_thing_bin(void);
// void __stdcall std___Xlen();
// _DWORD __stdcall std__logic_error__logic_error(_DWORD); weak
// int unknown_libname_34(void); weak
// void *__thiscall sub_57F445(LPVOID lpMem, char a2);
// _DWORD __thiscall std__logic_error__logic_error(std__logic_error *__hidden this, const struct std__logic_error *);
// weak int unknown_libname_35(void); weak void *__thiscall sub_57F518(LPVOID lpMem, char a2); void __cdecl
// std___Xran(); weak int unknown_libname_36(void); weak void __thiscall sub_57F5E3(const struct std__logic_error
// *this); void *__thiscall sub_57F600(LPVOID lpMem, char a2); std__logic_error *__thiscall sub_57F61C(std__logic_error
// *this, const struct std__logic_error *a2); int sub_57F65E(); _DWORD __thiscall std___Lockit___Lockit(std___Lockit
// *__hidden this); weak void __thiscall std___Lockit__destructor_Lockit(std___Lockit *__hidden this); weak LONG
// __stdcall ImmGetCompositionStringA(HIMC, DWORD, LPVOID lpBuf, DWORD dwBufLen); BOOL __stdcall
// ImmSetCompositionStringA(HIMC, DWORD dwIndex, LPVOID lpComp, DWORD dwCompLen, LPVOID lpRead, DWORD dwReadLen); BOOL
// __stdcall ImmNotifyIME(HIMC, DWORD dwAction, DWORD dwIndex, DWORD dwValue); BOOL __stdcall ImmGetOpenStatus(HIMC);
// BOOL __stdcall ImmSetOpenStatus(HIMC, BOOL);
// BOOL __stdcall ImmSetCompositionWindow(HIMC, LPCOMPOSITIONFORM lpCompForm);
// DWORD __stdcall ImmGetCandidateListA(HIMC, DWORD deIndex, LPCANDIDATELIST lpCandList, DWORD dwBufLen);
// BOOL __stdcall ImmDestroyContext(HIMC);
// UINT __stdcall ImmGetDescriptionA(HKL, LPSTR lpszDescription, UINT uBufLen);
// DWORD __stdcall ImmGetProperty(HKL, DWORD);
// HIMC __stdcall ImmCreateContext();
// HIMC __stdcall ImmGetContext(HWND);
// BOOL __stdcall ImmReleaseContext(HWND, HIMC);
// int __cdecl floor(double); idb
// int __cdecl fputs(const char *, FILE *);
// void __stdcall __noreturn _CxxThrowException(_DWORD, _DWORD); weak
// int __usercall sub_57FA20@<eax>(int a1@<ebp>);
// int __usercall sub_57FA28@<eax>(int a1@<ebp>);
int __cdecl SEH_40E260(int a1, int a2, int a3, int a4);
// int __usercall sub_57FA40@<eax>(int a1@<ebp>);
// int __usercall sub_57FA48@<eax>(int a1@<ebp>);
int __cdecl SEH_40E320(int a1, int a2, int a3, int a4);
// int __usercall sub_57FB20@<eax>(int a1@<ebp>);
// int __usercall sub_57FB2B@<eax>(int a1@<ebp>);
// int __usercall sub_57FB36@<eax>(int a1@<ebp>);
// int __usercall sub_57FB41@<eax>(int a1@<ebp>);
// int __usercall sub_57FB57@<eax>(int a1@<ebp>);
// int __usercall sub_57FB62@<eax>(int a1@<ebp>);
// int __usercall sub_57FB6D@<eax>(int a1@<ebp>);
// int __usercall sub_57FBB0@<eax>(int a1@<ebp>);
// int __usercall sub_57FBB8@<eax>(int a1@<ebp>);
int __cdecl SEH_556500(int a1, int a2, int a3, int a4);
// int __usercall sub_57FBD0@<eax>(int a1@<ebp>);
// int __usercall sub_57FBD8@<eax>(int a1@<ebp>);
int __cdecl SEH_556570(int a1, int a2, int a3, int a4);
// int __usercall sub_57FBF0@<eax>(int a1@<ebp>);
// int __usercall sub_57FBF8@<eax>(int a1@<ebp>);
int __cdecl SEH_5565E0(int a1, int a2, int a3, int a4);
// int __usercall sub_57FC50@<eax>(int a1@<ebp>);
int __cdecl SEH_557C70(int a1, int a2, int a3, int a4);
// wstring *__usercall sub_57FC90@<eax>(int a1@<ebp>);
int __cdecl SEH_5704C0(int a1, int a2, int a3, int a4);
// wstring *__usercall sub_57FCB0@<eax>(int a1@<ebp>);
int __cdecl SEH_570570(int a1, int a2, int a3, int a4);
void sub_57FCD0();
int __cdecl SEH_570B90(int a1, int a2, int a3, int a4);
// wstring *__usercall sub_57FD00@<eax>(int a1@<ebp>);
int __cdecl SEH_571810(int a1, int a2, int a3, int a4);
// int *__usercall sub_57FD13@<eax>(int a1@<ebp>);
// int *__usercall sub_57FD1C@<eax>(int a1@<ebp>);
// int *__usercall sub_57FD25@<eax>(int a1@<ebp>);
// int *__usercall sub_57FD2E@<eax>(int a1@<ebp>);
int __cdecl SEH_571C0E(int a1, int a2, int a3, int a4);
// wstring *__usercall sub_57FD41@<eax>(int a1@<ebp>);
// wstring *__usercall sub_57FD4D@<eax>(int a1@<ebp>);
// wstring *__usercall sub_57FD59@<eax>(int a1@<ebp>);
int __cdecl SEH_572045(int a1, int a2, int a3, int a4);
// wstring *__usercall sub_57FD6F@<eax>(int a1@<ebp>);
// wstring *__usercall sub_57FD7B@<eax>(int a1@<ebp>);
// wstring *__usercall sub_57FD87@<eax>(int a1@<ebp>);
int __cdecl SEH_57211C(int a1, int a2, int a3, int a4);
// BOOL __usercall sub_57FD9D@<eax>(int a1@<ebp>);
// wstring *__usercall sub_57FDA6@<eax>(int a1@<ebp>);
int __cdecl SEH_57222A(int a1, int a2, int a3, int a4);
// BOOL __usercall sub_57FDB9@<eax>(int a1@<ebp>);
// wstring *__usercall sub_57FDC2@<eax>(int a1@<ebp>);
int __cdecl SEH_572333(int a1, int a2, int a3, int a4);
// BOOL __usercall sub_57FDD5@<eax>(int a1@<ebp>);
// wstring *__usercall sub_57FDDE@<eax>(int a1@<ebp>);
int __cdecl SEH_572442(int a1, int a2, int a3, int a4);
// BOOL __usercall sub_57FE07@<eax>(int a1@<ebp>);
int __cdecl SEH_572A06(int a1, int a2, int a3, int a4);
// int *__usercall sub_57FE1A@<eax>(int a1@<ebp>);
// int *__usercall sub_57FE23@<eax>(int a1@<ebp>);
int __cdecl SEH_572BD6(int a1, int a2, int a3, int a4);
// BOOL __usercall sub_57FE36@<eax>(int a1@<ebp>);
// int **__usercall sub_57FE3F@<eax>(int a1@<ebp>);
int __cdecl SEH_573153(int a1, int a2, int a3, int a4);
// BOOL __usercall sub_57FE52@<eax>(int a1@<ebp>);
// int **__usercall sub_57FE5B@<eax>(int a1@<ebp>);
int __cdecl SEH_57330C(int a1, int a2, int a3, int a4);
// LPVOID *__usercall sub_57FE6E@<eax>(int a1@<ebp>);
// wstring *__usercall sub_57FE77@<eax>(int a1@<ebp>);
// wstring *__usercall sub_57FE80@<eax>(int a1@<ebp>);
int __cdecl SEH_573401(int a1, int a2, int a3, int a4);
// BOOL __usercall sub_57FE93@<eax>(int a1@<ebp>);
int __cdecl SEH_57366C(int a1, int a2, int a3, int a4);
// BOOL __usercall sub_57FEA6@<eax>(int a1@<ebp>);
int __cdecl SEH_57381A(int a1, int a2, int a3, int a4);
// BOOL __usercall sub_57FEB9@<eax>(int a1@<ebp>);
int __cdecl SEH_57390B(int a1, int a2, int a3, int a4);
// BOOL __usercall sub_57FECC@<eax>(int a1@<ebp>);
int __cdecl SEH_5739D2(int a1, int a2, int a3, int a4);
// void __usercall sub_57FEE0(int a1@<ebp>);
int __cdecl SEH_574410(int a1, int a2, int a3, int a4);
// void __usercall sub_57FF00(int a1@<ebp>);
int __cdecl SEH_5755F0(int a1, int a2, int a3, int a4);
// void __usercall sub_57FF20(int a1@<ebp>);
int __cdecl SEH_575E60(int a1, int a2, int a3, int a4);
// void __usercall sub_57FF40(int a1@<ebp>);
int __cdecl SEH_576A20(int a1, int a2, int a3, int a4);
// void __usercall sub_57FF80(int a1@<ebp>);
int __cdecl SEH_576DA0(int a1, int a2, int a3, int a4);
// void __usercall sub_57FFC0(int a1@<ebp>);
int __cdecl SEH_577060(int a1, int a2, int a3, int a4);
// void __usercall sub_57FFE0(int a1@<ebp>);
int __cdecl SEH_577100(int a1, int a2, int a3, int a4);
int __cdecl SEH_577760(int a1, int a2, int a3, int a4);
// void __usercall sub_580050(int a1@<ebp>);
int __cdecl SEH_577F90(int a1, int a2, int a3, int a4);
// void __usercall sub_580070(int a1@<ebp>);
int __cdecl SEH_578240(int a1, int a2, int a3, int a4);
// void __usercall sub_580090(int a1@<ebp>);
int __cdecl SEH_578370(int a1, int a2, int a3, int a4);
void sub_5800B0();
int __cdecl SEH_578810(int a1, int a2, int a3, int a4);
// int __usercall sub_5800D8@<eax>(int a1@<ebp>);
// int __usercall sub_5800F5@<eax>(int a1@<ebp>);
// int __usercall sub_58010A@<eax>(int a1@<ebp>);
int __cdecl SEH_57D1C0(int a1, int a2, int a3, int a4);
// int __usercall sub_580120@<eax>(int a1@<ebp>);
int __cdecl SEH_57DF00(int a1, int a2, int a3, int a4);
// int __usercall sub_580132@<eax>(int a1@<ebp>);
int __cdecl SEH_57DF70(int a1, int a2, int a3, int a4);
// int __usercall sub_580144@<eax>(int a1@<ebp>);
int __cdecl SEH_57E8A0(int a1, int a2, int a3, int a4);
// int __usercall sub_580156@<eax>(int a1@<ebp>);
int __cdecl SEH_57E910(int a1, int a2, int a3, int a4);
// int __usercall sub_58017A@<eax>(int a1@<ebp>);
int __cdecl SEH_57EA00(int a1, int a2, int a3, int a4);
// LSTATUS __stdcall RegCreateKeyExA(HKEY hKey, LPCSTR lpSubKey, DWORD Reserved, LPSTR lpClass, DWORD dwOptions, REGSAM
// samDesired, const LPSECURITY_ATTRIBUTES lpSecurityAttributes, PHKEY phkResult, LPDWORD lpdwDisposition); LSTATUS
// __stdcall RegCloseKey(HKEY hKey); LSTATUS __stdcall RegQueryValueExA(HKEY hKey, LPCSTR lpValueName, LPDWORD
// lpReserved, LPDWORD lpType, LPBYTE lpData, LPDWORD lpcbData); LSTATUS __stdcall RegOpenKeyExA(HKEY hKey, LPCSTR
// lpSubKey, DWORD ulOptions, REGSAM samDesired, PHKEY phkResult); LSTATUS __stdcall RegSetValueExA(HKEY hKey, LPCSTR
// lpValueName, DWORD Reserved, DWORD dwType, const BYTE *lpData, DWORD cbData); UINT __stdcall RealizePalette(HDC hdc);
// HPALETTE __stdcall SelectPalette(HDC hdc, HPALETTE hPal, BOOL bForceBkgd);
// HPALETTE __stdcall CreatePalette(const LOGPALETTE *plpal);
// BOOL __stdcall DeleteObject(HGDIOBJ ho);
// BOOL __stdcall DeleteDC(HDC hdc);
// UINT __stdcall GetSystemPaletteEntries(HDC hdc, UINT iStart, UINT cEntries, LPPALETTEENTRY pPalEntries);
// HDC __stdcall CreateICA(LPCSTR pszDriver, LPCSTR pszDevice, LPCSTR pszPort, const DEVMODEA *pdm);
// BOOL __stdcall TextOutA(HDC hdc, int x, int y, LPCSTR lpString, int c);
// HGDIOBJ __stdcall GetStockObject(int i);
// int __stdcall GetDeviceCaps(HDC hdc, int index);
// BOOL __stdcall HeapDestroy(HANDLE hHeap);
// LONG __stdcall InterlockedIncrement(volatile LONG *lpAddend);
// DWORD __stdcall SetFilePointer(HANDLE hFile, LONG lDistanceToMove, PLONG lpDistanceToMoveHigh, DWORD dwMoveMethod);
// LONG __stdcall InterlockedDecrement(volatile LONG *lpAddend);
// int __stdcall MultiByteToWideChar(UINT CodePage, DWORD dwFlags, LPCSTR lpMultiByteStr, int cbMultiByte, LPWSTR
// lpWideCharStr, int cchWideChar); BOOL __stdcall GetVersionExA(LPOSVERSIONINFOA lpVersionInformation); BOOL __stdcall
// CloseHandle(HANDLE hObject); int __stdcall WideCharToMultiByte(UINT CodePage, DWORD dwFlags, LPCWSTR lpWideCharStr,
// int cchWideChar, LPSTR lpMultiByteStr, int cbMultiByte, LPCSTR lpDefaultChar, LPBOOL lpUsedDefaultChar); void
// __stdcall LeaveCriticalSection(LPCRITICAL_SECTION lpCriticalSection); void __stdcall
// EnterCriticalSection(LPCRITICAL_SECTION lpCriticalSection); DWORD __stdcall
// GetTimeZoneInformation(LPTIME_ZONE_INFORMATION lpTimeZoneInformation); void __stdcall GetLocalTime(LPSYSTEMTIME
// lpSystemTime); BOOL __stdcall ReadFile(HANDLE hFile, LPVOID lpBuffer, DWORD nNumberOfBytesToRead, LPDWORD
// lpNumberOfBytesRead, LPOVERLAPPED lpOverlapped); HANDLE __stdcall CreateFileA(LPCSTR lpFileName, DWORD
// dwDesiredAccess, DWORD dwShareMode, LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwCreationDisposition, DWORD
// dwFlagsAndAttributes, HANDLE hTemplateFile); HANDLE __stdcall OpenMutexA(DWORD dwDesiredAccess, BOOL bInheritHandle,
// LPCSTR lpName); HANDLE __stdcall CreateMutexA(LPSECURITY_ATTRIBUTES lpMutexAttributes, BOOL bInitialOwner, LPCSTR
// lpName); BOOL __stdcall QueryPerformanceFrequency(LARGE_INTEGER *lpFrequency); BOOL __stdcall
// QueryPerformanceCounter(LARGE_INTEGER *lpPerformanceCount); BOOL __stdcall CreateDirectoryA(LPCSTR lpPathName,
// LPSECURITY_ATTRIBUTES lpSecurityAttributes); BOOL __stdcall SetVolumeLabelA(LPCSTR lpRootPathName, LPCSTR
// lpVolumeName); DWORD __stdcall GetLogicalDriveStringsA(DWORD nBufferLength, LPSTR lpBuffer); BOOL __stdcall
// GetVolumeInformationA(LPCSTR lpRootPathName, LPSTR lpVolumeNameBuffer, DWORD nVolumeNameSize, LPDWORD
// lpVolumeSerialNumber, LPDWORD lpMaximumComponentLength, LPDWORD lpFileSystemFlags, LPSTR lpFileSystemNameBuffer,
// DWORD nFileSystemNameSize); BOOL __stdcall GetDiskFreeSpaceA(LPCSTR lpRootPathName, LPDWORD lpSectorsPerCluster,
// LPDWORD lpBytesPerSector, LPDWORD lpNumberOfFreeClusters, LPDWORD lpTotalNumberOfClusters); BOOL __stdcall
// FindClose(HANDLE hFindFile); BOOL __stdcall FindNextFileA(HANDLE hFindFile, LPWIN32_FIND_DATAA lpFindFileData); LONG
// __stdcall CompareFileTime(const FILETIME *lpFileTime1, const FILETIME *lpFileTime2); BOOL __stdcall
// SystemTimeToFileTime(const SYSTEMTIME *lpSystemTime, LPFILETIME lpFileTime); int __stdcall GetTimeFormatA(LCID
// Locale, DWORD dwFlags, const SYSTEMTIME *lpTime, LPCSTR lpFormat, LPSTR lpTimeStr, int cchTime); int __stdcall
// GetDateFormatA(LCID Locale, DWORD dwFlags, const SYSTEMTIME *lpDate, LPCSTR lpFormat, LPSTR lpDateStr, int cchDate);
// BOOL __stdcall SetCurrentDirectoryA(LPCSTR lpPathName);
// DWORD __stdcall WaitForSingleObject(HANDLE hHandle, DWORD dwMilliseconds);
// int __stdcall MulDiv(int nNumber, int nNumerator, int nDenominator);
// BOOL __stdcall MoveFileA(LPCSTR lpExistingFileName, LPCSTR lpNewFileName);
// BOOL __stdcall RemoveDirectoryA(LPCSTR lpPathName);
// BOOL __stdcall CopyFileA(LPCSTR lpExistingFileName, LPCSTR lpNewFileName, BOOL bFailIfExists);
// BOOL __stdcall ReleaseMutex(HANDLE hMutex);
// BOOL __stdcall SetEvent(HANDLE hEvent);
// BOOL __stdcall FileTimeToSystemTime(const FILETIME *lpFileTime, LPSYSTEMTIME lpSystemTime);
// BOOL __stdcall FileTimeToLocalFileTime(const FILETIME *lpFileTime, LPFILETIME lpLocalFileTime);
// void __stdcall GetSystemTime(LPSYSTEMTIME lpSystemTime);
// LPTOP_LEVEL_EXCEPTION_FILTER __stdcall SetUnhandledExceptionFilter(LPTOP_LEVEL_EXCEPTION_FILTER
// lpTopLevelExceptionFilter); DWORD __stdcall GetModuleFileNameA(HMODULE hModule, LPSTR lpFilename, DWORD nSize); BOOL
// __stdcall DeleteFileA(LPCSTR lpFileName); UINT __stdcall GetDriveTypeA(LPCSTR lpRootPathName); DWORD __stdcall
// GetCurrentDirectoryA(DWORD nBufferLength, LPSTR lpBuffer); void __stdcall DeleteCriticalSection(LPCRITICAL_SECTION
// lpCriticalSection); void __stdcall GlobalMemoryStatus(LPMEMORYSTATUS lpBuffer); void __stdcall
// InitializeCriticalSection(LPCRITICAL_SECTION lpCriticalSection); DWORD __stdcall GetLastError(); HANDLE __stdcall
// FindFirstFileA(LPCSTR lpFileName, LPWIN32_FIND_DATAA lpFindFileData); BOOL __stdcall IsDBCSLeadByte(BYTE TestChar);
// HINSTANCE __stdcall ShellExecuteA(HWND hwnd, LPCSTR lpOperation, LPCSTR lpFile, LPCSTR lpParameters, LPCSTR
// lpDirectory, INT nShowCmd); SHORT __stdcall GetAsyncKeyState(int vKey); BOOL __stdcall CopyRect(LPRECT lprcDst, const
// RECT *lprcSrc); int __stdcall ShowCursor(BOOL bShow); HDC __stdcall GetWindowDC(HWND hWnd); int __stdcall
// GetSystemMetrics(int nIndex); BOOL __stdcall AdjustWindowRect(LPRECT lpRect, DWORD dwStyle, BOOL bMenu); BOOL
// __stdcall SetWindowPos(HWND hWnd, HWND hWndInsertAfter, int X, int Y, int cx, int cy, UINT uFlags); HMENU __stdcall
// GetMenu(HWND hWnd); BOOL __stdcall IsRectEmpty(const RECT *lprc); int __stdcall MessageBoxA(HWND hWnd, LPCSTR lpText,
// LPCSTR lpCaption, UINT uType); BOOL __stdcall PeekMessageA(LPMSG lpMsg, HWND hWnd, UINT wMsgFilterMin, UINT
// wMsgFilterMax, UINT wRemoveMsg); LRESULT __stdcall DispatchMessageA(const MSG *lpMsg); BOOL __stdcall
// GetMessageA(LPMSG lpMsg, HWND hWnd, UINT wMsgFilterMin, UINT wMsgFilterMax); BOOL __stdcall TranslateMessage(const
// MSG *lpMsg); BOOL __stdcall IsWindow(HWND hWnd); HCURSOR __stdcall SetCursor(HCURSOR hCursor); void __stdcall
// PostQuitMessage(int nExitCode); LONG __stdcall GetWindowLongA(HWND hWnd, int nIndex); BOOL __stdcall
// ClientToScreen(HWND hWnd, LPPOINT lpPoint); LONG __stdcall SetWindowLongA(HWND hWnd, int nIndex, LONG dwNewLong);
// BOOL __stdcall OffsetRect(LPRECT lprc, int dx, int dy);
// BOOL __stdcall SetRect(LPRECT lprc, int xLeft, int yTop, int xRight, int yBottom);
// HDC __stdcall GetDC(HWND hWnd);
// int __stdcall ReleaseDC(HWND hWnd, HDC hDC);
// BOOL __stdcall IsIconic(HWND hWnd);
// int wsprintfA(LPSTR, LPCSTR, ...);
// BOOL __stdcall PostMessageA(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam);
// BOOL __stdcall GetClientRect(HWND hWnd, LPRECT lpRect);
// LRESULT __stdcall DefWindowProcA(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam);
// BOOL __stdcall SetWindowTextA(HWND hWnd, LPCSTR lpString);
// HWND __stdcall FindWindowA(LPCSTR lpClassName, LPCSTR lpWindowName);
// BOOL __stdcall SetForegroundWindow(HWND hWnd);
// BOOL __stdcall ShowWindow(HWND hWnd, int nCmdShow);
// HICON __stdcall LoadIconA(HINSTANCE hInstance, LPCSTR lpIconName);
// HCURSOR __stdcall LoadCursorA(HINSTANCE hInstance, LPCSTR lpCursorName);
// ATOM __stdcall RegisterClassExA(const WNDCLASSEXA *);
// HWND __stdcall CreateWindowExA(DWORD dwExStyle, LPCSTR lpClassName, LPCSTR lpWindowName, DWORD dwStyle, int X, int Y,
// int nWidth, int nHeight, HWND hWndParent, HMENU hMenu, HINSTANCE hInstance, LPVOID lpParam); BOOL __stdcall
// DestroyWindow(HWND hWnd); BOOL __stdcall UnregisterClassA(LPCSTR lpClassName, HINSTANCE hInstance); BOOL __stdcall
// ClipCursor(const RECT *lpRect); SHORT __stdcall GetKeyState(int nVirtKey); HKL __stdcall GetKeyboardLayout(DWORD
// idThread); int __stdcall GetKeyboardLayoutList(int nBuff, HKL *lpList); MMRESULT __stdcall timeKillEvent(UINT
// uTimerID); MMRESULT __stdcall timeEndPeriod(UINT uPeriod); MMRESULT __stdcall timeBeginPeriod(UINT uPeriod); MMRESULT
// __stdcall timeSetEvent(UINT uDelay, UINT uResolution, LPTIMECALLBACK fptc, DWORD_PTR dwUser, UINT fuEvent); UINT
// __stdcall joyGetNumDevs(); MMRESULT __stdcall joyGetPosEx(UINT uJoyID, LPJOYINFOEX pji); MMRESULT __stdcall
// joyGetDevCapsA(UINT_PTR uJoyID, LPJOYCAPSA pjc, UINT cbjc); MCIERROR __stdcall mciSendCommandA(MCIDEVICEID mciId,
// UINT uMsg, DWORD_PTR dwParam1, DWORD_PTR dwParam2); SOCKET __stdcall socket(int af, int type, int protocol); int
// __stdcall WSACleanup(); int __stdcall setsockopt(SOCKET s, int level, int optname, const char *optval, int optlen);
// int __stdcall bind(SOCKET s, const struct sockaddr *name, int namelen);
// int __stdcall WSAGetLastError();
// int __stdcall gethostname(char *name, int namelen);
// struct hostent *__stdcall gethostbyname(const char *name);
// int __stdcall ioctlsocket(SOCKET s, int cmd, u_long *argp);
// int __stdcall recvfrom(SOCKET s, char *buf, int len, int flags, struct sockaddr *from, int *fromlen);
// int __stdcall sendto(SOCKET s, const char *buf, int len, int flags, const struct sockaddr *to, int tolen);
// unsigned int __stdcall inet_addr(const char *cp);
// char *__stdcall inet_ntoa(struct in_addr in);
// int __stdcall WSAStartup(WORD wVersionRequested, LPWSADATA lpWSAData);
// int __stdcall closesocket(SOCKET s);
// int __stdcall listen(SOCKET s, int backlog);
// int __stdcall shutdown(SOCKET s, int how);
// int __stdcall send(SOCKET s, const char *buf, int len, int flags);
// SOCKET __stdcall accept(SOCKET s, struct sockaddr *addr, int *addrlen);
// int __stdcall recv(SOCKET s, char *buf, int len, int flags);
// u_long __stdcall htonl(u_long hostlong);
// uint16_t __stdcall ntohs(uint16_t netshort);
// uint16_t __stdcall htons(uint16_t hostshort);
// int __stdcall AIL_waveOutClose(_DWORD); weak
// int __stdcall AIL_digital_configuration(_DWORD, _DWORD, _DWORD, _DWORD); weak
// int __stdcall AIL_set_timer_frequency(_DWORD, _DWORD); weak
// int __stdcall AIL_set_sample_user_data(_DWORD, _DWORD, _DWORD); weak
// int __stdcall AIL_allocate_sample_handle(_DWORD); weak
// _DWORD __stdcall AIL_last_error(); weak
// int __stdcall AIL_sample_user_data(_DWORD, _DWORD); weak
// int __stdcall AIL_register_EOS_callback(_DWORD, _DWORD); weak
// int __stdcall AIL_register_EOB_callback(_DWORD, _DWORD); weak
// int __stdcall AIL_set_sample_adpcm_block_size(_DWORD, _DWORD); weak
// int __stdcall AIL_set_sample_type(_DWORD, _DWORD, _DWORD); weak
// int __stdcall AIL_get_preference(_DWORD); weak
// int __stdcall AIL_set_preference(_DWORD, _DWORD); weak
// int __stdcall AIL_load_sample_buffer(_DWORD, _DWORD, _DWORD, _DWORD); weak
// int __stdcall AIL_sample_buffer_ready(_DWORD); weak
// int __stdcall AIL_end_sample(_DWORD); weak
// int __stdcall AIL_release_timer_handle(_DWORD); weak
// int __stdcall AIL_stop_timer(_DWORD); weak
// int __stdcall AIL_set_sample_playback_rate(_DWORD, _DWORD); weak
// int __stdcall AIL_set_sample_pan(_DWORD, _DWORD); weak
// int __stdcall AIL_set_sample_volume(_DWORD, _DWORD); weak
// _DWORD __stdcall AIL_serve(); weak
// _DWORD __stdcall AIL_unlock(); weak
// _DWORD __stdcall AIL_lock(); weak
// int __stdcall AIL_init_sample(_DWORD); weak
// _DWORD __stdcall AIL_shutdown(); weak
// int __stdcall AIL_start_timer(_DWORD); weak
// int __stdcall AIL_digital_handle_reacquire(_DWORD); weak
// int __stdcall AIL_digital_handle_release(_DWORD); weak
// int __stdcall AIL_stream_position(_DWORD); weak
// int __stdcall AIL_open_stream(_DWORD, _DWORD, _DWORD); weak
// int __stdcall AIL_set_stream_position(_DWORD, _DWORD); weak
// int __stdcall AIL_start_stream(_DWORD); weak
// int __stdcall AIL_pause_stream(_DWORD, _DWORD); weak
// int __stdcall AIL_close_stream(_DWORD); weak
// int __stdcall AIL_stream_status(_DWORD); weak
// _DWORD __stdcall AIL_startup(); weak
// int __stdcall AIL_register_timer(_DWORD); weak
// int __stdcall AIL_stop_sample(_DWORD); weak
// int __stdcall AIL_resume_sample(_DWORD); weak
// int __stdcall AIL_set_stream_volume(_DWORD, _DWORD); weak
// int __stdcall AIL_waveOutOpen(_DWORD, _DWORD, _DWORD, _DWORD); weak
// int __stdcall AIL_release_sample_handle(_DWORD); weak
// HRESULT __stdcall CoInitialize(LPVOID pvReserved);
// void __stdcall CoUninitialize();
// HRESULT __stdcall CoCreateInstance(const IID *const rclsid, LPUNKNOWN pUnkOuter, DWORD dwClsContext, const IID *const
// riid, LPVOID *ppv);
