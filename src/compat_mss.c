...
#else
	alBufferData(S->hwbuf[S->hwready], S->stereo ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16, decoded,
		     decoded_samples * 2, S->playback_rate);
#endif
	checkError();
...
static void stream_work(HSTREAM stream) {
	...
	while (stream->hwready) {
		...
		stream->hwready--;
		alBufferData(stream->hwbuf[stream->hwready], stream->stereo ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16,
			     buffer, offset * 2, stream->playback_rate);
		checkError();
