...
int nox_xxx_gui_4AD320(int a1) {
	...
	if (nox_xxx_check_flag_aaa_43AF70() == 1) {
		v3 = nox_new_window_from_file(*(const char**)getMemAt(0x587000, 173596 + 4 * v1),
									  nox_xxx_windowServerOptionsGeneralProc_4AD5D0);
	} else {
		v3 = nox_new_window_from_file(*(const char**)getMemAt(0x587000, 173556 + 4 * v1),
									  nox_xxx_windowServerOptionsGeneralProc_4AD5D0);
	}
...
int sub_4AD840() {
	...
	if (dword_5d4594_1309812) {
		...
		if (result == 1) {
			...
			v7 = nox_strman_loadString_40F1D0(*(char**)getMemAt(0x587000, 173536 + 4 * nox_server_connectionType_3596),
											  0, "C:\\NoxPost\\src\\client\\Gui\\ServOpts\\general.c", 391);
